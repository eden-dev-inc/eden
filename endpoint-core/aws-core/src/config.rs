use crate::AwsAsync;
use crate::connection::{AwsConnection, AwsCredentials, AwsTarget};

use super::comm::AwsClient;
...
#[schema(title = "AwsConfig")]
pub struct AwsConfig {
    pub target: AwsTarget,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub read_credentials: Option<AwsCredentials>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub write_credentials: Option<AwsCredentials>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub admin_credentials: Option<AwsCredentials>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub system_credentials: Option<AwsCredentials>,
}

impl_ep_config_target_auth!(AwsConfig, AwsConnection, AwsTarget, AwsCredentials, EpKind::Aws);
...
struct AwsConfigRaw {
    #[serde(default)]
    target: Option<AwsTarget>,
    #[serde(default)]
    read_credentials: Option<AwsCredentials>,
    #[serde(default)]
    write_credentials: Option<AwsCredentials>,
    #[serde(default)]
    admin_credentials: Option<AwsCredentials>,
    #[serde(default)]
    system_credentials: Option<AwsCredentials>,

    #[serde(default)]
    read_conn: Option<AwsConnection>,
    #[serde(default)]
    write_conn: Option<AwsConnection>,
    #[serde(default)]
    admin_conn: Option<AwsConnection>,
    #[serde(default)]
    system_conn: Option<AwsConnection>,
}

impl<'de> Deserialize<'de> for AwsConfig {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let raw = AwsConfigRaw::deserialize(deserializer)?;
        ...
        if let Some(target) = raw.target {
            ...
            let extract = |c: &Option<AwsConnection>| c.as_ref().and_then(|c| c.split().ok().map(|(_, creds)| creds));
...
impl RWPool<AwsAsync> for AwsConfig {
    #[named]
    async fn conn_async(
        &self,
        connection: Box<dyn EpConnection>,
        telemetry_wrapper: &mut TelemetryWrapper,
    ) -> Result<Pool<AwsClient>, EpError> {
        let _span = telemetry_wrapper.client_tracer(format!("{}.{}", self.kind(), function_name!()));

        let connection = match connection.as_any().downcast_ref::<AwsConnection>() {
            Some(aws_config) => aws_config.to_owned(),
            None => return Err(EpError::Connect(ConnectError::FailedToDowncastConfig)),
        };

        let mut clients = vec![];
        for _ in 0..4 {
            clients.push(AwsClient::new(&connection).await?)
        }
