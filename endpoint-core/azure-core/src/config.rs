use crate::AzureAsync;
use crate::connection::{AzureConnection, AzureCredentials, AzureTarget};

use super::comm::AzureClient;
...
#[schema(title = "AzureConfig")]
pub struct AzureConfig {
    pub target: AzureTarget,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub read_credentials: Option<AzureCredentials>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub write_credentials: Option<AzureCredentials>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub admin_credentials: Option<AzureCredentials>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub system_credentials: Option<AzureCredentials>,
}

impl_ep_config_target_auth!(AzureConfig, AzureConnection, AzureTarget, AzureCredentials, EpKind::Azure);

impl fmt::Display for AzureConfig {
...
#[derive(Deserialize)]
struct AzureConfigRaw {
    #[serde(default)]
    target: Option<AzureTarget>,
    #[serde(default)]
    read_credentials: Option<AzureCredentials>,
    #[serde(default)]
    write_credentials: Option<AzureCredentials>,
    #[serde(default)]
    admin_credentials: Option<AzureCredentials>,
    #[serde(default)]
    system_credentials: Option<AzureCredentials>,

    #[serde(default)]
    read_conn: Option<AzureConnection>,
    #[serde(default)]
    write_conn: Option<AzureConnection>,
    #[serde(default)]
    admin_conn: Option<AzureConnection>,
    #[serde(default)]
    system_conn: Option<AzureConnection>,
}

impl<'de> Deserialize<'de> for AzureConfig {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let raw = AzureConfigRaw::deserialize(deserializer)?;
        ...
        if let Some(target) = raw.target {
            ...
            let extract = |c: &Option<AzureConnection>| c.as_ref().and_then(|c| c.split().ok().map(|(_, creds)| creds));
            ...
            Ok(AzureConfig::default())
        }
    }
}

impl RWPool<AzureAsync> for AzureConfig {
    #[named]
    async fn conn_async(
        &self,
        connection: Box<dyn EpConnection>,
        telemetry_wrapper: &mut TelemetryWrapper,
    ) -> Result<Pool<AzureClient>, EpError> {
        let _span = telemetry_wrapper.client_tracer(format!("{}.{}", self.kind(), function_name!()));

        let connection = match connection.as_any().downcast_ref::<AzureConnection>() {
            Some(azure_config) => azure_config.to_owned(),
            None => return Err(EpError::Connect(ConnectError::FailedToDowncastConfig)),
        };

        let mut clients = vec![];
        for _ in 0..4 {
            clients.push(AzureClient::new(&connection).await?)
        }
