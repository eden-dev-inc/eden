...
const API_INFO: ApiInfo<ClickhouseApi, FetchOneInput> = ApiInfo::new(
    EpKind::Clickhouse,
    ClickhouseApi::FetchOne,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: ClickhouseAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        // let result: ClickhouseRow = query.fetch_one().await.map_err(EpError::request)?;
        let result = fetch_all_rows(query).await.map_err(EpError::request)?;
        if result.len() != 1 {
            return Err(EpError::request(format!("expect one row, returned {}", result.len())));
        }

        let value = result[0].to_json(); // serde_json::to_value(result[0]).map_err(EpError::request)?;
