...
const API_INFO: ApiInfo<ClickhouseApi, ExecuteInput> =
    ApiInfo::new(EpKind::Clickhouse, ClickhouseApi::Execute, "Execute a clickhouse query", ReqType::Write);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: ClickhouseAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        query.execute().await.map_err(EpError::request)?;
        // let result: Vec<ClickhouseRow> = query.fetch_all().await.map_err(EpError::request)?;
        ...
        Ok(Box::new(ClickhouseValueOutput(serde_json::Value::Null).to_output()) as Box<dyn EpOutput>)
    }
    #[named]
