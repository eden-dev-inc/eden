...
use clickhouse_client::query::RowCursor;
...
const API_INFO: ApiInfo<ClickhouseApi, FetchInput> = ApiInfo::new(
    EpKind::Clickhouse,
    ClickhouseApi::Fetch,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: ClickhouseAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let mut cursor: RowCursor<ClickhouseRow> = query.fetch().map_err(EpError::request)?;

        let mut result = vec![];
        while let Some(row) = cursor.next().await.map_err(EpError::request)? {
            result.push(row)
        }
