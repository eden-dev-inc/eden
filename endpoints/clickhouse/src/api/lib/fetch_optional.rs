...
const API_INFO: ApiInfo<ClickhouseApi, FetchOptionalInput> = ApiInfo::new(
    EpKind::Clickhouse,
    ClickhouseApi::FetchOptional,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: ClickhouseAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let result: Option<ClickhouseRow> = query.fetch_optional().await.map_err(EpError::request)?;
