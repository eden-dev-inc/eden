...
const API_INFO: ApiInfo<MysqlApi, QueryInput> = ApiInfo::new(
    EpKind::Mysql,
    MysqlApi::Query,
    "Executes an SQL query and returns results from MySql",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    pub async fn run_query(&self, context: MysqlAsync) -> ResultEP<Vec<Row>> {
        self.sql.to_string().fetch(context).await.map_err(EpError::request)
    }
