...
use std::any::Any;
use {ep_core::define_metadata_serializer_stuff, mysql_core::MysqlAsync};
...
impl SyncMetadata<MysqlAsync> for MysqlMetadata {
    fn jobs(&mut self, _: SyncFrequency) -> Vec<MetadataJob<MysqlAsync, Self>> {
        Vec::new()
    }
}

impl EpMetadata for MysqlMetadata {
    fn kind(&self) -> EpKind {
        EpKind::Mysql
    }
...
define_metadata_serializer_stuff!(EpKind::Mysql => MysqlMetadata);
