...
pub(crate) struct WeaviateValueOutput(pub Value);

impl ToOutput for WeaviateValueOutput {
    fn to_output(self) -> EndpointOutput<Self> {
        EndpointOutput::new(EpKind::Weaviate, EndpointResponse::Response(self))
    }
    ...
    fn try_borsh_serialize(&self) -> ResultEP<Vec<u8>> {
        Err(EpError::serde("borsh serialize not implemented for Weaviate"))
    }
}
