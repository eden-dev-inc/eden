...
const API_INFO: ApiInfo<WeaviateApi, GetObjectInput> =
    ApiInfo::new(EpKind::Weaviate, WeaviateApi::GetObject, "Weaviate GetObject", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: WeaviateAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = context.get_object(self.class(), self.id()).await?;
