...
const API_INFO: ApiInfo<WeaviateApi, CreateObjectInput> =
    ApiInfo::new(EpKind::Weaviate, WeaviateApi::CreateObject, "Weaviate CreateObject", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: WeaviateAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = context.create_object(self.body().to_owned()).await?;
