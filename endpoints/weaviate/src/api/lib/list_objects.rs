...
const API_INFO: ApiInfo<WeaviateApi, ListObjectsInput> =
    ApiInfo::new(EpKind::Weaviate, WeaviateApi::ListObjects, "Weaviate ListObjects", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: WeaviateAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = context.list_objects(self.class().as_deref(), *self.limit()).await?;
