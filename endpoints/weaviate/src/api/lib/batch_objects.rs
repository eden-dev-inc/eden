...
const API_INFO: ApiInfo<WeaviateApi, BatchObjectsInput> =
    ApiInfo::new(EpKind::Weaviate, WeaviateApi::BatchObjects, "Weaviate BatchObjects", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: WeaviateAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = context.batch_objects(self.body().to_owned()).await?;
