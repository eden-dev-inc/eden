...
const API_INFO: ApiInfo<WeaviateApi, UpdateObjectInput> =
    ApiInfo::new(EpKind::Weaviate, WeaviateApi::UpdateObject, "Weaviate UpdateObject", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: WeaviateAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = context.update_object(self.class(), self.id(), self.body().to_owned()).await?;
