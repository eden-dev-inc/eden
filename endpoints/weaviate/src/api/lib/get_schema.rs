...
const API_INFO: ApiInfo<WeaviateApi, GetSchemaInput> =
    ApiInfo::new(EpKind::Weaviate, WeaviateApi::GetSchema, "Weaviate GetSchema", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: WeaviateAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = context.get_schema().await?;
