...
const API_INFO: ApiInfo<AzureApi, FunctionsGetFunctionAppInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::FunctionsGetFunctionApp, "Get a function app", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FunctionsGetFunctionAppInputBuilder::default().resource_group("my-rg").name("my-func").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "functions_get_function_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-func"});
        let _: FunctionsGetFunctionAppInput = serde_json::from_value(json).unwrap();
    }
}
