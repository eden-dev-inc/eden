...
const API_INFO: ApiInfo<AzureApi, FunctionsStartFunctionAppInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::FunctionsStartFunctionApp, "Start a function app", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FunctionsStartFunctionAppInputBuilder::default().resource_group("my-rg").function_app_name("my-func").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "functions_start_function_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "function_app_name": "my-func"});
        let _: FunctionsStartFunctionAppInput = serde_json::from_value(json).unwrap();
    }
}
