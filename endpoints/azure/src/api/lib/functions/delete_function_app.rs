...
const API_INFO: ApiInfo<AzureApi, FunctionsDeleteFunctionAppInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::FunctionsDeleteFunctionApp, "Delete a function app", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.function_app_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FunctionsDeleteFunctionAppInputBuilder::default().resource_group("my-rg").function_app_name("my-func").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "functions_delete_function_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "function_app_name": "my-func"});
        let _: FunctionsDeleteFunctionAppInput = serde_json::from_value(json).unwrap();
    }
}
