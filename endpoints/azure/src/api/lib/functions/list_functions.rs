...
const API_INFO: ApiInfo<AzureApi, FunctionsListFunctionsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::FunctionsListFunctions,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FunctionsListFunctionsInputBuilder::default().resource_group("my-rg").name("my-func").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "functions_list_functions");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-func"});
        let _: FunctionsListFunctionsInput = serde_json::from_value(json).unwrap();
    }
}
