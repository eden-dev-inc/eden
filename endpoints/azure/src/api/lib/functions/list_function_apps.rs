...
const API_INFO: ApiInfo<AzureApi, FunctionsListFunctionAppsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::FunctionsListFunctionApps, "List function apps", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Web/sites", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Web/sites", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FunctionsListFunctionAppsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "functions_list_function_apps");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: FunctionsListFunctionAppsInput = serde_json::from_value(json).unwrap();
    }
}
