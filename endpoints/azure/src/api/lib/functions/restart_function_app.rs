...
const API_INFO: ApiInfo<AzureApi, FunctionsRestartFunctionAppInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::FunctionsRestartFunctionApp, "Restart a function app", ReqType::Write, true);
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            FunctionsRestartFunctionAppInputBuilder::default().resource_group("my-rg").function_app_name("my-func").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "functions_restart_function_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "function_app_name": "my-func"});
        let _: FunctionsRestartFunctionAppInput = serde_json::from_value(json).unwrap();
    }
}
