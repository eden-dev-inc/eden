...
const API_INFO: ApiInfo<AzureApi, ManagedIdentityGetUserAssignedInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedIdentityGetUserAssigned,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedIdentityGetUserAssignedInputBuilder::default()
            .resource_group("my-rg")
            .identity_name("my-identity")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_identity_get_user_assigned");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "identity_name": "my-identity"});
        let _: ManagedIdentityGetUserAssignedInput = serde_json::from_value(json).unwrap();
    }
}
