...
const API_INFO: ApiInfo<AzureApi, ManagedIdentityDeleteUserAssignedInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedIdentityDeleteUserAssigned,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedIdentityDeleteUserAssignedInputBuilder::default()
            .resource_group("my-rg")
            .identity_name("my-identity")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_identity_delete_user_assigned");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "identity_name": "my-identity"});
        let _: ManagedIdentityDeleteUserAssignedInput = serde_json::from_value(json).unwrap();
    }
}
