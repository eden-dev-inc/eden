...
const API_INFO: ApiInfo<AzureApi, ManagedIdentityListUserAssignedInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedIdentityListUserAssigned,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            ...
            format!("/subscriptions/{}/providers/Microsoft.ManagedIdentity/userAssignedIdentities", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedIdentityListUserAssignedInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_identity_list_user_assigned");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ManagedIdentityListUserAssignedInput = serde_json::from_value(json).unwrap();
    }
}
