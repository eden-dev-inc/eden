...
const API_INFO: ApiInfo<AzureApi, PostgresqlListFirewallRulesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlListFirewallRules,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlListFirewallRulesInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_list_firewall_rules");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: PostgresqlListFirewallRulesInput = serde_json::from_value(json).unwrap();
    }
}
