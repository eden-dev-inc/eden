...
const API_INFO: ApiInfo<AzureApi, PostgresqlListDatabasesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlListDatabases,
    "List databases on a PostgreSQL flexible server",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlListDatabasesInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_list_databases");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: PostgresqlListDatabasesInput = serde_json::from_value(json).unwrap();
    }
}
