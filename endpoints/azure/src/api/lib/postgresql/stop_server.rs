...
const API_INFO: ApiInfo<AzureApi, PostgresqlStopServerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlStopServer,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlStopServerInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_stop_server");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: PostgresqlStopServerInput = serde_json::from_value(json).unwrap();
    }
}
