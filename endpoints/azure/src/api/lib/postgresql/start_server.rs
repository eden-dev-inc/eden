...
const API_INFO: ApiInfo<AzureApi, PostgresqlStartServerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlStartServer,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlStartServerInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_start_server");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: PostgresqlStartServerInput = serde_json::from_value(json).unwrap();
    }
}
