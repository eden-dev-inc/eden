...
const API_INFO: ApiInfo<AzureApi, PostgresqlListServersInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlListServers,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.DBforPostgreSQL/flexibleServers", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.DBforPostgreSQL/flexibleServers", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlListServersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_list_servers");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: PostgresqlListServersInput = serde_json::from_value(json).unwrap();
    }
}
