...
const API_INFO: ApiInfo<AzureApi, PostgresqlListReplicasInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlListReplicas,
    "List PostgreSQL flexible server replicas",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlListReplicasInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_list_replicas");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: PostgresqlListReplicasInput = serde_json::from_value(json).unwrap();
    }
}
