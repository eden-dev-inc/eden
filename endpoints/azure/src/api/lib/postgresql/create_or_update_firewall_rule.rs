...
const API_VERSION: &str = "2024-08-01";

const API_INFO: ApiInfo<AzureApi, PostgresqlCreateOrUpdateFirewallRuleInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlCreateOrUpdateFirewallRule,
    "Create or update a firewall rule on a PostgreSQL flexible server",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.server_name, self.firewall_rule_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlCreateOrUpdateFirewallRuleInputBuilder::default()
            .resource_group("my-rg")
            .server_name("my-server")
            .firewall_rule_name("allow-all")
            .properties(serde_json::json!({"startIpAddress": "0.0.0.0", "endIpAddress": "255.255.255.255"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_create_or_update_firewall_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json =
            serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "firewall_rule_name": "allow-all", "properties": {}});
        let _: PostgresqlCreateOrUpdateFirewallRuleInput = serde_json::from_value(json).unwrap();
    }
}
