...
const API_INFO: ApiInfo<AzureApi, PostgresqlGetConfigurationInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlGetConfiguration,
    "Get a configuration on a PostgreSQL flexible server",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.server_name, self.configuration_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlGetConfigurationInputBuilder::default()
        ...
        assert_eq!(json["type"], "postgresql_get_configuration");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "configuration_name": "max_connections"});
        let _: PostgresqlGetConfigurationInput = serde_json::from_value(json).unwrap();
    }
}
