...
const API_INFO: ApiInfo<AzureApi, PostgresqlRestartServerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlRestartServer,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlRestartServerInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_restart_server");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: PostgresqlRestartServerInput = serde_json::from_value(json).unwrap();
    }
}
