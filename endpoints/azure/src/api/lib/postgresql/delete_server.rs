...
const API_INFO: ApiInfo<AzureApi, PostgresqlDeleteServerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlDeleteServer,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlDeleteServerInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_delete_server");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: PostgresqlDeleteServerInput = serde_json::from_value(json).unwrap();
    }
}
