...
const API_INFO: ApiInfo<AzureApi, PostgresqlGetDatabaseInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlGetDatabase,
    "Get a database on a PostgreSQL flexible server",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlGetDatabaseInputBuilder::default()
        ...
        assert_eq!(json["type"], "postgresql_get_database");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "database_name": "my-db"});
        let _: PostgresqlGetDatabaseInput = serde_json::from_value(json).unwrap();
    }
}
