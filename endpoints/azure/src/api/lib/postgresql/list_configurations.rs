...
const API_INFO: ApiInfo<AzureApi, PostgresqlListConfigurationsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlListConfigurations,
    "List configurations on a PostgreSQL flexible server",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlListConfigurationsInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_list_configurations");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: PostgresqlListConfigurationsInput = serde_json::from_value(json).unwrap();
    }
}
