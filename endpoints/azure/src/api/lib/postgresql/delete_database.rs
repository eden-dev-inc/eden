...
const API_INFO: ApiInfo<AzureApi, PostgresqlDeleteDatabaseInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlDeleteDatabase,
    "Delete a database on a PostgreSQL flexible server",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlDeleteDatabaseInputBuilder::default()
        ...
        assert_eq!(json["type"], "postgresql_delete_database");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "database_name": "my-db"});
        let _: PostgresqlDeleteDatabaseInput = serde_json::from_value(json).unwrap();
    }
}
