...
const API_INFO: ApiInfo<AzureApi, PostgresqlCreateOrUpdateServerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlCreateOrUpdateServer,
    "Create or update a PostgreSQL flexible server",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlCreateOrUpdateServerInputBuilder::default()
            .resource_group("my-rg")
            .server_name("my-server")
            .location("eastus")
            .properties(serde_json::json!({"version": "16"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_create_or_update_server");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "location": "eastus", "properties": {}});
        let _: PostgresqlCreateOrUpdateServerInput = serde_json::from_value(json).unwrap();
    }
}
