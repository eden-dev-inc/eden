...
const API_INFO: ApiInfo<AzureApi, PostgresqlGetServerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlGetServer,
    "Get a PostgreSQL flexible server",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlGetServerInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_get_server");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: PostgresqlGetServerInput = serde_json::from_value(json).unwrap();
    }
}
