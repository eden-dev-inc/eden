...
const API_INFO: ApiInfo<AzureApi, PostgresqlCreateOrUpdateDatabaseInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlCreateOrUpdateDatabase,
    "Create or update a database on a PostgreSQL flexible server",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlCreateOrUpdateDatabaseInputBuilder::default()
            .resource_group("my-rg")
            .server_name("my-server")
            .database_name("my-db")
            .properties(serde_json::json!({"charset": "UTF8"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "postgresql_create_or_update_database");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "database_name": "my-db", "properties": {}});
        let _: PostgresqlCreateOrUpdateDatabaseInput = serde_json::from_value(json).unwrap();
    }
}
