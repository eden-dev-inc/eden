...
const API_INFO: ApiInfo<AzureApi, PostgresqlDeleteFirewallRuleInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PostgresqlDeleteFirewallRule,
    "Delete a firewall rule on a PostgreSQL flexible server",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PostgresqlDeleteFirewallRuleInputBuilder::default()
        ...
        assert_eq!(json["type"], "postgresql_delete_firewall_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "firewall_rule_name": "allow-all"});
        let _: PostgresqlDeleteFirewallRuleInput = serde_json::from_value(json).unwrap();
    }
}
