...
const API_INFO: ApiInfo<AzureApi, StaticWebAppsCreateOrUpdateInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::StaticWebAppsCreateOrUpdate,
    "Create or update a Static Web App",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.static_site_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StaticWebAppsCreateOrUpdateInputBuilder::default()
            .resource_group("my-rg")
            .static_site_name("my-site")
            .location("eastus")
            .properties(serde_json::json!({"repositoryUrl": "https://github.com/example/repo"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "static_web_apps_create_or_update");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "static_site_name": "my-site", "location": "eastus", "properties": {}});
        let _: StaticWebAppsCreateOrUpdateInput = serde_json::from_value(json).unwrap();
    }
}
