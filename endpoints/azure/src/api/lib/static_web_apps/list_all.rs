...
const API_INFO: ApiInfo<AzureApi, StaticWebAppsListAllInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StaticWebAppsListAll, "List all Static Web Apps", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Web/staticSites", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Web/staticSites", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StaticWebAppsListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "static_web_apps_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: StaticWebAppsListAllInput = serde_json::from_value(json).unwrap();
    }
}
