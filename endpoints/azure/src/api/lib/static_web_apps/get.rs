...
const API_INFO: ApiInfo<AzureApi, StaticWebAppsGetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StaticWebAppsGet, "Get a Static Web App", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StaticWebAppsGetInputBuilder::default().resource_group("my-rg").static_site_name("my-site").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "static_web_apps_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "static_site_name": "my-site"});
        let _: StaticWebAppsGetInput = serde_json::from_value(json).unwrap();
    }
}
