...
const API_INFO: ApiInfo<AzureApi, StaticWebAppsDeleteInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StaticWebAppsDelete, "Delete a Static Web App", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StaticWebAppsDeleteInputBuilder::default().resource_group("my-rg").static_site_name("my-site").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "static_web_apps_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "static_site_name": "my-site"});
        let _: StaticWebAppsDeleteInput = serde_json::from_value(json).unwrap();
    }
}
