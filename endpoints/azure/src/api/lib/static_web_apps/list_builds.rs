...
const API_INFO: ApiInfo<AzureApi, StaticWebAppsListBuildsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::StaticWebAppsListBuilds,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StaticWebAppsListBuildsInputBuilder::default().resource_group("my-rg").static_site_name("my-site").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "static_web_apps_list_builds");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "static_site_name": "my-site"});
        let _: StaticWebAppsListBuildsInput = serde_json::from_value(json).unwrap();
    }
}
