...
const API_INFO: ApiInfo<AzureApi, StaticWebAppsListCustomDomainsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::StaticWebAppsListCustomDomains,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StaticWebAppsListCustomDomainsInputBuilder::default()
            .resource_group("my-rg")
            .static_site_name("my-site")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "static_web_apps_list_custom_domains");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "static_site_name": "my-site"});
        let _: StaticWebAppsListCustomDomainsInput = serde_json::from_value(json).unwrap();
    }
}
