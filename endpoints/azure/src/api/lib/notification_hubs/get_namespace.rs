...
const API_INFO: ApiInfo<AzureApi, NotificationHubsGetNamespaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NotificationHubsGetNamespace,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NotificationHubsGetNamespaceInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "notification_hubs_get_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: NotificationHubsGetNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
