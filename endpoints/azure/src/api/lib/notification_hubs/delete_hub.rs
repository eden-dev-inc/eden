...
const API_INFO: ApiInfo<AzureApi, NotificationHubsDeleteHubInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NotificationHubsDeleteHub,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NotificationHubsDeleteHubInputBuilder::default()
        ...
        assert_eq!(json["type"], "notification_hubs_delete_hub");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "hub_name": "my-hub"});
        let _: NotificationHubsDeleteHubInput = serde_json::from_value(json).unwrap();
    }
}
