...
const API_INFO: ApiInfo<AzureApi, NotificationHubsListHubsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NotificationHubsListHubs,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NotificationHubsListHubsInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "notification_hubs_list_hubs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: NotificationHubsListHubsInput = serde_json::from_value(json).unwrap();
    }
}
