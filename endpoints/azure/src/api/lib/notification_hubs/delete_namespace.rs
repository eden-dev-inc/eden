...
const API_INFO: ApiInfo<AzureApi, NotificationHubsDeleteNamespaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NotificationHubsDeleteNamespace,
    "Delete a Notification Hub namespace",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NotificationHubsDeleteNamespaceInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "notification_hubs_delete_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: NotificationHubsDeleteNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
