...
const API_INFO: ApiInfo<AzureApi, NotificationHubsListNamespacesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NotificationHubsListNamespaces,
    "List Notification Hub namespaces",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.NotificationHubs/namespaces", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.NotificationHubs/namespaces", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NotificationHubsListNamespacesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "notification_hubs_list_namespaces");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NotificationHubsListNamespacesInput = serde_json::from_value(json).unwrap();
    }
}
