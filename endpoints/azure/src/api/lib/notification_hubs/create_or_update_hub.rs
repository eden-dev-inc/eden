...
const API_INFO: ApiInfo<AzureApi, NotificationHubsCreateOrUpdateHubInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NotificationHubsCreateOrUpdateHub,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.namespace_name, self.hub_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NotificationHubsCreateOrUpdateHubInputBuilder::default()
        ...
        assert_eq!(json["type"], "notification_hubs_create_or_update_hub");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "hub_name": "my-hub", "location": "eastus", "properties": {}});
        let _: NotificationHubsCreateOrUpdateHubInput = serde_json::from_value(json).unwrap();
    }
}
