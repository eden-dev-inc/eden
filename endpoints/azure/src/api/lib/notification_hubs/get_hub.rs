...
const API_INFO: ApiInfo<AzureApi, NotificationHubsGetHubInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NotificationHubsGetHub, "Get a Notification Hub", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NotificationHubsGetHubInputBuilder::default()
        ...
        assert_eq!(json["type"], "notification_hubs_get_hub");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "hub_name": "my-hub"});
        let _: NotificationHubsGetHubInput = serde_json::from_value(json).unwrap();
    }
}
