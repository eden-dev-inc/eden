...
const API_INFO: ApiInfo<AzureApi, MonitorGetActionGroupInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MonitorGetActionGroup, "Get an action group", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.action_group_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorGetActionGroupInputBuilder::default().resource_group("my-rg").action_group_name("my-ag").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_get_action_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "action_group_name": "my-ag"});
        let _: MonitorGetActionGroupInput = serde_json::from_value(json).unwrap();
    }
}
