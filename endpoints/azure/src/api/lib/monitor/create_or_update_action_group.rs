...
const API_INFO: ApiInfo<AzureApi, MonitorCreateOrUpdateActionGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorCreateOrUpdateActionGroup,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorCreateOrUpdateActionGroupInputBuilder::default()
            .resource_group("my-rg")
            .action_group_name("my-ag")
            .location("global")
            .properties(serde_json::json!({"groupShortName": "myag", "enabled": true}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_create_or_update_action_group");
    }
    ...
    fn deserialize_minimal() {
        let json = serde_json::json!({
            ...
            "action_group_name": "my-ag",
            "location": "global",
            "properties": {"groupShortName": "myag", "enabled": true}
        });
        let _: MonitorCreateOrUpdateActionGroupInput = serde_json::from_value(json).unwrap();
    }
}
