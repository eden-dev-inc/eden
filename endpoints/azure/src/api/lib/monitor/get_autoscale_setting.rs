...
const API_INFO: ApiInfo<AzureApi, MonitorGetAutoscaleSettingInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MonitorGetAutoscaleSetting, "Get an autoscale setting", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.autoscale_setting_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorGetAutoscaleSettingInputBuilder::default()
            .resource_group("my-rg")
            .autoscale_setting_name("my-setting")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_get_autoscale_setting");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "autoscale_setting_name": "my-setting"});
        let _: MonitorGetAutoscaleSettingInput = serde_json::from_value(json).unwrap();
    }
}
