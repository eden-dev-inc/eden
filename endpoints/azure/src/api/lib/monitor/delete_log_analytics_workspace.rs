...
const API_INFO: ApiInfo<AzureApi, MonitorDeleteLogAnalyticsWorkspaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorDeleteLogAnalyticsWorkspace,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorDeleteLogAnalyticsWorkspaceInputBuilder::default()
            .resource_group("my-rg")
            .workspace_name("my-workspace")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_delete_log_analytics_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-workspace"});
        let _: MonitorDeleteLogAnalyticsWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
