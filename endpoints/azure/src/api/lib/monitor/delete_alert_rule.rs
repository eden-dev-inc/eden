...
const API_VERSION: &str = "2018-03-01";

const API_INFO: ApiInfo<AzureApi, MonitorDeleteAlertRuleInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MonitorDeleteAlertRule, "Delete a metric alert rule", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.rule_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorDeleteAlertRuleInputBuilder::default().resource_group("my-rg").rule_name("my-rule").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_delete_alert_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "rule_name": "my-rule"});
        let _: MonitorDeleteAlertRuleInput = serde_json::from_value(json).unwrap();
    }
}
