...
const API_INFO: ApiInfo<AzureApi, MonitorListMetricDefinitionsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorListMetricDefinitions,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/{}/providers/Microsoft.Insights/metricDefinitions", self.resource_uri.trim_start_matches('/'));
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorListMetricDefinitionsInputBuilder::default()
            .resource_uri("/subscriptions/sub1/resourceGroups/rg1/providers/Microsoft.Compute/virtualMachines/vm1")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_list_metric_definitions");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: MonitorListMetricDefinitionsInput = serde_json::from_value(json).unwrap();
    }
}
