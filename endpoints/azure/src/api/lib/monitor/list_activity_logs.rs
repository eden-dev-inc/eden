...
const API_VERSION: &str = "2015-04-01";

const API_INFO: ApiInfo<AzureApi, MonitorListActivityLogsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MonitorListActivityLogs, "List activity logs", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Insights/eventtypes/management/values", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorListActivityLogsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_list_activity_logs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: MonitorListActivityLogsInput = serde_json::from_value(json).unwrap();
    }
}
