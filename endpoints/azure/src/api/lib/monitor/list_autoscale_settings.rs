...
const API_INFO: ApiInfo<AzureApi, MonitorListAutoscaleSettingsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorListAutoscaleSettings,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Insights/autoscalesettings", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Insights/autoscalesettings", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorListAutoscaleSettingsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_list_autoscale_settings");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: MonitorListAutoscaleSettingsInput = serde_json::from_value(json).unwrap();
    }
}
