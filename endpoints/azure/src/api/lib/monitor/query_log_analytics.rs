...
const API_INFO: ApiInfo<AzureApi, MonitorQueryLogAnalyticsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorQueryLogAnalytics,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(ts) = &self.timespan {
            body["timespan"] = serde_json::json!(ts);
        }
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorQueryLogAnalyticsInputBuilder::default()
        ...
        assert_eq!(json["type"], "monitor_query_log_analytics");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-workspace", "query": "Heartbeat | take 10"});
        let _: MonitorQueryLogAnalyticsInput = serde_json::from_value(json).unwrap();
    }
}
