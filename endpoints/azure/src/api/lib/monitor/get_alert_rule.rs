...
const API_INFO: ApiInfo<AzureApi, MonitorGetAlertRuleInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MonitorGetAlertRule, "Get a metric alert rule", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorGetAlertRuleInputBuilder::default().resource_group("my-rg").rule_name("my-rule").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_get_alert_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "rule_name": "my-rule"});
        let _: MonitorGetAlertRuleInput = serde_json::from_value(json).unwrap();
    }
}
