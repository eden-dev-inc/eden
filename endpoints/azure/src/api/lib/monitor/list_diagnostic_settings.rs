...
const API_INFO: ApiInfo<AzureApi, MonitorListDiagnosticSettingsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorListDiagnosticSettings,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/{}/providers/Microsoft.Insights/diagnosticSettings", self.resource_uri.trim_start_matches('/'));
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorListDiagnosticSettingsInputBuilder::default()
            .resource_uri("/subscriptions/sub1/resourceGroups/rg1/providers/Microsoft.Compute/virtualMachines/vm1")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_list_diagnostic_settings");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: MonitorListDiagnosticSettingsInput = serde_json::from_value(json).unwrap();
    }
}
