...
const API_INFO: ApiInfo<AzureApi, MonitorListLogAnalyticsWorkspacesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorListLogAnalyticsWorkspaces,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.OperationalInsights/workspaces", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.OperationalInsights/workspaces", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorListLogAnalyticsWorkspacesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_list_log_analytics_workspaces");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: MonitorListLogAnalyticsWorkspacesInput = serde_json::from_value(json).unwrap();
    }
}
