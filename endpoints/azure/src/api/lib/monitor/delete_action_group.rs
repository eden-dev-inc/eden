...
const API_INFO: ApiInfo<AzureApi, MonitorDeleteActionGroupInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MonitorDeleteActionGroup, "Delete an action group", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorDeleteActionGroupInputBuilder::default()
            .resource_group("my-rg")
            .action_group_name("my-action-group")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_delete_action_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "action_group_name": "my-action-group"});
        let _: MonitorDeleteActionGroupInput = serde_json::from_value(json).unwrap();
    }
}
