...
const API_INFO: ApiInfo<AzureApi, MonitorDeleteAutoscaleSettingInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorDeleteAutoscaleSetting,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorDeleteAutoscaleSettingInputBuilder::default()
            .resource_group("my-rg")
            .autoscale_setting_name("my-autoscale")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_delete_autoscale_setting");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "autoscale_setting_name": "my-autoscale"});
        let _: MonitorDeleteAutoscaleSettingInput = serde_json::from_value(json).unwrap();
    }
}
