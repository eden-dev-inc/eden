...
const API_INFO: ApiInfo<AzureApi, MonitorGetLogAnalyticsWorkspaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorGetLogAnalyticsWorkspace,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorGetLogAnalyticsWorkspaceInputBuilder::default()
            .resource_group("my-rg")
            .workspace_name("my-workspace")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_get_log_analytics_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-workspace"});
        let _: MonitorGetLogAnalyticsWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
