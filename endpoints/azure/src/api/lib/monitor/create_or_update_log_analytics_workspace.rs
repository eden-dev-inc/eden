...
const API_INFO: ApiInfo<AzureApi, MonitorCreateOrUpdateLogAnalyticsWorkspaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorCreateOrUpdateLogAnalyticsWorkspace,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorCreateOrUpdateLogAnalyticsWorkspaceInputBuilder::default()
        ...
        assert_eq!(json["type"], "monitor_create_or_update_log_analytics_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-workspace", "location": "eastus"});
        let _: MonitorCreateOrUpdateLogAnalyticsWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
