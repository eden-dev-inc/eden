...
const API_INFO: ApiInfo<AzureApi, MonitorCreateOrUpdateAlertRuleInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MonitorCreateOrUpdateAlertRule,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorCreateOrUpdateAlertRuleInputBuilder::default()
            .resource_group("my-rg")
            .rule_name("my-rule")
            .location("global")
            .properties(serde_json::json!({"severity": 3}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_create_or_update_alert_rule");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: MonitorCreateOrUpdateAlertRuleInput = serde_json::from_value(json).unwrap();
    }
}
