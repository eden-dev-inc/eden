...
const API_INFO: ApiInfo<AzureApi, MonitorListAlertRulesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MonitorListAlertRules, "List metric alert rules", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Insights/metricAlerts", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Insights/metricAlerts", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorListAlertRulesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_list_alert_rules");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: MonitorListAlertRulesInput = serde_json::from_value(json).unwrap();
    }
}
