...
const API_INFO: ApiInfo<AzureApi, MonitorListActionGroupsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MonitorListActionGroups, "List action groups", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Insights/actionGroups", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Insights/actionGroups", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MonitorListActionGroupsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "monitor_list_action_groups");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: MonitorListActionGroupsInput = serde_json::from_value(json).unwrap();
    }
}
