...
const API_INFO: ApiInfo<AzureApi, AutomationCreateOrUpdateRunbookInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AutomationCreateOrUpdateRunbook,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.account_name, self.runbook_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AutomationCreateOrUpdateRunbookInputBuilder::default()
        ...
        assert_eq!(json["type"], "automation_create_or_update_runbook");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account", "runbook_name": "my-runbook", "location": "eastus", "properties": {}});
        let _: AutomationCreateOrUpdateRunbookInput = serde_json::from_value(json).unwrap();
    }
}
