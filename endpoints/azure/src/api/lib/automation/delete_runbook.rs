...
const API_INFO: ApiInfo<AzureApi, AutomationDeleteRunbookInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AutomationDeleteRunbook, "Delete a runbook", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AutomationDeleteRunbookInputBuilder::default()
        ...
        assert_eq!(json["type"], "automation_delete_runbook");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account", "runbook_name": "my-runbook"});
        let _: AutomationDeleteRunbookInput = serde_json::from_value(json).unwrap();
    }
}
