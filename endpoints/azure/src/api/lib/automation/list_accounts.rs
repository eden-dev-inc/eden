...
const API_INFO: ApiInfo<AzureApi, AutomationListAccountsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AutomationListAccounts, "List automation accounts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Automation/automationAccounts", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Automation/automationAccounts", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AutomationListAccountsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "automation_list_accounts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AutomationListAccountsInput = serde_json::from_value(json).unwrap();
    }
}
