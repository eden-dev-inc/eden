...
const API_INFO: ApiInfo<AzureApi, AutomationDeleteAccountInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AutomationDeleteAccount,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AutomationDeleteAccountInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "automation_delete_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: AutomationDeleteAccountInput = serde_json::from_value(json).unwrap();
    }
}
