...
const API_INFO: ApiInfo<AzureApi, AutomationGetJobInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AutomationGetJob, "Get a job", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.account_name, self.job_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AutomationGetJobInputBuilder::default()
        ...
        assert_eq!(json["type"], "automation_get_job");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account", "job_name": "my-job"});
        let _: AutomationGetJobInput = serde_json::from_value(json).unwrap();
    }
}
