...
const API_INFO: ApiInfo<AzureApi, AutomationGetAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AutomationGetAccount, "Get an automation account", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AutomationGetAccountInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "automation_get_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: AutomationGetAccountInput = serde_json::from_value(json).unwrap();
    }
}
