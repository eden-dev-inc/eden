...
const API_INFO: ApiInfo<AzureApi, AutomationListJobsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AutomationListJobs, "List jobs", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AutomationListJobsInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "automation_list_jobs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: AutomationListJobsInput = serde_json::from_value(json).unwrap();
    }
}
