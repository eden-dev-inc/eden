...
const API_INFO: ApiInfo<AzureApi, AutomationListRunbooksInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AutomationListRunbooks, "List runbooks", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AutomationListRunbooksInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "automation_list_runbooks");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: AutomationListRunbooksInput = serde_json::from_value(json).unwrap();
    }
}
