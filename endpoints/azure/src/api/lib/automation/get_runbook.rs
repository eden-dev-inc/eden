...
const API_INFO: ApiInfo<AzureApi, AutomationGetRunbookInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AutomationGetRunbook, "Get a runbook", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AutomationGetRunbookInputBuilder::default()
        ...
        assert_eq!(json["type"], "automation_get_runbook");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account", "runbook_name": "my-runbook"});
        let _: AutomationGetRunbookInput = serde_json::from_value(json).unwrap();
    }
}
