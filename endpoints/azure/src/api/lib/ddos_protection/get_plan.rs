...
const API_INFO: ApiInfo<AzureApi, DdosProtectionGetPlanInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DdosProtectionGetPlan,
    "Get an Azure DDoS protection plan",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DdosProtectionGetPlanInputBuilder::default().resource_group("my-rg").plan_name("my-plan").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "ddos_protection_get_plan");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "plan_name": "my-plan"});
        let _: DdosProtectionGetPlanInput = serde_json::from_value(json).unwrap();
    }
}
