...
const API_INFO: ApiInfo<AzureApi, DdosProtectionListPlansInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DdosProtectionListPlans,
    "List Azure DDoS protection plans",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/ddosProtectionPlans", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/ddosProtectionPlans", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DdosProtectionListPlansInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "ddos_protection_list_plans");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DdosProtectionListPlansInput = serde_json::from_value(json).unwrap();
    }
}
