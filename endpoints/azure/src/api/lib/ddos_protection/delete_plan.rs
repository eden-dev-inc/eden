...
const API_INFO: ApiInfo<AzureApi, DdosProtectionDeletePlanInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DdosProtectionDeletePlan,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DdosProtectionDeletePlanInputBuilder::default().resource_group("my-rg").plan_name("my-plan").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "ddos_protection_delete_plan");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "plan_name": "my-plan"});
        let _: DdosProtectionDeletePlanInput = serde_json::from_value(json).unwrap();
    }
}
