...
const API_INFO: ApiInfo<AzureApi, DdosProtectionCreateOrUpdatePlanInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DdosProtectionCreateOrUpdatePlan,
    "Create or update a DDoS protection plan",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.plan_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DdosProtectionCreateOrUpdatePlanInputBuilder::default()
        ...
        assert_eq!(json["type"], "ddos_protection_create_or_update_plan");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "plan_name": "my-plan", "location": "eastus", "properties": {}});
        let _: DdosProtectionCreateOrUpdatePlanInput = serde_json::from_value(json).unwrap();
    }
}
