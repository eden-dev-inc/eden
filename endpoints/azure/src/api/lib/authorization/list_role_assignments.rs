...
const API_INFO: ApiInfo<AzureApi, AuthorizationListRoleAssignmentsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AuthorizationListRoleAssignments,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Authorization/roleAssignments", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AuthorizationListRoleAssignmentsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "authorization_list_role_assignments");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AuthorizationListRoleAssignmentsInput = serde_json::from_value(json).unwrap();
    }
}
