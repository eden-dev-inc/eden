...
const API_INFO: ApiInfo<AzureApi, AuthorizationListRoleDefinitionsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AuthorizationListRoleDefinitions,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Authorization/roleDefinitions", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AuthorizationListRoleDefinitionsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "authorization_list_role_definitions");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AuthorizationListRoleDefinitionsInput = serde_json::from_value(json).unwrap();
    }
}
