...
const API_INFO: ApiInfo<AzureApi, AppServiceListPublishProfilesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppServiceListPublishProfiles,
    "List publish profiles for a web app",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceListPublishProfilesInputBuilder::default().resource_group("my-rg").name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_list_publish_profiles");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: AppServiceListPublishProfilesInput = serde_json::from_value(json).unwrap();
    }
}
