...
const API_INFO: ApiInfo<AzureApi, AppServiceListDeploymentsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppServiceListDeployments,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceListDeploymentsInputBuilder::default().resource_group("my-rg").name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_list_deployments");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: AppServiceListDeploymentsInput = serde_json::from_value(json).unwrap();
    }
}
