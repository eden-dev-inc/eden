...
const API_INFO: ApiInfo<AzureApi, AppServiceDeletePlanInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppServiceDeletePlan, "Delete an App Service plan", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceDeletePlanInputBuilder::default().resource_group("my-rg").name("my-plan").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_delete_plan");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-plan"});
        let _: AppServiceDeletePlanInput = serde_json::from_value(json).unwrap();
    }
}
