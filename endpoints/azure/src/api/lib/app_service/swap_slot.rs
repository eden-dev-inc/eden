...
const API_INFO: ApiInfo<AzureApi, AppServiceSwapSlotInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppServiceSwapSlot, "Swap deployment slots", ReqType::Write, true);
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            AppServiceSwapSlotInputBuilder::default().resource_group("my-rg").name("my-app").target_slot("staging").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_swap_slot");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-app", "target_slot": "staging"});
        let _: AppServiceSwapSlotInput = serde_json::from_value(json).unwrap();
    }
}
