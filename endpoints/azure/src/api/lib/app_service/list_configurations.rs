...
const API_INFO: ApiInfo<AzureApi, AppServiceListConfigurationsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppServiceListConfigurations,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceListConfigurationsInputBuilder::default().resource_group("my-rg").name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_list_configurations");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-app"});
        let _: AppServiceListConfigurationsInput = serde_json::from_value(json).unwrap();
    }
}
