...
const API_INFO: ApiInfo<AzureApi, AppServiceListPlansInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppServiceListPlans, "List App Service plans", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Web/serverfarms", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Web/serverfarms", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceListPlansInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_list_plans");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AppServiceListPlansInput = serde_json::from_value(json).unwrap();
    }
}
