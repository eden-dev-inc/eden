...
const API_INFO: ApiInfo<AzureApi, AppServiceListHybridConnectionsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppServiceListHybridConnections,
    "List hybrid connections for a web app",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceListHybridConnectionsInputBuilder::default().resource_group("my-rg").name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_list_hybrid_connections");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: AppServiceListHybridConnectionsInput = serde_json::from_value(json).unwrap();
    }
}
