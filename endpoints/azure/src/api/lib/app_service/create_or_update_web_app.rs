...
const API_INFO: ApiInfo<AzureApi, AppServiceCreateOrUpdateWebAppInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppServiceCreateOrUpdateWebApp,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceCreateOrUpdateWebAppInputBuilder::default()
        ...
        assert_eq!(json["type"], "app_service_create_or_update_web_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-app", "location": "eastus"});
        let _: AppServiceCreateOrUpdateWebAppInput = serde_json::from_value(json).unwrap();
    }
}
