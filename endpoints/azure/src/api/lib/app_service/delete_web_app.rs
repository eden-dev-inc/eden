...
const API_INFO: ApiInfo<AzureApi, AppServiceDeleteWebAppInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppServiceDeleteWebApp, "Delete a web app", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceDeleteWebAppInputBuilder::default().resource_group("my-rg").name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_delete_web_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-app"});
        let _: AppServiceDeleteWebAppInput = serde_json::from_value(json).unwrap();
    }
}
