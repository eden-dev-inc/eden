...
const API_INFO: ApiInfo<AzureApi, AppServiceListCustomDomainsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppServiceListCustomDomains,
    "List custom domain bindings for a web app",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceListCustomDomainsInputBuilder::default().resource_group("my-rg").name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_list_custom_domains");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-app"});
        let _: AppServiceListCustomDomainsInput = serde_json::from_value(json).unwrap();
    }
}
