...
const API_INFO: ApiInfo<AzureApi, AppServiceListDeploymentSlotsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppServiceListDeploymentSlots,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceListDeploymentSlotsInputBuilder::default().resource_group("my-rg").name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_list_deployment_slots");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-app"});
        let _: AppServiceListDeploymentSlotsInput = serde_json::from_value(json).unwrap();
    }
}
