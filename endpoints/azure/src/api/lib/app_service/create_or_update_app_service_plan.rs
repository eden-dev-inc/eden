...
const API_INFO: ApiInfo<AzureApi, AppServiceCreateOrUpdatePlanInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppServiceCreateOrUpdatePlan,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceCreateOrUpdatePlanInputBuilder::default()
        ...
        assert_eq!(json["type"], "app_service_create_or_update_plan");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-plan", "location": "eastus"});
        let _: AppServiceCreateOrUpdatePlanInput = serde_json::from_value(json).unwrap();
    }
}
