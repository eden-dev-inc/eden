...
const API_INFO: ApiInfo<AzureApi, AppServiceListWebAppsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppServiceListWebApps, "List web apps", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceListWebAppsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_list_web_apps");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AppServiceListWebAppsInput = serde_json::from_value(json).unwrap();
    }
}
