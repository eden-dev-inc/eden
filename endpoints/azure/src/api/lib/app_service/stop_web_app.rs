...
const API_INFO: ApiInfo<AzureApi, AppServiceStopWebAppInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppServiceStopWebApp, "Stop a web app", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceStopWebAppInputBuilder::default().resource_group("my-rg").name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_stop_web_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-app"});
        let _: AppServiceStopWebAppInput = serde_json::from_value(json).unwrap();
    }
}
