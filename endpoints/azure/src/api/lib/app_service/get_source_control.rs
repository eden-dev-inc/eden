...
const API_INFO: ApiInfo<AzureApi, AppServiceGetSourceControlInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppServiceGetSourceControl,
    "Get source control configuration for a web app",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceGetSourceControlInputBuilder::default().resource_group("my-rg").name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_get_source_control");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-app"});
        let _: AppServiceGetSourceControlInput = serde_json::from_value(json).unwrap();
    }
}
