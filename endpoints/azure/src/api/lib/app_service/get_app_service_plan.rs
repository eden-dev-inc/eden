...
const API_INFO: ApiInfo<AzureApi, AppServiceGetPlanInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppServiceGetPlan, "Get an App Service plan", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceGetPlanInputBuilder::default().resource_group("my-rg").name("my-plan").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_get_plan");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-plan"});
        let _: AppServiceGetPlanInput = serde_json::from_value(json).unwrap();
    }
}
