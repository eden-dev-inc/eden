...
const API_INFO: ApiInfo<AzureApi, AppServiceGetDeploymentSlotInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppServiceGetDeploymentSlot, "Get a deployment slot", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.name, self.slot
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceGetDeploymentSlotInputBuilder::default()
        ...
        assert_eq!(json["type"], "app_service_get_deployment_slot");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-app", "slot": "staging"});
        let _: AppServiceGetDeploymentSlotInput = serde_json::from_value(json).unwrap();
    }
}
