...
const API_INFO: ApiInfo<AzureApi, AppServiceGetWebAppInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppServiceGetWebApp, "Get a web app", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppServiceGetWebAppInputBuilder::default().resource_group("my-rg").name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_service_get_web_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-app"});
        let _: AppServiceGetWebAppInput = serde_json::from_value(json).unwrap();
    }
}
