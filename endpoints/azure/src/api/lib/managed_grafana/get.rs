...
const API_INFO: ApiInfo<AzureApi, ManagedGrafanaGetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ManagedGrafanaGet, "Get a Managed Grafana instance", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedGrafanaGetInputBuilder::default().resource_group("my-rg").grafana_name("my-grafana").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_grafana_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "grafana_name": "my-grafana"});
        let _: ManagedGrafanaGetInput = serde_json::from_value(json).unwrap();
    }
}
