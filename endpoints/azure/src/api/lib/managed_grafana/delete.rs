...
const API_INFO: ApiInfo<AzureApi, ManagedGrafanaDeleteInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedGrafanaDelete,
    "Delete a Managed Grafana instance",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedGrafanaDeleteInputBuilder::default().resource_group("my-rg").grafana_name("my-grafana").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_grafana_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "grafana_name": "my-grafana"});
        let _: ManagedGrafanaDeleteInput = serde_json::from_value(json).unwrap();
    }
}
