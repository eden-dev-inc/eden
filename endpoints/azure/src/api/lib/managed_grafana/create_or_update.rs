...
const API_INFO: ApiInfo<AzureApi, ManagedGrafanaCreateOrUpdateInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedGrafanaCreateOrUpdate,
    "Create or update a Managed Grafana instance",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.grafana_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedGrafanaCreateOrUpdateInputBuilder::default()
        ...
        assert_eq!(json["type"], "managed_grafana_create_or_update");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "grafana_name": "my-grafana", "location": "eastus", "properties": {}});
        let _: ManagedGrafanaCreateOrUpdateInput = serde_json::from_value(json).unwrap();
    }
}
