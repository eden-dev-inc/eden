...
const API_INFO: ApiInfo<AzureApi, ManagedGrafanaListAllInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedGrafanaListAll,
    "List all Managed Grafana instances",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Dashboard/grafana", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Dashboard/grafana", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedGrafanaListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_grafana_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ManagedGrafanaListAllInput = serde_json::from_value(json).unwrap();
    }
}
