...
const API_INFO: ApiInfo<AzureApi, LogicAppsListWorkflowsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::LogicAppsListWorkflows, "List Logic App workflows", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Logic/workflows", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Logic/workflows", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LogicAppsListWorkflowsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "logic_apps_list_workflows");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: LogicAppsListWorkflowsInput = serde_json::from_value(json).unwrap();
    }
}
