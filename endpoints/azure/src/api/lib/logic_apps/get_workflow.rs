...
const API_INFO: ApiInfo<AzureApi, LogicAppsGetWorkflowInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::LogicAppsGetWorkflow, "Get a Logic App workflow", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LogicAppsGetWorkflowInputBuilder::default().resource_group("my-rg").workflow_name("my-workflow").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "logic_apps_get_workflow");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workflow_name": "my-workflow"});
        let _: LogicAppsGetWorkflowInput = serde_json::from_value(json).unwrap();
    }
}
