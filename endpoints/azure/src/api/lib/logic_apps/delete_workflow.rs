...
const API_INFO: ApiInfo<AzureApi, LogicAppsDeleteWorkflowInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::LogicAppsDeleteWorkflow,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LogicAppsDeleteWorkflowInputBuilder::default().resource_group("my-rg").workflow_name("my-workflow").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "logic_apps_delete_workflow");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workflow_name": "my-workflow"});
        let _: LogicAppsDeleteWorkflowInput = serde_json::from_value(json).unwrap();
    }
}
