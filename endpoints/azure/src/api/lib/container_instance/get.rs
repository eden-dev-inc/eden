...
const API_INFO: ApiInfo<AzureApi, ContainerInstanceGetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerInstanceGet, "Get a container group", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerInstanceGetInputBuilder::default().resource_group("my-rg").container_group_name("my-cg").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_instance_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_group_name": "my-cg"});
        let _: ContainerInstanceGetInput = serde_json::from_value(json).unwrap();
    }
}
