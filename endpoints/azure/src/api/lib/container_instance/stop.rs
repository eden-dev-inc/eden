...
const API_INFO: ApiInfo<AzureApi, ContainerInstanceStopInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerInstanceStop, "Stop a container group", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerInstanceStopInputBuilder::default().resource_group("my-rg").container_group_name("my-cg").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_instance_stop");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_group_name": "my-cg"});
        let _: ContainerInstanceStopInput = serde_json::from_value(json).unwrap();
    }
}
