...
const API_INFO: ApiInfo<AzureApi, ContainerInstanceDeleteInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerInstanceDelete, "Delete a container group", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerInstanceDeleteInputBuilder::default().resource_group("my-rg").container_group_name("my-cg").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_instance_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_group_name": "my-cg"});
        let _: ContainerInstanceDeleteInput = serde_json::from_value(json).unwrap();
    }
}
