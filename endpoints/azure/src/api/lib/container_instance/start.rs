...
const API_INFO: ApiInfo<AzureApi, ContainerInstanceStartInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerInstanceStart, "Start a container group", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerInstanceStartInputBuilder::default().resource_group("my-rg").container_group_name("my-cg").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_instance_start");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_group_name": "my-cg"});
        let _: ContainerInstanceStartInput = serde_json::from_value(json).unwrap();
    }
}
