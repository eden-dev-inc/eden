...
const API_INFO: ApiInfo<AzureApi, ContainerInstanceListAllInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerInstanceListAll, "List all container groups", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.ContainerInstance/containerGroups", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.ContainerInstance/containerGroups", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerInstanceListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_instance_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ContainerInstanceListAllInput = serde_json::from_value(json).unwrap();
    }
}
