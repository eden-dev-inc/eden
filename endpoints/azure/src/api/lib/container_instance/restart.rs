...
const API_INFO: ApiInfo<AzureApi, ContainerInstanceRestartInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerInstanceRestart, "Restart a container group", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.container_group_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerInstanceRestartInputBuilder::default().resource_group("my-rg").container_group_name("my-cg").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_instance_restart");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_group_name": "my-cg"});
        let _: ContainerInstanceRestartInput = serde_json::from_value(json).unwrap();
    }
}
