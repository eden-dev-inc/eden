...
const API_INFO: ApiInfo<AzureApi, ContainerInstanceListLogsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerInstanceListLogs, "List container logs", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.container_group_name, self.container_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerInstanceListLogsInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_instance_list_logs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_group_name": "my-cg", "container_name": "my-container"});
        let _: ContainerInstanceListLogsInput = serde_json::from_value(json).unwrap();
    }
}
