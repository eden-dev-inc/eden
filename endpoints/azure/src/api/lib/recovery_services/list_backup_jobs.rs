...
const API_INFO: ApiInfo<AzureApi, RecoveryServicesListBackupJobsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RecoveryServicesListBackupJobs,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RecoveryServicesListBackupJobsInputBuilder::default().resource_group("my-rg").vault_name("my-vault").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "recovery_services_list_backup_jobs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vault_name": "my-vault"});
        let _: RecoveryServicesListBackupJobsInput = serde_json::from_value(json).unwrap();
    }
}
