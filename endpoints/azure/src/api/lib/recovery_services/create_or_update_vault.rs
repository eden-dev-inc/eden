...
const API_INFO: ApiInfo<AzureApi, RecoveryServicesCreateOrUpdateVaultInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RecoveryServicesCreateOrUpdateVault,
    "Create or update a Recovery Services vault",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RecoveryServicesCreateOrUpdateVaultInputBuilder::default()
        ...
        assert_eq!(json["type"], "recovery_services_create_or_update_vault");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vault_name": "my-vault", "location": "eastus", "properties": {}});
        let _: RecoveryServicesCreateOrUpdateVaultInput = serde_json::from_value(json).unwrap();
    }
}
