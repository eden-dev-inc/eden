...
const API_INFO: ApiInfo<AzureApi, RecoveryServicesDeleteVaultInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RecoveryServicesDeleteVault,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RecoveryServicesDeleteVaultInputBuilder::default().resource_group("my-rg").vault_name("my-vault").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "recovery_services_delete_vault");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vault_name": "my-vault"});
        let _: RecoveryServicesDeleteVaultInput = serde_json::from_value(json).unwrap();
    }
}
