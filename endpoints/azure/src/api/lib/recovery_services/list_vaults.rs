...
const API_INFO: ApiInfo<AzureApi, RecoveryServicesListVaultsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RecoveryServicesListVaults,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.RecoveryServices/vaults", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.RecoveryServices/vaults", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RecoveryServicesListVaultsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "recovery_services_list_vaults");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: RecoveryServicesListVaultsInput = serde_json::from_value(json).unwrap();
    }
}
