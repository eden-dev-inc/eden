...
const API_INFO: ApiInfo<AzureApi, RecoveryServicesListBackupItemsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RecoveryServicesListBackupItems,
    "List backup protected items in a vault",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.RecoveryServices/vaults/{}/backupFabrics/Azure/protectedItems",
            sub, self.resource_group, self.vault_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RecoveryServicesListBackupItemsInputBuilder::default().resource_group("my-rg").vault_name("my-vault").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "recovery_services_list_backup_items");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vault_name": "my-vault"});
        let _: RecoveryServicesListBackupItemsInput = serde_json::from_value(json).unwrap();
    }
}
