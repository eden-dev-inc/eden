...
const API_INFO: ApiInfo<AzureApi, RecoveryServicesListBackupPoliciesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RecoveryServicesListBackupPolicies,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RecoveryServicesListBackupPoliciesInputBuilder::default()
            .resource_group("my-rg")
            .vault_name("my-vault")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "recovery_services_list_backup_policies");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vault_name": "my-vault"});
        let _: RecoveryServicesListBackupPoliciesInput = serde_json::from_value(json).unwrap();
    }
}
