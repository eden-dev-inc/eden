...
const API_INFO: ApiInfo<AzureApi, RecoveryServicesGetVaultInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RecoveryServicesGetVault,
    "Get a Recovery Services vault",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RecoveryServicesGetVaultInputBuilder::default().resource_group("my-rg").vault_name("my-vault").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "recovery_services_get_vault");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vault_name": "my-vault"});
        let _: RecoveryServicesGetVaultInput = serde_json::from_value(json).unwrap();
    }
}
