...
const API_INFO: ApiInfo<AzureApi, ApimListCertificatesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ApimListCertificates,
    "List certificates in an API Management service",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimListCertificatesInputBuilder::default().resource_group("my-rg").service_name("my-apim").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "apim_list_certificates");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim"});
        let _: ApimListCertificatesInput = serde_json::from_value(json).unwrap();
    }
}
