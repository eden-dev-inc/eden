...
const API_INFO: ApiInfo<AzureApi, ApimListDiagnosticsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ApimListDiagnostics,
    "List diagnostics in an API Management service",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimListDiagnosticsInputBuilder::default().resource_group("my-rg").service_name("my-apim").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "apim_list_diagnostics");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim"});
        let _: ApimListDiagnosticsInput = serde_json::from_value(json).unwrap();
    }
}
