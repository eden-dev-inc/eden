...
const API_INFO: ApiInfo<AzureApi, ApimGetApiInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ApimGetApi,
    "Get an API from an API Management service",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimGetApiInputBuilder::default().resource_group("my-rg").service_name("my-apim").api_id("my-api").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "apim_get_api");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim", "api_id": "my-api"});
        let _: ApimGetApiInput = serde_json::from_value(json).unwrap();
    }
}
