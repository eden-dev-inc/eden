...
const API_INFO: ApiInfo<AzureApi, ApimListProductsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ApimListProducts,
    "List products in an API Management service",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimListProductsInputBuilder::default().resource_group("my-rg").service_name("my-apim").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "apim_list_products");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim"});
        let _: ApimListProductsInput = serde_json::from_value(json).unwrap();
    }
}
