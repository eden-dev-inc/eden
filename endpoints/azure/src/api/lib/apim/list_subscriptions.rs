...
const API_INFO: ApiInfo<AzureApi, ApimListSubscriptionsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ApimListSubscriptions,
    "List subscriptions in an API Management service",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimListSubscriptionsInputBuilder::default().resource_group("my-rg").service_name("my-apim").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "apim_list_subscriptions");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim"});
        let _: ApimListSubscriptionsInput = serde_json::from_value(json).unwrap();
    }
}
