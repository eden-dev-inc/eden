...
const API_INFO: ApiInfo<AzureApi, ApimDeleteProductInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ApimDeleteProduct, "Delete a product", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.service_name, self.product_id
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimDeleteProductInputBuilder::default()
        ...
        assert_eq!(json["type"], "apim_delete_product");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim", "product_id": "my-product"});
        let _: ApimDeleteProductInput = serde_json::from_value(json).unwrap();
    }
}
