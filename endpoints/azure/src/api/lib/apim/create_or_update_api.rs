...
const API_INFO: ApiInfo<AzureApi, ApimCreateOrUpdateApiInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ApimCreateOrUpdateApi, "Create or update an API", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimCreateOrUpdateApiInputBuilder::default()
        ...
        assert_eq!(json["type"], "apim_create_or_update_api");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim", "api_id": "my-api"});
        let _: ApimCreateOrUpdateApiInput = serde_json::from_value(json).unwrap();
    }
}
