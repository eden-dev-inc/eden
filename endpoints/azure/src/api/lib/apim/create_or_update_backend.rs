...
const API_INFO: ApiInfo<AzureApi, ApimCreateOrUpdateBackendInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ApimCreateOrUpdateBackend,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.service_name, self.backend_id
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimCreateOrUpdateBackendInputBuilder::default()
        ...
        assert_eq!(json["type"], "apim_create_or_update_backend");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim", "backend_id": "my-backend"});
        let _: ApimCreateOrUpdateBackendInput = serde_json::from_value(json).unwrap();
    }
}
