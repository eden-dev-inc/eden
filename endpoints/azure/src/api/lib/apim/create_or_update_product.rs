...
const API_INFO: ApiInfo<AzureApi, ApimCreateOrUpdateProductInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ApimCreateOrUpdateProduct,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimCreateOrUpdateProductInputBuilder::default()
        ...
        assert_eq!(json["type"], "apim_create_or_update_product");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim", "product_id": "my-product"});
        let _: ApimCreateOrUpdateProductInput = serde_json::from_value(json).unwrap();
    }
}
