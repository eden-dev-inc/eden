...
const API_INFO: ApiInfo<AzureApi, ApimDeleteApiInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ApimDeleteApi, "Delete an API", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.service_name, self.api_id
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimDeleteApiInputBuilder::default().resource_group("my-rg").service_name("my-apim").api_id("my-api").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "apim_delete_api");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim", "api_id": "my-api"});
        let _: ApimDeleteApiInput = serde_json::from_value(json).unwrap();
    }
}
