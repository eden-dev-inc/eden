...
const API_INFO: ApiInfo<AzureApi, ApimGetServiceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ApimGetService, "Get an API Management service", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimGetServiceInputBuilder::default().resource_group("my-rg").service_name("my-apim").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "apim_get_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim"});
        let _: ApimGetServiceInput = serde_json::from_value(json).unwrap();
    }
}
