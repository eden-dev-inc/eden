...
const API_INFO: ApiInfo<AzureApi, ApimListNamedValuesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ApimListNamedValues,
    "List named values in an API Management service",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimListNamedValuesInputBuilder::default().resource_group("my-rg").service_name("my-apim").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "apim_list_named_values");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim"});
        let _: ApimListNamedValuesInput = serde_json::from_value(json).unwrap();
    }
}
