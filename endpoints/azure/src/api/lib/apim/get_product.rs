...
const API_INFO: ApiInfo<AzureApi, ApimGetProductInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ApimGetProduct,
    "Get a product from an API Management service",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimGetProductInputBuilder::default()
        ...
        assert_eq!(json["type"], "apim_get_product");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim", "product_id": "my-product"});
        let _: ApimGetProductInput = serde_json::from_value(json).unwrap();
    }
}
