...
const API_INFO: ApiInfo<AzureApi, ApimDeleteServiceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ApimDeleteService, "Delete an API Management service", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimDeleteServiceInputBuilder::default().resource_group("my-rg").service_name("my-apim").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "apim_delete_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim"});
        let _: ApimDeleteServiceInput = serde_json::from_value(json).unwrap();
    }
}
