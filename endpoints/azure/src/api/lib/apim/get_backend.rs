...
const API_INFO: ApiInfo<AzureApi, ApimGetBackendInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ApimGetBackend, "Get a backend", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ApimGetBackendInputBuilder::default()
        ...
        assert_eq!(json["type"], "apim_get_backend");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-apim", "backend_id": "my-backend"});
        let _: ApimGetBackendInput = serde_json::from_value(json).unwrap();
    }
}
