...
const API_INFO: ApiInfo<AzureApi, DnsDeleteRecordSetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DnsDeleteRecordSet, "Delete a DNS record set", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DnsDeleteRecordSetInputBuilder::default()
        ...
        assert_eq!(json["type"], "dns_delete_record_set");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "zone_name": "example.com", "record_type": "A", "record_name": "www"});
        let _: DnsDeleteRecordSetInput = serde_json::from_value(json).unwrap();
    }
}
