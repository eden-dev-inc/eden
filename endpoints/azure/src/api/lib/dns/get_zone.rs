...
const API_INFO: ApiInfo<AzureApi, DnsGetZoneInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DnsGetZone, "Get a DNS zone", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DnsGetZoneInputBuilder::default().resource_group("my-rg").zone_name("example.com").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "dns_get_zone");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "zone_name": "example.com"});
        let _: DnsGetZoneInput = serde_json::from_value(json).unwrap();
    }
}
