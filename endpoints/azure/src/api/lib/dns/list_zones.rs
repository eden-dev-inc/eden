...
const API_INFO: ApiInfo<AzureApi, DnsListZonesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DnsListZones, "List DNS zones", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/dnszones", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/dnszones", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DnsListZonesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "dns_list_zones");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DnsListZonesInput = serde_json::from_value(json).unwrap();
    }
}
