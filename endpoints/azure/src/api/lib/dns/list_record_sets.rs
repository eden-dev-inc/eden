...
const API_INFO: ApiInfo<AzureApi, DnsListRecordSetsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DnsListRecordSets, "List DNS record sets", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DnsListRecordSetsInputBuilder::default().resource_group("my-rg").zone_name("example.com").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "dns_list_record_sets");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "zone_name": "example.com"});
        let _: DnsListRecordSetsInput = serde_json::from_value(json).unwrap();
    }
}
