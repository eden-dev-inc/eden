...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryRegenerateCredentialInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryRegenerateCredential,
    "Regenerate a login credential for a container registry",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryRegenerateCredentialInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_registry_regenerate_credential");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry", "password_name": "password"});
        let _: ContainerRegistryRegenerateCredentialInput = serde_json::from_value(json).unwrap();
    }
}
