...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryGetTaskInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryGetTask,
    "Get a container registry task",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryGetTaskInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_registry_get_task");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "my-registry", "task_name": "my-task"});
        let _: ContainerRegistryGetTaskInput = serde_json::from_value(json).unwrap();
    }
}
