...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryCreateOrUpdateWebhookInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryCreateOrUpdateWebhook,
    "Create or update a webhook for a container registry",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.registry_name, self.webhook_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryCreateOrUpdateWebhookInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_registry_create_or_update_webhook");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry", "webhook_name": "mywebhook", "location": "eastus"});
        let _: ContainerRegistryCreateOrUpdateWebhookInput = serde_json::from_value(json).unwrap();
    }
}
