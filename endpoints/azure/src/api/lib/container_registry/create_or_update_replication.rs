...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryCreateOrUpdateReplicationInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryCreateOrUpdateReplication,
    "Create or update a replication for a container registry",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryCreateOrUpdateReplicationInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_registry_create_or_update_replication");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry", "replication_name": "eastus", "location": "eastus"});
        let _: ContainerRegistryCreateOrUpdateReplicationInput = serde_json::from_value(json).unwrap();
    }
}
