...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryListRegistriesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryListRegistries,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.ContainerRegistry/registries", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.ContainerRegistry/registries", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryListRegistriesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_registry_list_registries");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ContainerRegistryListRegistriesInput = serde_json::from_value(json).unwrap();
    }
}
