...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryDeleteTaskInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryDeleteTask,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryDeleteTaskInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_registry_delete_task");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "my-registry", "task_name": "my-task"});
        let _: ContainerRegistryDeleteTaskInput = serde_json::from_value(json).unwrap();
    }
}
