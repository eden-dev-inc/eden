...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryDeleteRegistryInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryDeleteRegistry,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryDeleteRegistryInputBuilder::default()
            .resource_group("my-rg")
            .registry_name("myregistry")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_registry_delete_registry");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry"});
        let _: ContainerRegistryDeleteRegistryInput = serde_json::from_value(json).unwrap();
    }
}
