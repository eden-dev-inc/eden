...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryGetWebhookInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryGetWebhook,
    "Get a webhook for a container registry",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryGetWebhookInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_registry_get_webhook");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry", "webhook_name": "mywebhook"});
        let _: ContainerRegistryGetWebhookInput = serde_json::from_value(json).unwrap();
    }
}
