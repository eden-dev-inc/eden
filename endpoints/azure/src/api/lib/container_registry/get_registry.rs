...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryGetRegistryInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryGetRegistry,
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            ContainerRegistryGetRegistryInputBuilder::default().resource_group("my-rg").registry_name("myregistry").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_registry_get_registry");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry"});
        let _: ContainerRegistryGetRegistryInput = serde_json::from_value(json).unwrap();
    }
}
