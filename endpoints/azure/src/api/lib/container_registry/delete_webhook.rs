...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryDeleteWebhookInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryDeleteWebhook,
    "Delete a webhook for a container registry",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryDeleteWebhookInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_registry_delete_webhook");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry", "webhook_name": "mywebhook"});
        let _: ContainerRegistryDeleteWebhookInput = serde_json::from_value(json).unwrap();
    }
}
