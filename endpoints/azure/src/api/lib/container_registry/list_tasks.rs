...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryListTasksInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryListTasks,
    "List container registry tasks",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryListTasksInputBuilder::default().resource_group("my-rg").registry_name("my-registry").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_registry_list_tasks");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "my-registry"});
        let _: ContainerRegistryListTasksInput = serde_json::from_value(json).unwrap();
    }
}
