...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryListWebhooksInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryListWebhooks,
    "List webhooks for a container registry",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryListWebhooksInputBuilder::default()
            .resource_group("my-rg")
            .registry_name("myregistry")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_registry_list_webhooks");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry"});
        let _: ContainerRegistryListWebhooksInput = serde_json::from_value(json).unwrap();
    }
}
