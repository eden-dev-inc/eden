...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryListReplicationsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryListReplications,
    "List replications for a container registry",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryListReplicationsInputBuilder::default()
            .resource_group("my-rg")
            .registry_name("myregistry")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_registry_list_replications");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry"});
        let _: ContainerRegistryListReplicationsInput = serde_json::from_value(json).unwrap();
    }
}
