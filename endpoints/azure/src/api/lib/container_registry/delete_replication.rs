...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryDeleteReplicationInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryDeleteReplication,
    "Delete a replication for a container registry",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryDeleteReplicationInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_registry_delete_replication");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry", "replication_name": "eastus"});
        let _: ContainerRegistryDeleteReplicationInput = serde_json::from_value(json).unwrap();
    }
}
