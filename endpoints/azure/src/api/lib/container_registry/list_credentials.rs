...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryListCredentialsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryListCredentials,
    "List login credentials for a container registry",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryListCredentialsInputBuilder::default()
            .resource_group("my-rg")
            .registry_name("myregistry")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_registry_list_credentials");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry"});
        let _: ContainerRegistryListCredentialsInput = serde_json::from_value(json).unwrap();
    }
}
