...
const API_INFO: ApiInfo<AzureApi, ContainerRegistryListRepositoriesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRegistryListRepositories,
    "List container registry usages",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRegistryListRepositoriesInputBuilder::default()
            .resource_group("my-rg")
            .registry_name("myregistry")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_registry_list_repositories");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "registry_name": "myregistry"});
        let _: ContainerRegistryListRepositoriesInput = serde_json::from_value(json).unwrap();
    }
}
