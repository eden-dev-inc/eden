...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteVnetPeeringInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkDeleteVnetPeering, "Delete a VNet peering", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteVnetPeeringInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_delete_vnet_peering");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "virtual_network_name": "my-vnet", "peering_name": "my-peering"});
        let _: NetworkDeleteVnetPeeringInput = serde_json::from_value(json).unwrap();
    }
}
