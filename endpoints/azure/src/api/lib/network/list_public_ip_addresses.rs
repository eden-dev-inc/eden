...
const API_INFO: ApiInfo<AzureApi, NetworkListPublicIpAddressesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListPublicIpAddresses,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/publicIPAddresses", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/publicIPAddresses", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListPublicIpAddressesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_public_ip_addresses");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListPublicIpAddressesInput = serde_json::from_value(json).unwrap();
    }
}
