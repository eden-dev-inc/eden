...
const API_INFO: ApiInfo<AzureApi, NetworkGetWafPolicyInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetWafPolicy, "Get a WAF policy", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetWafPolicyInputBuilder::default().resource_group("my-rg").policy_name("my-waf-policy").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_waf_policy");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "policy_name": "my-waf-policy"});
        let _: NetworkGetWafPolicyInput = serde_json::from_value(json).unwrap();
    }
}
