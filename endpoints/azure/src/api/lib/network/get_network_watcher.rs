...
const API_INFO: ApiInfo<AzureApi, NetworkGetNetworkWatcherInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkGetNetworkWatcher,
    "Get an Azure network watcher",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetNetworkWatcherInputBuilder::default().resource_group("my-rg").network_watcher_name("my-nw").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_network_watcher");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "network_watcher_name": "my-nw"});
        let _: NetworkGetNetworkWatcherInput = serde_json::from_value(json).unwrap();
    }
}
