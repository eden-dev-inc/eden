...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteLoadBalancerInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkDeleteLoadBalancer, "Delete a load balancer", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteLoadBalancerInputBuilder::default().resource_group("my-rg").load_balancer_name("my-lb").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_load_balancer");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "load_balancer_name": "my-lb"});
        let _: NetworkDeleteLoadBalancerInput = serde_json::from_value(json).unwrap();
    }
}
