...
const API_INFO: ApiInfo<AzureApi, NetworkCreateOrUpdateExpressRouteCircuitInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkCreateOrUpdateExpressRouteCircuit,
    "Create or update an express route circuit",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.circuit_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkCreateOrUpdateExpressRouteCircuitInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_create_or_update_express_route_circuit");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "circuit_name": "my-circuit", "location": "eastus", "properties": {}});
        let _: NetworkCreateOrUpdateExpressRouteCircuitInput = serde_json::from_value(json).unwrap();
    }
}
