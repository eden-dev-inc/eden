...
const API_INFO: ApiInfo<AzureApi, NetworkListSecurityRulesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkListSecurityRules, "List security rules", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListSecurityRulesInputBuilder::default().resource_group("my-rg").nsg_name("my-nsg").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_security_rules");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "nsg_name": "my-nsg"});
        let _: NetworkListSecurityRulesInput = serde_json::from_value(json).unwrap();
    }
}
