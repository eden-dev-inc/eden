...
const API_INFO: ApiInfo<AzureApi, NetworkCreateOrUpdateSubnetInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkCreateOrUpdateSubnet,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.vnet_name, self.subnet_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkCreateOrUpdateSubnetInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_create_or_update_subnet");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vnet_name": "my-vnet", "subnet_name": "my-subnet", "properties": {}});
        let _: NetworkCreateOrUpdateSubnetInput = serde_json::from_value(json).unwrap();
    }
}
