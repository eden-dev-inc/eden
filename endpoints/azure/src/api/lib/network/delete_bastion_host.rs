...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteBastionHostInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkDeleteBastionHost, "Delete a bastion host", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.bastion_host_name
        );
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            NetworkDeleteBastionHostInputBuilder::default().resource_group("my-rg").bastion_host_name("my-bastion").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_bastion_host");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "bastion_host_name": "my-bastion"});
        let _: NetworkDeleteBastionHostInput = serde_json::from_value(json).unwrap();
    }
}
