...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteVirtualNetworkInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeleteVirtualNetwork,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteVirtualNetworkInputBuilder::default().resource_group("my-rg").vnet_name("my-vnet").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_virtual_network");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vnet_name": "my-vnet"});
        let _: NetworkDeleteVirtualNetworkInput = serde_json::from_value(json).unwrap();
    }
}
