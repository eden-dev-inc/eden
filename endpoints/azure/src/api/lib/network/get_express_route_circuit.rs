...
const API_INFO: ApiInfo<AzureApi, NetworkGetExpressRouteCircuitInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkGetExpressRouteCircuit,
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            NetworkGetExpressRouteCircuitInputBuilder::default().resource_group("my-rg").circuit_name("my-circuit").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_express_route_circuit");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "circuit_name": "my-circuit"});
        let _: NetworkGetExpressRouteCircuitInput = serde_json::from_value(json).unwrap();
    }
}
