...
const API_INFO: ApiInfo<AzureApi, NetworkGetNetworkInterfaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetNetworkInterface, "Get a network interface", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.nic_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetNetworkInterfaceInputBuilder::default().resource_group("my-rg").nic_name("my-nic").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_network_interface");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "nic_name": "my-nic"});
        let _: NetworkGetNetworkInterfaceInput = serde_json::from_value(json).unwrap();
    }
}
