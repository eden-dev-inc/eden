...
const API_INFO: ApiInfo<AzureApi, NetworkListSubnetsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListSubnets,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListSubnetsInputBuilder::default().resource_group("my-rg").vnet_name("my-vnet").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_subnets");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vnet_name": "my-vnet"});
        let _: NetworkListSubnetsInput = serde_json::from_value(json).unwrap();
    }
}
