...
const API_INFO: ApiInfo<AzureApi, NetworkListNetworkSecurityGroupsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListNetworkSecurityGroups,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/networkSecurityGroups", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/networkSecurityGroups", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListNetworkSecurityGroupsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_network_security_groups");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListNetworkSecurityGroupsInput = serde_json::from_value(json).unwrap();
    }
}
