...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteTrafficManagerProfileInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeleteTrafficManagerProfile,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteTrafficManagerProfileInputBuilder::default()
            .resource_group("my-rg")
            .profile_name("my-profile")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_traffic_manager_profile");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "profile_name": "my-profile"});
        let _: NetworkDeleteTrafficManagerProfileInput = serde_json::from_value(json).unwrap();
    }
}
