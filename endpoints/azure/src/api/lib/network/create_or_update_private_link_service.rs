...
const API_INFO: ApiInfo<AzureApi, NetworkCreateOrUpdatePrivateLinkServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkCreateOrUpdatePrivateLinkService,
    "Create or update a private link service",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.service_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkCreateOrUpdatePrivateLinkServiceInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_create_or_update_private_link_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-pls", "location": "eastus", "properties": {}});
        let _: NetworkCreateOrUpdatePrivateLinkServiceInput = serde_json::from_value(json).unwrap();
    }
}
