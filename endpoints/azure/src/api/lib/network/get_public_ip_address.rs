...
const API_INFO: ApiInfo<AzureApi, NetworkGetPublicIpAddressInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetPublicIpAddress, "Get a public IP address", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.public_ip_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetPublicIpAddressInputBuilder::default().resource_group("my-rg").public_ip_name("my-pip").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_public_ip_address");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "public_ip_name": "my-pip"});
        let _: NetworkGetPublicIpAddressInput = serde_json::from_value(json).unwrap();
    }
}
