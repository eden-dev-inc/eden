...
const API_INFO: ApiInfo<AzureApi, NetworkListLbRulesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkListLbRules, "List load balancer rules", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListLbRulesInputBuilder::default().resource_group("my-rg").load_balancer_name("my-lb").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_lb_rules");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "load_balancer_name": "my-lb"});
        let _: NetworkListLbRulesInput = serde_json::from_value(json).unwrap();
    }
}
