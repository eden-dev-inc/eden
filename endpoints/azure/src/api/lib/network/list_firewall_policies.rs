...
const API_INFO: ApiInfo<AzureApi, NetworkListFirewallPoliciesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListFirewallPolicies,
    "List Azure firewall policies",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/firewallPolicies", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/firewallPolicies", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListFirewallPoliciesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_firewall_policies");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListFirewallPoliciesInput = serde_json::from_value(json).unwrap();
    }
}
