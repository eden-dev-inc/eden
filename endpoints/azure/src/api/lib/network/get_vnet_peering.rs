...
const API_INFO: ApiInfo<AzureApi, NetworkGetVnetPeeringInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetVnetPeering, "Get a VNet peering", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetVnetPeeringInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_get_vnet_peering");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "virtual_network_name": "my-vnet", "peering_name": "my-peering"});
        let _: NetworkGetVnetPeeringInput = serde_json::from_value(json).unwrap();
    }
}
