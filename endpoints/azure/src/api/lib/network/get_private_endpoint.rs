...
const API_INFO: ApiInfo<AzureApi, NetworkGetPrivateEndpointInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetPrivateEndpoint, "Get a private endpoint", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.private_endpoint_name
        );
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            NetworkGetPrivateEndpointInputBuilder::default().resource_group("my-rg").private_endpoint_name("my-pe").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_private_endpoint");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "private_endpoint_name": "my-pe"});
        let _: NetworkGetPrivateEndpointInput = serde_json::from_value(json).unwrap();
    }
}
