...
const API_INFO: ApiInfo<AzureApi, NetworkGetNetworkWatcherTopologyInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkGetNetworkWatcherTopology,
    "Get network watcher topology",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.network_watcher_name
        );

        let body = self.properties.clone();
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetNetworkWatcherTopologyInputBuilder::default()
            .resource_group("my-rg")
            .network_watcher_name("my-nw")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_network_watcher_topology");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "network_watcher_name": "my-nw", "properties": {}});
        let _: NetworkGetNetworkWatcherTopologyInput = serde_json::from_value(json).unwrap();
    }
}
