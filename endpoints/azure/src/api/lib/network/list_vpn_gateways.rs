...
const API_INFO: ApiInfo<AzureApi, NetworkListVpnGatewaysInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkListVpnGateways, "List VPN gateways", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/virtualNetworkGateways", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/virtualNetworkGateways", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListVpnGatewaysInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_vpn_gateways");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListVpnGatewaysInput = serde_json::from_value(json).unwrap();
    }
}
