...
const API_INFO: ApiInfo<AzureApi, NetworkListAppGatewaySslCertificatesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListAppGatewaySslCertificates,
    "List application gateway SSL certificates",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListAppGatewaySslCertificatesInputBuilder::default()
            .resource_group("my-rg")
            .application_gateway_name("my-appgw")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_app_gateway_ssl_certificates");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "application_gateway_name": "my-appgw"});
        let _: NetworkListAppGatewaySslCertificatesInput = serde_json::from_value(json).unwrap();
    }
}
