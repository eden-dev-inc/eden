...
const API_INFO: ApiInfo<AzureApi, NetworkListNetworkInterfacesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListNetworkInterfaces,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/networkInterfaces", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/networkInterfaces", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListNetworkInterfacesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_network_interfaces");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListNetworkInterfacesInput = serde_json::from_value(json).unwrap();
    }
}
