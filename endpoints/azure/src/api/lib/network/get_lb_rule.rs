...
const API_INFO: ApiInfo<AzureApi, NetworkGetLbRuleInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetLbRule, "Get a load balancer rule", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.load_balancer_name, self.rule_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetLbRuleInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_get_lb_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "load_balancer_name": "my-lb", "rule_name": "my-rule"});
        let _: NetworkGetLbRuleInput = serde_json::from_value(json).unwrap();
    }
}
