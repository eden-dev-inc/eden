...
const API_INFO: ApiInfo<AzureApi, NetworkDeletePrivateDnsZoneInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeletePrivateDnsZone,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeletePrivateDnsZoneInputBuilder::default().resource_group("my-rg").zone_name("my-zone").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_private_dns_zone");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "zone_name": "my-zone"});
        let _: NetworkDeletePrivateDnsZoneInput = serde_json::from_value(json).unwrap();
    }
}
