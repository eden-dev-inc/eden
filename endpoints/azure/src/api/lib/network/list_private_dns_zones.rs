...
const API_INFO: ApiInfo<AzureApi, NetworkListPrivateDnsZonesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkListPrivateDnsZones, "List private DNS zones", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/privateDnsZones", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/privateDnsZones", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListPrivateDnsZonesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_private_dns_zones");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListPrivateDnsZonesInput = serde_json::from_value(json).unwrap();
    }
}
