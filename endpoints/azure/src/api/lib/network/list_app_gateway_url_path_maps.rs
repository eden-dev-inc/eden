...
const API_INFO: ApiInfo<AzureApi, NetworkListAppGatewayUrlPathMapsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListAppGatewayUrlPathMaps,
    "List application gateway URL path maps",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/applicationGateways/{}/urlPathMaps",
            sub, self.resource_group, self.application_gateway_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListAppGatewayUrlPathMapsInputBuilder::default()
            .resource_group("my-rg")
            .application_gateway_name("my-appgw")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_app_gateway_url_path_maps");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "application_gateway_name": "my-appgw"});
        let _: NetworkListAppGatewayUrlPathMapsInput = serde_json::from_value(json).unwrap();
    }
}
