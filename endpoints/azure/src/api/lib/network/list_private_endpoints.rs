...
const API_INFO: ApiInfo<AzureApi, NetworkListPrivateEndpointsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkListPrivateEndpoints, "List private endpoints", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/privateEndpoints", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/privateEndpoints", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListPrivateEndpointsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_private_endpoints");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListPrivateEndpointsInput = serde_json::from_value(json).unwrap();
    }
}
