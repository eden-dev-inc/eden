...
const API_INFO: ApiInfo<AzureApi, NetworkGetVpnGatewayInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetVpnGateway, "Get a VPN gateway", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetVpnGatewayInputBuilder::default().resource_group("my-rg").gateway_name("my-gw").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_vpn_gateway");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "gateway_name": "my-gw"});
        let _: NetworkGetVpnGatewayInput = serde_json::from_value(json).unwrap();
    }
}
