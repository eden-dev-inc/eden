...
const API_INFO: ApiInfo<AzureApi, NetworkDeletePrivateLinkServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeletePrivateLinkService,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeletePrivateLinkServiceInputBuilder::default().resource_group("my-rg").service_name("my-pls").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_private_link_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-pls"});
        let _: NetworkDeletePrivateLinkServiceInput = serde_json::from_value(json).unwrap();
    }
}
