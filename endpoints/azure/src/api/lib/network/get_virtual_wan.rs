...
const API_INFO: ApiInfo<AzureApi, NetworkGetVirtualWanInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetVirtualWan, "Get a virtual WAN", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetVirtualWanInputBuilder::default().resource_group("my-rg").virtual_wan_name("my-wan").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_virtual_wan");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "virtual_wan_name": "my-wan"});
        let _: NetworkGetVirtualWanInput = serde_json::from_value(json).unwrap();
    }
}
