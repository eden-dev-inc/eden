...
const API_INFO: ApiInfo<AzureApi, NetworkListNetworkWatchersInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListNetworkWatchers,
    "List Azure network watchers",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/networkWatchers", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/networkWatchers", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListNetworkWatchersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_network_watchers");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListNetworkWatchersInput = serde_json::from_value(json).unwrap();
    }
}
