...
const API_INFO: ApiInfo<AzureApi, NetworkGetVirtualNetworkInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetVirtualNetwork, "Get a virtual network", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.vnet_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetVirtualNetworkInputBuilder::default().resource_group("my-rg").vnet_name("my-vnet").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_virtual_network");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vnet_name": "my-vnet"});
        let _: NetworkGetVirtualNetworkInput = serde_json::from_value(json).unwrap();
    }
}
