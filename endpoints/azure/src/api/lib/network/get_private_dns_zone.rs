...
const API_INFO: ApiInfo<AzureApi, NetworkGetPrivateDnsZoneInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetPrivateDnsZone, "Get a private DNS zone", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetPrivateDnsZoneInputBuilder::default().resource_group("my-rg").zone_name("my-zone").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_private_dns_zone");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "zone_name": "my-zone"});
        let _: NetworkGetPrivateDnsZoneInput = serde_json::from_value(json).unwrap();
    }
}
