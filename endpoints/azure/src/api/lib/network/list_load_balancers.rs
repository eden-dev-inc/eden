...
const API_INFO: ApiInfo<AzureApi, NetworkListLoadBalancersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkListLoadBalancers, "List load balancers", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/loadBalancers", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/loadBalancers", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListLoadBalancersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_load_balancers");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListLoadBalancersInput = serde_json::from_value(json).unwrap();
    }
}
