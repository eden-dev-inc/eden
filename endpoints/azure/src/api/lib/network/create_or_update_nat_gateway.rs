...
const API_INFO: ApiInfo<AzureApi, NetworkCreateOrUpdateNatGatewayInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkCreateOrUpdateNatGateway,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkCreateOrUpdateNatGatewayInputBuilder::default()
            .resource_group("my-rg")
            .nat_gateway_name("my-natgw")
            .location("eastus")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_create_or_update_nat_gateway");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "nat_gateway_name": "my-natgw", "location": "eastus", "properties": {}});
        let _: NetworkCreateOrUpdateNatGatewayInput = serde_json::from_value(json).unwrap();
    }
}
