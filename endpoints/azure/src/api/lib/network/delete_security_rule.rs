...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteSecurityRuleInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkDeleteSecurityRule, "Delete a security rule", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.nsg_name, self.security_rule_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteSecurityRuleInputBuilder::default()
            .resource_group("my-rg")
            .nsg_name("my-nsg")
            .security_rule_name("my-rule")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_security_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "nsg_name": "my-nsg", "security_rule_name": "my-rule"});
        let _: NetworkDeleteSecurityRuleInput = serde_json::from_value(json).unwrap();
    }
}
