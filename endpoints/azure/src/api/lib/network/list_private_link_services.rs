...
const API_INFO: ApiInfo<AzureApi, NetworkListPrivateLinkServicesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListPrivateLinkServices,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/privateLinkServices", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/privateLinkServices", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListPrivateLinkServicesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_private_link_services");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListPrivateLinkServicesInput = serde_json::from_value(json).unwrap();
    }
}
