...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteFirewallPolicyInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeleteFirewallPolicy,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteFirewallPolicyInputBuilder::default()
            .resource_group("my-rg")
            .firewall_policy_name("my-fp")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_firewall_policy");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "firewall_policy_name": "my-fp"});
        let _: NetworkDeleteFirewallPolicyInput = serde_json::from_value(json).unwrap();
    }
}
