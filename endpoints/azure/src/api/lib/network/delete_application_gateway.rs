...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteApplicationGatewayInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeleteApplicationGateway,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteApplicationGatewayInputBuilder::default()
            .resource_group("my-rg")
            .application_gateway_name("my-appgw")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_application_gateway");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "application_gateway_name": "my-appgw"});
        let _: NetworkDeleteApplicationGatewayInput = serde_json::from_value(json).unwrap();
    }
}
