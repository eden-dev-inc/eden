...
const API_INFO: ApiInfo<AzureApi, NetworkGetLbProbeInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetLbProbe, "Get a load balancer probe", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.load_balancer_name, self.probe_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetLbProbeInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_get_lb_probe");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "load_balancer_name": "my-lb", "probe_name": "my-probe"});
        let _: NetworkGetLbProbeInput = serde_json::from_value(json).unwrap();
    }
}
