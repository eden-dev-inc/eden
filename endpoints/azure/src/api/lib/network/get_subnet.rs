...
const API_INFO: ApiInfo<AzureApi, NetworkGetSubnetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetSubnet, "Get a subnet", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetSubnetInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_get_subnet");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vnet_name": "my-vnet", "subnet_name": "my-subnet"});
        let _: NetworkGetSubnetInput = serde_json::from_value(json).unwrap();
    }
}
