...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteVpnGatewayInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkDeleteVpnGateway, "Delete a VPN gateway", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.gateway_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteVpnGatewayInputBuilder::default().resource_group("my-rg").gateway_name("my-gw").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_vpn_gateway");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "gateway_name": "my-gw"});
        let _: NetworkDeleteVpnGatewayInput = serde_json::from_value(json).unwrap();
    }
}
