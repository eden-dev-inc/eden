...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteSubnetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkDeleteSubnet, "Delete a subnet", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteSubnetInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_delete_subnet");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vnet_name": "my-vnet", "subnet_name": "my-subnet"});
        let _: NetworkDeleteSubnetInput = serde_json::from_value(json).unwrap();
    }
}
