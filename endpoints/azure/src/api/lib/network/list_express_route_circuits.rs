...
const API_INFO: ApiInfo<AzureApi, NetworkListExpressRouteCircuitsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListExpressRouteCircuits,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/expressRouteCircuits", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/expressRouteCircuits", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListExpressRouteCircuitsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_express_route_circuits");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListExpressRouteCircuitsInput = serde_json::from_value(json).unwrap();
    }
}
