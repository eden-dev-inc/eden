...
const API_INFO: ApiInfo<AzureApi, NetworkListLbInboundNatRulesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListLbInboundNatRules,
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            NetworkListLbInboundNatRulesInputBuilder::default().resource_group("my-rg").load_balancer_name("my-lb").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_lb_inbound_nat_rules");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "load_balancer_name": "my-lb"});
        let _: NetworkListLbInboundNatRulesInput = serde_json::from_value(json).unwrap();
    }
}
