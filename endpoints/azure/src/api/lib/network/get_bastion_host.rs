...
const API_INFO: ApiInfo<AzureApi, NetworkGetBastionHostInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetBastionHost, "Get a Bastion host", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetBastionHostInputBuilder::default().resource_group("my-rg").bastion_host_name("my-bastion").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_bastion_host");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "bastion_host_name": "my-bastion"});
        let _: NetworkGetBastionHostInput = serde_json::from_value(json).unwrap();
    }
}
