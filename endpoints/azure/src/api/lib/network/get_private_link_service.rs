...
const API_INFO: ApiInfo<AzureApi, NetworkGetPrivateLinkServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkGetPrivateLinkService,
    "Get a private link service",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetPrivateLinkServiceInputBuilder::default().resource_group("my-rg").service_name("my-pls").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_private_link_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-pls"});
        let _: NetworkGetPrivateLinkServiceInput = serde_json::from_value(json).unwrap();
    }
}
