...
const API_INFO: ApiInfo<AzureApi, NetworkListVnetPeeringsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkListVnetPeerings, "List VNet peerings", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.virtual_network_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListVnetPeeringsInputBuilder::default().resource_group("my-rg").virtual_network_name("my-vnet").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_vnet_peerings");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "virtual_network_name": "my-vnet"});
        let _: NetworkListVnetPeeringsInput = serde_json::from_value(json).unwrap();
    }
}
