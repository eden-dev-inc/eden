...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteFirewallInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkDeleteFirewall, "Delete a firewall", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.firewall_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteFirewallInputBuilder::default().resource_group("my-rg").firewall_name("my-fw").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_firewall");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "firewall_name": "my-fw"});
        let _: NetworkDeleteFirewallInput = serde_json::from_value(json).unwrap();
    }
}
