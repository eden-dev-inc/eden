...
const API_INFO: ApiInfo<AzureApi, NetworkCreateOrUpdateVpnGatewayInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkCreateOrUpdateVpnGateway,
    "Create or update a VPN gateway",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkCreateOrUpdateVpnGatewayInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_create_or_update_vpn_gateway");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "gateway_name": "my-gw", "location": "eastus", "properties": {}});
        let _: NetworkCreateOrUpdateVpnGatewayInput = serde_json::from_value(json).unwrap();
    }
}
