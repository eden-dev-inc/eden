...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteNetworkInterfaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeleteNetworkInterface,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteNetworkInterfaceInputBuilder::default()
            .resource_group("my-rg")
            .network_interface_name("my-nic")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_network_interface");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "network_interface_name": "my-nic"});
        let _: NetworkDeleteNetworkInterfaceInput = serde_json::from_value(json).unwrap();
    }
}
