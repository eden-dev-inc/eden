...
const API_INFO: ApiInfo<AzureApi, NetworkCreateOrUpdatePrivateDnsRecordSetInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkCreateOrUpdatePrivateDnsRecordSet,
    "Create or update a private DNS record set",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkCreateOrUpdatePrivateDnsRecordSetInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_create_or_update_private_dns_record_set");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "zone_name": "my-zone", "record_type": "A", "record_set_name": "my-record", "properties": {}});
        let _: NetworkCreateOrUpdatePrivateDnsRecordSetInput = serde_json::from_value(json).unwrap();
    }
}
