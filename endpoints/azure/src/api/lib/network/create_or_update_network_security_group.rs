...
const API_INFO: ApiInfo<AzureApi, NetworkCreateOrUpdateNetworkSecurityGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkCreateOrUpdateNetworkSecurityGroup,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.nsg_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkCreateOrUpdateNetworkSecurityGroupInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_create_or_update_network_security_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "nsg_name": "my-nsg", "location": "eastus", "properties": {}});
        let _: NetworkCreateOrUpdateNetworkSecurityGroupInput = serde_json::from_value(json).unwrap();
    }
}
