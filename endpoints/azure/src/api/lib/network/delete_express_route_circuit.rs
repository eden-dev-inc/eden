...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteExpressRouteCircuitInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeleteExpressRouteCircuit,
    "Delete an express route circuit",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteExpressRouteCircuitInputBuilder::default()
            .resource_group("my-rg")
            .circuit_name("my-circuit")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_express_route_circuit");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "circuit_name": "my-circuit"});
        let _: NetworkDeleteExpressRouteCircuitInput = serde_json::from_value(json).unwrap();
    }
}
