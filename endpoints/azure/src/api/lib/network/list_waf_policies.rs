...
const API_INFO: ApiInfo<AzureApi, NetworkListWafPoliciesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkListWafPolicies, "List WAF policies", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListWafPoliciesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_waf_policies");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListWafPoliciesInput = serde_json::from_value(json).unwrap();
    }
}
