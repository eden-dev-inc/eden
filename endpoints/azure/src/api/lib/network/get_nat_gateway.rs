...
const API_INFO: ApiInfo<AzureApi, NetworkGetNatGatewayInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetNatGateway, "Get a NAT gateway", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetNatGatewayInputBuilder::default().resource_group("my-rg").nat_gateway_name("my-natgw").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_nat_gateway");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "nat_gateway_name": "my-natgw"});
        let _: NetworkGetNatGatewayInput = serde_json::from_value(json).unwrap();
    }
}
