...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteVirtualWanInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkDeleteVirtualWan, "Delete a virtual WAN", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.virtual_wan_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteVirtualWanInputBuilder::default().resource_group("my-rg").virtual_wan_name("my-wan").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_virtual_wan");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "virtual_wan_name": "my-wan"});
        let _: NetworkDeleteVirtualWanInput = serde_json::from_value(json).unwrap();
    }
}
