...
const API_INFO: ApiInfo<AzureApi, NetworkGetApplicationGatewayInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkGetApplicationGateway,
    "Get an application gateway",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetApplicationGatewayInputBuilder::default()
            .resource_group("my-rg")
            .application_gateway_name("my-appgw")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_application_gateway");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "application_gateway_name": "my-appgw"});
        let _: NetworkGetApplicationGatewayInput = serde_json::from_value(json).unwrap();
    }
}
