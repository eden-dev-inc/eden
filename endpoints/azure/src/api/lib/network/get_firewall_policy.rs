...
const API_INFO: ApiInfo<AzureApi, NetworkGetFirewallPolicyInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkGetFirewallPolicy,
    "Get an Azure firewall policy",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.firewall_policy_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetFirewallPolicyInputBuilder::default().resource_group("my-rg").firewall_policy_name("my-fp").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_firewall_policy");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "firewall_policy_name": "my-fp"});
        let _: NetworkGetFirewallPolicyInput = serde_json::from_value(json).unwrap();
    }
}
