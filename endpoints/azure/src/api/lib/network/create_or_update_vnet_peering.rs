...
const API_INFO: ApiInfo<AzureApi, NetworkCreateOrUpdateVnetPeeringInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkCreateOrUpdateVnetPeering,
    "Create or update a VNet peering",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.virtual_network_name, self.peering_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkCreateOrUpdateVnetPeeringInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_create_or_update_vnet_peering");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "virtual_network_name": "my-vnet", "peering_name": "my-peering", "properties": {}});
        let _: NetworkCreateOrUpdateVnetPeeringInput = serde_json::from_value(json).unwrap();
    }
}
