...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteNatGatewayInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkDeleteNatGateway, "Delete a NAT gateway", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.nat_gateway_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteNatGatewayInputBuilder::default().resource_group("my-rg").nat_gateway_name("my-natgw").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_nat_gateway");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "nat_gateway_name": "my-natgw"});
        let _: NetworkDeleteNatGatewayInput = serde_json::from_value(json).unwrap();
    }
}
