...
const API_INFO: ApiInfo<AzureApi, NetworkGetRouteTableInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetRouteTable, "Get a route table", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.route_table_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetRouteTableInputBuilder::default().resource_group("my-rg").route_table_name("my-rt").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_route_table");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "route_table_name": "my-rt"});
        let _: NetworkGetRouteTableInput = serde_json::from_value(json).unwrap();
    }
}
