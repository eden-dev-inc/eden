...
const API_INFO: ApiInfo<AzureApi, NetworkCreateOrUpdateVirtualWanInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkCreateOrUpdateVirtualWan,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkCreateOrUpdateVirtualWanInputBuilder::default()
            .resource_group("my-rg")
            .virtual_wan_name("my-wan")
            .location("eastus")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_create_or_update_virtual_wan");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "virtual_wan_name": "my-wan", "location": "eastus", "properties": {}});
        let _: NetworkCreateOrUpdateVirtualWanInput = serde_json::from_value(json).unwrap();
    }
}
