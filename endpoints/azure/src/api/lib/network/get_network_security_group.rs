...
const API_INFO: ApiInfo<AzureApi, NetworkGetNetworkSecurityGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkGetNetworkSecurityGroup,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetNetworkSecurityGroupInputBuilder::default().resource_group("my-rg").nsg_name("my-nsg").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_network_security_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "nsg_name": "my-nsg"});
        let _: NetworkGetNetworkSecurityGroupInput = serde_json::from_value(json).unwrap();
    }
}
