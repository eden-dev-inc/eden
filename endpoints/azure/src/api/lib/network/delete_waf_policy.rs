...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteWafPolicyInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkDeleteWafPolicy, "Delete a WAF policy", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.policy_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteWafPolicyInputBuilder::default().resource_group("my-rg").policy_name("my-policy").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_waf_policy");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "policy_name": "my-policy"});
        let _: NetworkDeleteWafPolicyInput = serde_json::from_value(json).unwrap();
    }
}
