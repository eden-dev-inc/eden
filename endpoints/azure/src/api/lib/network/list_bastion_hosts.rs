...
const API_INFO: ApiInfo<AzureApi, NetworkListBastionHostsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkListBastionHosts, "List Bastion hosts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/bastionHosts", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/bastionHosts", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListBastionHostsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_bastion_hosts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListBastionHostsInput = serde_json::from_value(json).unwrap();
    }
}
