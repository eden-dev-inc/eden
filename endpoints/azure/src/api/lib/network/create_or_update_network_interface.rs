...
const API_INFO: ApiInfo<AzureApi, NetworkCreateOrUpdateNetworkInterfaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkCreateOrUpdateNetworkInterface,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.network_interface_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkCreateOrUpdateNetworkInterfaceInputBuilder::default()
        ...
        assert_eq!(json["type"], "network_create_or_update_network_interface");
    }

    #[test]
    fn deserialize_minimal() {
        let json =
            serde_json::json!({"resource_group": "my-rg", "network_interface_name": "my-nic", "location": "eastus", "properties": {}});
        let _: NetworkCreateOrUpdateNetworkInterfaceInput = serde_json::from_value(json).unwrap();
    }
}
