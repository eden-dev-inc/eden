...
const API_INFO: ApiInfo<AzureApi, NetworkDeleteNetworkWatcherInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeleteNetworkWatcher,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeleteNetworkWatcherInputBuilder::default()
            .resource_group("my-rg")
            .network_watcher_name("my-nw")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_network_watcher");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "network_watcher_name": "my-nw"});
        let _: NetworkDeleteNetworkWatcherInput = serde_json::from_value(json).unwrap();
    }
}
