...
const API_INFO: ApiInfo<AzureApi, NetworkListFlowLogsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkListFlowLogs,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListFlowLogsInputBuilder::default().resource_group("my-rg").network_watcher_name("my-nw").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_flow_logs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "network_watcher_name": "my-nw"});
        let _: NetworkListFlowLogsInput = serde_json::from_value(json).unwrap();
    }
}
