...
const API_INFO: ApiInfo<AzureApi, NetworkListVirtualNetworksInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkListVirtualNetworks, "List virtual networks", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/virtualNetworks", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/virtualNetworks", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkListVirtualNetworksInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_list_virtual_networks");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetworkListVirtualNetworksInput = serde_json::from_value(json).unwrap();
    }
}
