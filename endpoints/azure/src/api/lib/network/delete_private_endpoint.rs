...
const API_INFO: ApiInfo<AzureApi, NetworkDeletePrivateEndpointInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeletePrivateEndpoint,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeletePrivateEndpointInputBuilder::default()
            .resource_group("my-rg")
            .private_endpoint_name("my-pe")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_private_endpoint");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "private_endpoint_name": "my-pe"});
        let _: NetworkDeletePrivateEndpointInput = serde_json::from_value(json).unwrap();
    }
}
