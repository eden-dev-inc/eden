...
const API_INFO: ApiInfo<AzureApi, NetworkGetLoadBalancerInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetLoadBalancer, "Get a load balancer", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetLoadBalancerInputBuilder::default().resource_group("my-rg").load_balancer_name("my-lb").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_load_balancer");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "load_balancer_name": "my-lb"});
        let _: NetworkGetLoadBalancerInput = serde_json::from_value(json).unwrap();
    }
}
