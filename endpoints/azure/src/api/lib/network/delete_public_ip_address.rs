...
const API_INFO: ApiInfo<AzureApi, NetworkDeletePublicIpAddressInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetworkDeletePublicIpAddress,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkDeletePublicIpAddressInputBuilder::default().resource_group("my-rg").public_ip_name("my-pip").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_delete_public_ip_address");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "public_ip_name": "my-pip"});
        let _: NetworkDeletePublicIpAddressInput = serde_json::from_value(json).unwrap();
    }
}
