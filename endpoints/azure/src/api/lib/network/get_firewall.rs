...
const API_INFO: ApiInfo<AzureApi, NetworkGetFirewallInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetworkGetFirewall, "Get an Azure firewall", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetworkGetFirewallInputBuilder::default().resource_group("my-rg").firewall_name("my-fw").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "network_get_firewall");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "firewall_name": "my-fw"});
        let _: NetworkGetFirewallInput = serde_json::from_value(json).unwrap();
    }
}
