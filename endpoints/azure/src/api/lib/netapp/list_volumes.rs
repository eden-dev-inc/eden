...
const API_INFO: ApiInfo<AzureApi, NetAppListVolumesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetAppListVolumes,
    "List volumes in a NetApp capacity pool",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetAppListVolumesInputBuilder::default()
        ...
        assert_eq!(json["type"], "net_app_list_volumes");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account", "pool_name": "my-pool"});
        let _: NetAppListVolumesInput = serde_json::from_value(json).unwrap();
    }
}
