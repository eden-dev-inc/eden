...
const API_INFO: ApiInfo<AzureApi, NetAppDeleteAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetAppDeleteAccount, "Delete a NetApp account", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetAppDeleteAccountInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "net_app_delete_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: NetAppDeleteAccountInput = serde_json::from_value(json).unwrap();
    }
}
