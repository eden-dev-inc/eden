...
const API_INFO: ApiInfo<AzureApi, NetAppListPoolsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::NetAppListPools,
    "List capacity pools for a NetApp account",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetAppListPoolsInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "net_app_list_pools");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: NetAppListPoolsInput = serde_json::from_value(json).unwrap();
    }
}
