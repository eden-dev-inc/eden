...
const API_INFO: ApiInfo<AzureApi, NetAppListAccountsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetAppListAccounts, "List all NetApp accounts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.NetApp/netAppAccounts", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.NetApp/netAppAccounts", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetAppListAccountsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "net_app_list_accounts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: NetAppListAccountsInput = serde_json::from_value(json).unwrap();
    }
}
