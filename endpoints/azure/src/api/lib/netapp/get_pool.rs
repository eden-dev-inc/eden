...
const API_INFO: ApiInfo<AzureApi, NetAppGetPoolInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetAppGetPool, "Get a NetApp capacity pool", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetAppGetPoolInputBuilder::default()
        ...
        assert_eq!(json["type"], "net_app_get_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account", "pool_name": "my-pool"});
        let _: NetAppGetPoolInput = serde_json::from_value(json).unwrap();
    }
}
