...
const API_INFO: ApiInfo<AzureApi, NetAppGetAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::NetAppGetAccount, "Get a NetApp account", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = NetAppGetAccountInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "net_app_get_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: NetAppGetAccountInput = serde_json::from_value(json).unwrap();
    }
}
