...
const API_INFO: ApiInfo<AzureApi, ArcListMachinesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ArcListMachines, "List Arc machines", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.HybridCompute/machines", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.HybridCompute/machines", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ArcListMachinesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "arc_list_machines");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ArcListMachinesInput = serde_json::from_value(json).unwrap();
    }
}
