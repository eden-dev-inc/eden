...
const API_INFO: ApiInfo<AzureApi, ArcListExtensionsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ArcListExtensions, "List Arc machine extensions", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.machine_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ArcListExtensionsInputBuilder::default().resource_group("my-rg").machine_name("my-machine").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "arc_list_extensions");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "machine_name": "my-machine"});
        let _: ArcListExtensionsInput = serde_json::from_value(json).unwrap();
    }
}
