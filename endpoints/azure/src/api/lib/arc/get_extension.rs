...
const API_INFO: ApiInfo<AzureApi, ArcGetExtensionInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ArcGetExtension, "Get Arc machine extension", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ArcGetExtensionInputBuilder::default()
        ...
        assert_eq!(json["type"], "arc_get_extension");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "machine_name": "my-machine", "extension_name": "my-ext"});
        let _: ArcGetExtensionInput = serde_json::from_value(json).unwrap();
    }
}
