...
const API_INFO: ApiInfo<AzureApi, ArcGetMachineInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ArcGetMachine, "Get Arc machine", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ArcGetMachineInputBuilder::default().resource_group("my-rg").machine_name("my-machine").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "arc_get_machine");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "machine_name": "my-machine"});
        let _: ArcGetMachineInput = serde_json::from_value(json).unwrap();
    }
}
