...
const API_INFO: ApiInfo<AzureApi, ArcDeleteExtensionInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ArcDeleteExtension, "Delete Arc machine extension", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ArcDeleteExtensionInputBuilder::default()
        ...
        assert_eq!(json["type"], "arc_delete_extension");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "machine_name": "my-machine", "extension_name": "my-ext"});
        let _: ArcDeleteExtensionInput = serde_json::from_value(json).unwrap();
    }
}
