...
const API_INFO: ApiInfo<AzureApi, ArcDeleteMachineInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ArcDeleteMachine, "Delete Arc machine", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ArcDeleteMachineInputBuilder::default().resource_group("my-rg").machine_name("my-machine").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "arc_delete_machine");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "machine_name": "my-machine"});
        let _: ArcDeleteMachineInput = serde_json::from_value(json).unwrap();
    }
}
