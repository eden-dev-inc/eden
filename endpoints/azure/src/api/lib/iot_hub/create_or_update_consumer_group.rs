...
const API_VERSION: &str = "2023-06-30";

const API_INFO: ApiInfo<AzureApi, IotHubCreateOrUpdateConsumerGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::IotHubCreateOrUpdateConsumerGroup,
    "Create or update a consumer group for an IoT Hub",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.iot_hub_name, self.event_hub_endpoint_name, self.consumer_group_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubCreateOrUpdateConsumerGroupInputBuilder::default()
            .resource_group("my-rg")
            .iot_hub_name("my-hub")
            .event_hub_endpoint_name("events")
            .consumer_group_name("my-cg")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iot_hub_create_or_update_consumer_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "iot_hub_name": "my-hub", "event_hub_endpoint_name": "events", "consumer_group_name": "my-cg", "properties": {}});
        let _: IotHubCreateOrUpdateConsumerGroupInput = serde_json::from_value(json).unwrap();
    }
}
