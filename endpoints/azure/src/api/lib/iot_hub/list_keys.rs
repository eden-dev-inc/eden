...
const API_INFO: ApiInfo<AzureApi, IotHubListKeysInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::IotHubListKeys, "List keys for an IoT Hub", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubListKeysInputBuilder::default().resource_group("my-rg").iot_hub_name("my-hub").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iot_hub_list_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "iot_hub_name": "my-hub"});
        let _: IotHubListKeysInput = serde_json::from_value(json).unwrap();
    }
}
