...
const API_INFO: ApiInfo<AzureApi, IotHubGetIotHubInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::IotHubGetIotHub, "Get an IoT Hub", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubGetIotHubInputBuilder::default().resource_group("my-rg").iot_hub_name("my-hub").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iot_hub_get_iot_hub");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "iot_hub_name": "my-hub"});
        let _: IotHubGetIotHubInput = serde_json::from_value(json).unwrap();
    }
}
