...
const API_INFO: ApiInfo<AzureApi, IotHubGetQuotaMetricsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::IotHubGetQuotaMetrics, "Get IoT Hub quota metrics", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubGetQuotaMetricsInputBuilder::default().resource_group("my-rg").iot_hub_name("my-hub").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iot_hub_get_quota_metrics");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "iot_hub_name": "my-hub"});
        let _: IotHubGetQuotaMetricsInput = serde_json::from_value(json).unwrap();
    }
}
