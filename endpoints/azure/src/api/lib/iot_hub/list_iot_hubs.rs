...
const API_INFO: ApiInfo<AzureApi, IotHubListIotHubsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::IotHubListIotHubs, "List IoT Hubs", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Devices/IotHubs", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Devices/IotHubs", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubListIotHubsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iot_hub_list_iot_hubs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: IotHubListIotHubsInput = serde_json::from_value(json).unwrap();
    }
}
