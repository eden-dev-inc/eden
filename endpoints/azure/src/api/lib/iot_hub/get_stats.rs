...
const API_INFO: ApiInfo<AzureApi, IotHubGetStatsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::IotHubGetStats, "Get IoT Hub statistics", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubGetStatsInputBuilder::default().resource_group("my-rg").iot_hub_name("my-hub").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iot_hub_get_stats");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "iot_hub_name": "my-hub"});
        let _: IotHubGetStatsInput = serde_json::from_value(json).unwrap();
    }
}
