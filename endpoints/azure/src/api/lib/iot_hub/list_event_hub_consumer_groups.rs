...
const API_INFO: ApiInfo<AzureApi, IotHubListEventHubConsumerGroupsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::IotHubListEventHubConsumerGroups,
    "List IoT Hub Event Hub consumer groups",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.iot_hub_name, self.event_hub_endpoint_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubListEventHubConsumerGroupsInputBuilder::default()
        ...
        assert_eq!(json["type"], "iot_hub_list_event_hub_consumer_groups");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "iot_hub_name": "my-hub", "event_hub_endpoint_name": "events"});
        let _: IotHubListEventHubConsumerGroupsInput = serde_json::from_value(json).unwrap();
    }
}
