...
const API_INFO: ApiInfo<AzureApi, IotHubListConsumerGroupsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::IotHubListConsumerGroups,
    "List consumer groups for an IoT Hub",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Devices/IotHubs/{}/eventHubEndpoints/events/ConsumerGroups",
            sub, self.resource_group, self.iot_hub_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubListConsumerGroupsInputBuilder::default().resource_group("my-rg").iot_hub_name("my-hub").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iot_hub_list_consumer_groups");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "iot_hub_name": "my-hub"});
        let _: IotHubListConsumerGroupsInput = serde_json::from_value(json).unwrap();
    }
}
