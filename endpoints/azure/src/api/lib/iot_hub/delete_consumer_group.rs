...
const API_INFO: ApiInfo<AzureApi, IotHubDeleteConsumerGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::IotHubDeleteConsumerGroup,
    "Delete a consumer group for an IoT Hub",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubDeleteConsumerGroupInputBuilder::default()
        ...
        assert_eq!(json["type"], "iot_hub_delete_consumer_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "iot_hub_name": "my-hub", "event_hub_endpoint_name": "events", "consumer_group_name": "my-cg"});
        let _: IotHubDeleteConsumerGroupInput = serde_json::from_value(json).unwrap();
    }
}
