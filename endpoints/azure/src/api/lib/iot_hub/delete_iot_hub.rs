...
const API_INFO: ApiInfo<AzureApi, IotHubDeleteIotHubInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::IotHubDeleteIotHub, "Delete an IoT Hub", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubDeleteIotHubInputBuilder::default().resource_group("my-rg").iot_hub_name("my-hub").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iot_hub_delete_iot_hub");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "iot_hub_name": "my-hub"});
        let _: IotHubDeleteIotHubInput = serde_json::from_value(json).unwrap();
    }
}
