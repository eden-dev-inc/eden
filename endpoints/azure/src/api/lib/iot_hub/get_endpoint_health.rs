...
const API_INFO: ApiInfo<AzureApi, IotHubGetEndpointHealthInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::IotHubGetEndpointHealth, "Get IoT Hub endpoint health", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IotHubGetEndpointHealthInputBuilder::default().resource_group("my-rg").iot_hub_name("my-hub").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iot_hub_get_endpoint_health");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "iot_hub_name": "my-hub"});
        let _: IotHubGetEndpointHealthInput = serde_json::from_value(json).unwrap();
    }
}
