...
const API_INFO: ApiInfo<AzureApi, CostManagementGetBudgetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::CostManagementGetBudget, "Get budget", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Consumption/budgets/{}", sub, self.budget_name);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CostManagementGetBudgetInputBuilder::default().budget_name("my-budget").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cost_management_get_budget");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"budget_name": "my-budget"});
        let _: CostManagementGetBudgetInput = serde_json::from_value(json).unwrap();
    }
}
