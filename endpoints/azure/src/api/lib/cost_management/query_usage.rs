...
const API_INFO: ApiInfo<AzureApi, CostManagementQueryUsageInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CostManagementQueryUsage,
    "Query cost management usage",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.CostManagement/query", sub);

        let result = client.execute("POST", &path, API_VERSION, Some(&self.query_body), None).await?;
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CostManagementQueryUsageInputBuilder::default()
            .query_body(serde_json::json!({"type": "Usage", "timeframe": "MonthToDate", "dataset": {}}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cost_management_query_usage");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"query_body": {"type": "Usage", "timeframe": "MonthToDate", "dataset": {}}});
        let _: CostManagementQueryUsageInput = serde_json::from_value(json).unwrap();
    }
}
