...
const API_INFO: ApiInfo<AzureApi, CostManagementListExportsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CostManagementListExports,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.CostManagement/exports", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CostManagementListExportsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cost_management_list_exports");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: CostManagementListExportsInput = serde_json::from_value(json).unwrap();
    }
}
