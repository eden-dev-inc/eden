...
const API_INFO: ApiInfo<AzureApi, CostManagementListBudgetsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::CostManagementListBudgets, "List budgets", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Consumption/budgets", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CostManagementListBudgetsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cost_management_list_budgets");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: CostManagementListBudgetsInput = serde_json::from_value(json).unwrap();
    }
}
