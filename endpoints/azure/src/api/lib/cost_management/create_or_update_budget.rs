...
const API_INFO: ApiInfo<AzureApi, CostManagementCreateOrUpdateBudgetInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CostManagementCreateOrUpdateBudget,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CostManagementCreateOrUpdateBudgetInputBuilder::default()
            .budget_name("my-budget")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cost_management_create_or_update_budget");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"budget_name": "my-budget", "properties": {}});
        let _: CostManagementCreateOrUpdateBudgetInput = serde_json::from_value(json).unwrap();
    }
}
