...
const API_INFO: ApiInfo<AzureApi, ServiceFabricGetClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ServiceFabricGetCluster, "Get Service Fabric cluster", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceFabricGetClusterInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "service_fabric_get_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: ServiceFabricGetClusterInput = serde_json::from_value(json).unwrap();
    }
}
