...
const API_VERSION: &str = "2023-11-01-preview";

const API_INFO: ApiInfo<AzureApi, ServiceFabricCreateOrUpdateApplicationInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceFabricCreateOrUpdateApplication,
    "Create or update a Service Fabric application",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.ServiceFabric/clusters/{}/applications/{}",
            sub, self.resource_group, self.cluster_name, self.application_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceFabricCreateOrUpdateApplicationInputBuilder::default()
        ...
        assert_eq!(json["type"], "service_fabric_create_or_update_application");
    }

    #[test]
    fn deserialize_minimal() {
        let json =
            serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "application_name": "my-app", "properties": {}});
        let _: ServiceFabricCreateOrUpdateApplicationInput = serde_json::from_value(json).unwrap();
    }
}
