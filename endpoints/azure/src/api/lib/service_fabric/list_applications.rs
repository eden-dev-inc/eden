...
const API_INFO: ApiInfo<AzureApi, ServiceFabricListApplicationsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceFabricListApplications,
    "List Service Fabric applications",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.cluster_name
        );
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            ServiceFabricListApplicationsInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "service_fabric_list_applications");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: ServiceFabricListApplicationsInput = serde_json::from_value(json).unwrap();
    }
}
