...
const API_INFO: ApiInfo<AzureApi, ServiceFabricListClustersInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceFabricListClusters,
    "List Service Fabric clusters",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.ServiceFabric/clusters", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceFabricListClustersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "service_fabric_list_clusters");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ServiceFabricListClustersInput = serde_json::from_value(json).unwrap();
    }
}
