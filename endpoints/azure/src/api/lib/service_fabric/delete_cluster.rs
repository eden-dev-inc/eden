...
const API_INFO: ApiInfo<AzureApi, ServiceFabricDeleteClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceFabricDeleteCluster,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceFabricDeleteClusterInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "service_fabric_delete_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: ServiceFabricDeleteClusterInput = serde_json::from_value(json).unwrap();
    }
}
