...
const API_INFO: ApiInfo<AzureApi, ServiceFabricGetApplicationInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceFabricGetApplication,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceFabricGetApplicationInputBuilder::default()
        ...
        assert_eq!(json["type"], "service_fabric_get_application");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "application_name": "my-app"});
        let _: ServiceFabricGetApplicationInput = serde_json::from_value(json).unwrap();
    }
}
