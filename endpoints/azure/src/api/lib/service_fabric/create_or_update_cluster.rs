...
const API_INFO: ApiInfo<AzureApi, ServiceFabricCreateOrUpdateClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceFabricCreateOrUpdateCluster,
    "Create or update Service Fabric cluster",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.ServiceFabric/clusters/{}",
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceFabricCreateOrUpdateClusterInputBuilder::default()
        ...
        assert_eq!(json["type"], "service_fabric_create_or_update_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "location": "eastus", "properties": {}});
        let _: ServiceFabricCreateOrUpdateClusterInput = serde_json::from_value(json).unwrap();
    }
}
