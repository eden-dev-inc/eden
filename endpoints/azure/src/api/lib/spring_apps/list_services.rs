...
const API_INFO: ApiInfo<AzureApi, SpringAppsListServicesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SpringAppsListServices, "List Spring Apps services", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.AppPlatform/Spring", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.AppPlatform/Spring", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsListServicesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "spring_apps_list_services");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SpringAppsListServicesInput = serde_json::from_value(json).unwrap();
    }
}
