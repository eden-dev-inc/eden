...
const API_INFO: ApiInfo<AzureApi, SpringAppsDeleteServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SpringAppsDeleteService,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsDeleteServiceInputBuilder::default().resource_group("my-rg").service_name("my-svc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "spring_apps_delete_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc"});
        let _: SpringAppsDeleteServiceInput = serde_json::from_value(json).unwrap();
    }
}
