...
const API_INFO: ApiInfo<AzureApi, SpringAppsGetAppInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SpringAppsGetApp,
    "Get an app in a Spring Apps service",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsGetAppInputBuilder::default()
        ...
        assert_eq!(json["type"], "spring_apps_get_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc", "app_name": "my-app"});
        let _: SpringAppsGetAppInput = serde_json::from_value(json).unwrap();
    }
}
