...
const API_INFO: ApiInfo<AzureApi, SpringAppsListDeploymentsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SpringAppsListDeployments,
    "List deployments for a Spring Apps app",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsListDeploymentsInputBuilder::default()
        ...
        assert_eq!(json["type"], "spring_apps_list_deployments");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc", "app_name": "my-app"});
        let _: SpringAppsListDeploymentsInput = serde_json::from_value(json).unwrap();
    }
}
