...
const API_INFO: ApiInfo<AzureApi, SpringAppsCreateOrUpdateAppInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SpringAppsCreateOrUpdateApp,
    "Create or update an app in a Spring Apps service",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.service_name, self.app_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsCreateOrUpdateAppInputBuilder::default()
        ...
        assert_eq!(json["type"], "spring_apps_create_or_update_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc", "app_name": "my-app", "properties": {}});
        let _: SpringAppsCreateOrUpdateAppInput = serde_json::from_value(json).unwrap();
    }
}
