...
const API_INFO: ApiInfo<AzureApi, SpringAppsDeleteAppInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SpringAppsDeleteApp,
    "Delete an app in a Spring Apps service",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsDeleteAppInputBuilder::default()
        ...
        assert_eq!(json["type"], "spring_apps_delete_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc", "app_name": "my-app"});
        let _: SpringAppsDeleteAppInput = serde_json::from_value(json).unwrap();
    }
}
