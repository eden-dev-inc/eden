...
const API_INFO: ApiInfo<AzureApi, SpringAppsCreateOrUpdateDeploymentInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SpringAppsCreateOrUpdateDeployment,
    "Create or update a deployment for a Spring Apps app",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.service_name, self.app_name, self.deployment_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsCreateOrUpdateDeploymentInputBuilder::default()
        ...
        assert_eq!(json["type"], "spring_apps_create_or_update_deployment");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc", "app_name": "my-app", "deployment_name": "my-deploy", "properties": {}});
        let _: SpringAppsCreateOrUpdateDeploymentInput = serde_json::from_value(json).unwrap();
    }
}
