...
const API_INFO: ApiInfo<AzureApi, SpringAppsGetServiceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SpringAppsGetService, "Get a Spring Apps service", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsGetServiceInputBuilder::default().resource_group("my-rg").service_name("my-svc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "spring_apps_get_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc"});
        let _: SpringAppsGetServiceInput = serde_json::from_value(json).unwrap();
    }
}
