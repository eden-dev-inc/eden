...
const API_INFO: ApiInfo<AzureApi, SpringAppsListAppsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SpringAppsListApps,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsListAppsInputBuilder::default().resource_group("my-rg").service_name("my-svc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "spring_apps_list_apps");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc"});
        let _: SpringAppsListAppsInput = serde_json::from_value(json).unwrap();
    }
}
