...
const API_INFO: ApiInfo<AzureApi, SpringAppsCreateOrUpdateServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SpringAppsCreateOrUpdateService,
    "Create or update a Spring Apps service",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsCreateOrUpdateServiceInputBuilder::default()
        ...
        assert_eq!(json["type"], "spring_apps_create_or_update_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc", "location": "eastus", "properties": {}});
        let _: SpringAppsCreateOrUpdateServiceInput = serde_json::from_value(json).unwrap();
    }
}
