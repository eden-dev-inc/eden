...
const API_INFO: ApiInfo<AzureApi, SpringAppsDeleteDeploymentInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SpringAppsDeleteDeployment,
    "Delete a deployment for a Spring Apps app",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SpringAppsDeleteDeploymentInputBuilder::default()
        ...
        assert_eq!(json["type"], "spring_apps_delete_deployment");
    }

    #[test]
    fn deserialize_minimal() {
        let json =
            serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc", "app_name": "my-app", "deployment_name": "my-deploy"});
        let _: SpringAppsDeleteDeploymentInput = serde_json::from_value(json).unwrap();
    }
}
