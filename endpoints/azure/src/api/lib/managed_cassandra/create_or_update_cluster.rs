...
const API_INFO: ApiInfo<AzureApi, ManagedCassandraCreateOrUpdateClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedCassandraCreateOrUpdateCluster,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedCassandraCreateOrUpdateClusterInputBuilder::default()
        ...
        assert_eq!(json["type"], "managed_cassandra_create_or_update_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "location": "eastus", "properties": {}});
        let _: ManagedCassandraCreateOrUpdateClusterInput = serde_json::from_value(json).unwrap();
    }
}
