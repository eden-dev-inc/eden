...
const API_INFO: ApiInfo<AzureApi, ManagedCassandraDeleteClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedCassandraDeleteCluster,
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            ManagedCassandraDeleteClusterInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_cassandra_delete_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: ManagedCassandraDeleteClusterInput = serde_json::from_value(json).unwrap();
    }
}
