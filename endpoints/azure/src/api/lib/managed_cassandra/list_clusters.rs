...
const API_INFO: ApiInfo<AzureApi, ManagedCassandraListClustersInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedCassandraListClusters,
    "List Managed Cassandra clusters",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.DocumentDB/cassandraClusters", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.DocumentDB/cassandraClusters", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedCassandraListClustersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_cassandra_list_clusters");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ManagedCassandraListClustersInput = serde_json::from_value(json).unwrap();
    }
}
