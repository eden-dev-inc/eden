...
const API_INFO: ApiInfo<AzureApi, ManagedCassandraGetClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedCassandraGetCluster,
    "Get a Managed Cassandra cluster",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedCassandraGetClusterInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_cassandra_get_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: ManagedCassandraGetClusterInput = serde_json::from_value(json).unwrap();
    }
}
