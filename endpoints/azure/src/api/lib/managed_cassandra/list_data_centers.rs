...
const API_INFO: ApiInfo<AzureApi, ManagedCassandraListDataCentersInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedCassandraListDataCenters,
    "List Managed Cassandra data centers",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedCassandraListDataCentersInputBuilder::default()
            .resource_group("my-rg")
            .cluster_name("my-cluster")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_cassandra_list_data_centers");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: ManagedCassandraListDataCentersInput = serde_json::from_value(json).unwrap();
    }
}
