...
const API_INFO: ApiInfo<AzureApi, WebPubSubCreateOrUpdateInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::WebPubSubCreateOrUpdate,
    "Create or update Web PubSub resource",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = WebPubSubCreateOrUpdateInputBuilder::default()
        ...
        assert_eq!(json["type"], "web_pub_sub_create_or_update");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "resource_name": "my-pubsub", "location": "eastus"});
        let _: WebPubSubCreateOrUpdateInput = serde_json::from_value(json).unwrap();
    }
}
