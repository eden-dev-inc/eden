...
const API_INFO: ApiInfo<AzureApi, WebPubSubDeleteInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::WebPubSubDelete, "Delete Web PubSub resource", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = WebPubSubDeleteInputBuilder::default().resource_group("my-rg").resource_name("my-pubsub").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "web_pub_sub_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "resource_name": "my-pubsub"});
        let _: WebPubSubDeleteInput = serde_json::from_value(json).unwrap();
    }
}
