...
const API_INFO: ApiInfo<AzureApi, WebPubSubListKeysInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::WebPubSubListKeys, "List Web PubSub keys", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = WebPubSubListKeysInputBuilder::default().resource_group("my-rg").resource_name("my-pubsub").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "web_pub_sub_list_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "resource_name": "my-pubsub"});
        let _: WebPubSubListKeysInput = serde_json::from_value(json).unwrap();
    }
}
