...
const API_INFO: ApiInfo<AzureApi, WebPubSubListAllInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::WebPubSubListAll, "List Web PubSub resources", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.SignalRService/webPubSub", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = WebPubSubListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "web_pub_sub_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: WebPubSubListAllInput = serde_json::from_value(json).unwrap();
    }
}
