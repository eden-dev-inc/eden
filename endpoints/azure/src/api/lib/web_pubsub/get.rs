...
const API_INFO: ApiInfo<AzureApi, WebPubSubGetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::WebPubSubGet, "Get Web PubSub resource", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = WebPubSubGetInputBuilder::default().resource_group("my-rg").resource_name("my-pubsub").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "web_pub_sub_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "resource_name": "my-pubsub"});
        let _: WebPubSubGetInput = serde_json::from_value(json).unwrap();
    }
}
