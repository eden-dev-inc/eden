...
const API_INFO: ApiInfo<AzureApi, WebPubSubRegenerateKeyInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::WebPubSubRegenerateKey, "Regenerate a Web PubSub key", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.resource_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = WebPubSubRegenerateKeyInputBuilder::default()
            .resource_group("my-rg")
            .resource_name("my-pubsub")
            .key_type("Primary")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "web_pub_sub_regenerate_key");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "resource_name": "my-pubsub", "key_type": "Primary"});
        let _: WebPubSubRegenerateKeyInput = serde_json::from_value(json).unwrap();
    }
}
