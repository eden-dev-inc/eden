...
const API_INFO: ApiInfo<AzureApi, ManagedApplicationsListAllInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedApplicationsListAll,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Solutions/applications", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Solutions/applications", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedApplicationsListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_applications_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ManagedApplicationsListAllInput = serde_json::from_value(json).unwrap();
    }
}
