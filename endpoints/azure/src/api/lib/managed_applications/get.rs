...
const API_INFO: ApiInfo<AzureApi, ManagedApplicationsGetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ManagedApplicationsGet, "Get a managed application", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.application_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedApplicationsGetInputBuilder::default().resource_group("my-rg").application_name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_applications_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "application_name": "my-app"});
        let _: ManagedApplicationsGetInput = serde_json::from_value(json).unwrap();
    }
}
