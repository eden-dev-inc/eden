...
const API_INFO: ApiInfo<AzureApi, ManagedApplicationsDeleteInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ManagedApplicationsDelete,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ManagedApplicationsDeleteInputBuilder::default().resource_group("my-rg").application_name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "managed_applications_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "application_name": "my-app"});
        let _: ManagedApplicationsDeleteInput = serde_json::from_value(json).unwrap();
    }
}
