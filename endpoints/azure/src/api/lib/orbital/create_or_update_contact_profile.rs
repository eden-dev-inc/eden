...
const API_INFO: ApiInfo<AzureApi, OrbitalCreateOrUpdateContactProfileInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::OrbitalCreateOrUpdateContactProfile,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.contact_profile_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = OrbitalCreateOrUpdateContactProfileInputBuilder::default()
            .resource_group("my-rg")
            .contact_profile_name("my-cp")
            .location("eastus")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "orbital_create_or_update_contact_profile");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "contact_profile_name": "my-cp", "location": "eastus", "properties": {}});
        let _: OrbitalCreateOrUpdateContactProfileInput = serde_json::from_value(json).unwrap();
    }
}
