...
const API_INFO: ApiInfo<AzureApi, OrbitalDeleteSpacecraftInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::OrbitalDeleteSpacecraft, "Delete a spacecraft", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = OrbitalDeleteSpacecraftInputBuilder::default().resource_group("my-rg").spacecraft_name("my-sc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "orbital_delete_spacecraft");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "spacecraft_name": "my-sc"});
        let _: OrbitalDeleteSpacecraftInput = serde_json::from_value(json).unwrap();
    }
}
