...
const API_INFO: ApiInfo<AzureApi, OrbitalListSpacecraftsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::OrbitalListSpacecrafts, "List Orbital spacecrafts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Orbital/spacecrafts", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Orbital/spacecrafts", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = OrbitalListSpacecraftsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "orbital_list_spacecrafts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: OrbitalListSpacecraftsInput = serde_json::from_value(json).unwrap();
    }
}
