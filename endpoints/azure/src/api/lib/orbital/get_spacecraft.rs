...
const API_INFO: ApiInfo<AzureApi, OrbitalGetSpacecraftInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::OrbitalGetSpacecraft, "Get an Orbital spacecraft", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = OrbitalGetSpacecraftInputBuilder::default().resource_group("my-rg").spacecraft_name("my-sc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "orbital_get_spacecraft");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "spacecraft_name": "my-sc"});
        let _: OrbitalGetSpacecraftInput = serde_json::from_value(json).unwrap();
    }
}
