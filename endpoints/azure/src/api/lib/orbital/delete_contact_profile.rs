...
const API_INFO: ApiInfo<AzureApi, OrbitalDeleteContactProfileInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::OrbitalDeleteContactProfile,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = OrbitalDeleteContactProfileInputBuilder::default()
            .resource_group("my-rg")
            .contact_profile_name("my-cp")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "orbital_delete_contact_profile");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "contact_profile_name": "my-cp"});
        let _: OrbitalDeleteContactProfileInput = serde_json::from_value(json).unwrap();
    }
}
