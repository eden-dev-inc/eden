...
const API_INFO: ApiInfo<AzureApi, OrbitalCreateOrUpdateSpacecraftInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::OrbitalCreateOrUpdateSpacecraft,
    "Create or update a spacecraft",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.spacecraft_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = OrbitalCreateOrUpdateSpacecraftInputBuilder::default()
            .resource_group("my-rg")
            .spacecraft_name("my-sc")
            .location("eastus")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "orbital_create_or_update_spacecraft");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "spacecraft_name": "my-sc", "location": "eastus", "properties": {}});
        let _: OrbitalCreateOrUpdateSpacecraftInput = serde_json::from_value(json).unwrap();
    }
}
