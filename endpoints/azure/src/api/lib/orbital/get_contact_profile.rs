...
const API_INFO: ApiInfo<AzureApi, OrbitalGetContactProfileInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::OrbitalGetContactProfile,
    "Get an Orbital contact profile",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = OrbitalGetContactProfileInputBuilder::default().resource_group("my-rg").contact_profile_name("my-cp").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "orbital_get_contact_profile");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "contact_profile_name": "my-cp"});
        let _: OrbitalGetContactProfileInput = serde_json::from_value(json).unwrap();
    }
}
