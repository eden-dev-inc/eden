...
const API_VERSION: &str = "2024-03-01-preview";

const API_INFO: ApiInfo<AzureApi, OrbitalListContactProfilesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::OrbitalListContactProfiles,
    "List Orbital contact profiles",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Orbital/contactProfiles", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Orbital/contactProfiles", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = OrbitalListContactProfilesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "orbital_list_contact_profiles");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: OrbitalListContactProfilesInput = serde_json::from_value(json).unwrap();
    }
}
