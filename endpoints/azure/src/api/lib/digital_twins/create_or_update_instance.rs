...
const API_INFO: ApiInfo<AzureApi, DigitalTwinsCreateOrUpdateInstanceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DigitalTwinsCreateOrUpdateInstance,
    "Create or update Digital Twins instance",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DigitalTwinsCreateOrUpdateInstanceInputBuilder::default()
        ...
        assert_eq!(json["type"], "digital_twins_create_or_update_instance");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "instance_name": "my-instance", "location": "eastus"});
        let _: DigitalTwinsCreateOrUpdateInstanceInput = serde_json::from_value(json).unwrap();
    }
}
