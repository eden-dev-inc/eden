...
const API_INFO: ApiInfo<AzureApi, DigitalTwinsListInstancesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DigitalTwinsListInstances,
    "List Digital Twins instances",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.DigitalTwins/digitalTwinsInstances", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DigitalTwinsListInstancesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "digital_twins_list_instances");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DigitalTwinsListInstancesInput = serde_json::from_value(json).unwrap();
    }
}
