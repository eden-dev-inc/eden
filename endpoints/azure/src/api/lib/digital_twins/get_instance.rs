...
const API_INFO: ApiInfo<AzureApi, DigitalTwinsGetInstanceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DigitalTwinsGetInstance, "Get Digital Twins instance", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.instance_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DigitalTwinsGetInstanceInputBuilder::default().resource_group("my-rg").instance_name("my-instance").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "digital_twins_get_instance");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "instance_name": "my-instance"});
        let _: DigitalTwinsGetInstanceInput = serde_json::from_value(json).unwrap();
    }
}
