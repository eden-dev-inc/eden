...
const API_INFO: ApiInfo<AzureApi, DigitalTwinsDeleteInstanceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DigitalTwinsDeleteInstance,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DigitalTwinsDeleteInstanceInputBuilder::default().resource_group("my-rg").instance_name("my-instance").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "digital_twins_delete_instance");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "instance_name": "my-instance"});
        let _: DigitalTwinsDeleteInstanceInput = serde_json::from_value(json).unwrap();
    }
}
