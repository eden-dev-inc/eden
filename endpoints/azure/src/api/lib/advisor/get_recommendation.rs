...
const API_INFO: ApiInfo<AzureApi, AdvisorGetRecommendationInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AdvisorGetRecommendation, "Get advisor recommendation", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("{}/providers/Microsoft.Advisor/recommendations/{}", self.resource_uri, self.recommendation_id);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AdvisorGetRecommendationInputBuilder::default()
            .resource_uri("/subscriptions/sub-id")
            .recommendation_id("rec-id")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "advisor_get_recommendation");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_uri": "/subscriptions/sub-id", "recommendation_id": "rec-id"});
        let _: AdvisorGetRecommendationInput = serde_json::from_value(json).unwrap();
    }
}
