...
const API_INFO: ApiInfo<AzureApi, AdvisorListRecommendationsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AdvisorListRecommendations,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Advisor/recommendations", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AdvisorListRecommendationsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "advisor_list_recommendations");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AdvisorListRecommendationsInput = serde_json::from_value(json).unwrap();
    }
}
