...
const API_INFO: ApiInfo<AzureApi, SqlDeleteServerInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SqlDeleteServer, "Delete a SQL server", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SqlDeleteServerInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "sql_delete_server");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: SqlDeleteServerInput = serde_json::from_value(json).unwrap();
    }
}
