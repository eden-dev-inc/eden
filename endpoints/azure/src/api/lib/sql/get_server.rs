...
const API_INFO: ApiInfo<AzureApi, SqlGetServerInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SqlGetServer, "Get a SQL server", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SqlGetServerInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "sql_get_server");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: SqlGetServerInput = serde_json::from_value(json).unwrap();
    }
}
