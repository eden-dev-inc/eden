...
const API_INFO: ApiInfo<AzureApi, SqlCreateOrUpdateElasticPoolInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SqlCreateOrUpdateElasticPool,
    "Create or update an elastic pool",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.server_name, self.elastic_pool_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SqlCreateOrUpdateElasticPoolInputBuilder::default()
            .resource_group("my-rg")
            .server_name("my-server")
            .elastic_pool_name("my-pool")
            .location("eastus")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "sql_create_or_update_elastic_pool");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: SqlCreateOrUpdateElasticPoolInput = serde_json::from_value(json).unwrap();
    }
}
