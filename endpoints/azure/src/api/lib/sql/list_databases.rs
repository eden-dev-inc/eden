...
const API_INFO: ApiInfo<AzureApi, SqlListDatabasesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SqlListDatabases, "List SQL databases", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SqlListDatabasesInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "sql_list_databases");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: SqlListDatabasesInput = serde_json::from_value(json).unwrap();
    }
}
