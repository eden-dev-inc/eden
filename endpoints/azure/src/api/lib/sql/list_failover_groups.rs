...
const API_INFO: ApiInfo<AzureApi, SqlListFailoverGroupsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SqlListFailoverGroups, "List SQL failover groups", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SqlListFailoverGroupsInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "sql_list_failover_groups");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: SqlListFailoverGroupsInput = serde_json::from_value(json).unwrap();
    }
}
