...
const API_INFO: ApiInfo<AzureApi, SqlGetFailoverGroupInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SqlGetFailoverGroup, "Get a SQL failover group", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.server_name, self.failover_group_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SqlGetFailoverGroupInputBuilder::default()
            .resource_group("my-rg")
            .server_name("my-server")
            .failover_group_name("my-fg")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "sql_get_failover_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "failover_group_name": "my-fg"});
        let _: SqlGetFailoverGroupInput = serde_json::from_value(json).unwrap();
    }
}
