...
const API_INFO: ApiInfo<AzureApi, SqlCreateOrUpdateFirewallRuleInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SqlCreateOrUpdateFirewallRule,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let body = serde_json::json!({
            "properties": {
                "startIpAddress": self.start_ip_address,
                "endIpAddress": self.end_ip_address
            }
        });
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SqlCreateOrUpdateFirewallRuleInputBuilder::default()
        ...
        assert_eq!(json["type"], "sql_create_or_update_firewall_rule");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: SqlCreateOrUpdateFirewallRuleInput = serde_json::from_value(json).unwrap();
    }
}
