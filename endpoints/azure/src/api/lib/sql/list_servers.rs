...
const API_INFO: ApiInfo<AzureApi, SqlListServersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SqlListServers, "List SQL servers", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Sql/servers", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SqlListServersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "sql_list_servers");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SqlListServersInput = serde_json::from_value(json).unwrap();
    }
}
