...
const API_INFO: ApiInfo<AzureApi, SqlListFirewallRulesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SqlListFirewallRules, "List SQL server firewall rules", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SqlListFirewallRulesInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "sql_list_firewall_rules");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: SqlListFirewallRulesInput = serde_json::from_value(json).unwrap();
    }
}
