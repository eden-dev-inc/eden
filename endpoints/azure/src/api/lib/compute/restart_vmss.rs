...
const API_INFO: ApiInfo<AzureApi, ComputeRestartVmssInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeRestartVmss,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeRestartVmssInputBuilder::default().resource_group("my-rg").vmss_name("my-vmss").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_restart_vmss");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vmss_name": "my-vmss"});
        let _: ComputeRestartVmssInput = serde_json::from_value(json).unwrap();
    }
}
