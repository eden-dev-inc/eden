...
const API_INFO: ApiInfo<AzureApi, ComputeListDisksInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeListDisks, "List disks", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Compute/disks", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Compute/disks", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListDisksInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_disks");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ComputeListDisksInput = serde_json::from_value(json).unwrap();
    }
}
