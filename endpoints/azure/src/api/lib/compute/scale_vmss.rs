...
const API_INFO: ApiInfo<AzureApi, ComputeScaleVmssInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeScaleVmss, "Scale a virtual machine scale set", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.vmss_name
        );

        let body = serde_json::json!({
            "sku": {"capacity": self.capacity}
        });

        let result = client.execute("PATCH", &path, API_VERSION, Some(&body), None).await?;
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeScaleVmssInputBuilder::default().resource_group("my-rg").vmss_name("my-vmss").capacity(3).build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_scale_vmss");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vmss_name": "my-vmss", "capacity": 3});
        let _: ComputeScaleVmssInput = serde_json::from_value(json).unwrap();
    }
}
