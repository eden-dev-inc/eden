...
const API_INFO: ApiInfo<AzureApi, ComputeReimageVirtualMachineInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeReimageVirtualMachine,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeReimageVirtualMachineInputBuilder::default().resource_group("my-rg").vm_name("my-vm").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_reimage_virtual_machine");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm"});
        let _: ComputeReimageVirtualMachineInput = serde_json::from_value(json).unwrap();
    }
}
