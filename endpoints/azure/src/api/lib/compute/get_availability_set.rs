...
const API_INFO: ApiInfo<AzureApi, ComputeGetAvailabilitySetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeGetAvailabilitySet, "Get an availability set", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.availability_set_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetAvailabilitySetInputBuilder::default()
            .resource_group("my-rg")
            .availability_set_name("my-avset")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_availability_set");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "availability_set_name": "my-avset"});
        let _: ComputeGetAvailabilitySetInput = serde_json::from_value(json).unwrap();
    }
}
