...
const API_INFO: ApiInfo<AzureApi, ComputeCreateOrUpdateVmExtensionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeCreateOrUpdateVmExtension,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.vm_name, self.extension_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeCreateOrUpdateVmExtensionInputBuilder::default()
        ...
        assert_eq!(json["type"], "compute_create_or_update_vm_extension");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm", "extension_name": "my-ext", "location": "eastus", "properties": {}});
        let _: ComputeCreateOrUpdateVmExtensionInput = serde_json::from_value(json).unwrap();
    }
}
