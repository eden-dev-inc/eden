...
const API_INFO: ApiInfo<AzureApi, ComputeListGalleryImageVersionsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeListGalleryImageVersions,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.gallery_name, self.image_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListGalleryImageVersionsInputBuilder::default()
        ...
        assert_eq!(json["type"], "compute_list_gallery_image_versions");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "gallery_name": "my-gallery", "image_name": "my-image"});
        let _: ComputeListGalleryImageVersionsInput = serde_json::from_value(json).unwrap();
    }
}
