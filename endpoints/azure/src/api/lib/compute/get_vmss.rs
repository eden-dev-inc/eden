...
const API_INFO: ApiInfo<AzureApi, ComputeGetVmssInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeGetVmss, "Get a virtual machine scale set", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetVmssInputBuilder::default().resource_group("my-rg").vmss_name("my-vmss").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_vmss");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vmss_name": "my-vmss"});
        let _: ComputeGetVmssInput = serde_json::from_value(json).unwrap();
    }
}
