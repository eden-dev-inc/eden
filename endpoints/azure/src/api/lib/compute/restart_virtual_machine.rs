...
const API_INFO: ApiInfo<AzureApi, ComputeRestartVirtualMachineInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeRestartVirtualMachine,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeRestartVirtualMachineInputBuilder::default().resource_group("my-rg").vm_name("my-vm").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_restart_virtual_machine");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm"});
        let _: ComputeRestartVirtualMachineInput = serde_json::from_value(json).unwrap();
    }
}
