...
const API_INFO: ApiInfo<AzureApi, ComputeDeleteVmssInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeDeleteVmss,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeDeleteVmssInputBuilder::default().resource_group("my-rg").vmss_name("my-vmss").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_delete_vmss");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vmss_name": "my-vmss"});
        let _: ComputeDeleteVmssInput = serde_json::from_value(json).unwrap();
    }
}
