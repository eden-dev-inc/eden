...
const API_INFO: ApiInfo<AzureApi, ComputeGetCapacityReservationGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeGetCapacityReservationGroup,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.capacity_reservation_group_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetCapacityReservationGroupInputBuilder::default()
            .resource_group("my-rg")
            .capacity_reservation_group_name("my-crg")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_capacity_reservation_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "capacity_reservation_group_name": "my-crg"});
        let _: ComputeGetCapacityReservationGroupInput = serde_json::from_value(json).unwrap();
    }
}
