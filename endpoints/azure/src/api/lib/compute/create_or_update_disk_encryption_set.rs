...
const API_INFO: ApiInfo<AzureApi, ComputeCreateOrUpdateDiskEncryptionSetInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeCreateOrUpdateDiskEncryptionSet,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.disk_encryption_set_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeCreateOrUpdateDiskEncryptionSetInputBuilder::default()
        ...
        assert_eq!(json["type"], "compute_create_or_update_disk_encryption_set");
    }

    #[test]
    fn deserialize_minimal() {
        let json =
            serde_json::json!({"resource_group": "my-rg", "disk_encryption_set_name": "my-des", "location": "eastus", "properties": {}});
        let _: ComputeCreateOrUpdateDiskEncryptionSetInput = serde_json::from_value(json).unwrap();
    }
}
