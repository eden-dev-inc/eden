...
const API_INFO: ApiInfo<AzureApi, ComputeStartVmssInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeStartVmss, "Start a virtual machine scale set", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeStartVmssInputBuilder::default().resource_group("my-rg").vmss_name("my-vmss").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_start_vmss");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vmss_name": "my-vmss"});
        let _: ComputeStartVmssInput = serde_json::from_value(json).unwrap();
    }
}
