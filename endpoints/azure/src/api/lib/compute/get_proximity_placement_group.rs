...
const API_INFO: ApiInfo<AzureApi, ComputeGetProximityPlacementGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeGetProximityPlacementGroup,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.proximity_placement_group_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetProximityPlacementGroupInputBuilder::default()
            .resource_group("my-rg")
            .proximity_placement_group_name("my-ppg")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_proximity_placement_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "proximity_placement_group_name": "my-ppg"});
        let _: ComputeGetProximityPlacementGroupInput = serde_json::from_value(json).unwrap();
    }
}
