...
const API_INFO: ApiInfo<AzureApi, ComputeListVmExtensionsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeListVmExtensions, "List VM extensions", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListVmExtensionsInputBuilder::default().resource_group("my-rg").vm_name("my-vm").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_vm_extensions");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm"});
        let _: ComputeListVmExtensionsInput = serde_json::from_value(json).unwrap();
    }
}
