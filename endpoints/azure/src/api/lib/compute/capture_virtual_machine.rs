...
const API_INFO: ApiInfo<AzureApi, ComputeCaptureVirtualMachineInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeCaptureVirtualMachine,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let result = client.execute("POST", &path, API_VERSION, Some(&self.properties), None).await?;
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeCaptureVirtualMachineInputBuilder::default()
            .resource_group("my-rg")
            .vm_name("my-vm")
            .properties(serde_json::json!({"vhdPrefix": "prefix"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_capture_virtual_machine");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm", "properties": {}});
        let _: ComputeCaptureVirtualMachineInput = serde_json::from_value(json).unwrap();
    }
}
