...
const API_INFO: ApiInfo<AzureApi, ComputeGetSnapshotInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeGetSnapshot, "Get a snapshot", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetSnapshotInputBuilder::default().resource_group("my-rg").snapshot_name("my-snapshot").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_snapshot");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "snapshot_name": "my-snapshot"});
        let _: ComputeGetSnapshotInput = serde_json::from_value(json).unwrap();
    }
}
