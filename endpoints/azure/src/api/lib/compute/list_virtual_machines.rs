...
const API_INFO: ApiInfo<AzureApi, ComputeListVirtualMachinesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeListVirtualMachines, "List virtual machines", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Compute/virtualMachines", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Compute/virtualMachines", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListVirtualMachinesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_virtual_machines");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ComputeListVirtualMachinesInput = serde_json::from_value(json).unwrap();
    }
}
