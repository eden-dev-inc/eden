...
const API_INFO: ApiInfo<AzureApi, ComputeListDedicatedHostsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeListDedicatedHosts, "List dedicated hosts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.host_group_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListDedicatedHostsInputBuilder::default()
            .resource_group("my-rg")
            .host_group_name("my-host-group")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_dedicated_hosts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "host_group_name": "my-host-group"});
        let _: ComputeListDedicatedHostsInput = serde_json::from_value(json).unwrap();
    }
}
