...
const API_INFO: ApiInfo<AzureApi, ComputeStartVirtualMachineInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeStartVirtualMachine, "Start a virtual machine", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeStartVirtualMachineInputBuilder::default().resource_group("my-rg").vm_name("my-vm").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_start_virtual_machine");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm"});
        let _: ComputeStartVirtualMachineInput = serde_json::from_value(json).unwrap();
    }
}
