...
const API_INFO: ApiInfo<AzureApi, ComputeListSnapshotsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeListSnapshots, "List snapshots", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Compute/snapshots", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Compute/snapshots", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListSnapshotsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_snapshots");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ComputeListSnapshotsInput = serde_json::from_value(json).unwrap();
    }
}
