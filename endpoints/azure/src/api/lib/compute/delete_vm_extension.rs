...
const API_INFO: ApiInfo<AzureApi, ComputeDeleteVmExtensionInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeDeleteVmExtension, "Delete a VM extension", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeDeleteVmExtensionInputBuilder::default()
        ...
        assert_eq!(json["type"], "compute_delete_vm_extension");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm", "extension_name": "my-ext"});
        let _: ComputeDeleteVmExtensionInput = serde_json::from_value(json).unwrap();
    }
}
