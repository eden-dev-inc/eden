...
const API_INFO: ApiInfo<AzureApi, ComputeCreateOrUpdateDiskInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeCreateOrUpdateDisk, "Create or update a disk", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.disk_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeCreateOrUpdateDiskInputBuilder::default()
        ...
        assert_eq!(json["type"], "compute_create_or_update_disk");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "disk_name": "my-disk", "location": "eastus", "properties": {}});
        let _: ComputeCreateOrUpdateDiskInput = serde_json::from_value(json).unwrap();
    }
}
