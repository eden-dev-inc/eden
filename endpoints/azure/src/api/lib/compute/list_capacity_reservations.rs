...
const API_INFO: ApiInfo<AzureApi, ComputeListCapacityReservationGroupsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeListCapacityReservationGroups,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Compute/capacityReservationGroups", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Compute/capacityReservationGroups", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListCapacityReservationGroupsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_capacity_reservation_groups");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ComputeListCapacityReservationGroupsInput = serde_json::from_value(json).unwrap();
    }
}
