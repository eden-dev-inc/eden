...
const API_INFO: ApiInfo<AzureApi, ComputeStopVmssInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeStopVmss,
    "Stop (power off) a virtual machine scale set",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeStopVmssInputBuilder::default().resource_group("my-rg").vmss_name("my-vmss").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_stop_vmss");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vmss_name": "my-vmss"});
        let _: ComputeStopVmssInput = serde_json::from_value(json).unwrap();
    }
}
