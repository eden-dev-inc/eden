...
const API_INFO: ApiInfo<AzureApi, ComputeGetImageInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeGetImage, "Get an image", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.image_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetImageInputBuilder::default().resource_group("my-rg").image_name("my-image").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_image");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "image_name": "my-image"});
        let _: ComputeGetImageInput = serde_json::from_value(json).unwrap();
    }
}
