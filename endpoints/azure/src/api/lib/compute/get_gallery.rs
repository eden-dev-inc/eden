...
const API_INFO: ApiInfo<AzureApi, ComputeGetGalleryInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeGetGallery, "Get a gallery", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetGalleryInputBuilder::default().resource_group("my-rg").gallery_name("my-gallery").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_gallery");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "gallery_name": "my-gallery"});
        let _: ComputeGetGalleryInput = serde_json::from_value(json).unwrap();
    }
}
