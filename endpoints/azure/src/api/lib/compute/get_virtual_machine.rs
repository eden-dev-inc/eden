...
const API_INFO: ApiInfo<AzureApi, ComputeGetVirtualMachineInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeGetVirtualMachine, "Get a virtual machine", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetVirtualMachineInputBuilder::default().resource_group("my-rg").vm_name("my-vm").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_virtual_machine");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm"});
        let _: ComputeGetVirtualMachineInput = serde_json::from_value(json).unwrap();
    }
}
