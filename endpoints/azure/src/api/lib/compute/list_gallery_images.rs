...
const API_INFO: ApiInfo<AzureApi, ComputeListGalleryImagesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeListGalleryImages, "List gallery images", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.gallery_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListGalleryImagesInputBuilder::default().resource_group("my-rg").gallery_name("my-gallery").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_gallery_images");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "gallery_name": "my-gallery"});
        let _: ComputeListGalleryImagesInput = serde_json::from_value(json).unwrap();
    }
}
