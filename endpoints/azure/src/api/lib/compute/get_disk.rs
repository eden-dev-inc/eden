...
const API_INFO: ApiInfo<AzureApi, ComputeGetDiskInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeGetDisk, "Get a disk", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetDiskInputBuilder::default().resource_group("my-rg").disk_name("my-disk").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_disk");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "disk_name": "my-disk"});
        let _: ComputeGetDiskInput = serde_json::from_value(json).unwrap();
    }
}
