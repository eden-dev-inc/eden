...
const API_INFO: ApiInfo<AzureApi, ComputeDeleteDiskInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeDeleteDisk, "Delete a disk", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeDeleteDiskInputBuilder::default().resource_group("my-rg").disk_name("my-disk").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_delete_disk");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "disk_name": "my-disk"});
        let _: ComputeDeleteDiskInput = serde_json::from_value(json).unwrap();
    }
}
