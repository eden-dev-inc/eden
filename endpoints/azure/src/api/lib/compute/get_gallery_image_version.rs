...
const API_INFO: ApiInfo<AzureApi, ComputeGetGalleryImageVersionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeGetGalleryImageVersion,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.gallery_name, self.image_name, self.version_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetGalleryImageVersionInputBuilder::default()
        ...
        assert_eq!(json["type"], "compute_get_gallery_image_version");
    }

    #[test]
    fn deserialize_minimal() {
        let json =
            serde_json::json!({"resource_group": "my-rg", "gallery_name": "my-gallery", "image_name": "my-image", "version_name": "1.0.0"});
        let _: ComputeGetGalleryImageVersionInput = serde_json::from_value(json).unwrap();
    }
}
