...
const API_INFO: ApiInfo<AzureApi, ComputeStopVirtualMachineInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeStopVirtualMachine,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeStopVirtualMachineInputBuilder::default().resource_group("my-rg").vm_name("my-vm").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_stop_virtual_machine");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm"});
        let _: ComputeStopVirtualMachineInput = serde_json::from_value(json).unwrap();
    }
}
