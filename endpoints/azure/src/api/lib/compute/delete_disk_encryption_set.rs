...
const API_INFO: ApiInfo<AzureApi, ComputeDeleteDiskEncryptionSetInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeDeleteDiskEncryptionSet,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeDeleteDiskEncryptionSetInputBuilder::default()
            .resource_group("my-rg")
            .disk_encryption_set_name("my-des")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_delete_disk_encryption_set");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "disk_encryption_set_name": "my-des"});
        let _: ComputeDeleteDiskEncryptionSetInput = serde_json::from_value(json).unwrap();
    }
}
