...
const API_INFO: ApiInfo<AzureApi, ComputeGetVmExtensionInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeGetVmExtension, "Get a VM extension", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetVmExtensionInputBuilder::default()
        ...
        assert_eq!(json["type"], "compute_get_vm_extension");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm", "extension_name": "my-ext"});
        let _: ComputeGetVmExtensionInput = serde_json::from_value(json).unwrap();
    }
}
