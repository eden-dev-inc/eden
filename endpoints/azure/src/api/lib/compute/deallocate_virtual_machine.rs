...
const API_INFO: ApiInfo<AzureApi, ComputeDeallocateVirtualMachineInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeDeallocateVirtualMachine,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeDeallocateVirtualMachineInputBuilder::default().resource_group("my-rg").vm_name("my-vm").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_deallocate_virtual_machine");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm"});
        let _: ComputeDeallocateVirtualMachineInput = serde_json::from_value(json).unwrap();
    }
}
