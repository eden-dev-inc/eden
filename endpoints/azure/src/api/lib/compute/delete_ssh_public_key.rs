...
const API_INFO: ApiInfo<AzureApi, ComputeDeleteSshPublicKeyInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeDeleteSshPublicKey, "Delete an SSH public key", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeDeleteSshPublicKeyInputBuilder::default().resource_group("my-rg").ssh_public_key_name("my-key").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_delete_ssh_public_key");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "ssh_public_key_name": "my-key"});
        let _: ComputeDeleteSshPublicKeyInput = serde_json::from_value(json).unwrap();
    }
}
