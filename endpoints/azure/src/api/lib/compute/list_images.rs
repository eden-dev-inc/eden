...
const API_INFO: ApiInfo<AzureApi, ComputeListImagesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeListImages, "List images", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Compute/images", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Compute/images", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListImagesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_images");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ComputeListImagesInput = serde_json::from_value(json).unwrap();
    }
}
