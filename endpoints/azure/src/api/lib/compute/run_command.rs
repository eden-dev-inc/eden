...
const API_INFO: ApiInfo<AzureApi, ComputeRunCommandInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeRunCommand,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let mut body = serde_json::json!({"commandId": self.command_id});
        if let Some(s) = &self.script {
            body["script"] = serde_json::json!(s);
        }
        if let Some(p) = &self.parameters {
            body["parameters"] = p.clone();
        }
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeRunCommandInputBuilder::default()
        ...
        assert_eq!(json["type"], "compute_run_command");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vm_name": "my-vm", "command_id": "RunShellScript"});
        let _: ComputeRunCommandInput = serde_json::from_value(json).unwrap();
    }
}
