...
const API_INFO: ApiInfo<AzureApi, ComputeListVmSizesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeListVmSizes, "List virtual machine sizes", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Compute/locations/{}/vmSizes", sub, self.location);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListVmSizesInputBuilder::default().location("eastus").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_vm_sizes");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"location": "eastus"});
        let _: ComputeListVmSizesInput = serde_json::from_value(json).unwrap();
    }
}
