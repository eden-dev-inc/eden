...
const API_INFO: ApiInfo<AzureApi, ComputeListVmssInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeListVmss, "List virtual machine scale sets", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Compute/virtualMachineScaleSets", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Compute/virtualMachineScaleSets", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListVmssInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_vmss");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ComputeListVmssInput = serde_json::from_value(json).unwrap();
    }
}
