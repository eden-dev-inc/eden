...
const API_INFO: ApiInfo<AzureApi, ComputeCreateOrUpdateSshPublicKeyInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeCreateOrUpdateSshPublicKey,
    "Create or update an SSH public key",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.ssh_public_key_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeCreateOrUpdateSshPublicKeyInputBuilder::default()
            .resource_group("my-rg")
            .ssh_public_key_name("my-key")
            .location("eastus")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_create_or_update_ssh_public_key");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "ssh_public_key_name": "my-key", "location": "eastus", "properties": {}});
        let _: ComputeCreateOrUpdateSshPublicKeyInput = serde_json::from_value(json).unwrap();
    }
}
