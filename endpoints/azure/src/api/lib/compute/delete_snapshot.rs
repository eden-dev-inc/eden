...
const API_INFO: ApiInfo<AzureApi, ComputeDeleteSnapshotInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeDeleteSnapshot, "Delete a snapshot", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.snapshot_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeDeleteSnapshotInputBuilder::default().resource_group("my-rg").snapshot_name("my-snapshot").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_delete_snapshot");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "snapshot_name": "my-snapshot"});
        let _: ComputeDeleteSnapshotInput = serde_json::from_value(json).unwrap();
    }
}
