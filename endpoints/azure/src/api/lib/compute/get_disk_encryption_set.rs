...
const API_INFO: ApiInfo<AzureApi, ComputeGetDiskEncryptionSetInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeGetDiskEncryptionSet,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetDiskEncryptionSetInputBuilder::default()
            .resource_group("my-rg")
            .disk_encryption_set_name("my-des")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_disk_encryption_set");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "disk_encryption_set_name": "my-des"});
        let _: ComputeGetDiskEncryptionSetInput = serde_json::from_value(json).unwrap();
    }
}
