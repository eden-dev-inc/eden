...
const API_INFO: ApiInfo<AzureApi, ComputeListDiskEncryptionSetsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ComputeListDiskEncryptionSets,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Compute/diskEncryptionSets", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Compute/diskEncryptionSets", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListDiskEncryptionSetsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_disk_encryption_sets");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ComputeListDiskEncryptionSetsInput = serde_json::from_value(json).unwrap();
    }
}
