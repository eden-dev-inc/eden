...
const API_INFO: ApiInfo<AzureApi, ComputeListVmssInstancesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeListVmssInstances, "List VMSS instances", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeListVmssInstancesInputBuilder::default().resource_group("my-rg").vmss_name("my-vmss").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_list_vmss_instances");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vmss_name": "my-vmss"});
        let _: ComputeListVmssInstancesInput = serde_json::from_value(json).unwrap();
    }
}
