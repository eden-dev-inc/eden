...
const API_INFO: ApiInfo<AzureApi, ComputeGetSshPublicKeyInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ComputeGetSshPublicKey, "Get an SSH public key", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ComputeGetSshPublicKeyInputBuilder::default().resource_group("my-rg").ssh_public_key_name("my-key").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "compute_get_ssh_public_key");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "ssh_public_key_name": "my-key"});
        let _: ComputeGetSshPublicKeyInput = serde_json::from_value(json).unwrap();
    }
}
