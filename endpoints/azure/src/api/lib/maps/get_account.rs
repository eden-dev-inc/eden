...
const API_INFO: ApiInfo<AzureApi, MapsGetAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MapsGetAccount, "Get a Maps account", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MapsGetAccountInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "maps_get_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: MapsGetAccountInput = serde_json::from_value(json).unwrap();
    }
}
