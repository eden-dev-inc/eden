...
const API_INFO: ApiInfo<AzureApi, MapsCreateOrUpdateAccountInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MapsCreateOrUpdateAccount,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MapsCreateOrUpdateAccountInputBuilder::default()
            .resource_group("my-rg")
            .account_name("my-account")
            .location("eastus")
            .properties(serde_json::json!({}))
            .sku(serde_json::json!({"name": "S0"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "maps_create_or_update_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account", "location": "eastus", "properties": {}, "sku": {"name": "S0"}});
        let _: MapsCreateOrUpdateAccountInput = serde_json::from_value(json).unwrap();
    }
}
