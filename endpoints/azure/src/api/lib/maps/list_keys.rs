...
const API_INFO: ApiInfo<AzureApi, MapsListKeysInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MapsListKeys, "List keys for a Maps account", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MapsListKeysInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "maps_list_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: MapsListKeysInput = serde_json::from_value(json).unwrap();
    }
}
