...
const API_VERSION: &str = "2024-01-01-preview";

const API_INFO: ApiInfo<AzureApi, MapsListAccountsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MapsListAccounts, "List Maps accounts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Maps/accounts", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Maps/accounts", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MapsListAccountsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "maps_list_accounts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: MapsListAccountsInput = serde_json::from_value(json).unwrap();
    }
}
