...
const API_INFO: ApiInfo<AzureApi, MapsDeleteAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MapsDeleteAccount, "Delete a Maps account", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MapsDeleteAccountInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "maps_delete_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: MapsDeleteAccountInput = serde_json::from_value(json).unwrap();
    }
}
