...
const API_VERSION: &str = "2023-03-15";

const API_INFO: ApiInfo<AzureApi, MigrateListProjectsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MigrateListProjects, "List Migrate projects", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Migrate/migrateProjects", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Migrate/migrateProjects", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MigrateListProjectsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "migrate_list_projects");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: MigrateListProjectsInput = serde_json::from_value(json).unwrap();
    }
}
