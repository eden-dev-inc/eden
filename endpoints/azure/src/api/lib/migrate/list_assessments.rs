...
const API_INFO: ApiInfo<AzureApi, MigrateListAssessmentsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MigrateListAssessments, "List Migrate assessments", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Migrate/migrateProjects/{}/assessments",
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MigrateListAssessmentsInputBuilder::default().resource_group("my-rg").project_name("my-project").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "migrate_list_assessments");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "project_name": "my-project"});
        let _: MigrateListAssessmentsInput = serde_json::from_value(json).unwrap();
    }
}
