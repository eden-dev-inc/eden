...
const API_INFO: ApiInfo<AzureApi, MigrateGetProjectInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MigrateGetProject, "Get Migrate project", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MigrateGetProjectInputBuilder::default().resource_group("my-rg").project_name("my-project").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "migrate_get_project");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "project_name": "my-project"});
        let _: MigrateGetProjectInput = serde_json::from_value(json).unwrap();
    }
}
