...
const API_VERSION: &str = "2024-03-31";

const API_INFO: ApiInfo<AzureApi, HealthcareListWorkspacesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::HealthcareListWorkspaces, "List Healthcare workspaces", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.HealthcareApis/workspaces", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.HealthcareApis/workspaces", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = HealthcareListWorkspacesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "healthcare_list_workspaces");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: HealthcareListWorkspacesInput = serde_json::from_value(json).unwrap();
    }
}
