...
const API_INFO: ApiInfo<AzureApi, HealthcareGetServiceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::HealthcareGetService, "Get a Healthcare service", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = HealthcareGetServiceInputBuilder::default().resource_group("my-rg").service_name("my-svc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "healthcare_get_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc"});
        let _: HealthcareGetServiceInput = serde_json::from_value(json).unwrap();
    }
}
