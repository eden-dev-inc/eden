...
const API_INFO: ApiInfo<AzureApi, HealthcareListServicesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::HealthcareListServices, "List Healthcare services", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.HealthcareApis/services", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.HealthcareApis/services", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = HealthcareListServicesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "healthcare_list_services");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: HealthcareListServicesInput = serde_json::from_value(json).unwrap();
    }
}
