...
const API_INFO: ApiInfo<AzureApi, HealthcareDeleteServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::HealthcareDeleteService,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = HealthcareDeleteServiceInputBuilder::default().resource_group("my-rg").service_name("my-svc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "healthcare_delete_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc"});
        let _: HealthcareDeleteServiceInput = serde_json::from_value(json).unwrap();
    }
}
