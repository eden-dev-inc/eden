...
const API_INFO: ApiInfo<AzureApi, ChaosDeleteExperimentInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ChaosDeleteExperiment, "Delete Chaos experiment", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ChaosDeleteExperimentInputBuilder::default().resource_group("my-rg").experiment_name("my-experiment").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "chaos_delete_experiment");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "experiment_name": "my-experiment"});
        let _: ChaosDeleteExperimentInput = serde_json::from_value(json).unwrap();
    }
}
