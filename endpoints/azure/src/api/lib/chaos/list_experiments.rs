...
const API_INFO: ApiInfo<AzureApi, ChaosListExperimentsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ChaosListExperiments, "List Chaos experiments", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Chaos/experiments", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Chaos/experiments", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ChaosListExperimentsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "chaos_list_experiments");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ChaosListExperimentsInput = serde_json::from_value(json).unwrap();
    }
}
