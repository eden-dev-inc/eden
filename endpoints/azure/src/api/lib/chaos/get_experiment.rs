...
const API_INFO: ApiInfo<AzureApi, ChaosGetExperimentInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ChaosGetExperiment, "Get Chaos experiment", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ChaosGetExperimentInputBuilder::default().resource_group("my-rg").experiment_name("my-experiment").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "chaos_get_experiment");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "experiment_name": "my-experiment"});
        let _: ChaosGetExperimentInput = serde_json::from_value(json).unwrap();
    }
}
