...
const API_INFO: ApiInfo<AzureApi, ChaosListTargetsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ChaosListTargets, "List Chaos targets", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/{}/providers/Microsoft.Chaos/targets", self.scope);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ChaosListTargetsInputBuilder::default()
            .scope("subscriptions/sub1/resourceGroups/rg1/providers/Microsoft.Compute/virtualMachines/vm1")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "chaos_list_targets");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"scope": "subscriptions/sub1/resourceGroups/rg1"});
        let _: ChaosListTargetsInput = serde_json::from_value(json).unwrap();
    }
}
