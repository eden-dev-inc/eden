...
const API_INFO: ApiInfo<AzureApi, HdInsightDeleteClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::HdInsightDeleteCluster, "Delete an HDInsight cluster", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = HdInsightDeleteClusterInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "hd_insight_delete_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: HdInsightDeleteClusterInput = serde_json::from_value(json).unwrap();
    }
}
