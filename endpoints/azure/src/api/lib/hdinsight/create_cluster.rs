...
const API_INFO: ApiInfo<AzureApi, HdInsightCreateClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::HdInsightCreateCluster, "Create an HDInsight cluster", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = HdInsightCreateClusterInputBuilder::default()
        ...
        assert_eq!(json["type"], "hd_insight_create_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "location": "eastus", "properties": {}});
        let _: HdInsightCreateClusterInput = serde_json::from_value(json).unwrap();
    }
}
