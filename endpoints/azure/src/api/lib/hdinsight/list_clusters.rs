...
const API_INFO: ApiInfo<AzureApi, HdInsightListClustersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::HdInsightListClusters, "List HDInsight clusters", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.HDInsight/clusters", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.HDInsight/clusters", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = HdInsightListClustersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "hd_insight_list_clusters");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: HdInsightListClustersInput = serde_json::from_value(json).unwrap();
    }
}
