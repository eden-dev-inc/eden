...
const API_INFO: ApiInfo<AzureApi, HdInsightGetClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::HdInsightGetCluster, "Get an HDInsight cluster", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = HdInsightGetClusterInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "hd_insight_get_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: HdInsightGetClusterInput = serde_json::from_value(json).unwrap();
    }
}
