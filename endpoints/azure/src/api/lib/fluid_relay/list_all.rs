...
const API_VERSION: &str = "2022-06-01";

const API_INFO: ApiInfo<AzureApi, FluidRelayListAllInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::FluidRelayListAll, "List Fluid Relay servers", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.FluidRelay/fluidRelayServers", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.FluidRelay/fluidRelayServers", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FluidRelayListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "fluid_relay_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: FluidRelayListAllInput = serde_json::from_value(json).unwrap();
    }
}
