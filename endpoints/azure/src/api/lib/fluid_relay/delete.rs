...
const API_INFO: ApiInfo<AzureApi, FluidRelayDeleteInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::FluidRelayDelete, "Delete a Fluid Relay server", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FluidRelayDeleteInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "fluid_relay_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: FluidRelayDeleteInput = serde_json::from_value(json).unwrap();
    }
}
