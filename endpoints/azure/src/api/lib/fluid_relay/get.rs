...
const API_INFO: ApiInfo<AzureApi, FluidRelayGetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::FluidRelayGet, "Get a Fluid Relay server", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FluidRelayGetInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "fluid_relay_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: FluidRelayGetInput = serde_json::from_value(json).unwrap();
    }
}
