...
const API_INFO: ApiInfo<AzureApi, CommunicationListServicesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CommunicationListServices,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            ...
            format!("/subscriptions/{}/providers/Microsoft.Communication/communicationServices", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CommunicationListServicesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "communication_list_services");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: CommunicationListServicesInput = serde_json::from_value(json).unwrap();
    }
}
