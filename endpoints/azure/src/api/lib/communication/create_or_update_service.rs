...
const API_INFO: ApiInfo<AzureApi, CommunicationCreateOrUpdateServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CommunicationCreateOrUpdateService,
    "Create or update a Communication Service",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CommunicationCreateOrUpdateServiceInputBuilder::default()
        ...
        assert_eq!(json["type"], "communication_create_or_update_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc", "location": "eastus", "properties": {}});
        let _: CommunicationCreateOrUpdateServiceInput = serde_json::from_value(json).unwrap();
    }
}
