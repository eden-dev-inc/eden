...
const API_INFO: ApiInfo<AzureApi, CommunicationListKeysInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CommunicationListKeys,
    "List keys for a Communication Service",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CommunicationListKeysInputBuilder::default().resource_group("my-rg").service_name("my-svc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "communication_list_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc"});
        let _: CommunicationListKeysInput = serde_json::from_value(json).unwrap();
    }
}
