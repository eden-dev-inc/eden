...
const API_INFO: ApiInfo<AzureApi, CommunicationDeleteServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CommunicationDeleteService,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CommunicationDeleteServiceInputBuilder::default().resource_group("my-rg").service_name("my-svc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "communication_delete_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc"});
        let _: CommunicationDeleteServiceInput = serde_json::from_value(json).unwrap();
    }
}
