...
const API_INFO: ApiInfo<AzureApi, CommunicationGetServiceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::CommunicationGetService, "Get a Communication Service", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CommunicationGetServiceInputBuilder::default().resource_group("my-rg").service_name("my-svc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "communication_get_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc"});
        let _: CommunicationGetServiceInput = serde_json::from_value(json).unwrap();
    }
}
