...
const API_INFO: ApiInfo<AzureApi, CommunicationRegenerateKeyInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CommunicationRegenerateKey,
    "Regenerate a key for a Communication Service",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CommunicationRegenerateKeyInputBuilder::default()
        ...
        assert_eq!(json["type"], "communication_regenerate_key");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-svc", "key_type": "Primary"});
        let _: CommunicationRegenerateKeyInput = serde_json::from_value(json).unwrap();
    }
}
