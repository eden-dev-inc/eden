...
const API_INFO: ApiInfo<AzureApi, CdnDeleteEndpointInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::CdnDeleteEndpoint, "Delete a CDN endpoint", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CdnDeleteEndpointInputBuilder::default()
        ...
        assert_eq!(json["type"], "cdn_delete_endpoint");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "profile_name": "my-profile", "endpoint_name": "my-endpoint"});
        let _: CdnDeleteEndpointInput = serde_json::from_value(json).unwrap();
    }
}
