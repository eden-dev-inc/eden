...
const API_INFO: ApiInfo<AzureApi, CdnCreateOrUpdateEndpointInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CdnCreateOrUpdateEndpoint,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CdnCreateOrUpdateEndpointInputBuilder::default()
            .resource_group("my-rg")
            .profile_name("my-profile")
            .endpoint_name("my-endpoint")
            .location("eastus")
            .properties(serde_json::json!({"origins": []}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cdn_create_or_update_endpoint");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "profile_name": "my-profile", "endpoint_name": "my-endpoint", "location": "eastus", "properties": {}});
        let _: CdnCreateOrUpdateEndpointInput = serde_json::from_value(json).unwrap();
    }
}
