...
const API_INFO: ApiInfo<AzureApi, CdnListEndpointsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::CdnListEndpoints, "List CDN endpoints", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CdnListEndpointsInputBuilder::default().resource_group("my-rg").profile_name("my-profile").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cdn_list_endpoints");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "profile_name": "my-profile"});
        let _: CdnListEndpointsInput = serde_json::from_value(json).unwrap();
    }
}
