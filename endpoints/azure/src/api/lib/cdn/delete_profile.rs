...
const API_INFO: ApiInfo<AzureApi, CdnDeleteProfileInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::CdnDeleteProfile, "Delete a CDN profile", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CdnDeleteProfileInputBuilder::default().resource_group("my-rg").profile_name("my-profile").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cdn_delete_profile");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "profile_name": "my-profile"});
        let _: CdnDeleteProfileInput = serde_json::from_value(json).unwrap();
    }
}
