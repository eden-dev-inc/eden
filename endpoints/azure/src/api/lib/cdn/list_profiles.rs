...
const API_INFO: ApiInfo<AzureApi, CdnListProfilesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::CdnListProfiles, "List CDN profiles", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Cdn/profiles", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Cdn/profiles", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CdnListProfilesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cdn_list_profiles");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: CdnListProfilesInput = serde_json::from_value(json).unwrap();
    }
}
