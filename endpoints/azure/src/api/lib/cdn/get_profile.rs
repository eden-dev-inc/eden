...
const API_INFO: ApiInfo<AzureApi, CdnGetProfileInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::CdnGetProfile, "Get a CDN profile", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CdnGetProfileInputBuilder::default().resource_group("my-rg").profile_name("my-profile").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cdn_get_profile");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "profile_name": "my-profile"});
        let _: CdnGetProfileInput = serde_json::from_value(json).unwrap();
    }
}
