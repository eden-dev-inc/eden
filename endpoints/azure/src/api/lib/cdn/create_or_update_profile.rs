...
const API_INFO: ApiInfo<AzureApi, CdnCreateOrUpdateProfileInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CdnCreateOrUpdateProfile,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CdnCreateOrUpdateProfileInputBuilder::default()
            .resource_group("my-rg")
            .profile_name("my-profile")
            .location("eastus")
            .sku(serde_json::json!({"name": "Standard_Microsoft"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cdn_create_or_update_profile");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "profile_name": "my-profile", "location": "eastus", "sku": {"name": "Standard_Microsoft"}});
        let _: CdnCreateOrUpdateProfileInput = serde_json::from_value(json).unwrap();
    }
}
