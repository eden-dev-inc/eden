...
const API_INFO: ApiInfo<AzureApi, SignalRListAllInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SignalRListAll, "List all SignalR instances", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.SignalRService/signalR", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.SignalRService/signalR", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SignalRListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "signalr_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SignalRListAllInput = serde_json::from_value(json).unwrap();
    }
}
