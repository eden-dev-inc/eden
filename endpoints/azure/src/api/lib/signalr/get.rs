...
const API_INFO: ApiInfo<AzureApi, SignalRGetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SignalRGet, "Get a SignalR instance", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SignalRGetInputBuilder::default().resource_group("my-rg").signalr_name("my-signalr").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "signalr_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "signalr_name": "my-signalr"});
        let _: SignalRGetInput = serde_json::from_value(json).unwrap();
    }
}
