...
const API_INFO: ApiInfo<AzureApi, SignalRDeleteInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SignalRDelete, "Delete a SignalR instance", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.signalr_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SignalRDeleteInputBuilder::default().resource_group("my-rg").signalr_name("my-signalr").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "signalr_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "signalr_name": "my-signalr"});
        let _: SignalRDeleteInput = serde_json::from_value(json).unwrap();
    }
}
