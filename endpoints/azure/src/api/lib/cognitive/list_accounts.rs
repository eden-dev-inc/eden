...
const API_INFO: ApiInfo<AzureApi, CognitiveListAccountsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CognitiveListAccounts,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.CognitiveServices/accounts", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.CognitiveServices/accounts", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CognitiveListAccountsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cognitive_list_accounts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: CognitiveListAccountsInput = serde_json::from_value(json).unwrap();
    }
}
