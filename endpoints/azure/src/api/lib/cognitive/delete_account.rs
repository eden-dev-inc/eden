...
const API_INFO: ApiInfo<AzureApi, CognitiveDeleteAccountInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CognitiveDeleteAccount,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CognitiveDeleteAccountInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cognitive_delete_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: CognitiveDeleteAccountInput = serde_json::from_value(json).unwrap();
    }
}
