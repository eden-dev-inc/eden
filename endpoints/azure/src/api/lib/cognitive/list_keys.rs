...
const API_INFO: ApiInfo<AzureApi, CognitiveListKeysInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CognitiveListKeys,
    "List keys for a Cognitive Services account",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CognitiveListKeysInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cognitive_list_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: CognitiveListKeysInput = serde_json::from_value(json).unwrap();
    }
}
