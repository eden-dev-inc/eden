...
const API_INFO: ApiInfo<AzureApi, CognitiveGetAccountInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CognitiveGetAccount,
    "Get a Cognitive Services account",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CognitiveGetAccountInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cognitive_get_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: CognitiveGetAccountInput = serde_json::from_value(json).unwrap();
    }
}
