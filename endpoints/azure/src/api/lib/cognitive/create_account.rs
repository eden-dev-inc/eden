...
const API_INFO: ApiInfo<AzureApi, CognitiveCreateAccountInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CognitiveCreateAccount,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CognitiveCreateAccountInputBuilder::default()
        ...
        assert_eq!(json["type"], "cognitive_create_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account", "location": "eastus", "kind": "TextAnalytics", "sku": {"name": "S0"}});
        let _: CognitiveCreateAccountInput = serde_json::from_value(json).unwrap();
    }
}
