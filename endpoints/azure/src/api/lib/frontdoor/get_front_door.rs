...
const API_INFO: ApiInfo<AzureApi, FrontDoorGetFrontDoorInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::FrontDoorGetFrontDoor, "Get a Front Door instance", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FrontDoorGetFrontDoorInputBuilder::default().resource_group("my-rg").front_door_name("my-fd").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "frontdoor_get_front_door");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "front_door_name": "my-fd"});
        let _: FrontDoorGetFrontDoorInput = serde_json::from_value(json).unwrap();
    }
}
