...
const API_INFO: ApiInfo<AzureApi, FrontDoorListFrontDoorsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::FrontDoorListFrontDoors, "List Front Door instances", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Network/frontDoors", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Network/frontDoors", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FrontDoorListFrontDoorsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "frontdoor_list_front_doors");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: FrontDoorListFrontDoorsInput = serde_json::from_value(json).unwrap();
    }
}
