...
const API_INFO: ApiInfo<AzureApi, FrontDoorCreateOrUpdateFrontDoorInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::FrontDoorCreateOrUpdateFrontDoor,
    "Create or update a Front Door instance",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.front_door_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FrontDoorCreateOrUpdateFrontDoorInputBuilder::default()
            .resource_group("my-rg")
            .front_door_name("my-fd")
            .location("global")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "frontdoor_create_or_update_front_door");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "front_door_name": "my-fd", "location": "global"});
        let _: FrontDoorCreateOrUpdateFrontDoorInput = serde_json::from_value(json).unwrap();
    }
}
