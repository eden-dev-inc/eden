...
const API_INFO: ApiInfo<AzureApi, FrontDoorDeleteFrontDoorInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::FrontDoorDeleteFrontDoor,
    "Delete a Front Door instance",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = FrontDoorDeleteFrontDoorInputBuilder::default().resource_group("my-rg").front_door_name("my-fd").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "frontdoor_delete_front_door");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "front_door_name": "my-fd"});
        let _: FrontDoorDeleteFrontDoorInput = serde_json::from_value(json).unwrap();
    }
}
