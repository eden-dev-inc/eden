...
const API_INFO: ApiInfo<AzureApi, ServiceBusListQueuesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ServiceBusListQueues, "List Service Bus queues", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusListQueuesInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "servicebus_list_queues");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: ServiceBusListQueuesInput = serde_json::from_value(json).unwrap();
    }
}
