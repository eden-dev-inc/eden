...
const API_INFO: ApiInfo<AzureApi, ServiceBusGetNamespaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ServiceBusGetNamespace, "Get a Service Bus namespace", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusGetNamespaceInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "servicebus_get_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: ServiceBusGetNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
