...
const API_INFO: ApiInfo<AzureApi, ServiceBusGetSubscriptionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceBusGetSubscription,
    "Get a Service Bus topic subscription",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusGetSubscriptionInputBuilder::default()
        ...
        assert_eq!(json["type"], "servicebus_get_subscription");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "topic_name": "my-topic", "subscription_name": "my-sub"});
        let _: ServiceBusGetSubscriptionInput = serde_json::from_value(json).unwrap();
    }
}
