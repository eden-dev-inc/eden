...
const API_INFO: ApiInfo<AzureApi, ServiceBusDeleteNamespaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceBusDeleteNamespace,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusDeleteNamespaceInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "servicebus_delete_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: ServiceBusDeleteNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
