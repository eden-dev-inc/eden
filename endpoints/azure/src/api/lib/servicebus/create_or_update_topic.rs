...
const API_INFO: ApiInfo<AzureApi, ServiceBusCreateOrUpdateTopicInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceBusCreateOrUpdateTopic,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusCreateOrUpdateTopicInputBuilder::default()
        ...
        assert_eq!(json["type"], "servicebus_create_or_update_topic");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "topic_name": "my-topic"});
        let _: ServiceBusCreateOrUpdateTopicInput = serde_json::from_value(json).unwrap();
    }
}
