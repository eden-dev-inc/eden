...
const API_INFO: ApiInfo<AzureApi, ServiceBusDeleteQueueInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ServiceBusDeleteQueue, "Delete a Service Bus queue", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusDeleteQueueInputBuilder::default()
        ...
        assert_eq!(json["type"], "servicebus_delete_queue");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "queue_name": "my-queue"});
        let _: ServiceBusDeleteQueueInput = serde_json::from_value(json).unwrap();
    }
}
