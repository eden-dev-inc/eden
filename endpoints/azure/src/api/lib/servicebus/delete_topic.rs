...
const API_INFO: ApiInfo<AzureApi, ServiceBusDeleteTopicInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ServiceBusDeleteTopic, "Delete a Service Bus topic", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.namespace_name, self.topic_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusDeleteTopicInputBuilder::default()
        ...
        assert_eq!(json["type"], "servicebus_delete_topic");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "topic_name": "my-topic"});
        let _: ServiceBusDeleteTopicInput = serde_json::from_value(json).unwrap();
    }
}
