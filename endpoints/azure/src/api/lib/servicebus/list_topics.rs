...
const API_INFO: ApiInfo<AzureApi, ServiceBusListTopicsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ServiceBusListTopics, "List Service Bus topics", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusListTopicsInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "servicebus_list_topics");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: ServiceBusListTopicsInput = serde_json::from_value(json).unwrap();
    }
}
