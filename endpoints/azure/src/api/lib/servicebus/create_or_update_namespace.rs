...
const API_INFO: ApiInfo<AzureApi, ServiceBusCreateOrUpdateNamespaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceBusCreateOrUpdateNamespace,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusCreateOrUpdateNamespaceInputBuilder::default()
        ...
        assert_eq!(json["type"], "servicebus_create_or_update_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "location": "eastus"});
        let _: ServiceBusCreateOrUpdateNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
