...
const API_INFO: ApiInfo<AzureApi, ServiceBusListSubscriptionsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ServiceBusListSubscriptions,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusListSubscriptionsInputBuilder::default()
        ...
        assert_eq!(json["type"], "servicebus_list_subscriptions");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "topic_name": "my-topic"});
        let _: ServiceBusListSubscriptionsInput = serde_json::from_value(json).unwrap();
    }
}
