...
const API_INFO: ApiInfo<AzureApi, ServiceBusGetQueueInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ServiceBusGetQueue, "Get a Service Bus queue", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ServiceBusGetQueueInputBuilder::default()
        ...
        assert_eq!(json["type"], "servicebus_get_queue");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "queue_name": "my-queue"});
        let _: ServiceBusGetQueueInput = serde_json::from_value(json).unwrap();
    }
}
