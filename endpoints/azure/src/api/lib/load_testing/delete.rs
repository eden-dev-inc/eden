...
const API_INFO: ApiInfo<AzureApi, LoadTestingDeleteInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::LoadTestingDelete, "Delete load test", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.load_test_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LoadTestingDeleteInputBuilder::default().resource_group("my-rg").load_test_name("my-test").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "load_testing_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "load_test_name": "my-test"});
        let _: LoadTestingDeleteInput = serde_json::from_value(json).unwrap();
    }
}
