...
const API_INFO: ApiInfo<AzureApi, LoadTestingListAllInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::LoadTestingListAll, "List all load tests", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.LoadTestService/loadTests", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.LoadTestService/loadTests", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LoadTestingListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "load_testing_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: LoadTestingListAllInput = serde_json::from_value(json).unwrap();
    }
}
