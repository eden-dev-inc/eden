...
const API_INFO: ApiInfo<AzureApi, LoadTestingGetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::LoadTestingGet, "Get load test", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LoadTestingGetInputBuilder::default().resource_group("my-rg").load_test_name("my-test").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "load_testing_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "load_test_name": "my-test"});
        let _: LoadTestingGetInput = serde_json::from_value(json).unwrap();
    }
}
