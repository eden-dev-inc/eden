...
const API_INFO: ApiInfo<AzureApi, LoadTestingCreateOrUpdateInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::LoadTestingCreateOrUpdate,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LoadTestingCreateOrUpdateInputBuilder::default()
            .resource_group("my-rg")
            .load_test_name("my-test")
            .location("eastus")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "load_testing_create_or_update");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "load_test_name": "my-test", "location": "eastus", "properties": {}});
        let _: LoadTestingCreateOrUpdateInput = serde_json::from_value(json).unwrap();
    }
}
