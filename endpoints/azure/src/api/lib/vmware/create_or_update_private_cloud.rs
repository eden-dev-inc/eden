...
const API_INFO: ApiInfo<AzureApi, VmwareCreateOrUpdatePrivateCloudInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::VmwareCreateOrUpdatePrivateCloud,
    "Create or update VMware private cloud",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.private_cloud_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = VmwareCreateOrUpdatePrivateCloudInputBuilder::default()
            .resource_group("my-rg")
            .private_cloud_name("my-cloud")
            .location("eastus")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "vmware_create_or_update_private_cloud");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "private_cloud_name": "my-cloud", "location": "eastus", "properties": {}});
        let _: VmwareCreateOrUpdatePrivateCloudInput = serde_json::from_value(json).unwrap();
    }
}
