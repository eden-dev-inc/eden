...
const API_INFO: ApiInfo<AzureApi, VmwareCreateOrUpdateClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::VmwareCreateOrUpdateCluster,
    "Create or update a VMware cluster",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.private_cloud_name, self.cluster_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = VmwareCreateOrUpdateClusterInputBuilder::default()
        ...
        assert_eq!(json["type"], "vmware_create_or_update_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "private_cloud_name": "my-cloud", "cluster_name": "my-cluster", "properties": {}});
        let _: VmwareCreateOrUpdateClusterInput = serde_json::from_value(json).unwrap();
    }
}
