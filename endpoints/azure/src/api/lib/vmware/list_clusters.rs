...
const API_INFO: ApiInfo<AzureApi, VmwareListClustersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::VmwareListClusters, "List VMware clusters", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = VmwareListClustersInputBuilder::default().resource_group("my-rg").private_cloud_name("my-cloud").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "vmware_list_clusters");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "private_cloud_name": "my-cloud"});
        let _: VmwareListClustersInput = serde_json::from_value(json).unwrap();
    }
}
