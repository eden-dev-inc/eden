...
const API_INFO: ApiInfo<AzureApi, VmwareDeleteClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::VmwareDeleteCluster, "Delete a VMware cluster", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = VmwareDeleteClusterInputBuilder::default()
        ...
        assert_eq!(json["type"], "vmware_delete_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "private_cloud_name": "my-cloud", "cluster_name": "my-cluster"});
        let _: VmwareDeleteClusterInput = serde_json::from_value(json).unwrap();
    }
}
