...
const API_INFO: ApiInfo<AzureApi, VmwareListPrivateCloudsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::VmwareListPrivateClouds, "List VMware private clouds", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.AVS/privateClouds", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.AVS/privateClouds", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = VmwareListPrivateCloudsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "vmware_list_private_clouds");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: VmwareListPrivateCloudsInput = serde_json::from_value(json).unwrap();
    }
}
