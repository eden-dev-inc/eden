...
const API_INFO: ApiInfo<AzureApi, VmwareDeletePrivateCloudInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::VmwareDeletePrivateCloud,
    "Delete VMware private cloud",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = VmwareDeletePrivateCloudInputBuilder::default().resource_group("my-rg").private_cloud_name("my-cloud").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "vmware_delete_private_cloud");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "private_cloud_name": "my-cloud"});
        let _: VmwareDeletePrivateCloudInput = serde_json::from_value(json).unwrap();
    }
}
