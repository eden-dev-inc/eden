...
const API_INFO: ApiInfo<AzureApi, VmwareGetPrivateCloudInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::VmwareGetPrivateCloud, "Get VMware private cloud", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = VmwareGetPrivateCloudInputBuilder::default().resource_group("my-rg").private_cloud_name("my-cloud").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "vmware_get_private_cloud");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "private_cloud_name": "my-cloud"});
        let _: VmwareGetPrivateCloudInput = serde_json::from_value(json).unwrap();
    }
}
