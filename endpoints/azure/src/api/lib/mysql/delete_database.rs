...
const API_INFO: ApiInfo<AzureApi, MysqlDeleteDatabaseInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MysqlDeleteDatabase, "Delete a MySQL database", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlDeleteDatabaseInputBuilder::default()
        ...
        assert_eq!(json["type"], "mysql_delete_database");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "database_name": "my-db"});
        let _: MysqlDeleteDatabaseInput = serde_json::from_value(json).unwrap();
    }
}
