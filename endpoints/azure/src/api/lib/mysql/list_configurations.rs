...
const API_INFO: ApiInfo<AzureApi, MysqlListConfigurationsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MysqlListConfigurations, "List MySQL configurations", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlListConfigurationsInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "mysql_list_configurations");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: MysqlListConfigurationsInput = serde_json::from_value(json).unwrap();
    }
}
