...
const API_INFO: ApiInfo<AzureApi, MysqlGetDatabaseInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MysqlGetDatabase, "Get a MySQL database", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlGetDatabaseInputBuilder::default()
        ...
        assert_eq!(json["type"], "mysql_get_database");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "database_name": "my-db"});
        let _: MysqlGetDatabaseInput = serde_json::from_value(json).unwrap();
    }
}
