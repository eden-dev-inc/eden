...
const API_INFO: ApiInfo<AzureApi, MysqlDeleteFirewallRuleInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MysqlDeleteFirewallRule,
    "Delete a MySQL firewall rule",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlDeleteFirewallRuleInputBuilder::default()
        ...
        assert_eq!(json["type"], "mysql_delete_firewall_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "firewall_rule_name": "my-rule"});
        let _: MysqlDeleteFirewallRuleInput = serde_json::from_value(json).unwrap();
    }
}
