...
const API_INFO: ApiInfo<AzureApi, MysqlDeleteServerInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MysqlDeleteServer, "Delete a MySQL flexible server", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlDeleteServerInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "mysql_delete_server");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: MysqlDeleteServerInput = serde_json::from_value(json).unwrap();
    }
}
