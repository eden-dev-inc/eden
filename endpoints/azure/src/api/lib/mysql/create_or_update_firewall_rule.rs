...
const API_INFO: ApiInfo<AzureApi, MysqlCreateOrUpdateFirewallRuleInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MysqlCreateOrUpdateFirewallRule,
    "Create or update a MySQL firewall rule",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlCreateOrUpdateFirewallRuleInputBuilder::default()
        ...
        assert_eq!(json["type"], "mysql_create_or_update_firewall_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json =
            serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "firewall_rule_name": "my-rule", "properties": {}});
        let _: MysqlCreateOrUpdateFirewallRuleInput = serde_json::from_value(json).unwrap();
    }
}
