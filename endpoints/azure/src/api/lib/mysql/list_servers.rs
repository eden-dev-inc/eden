...
const API_INFO: ApiInfo<AzureApi, MysqlListServersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MysqlListServers, "List MySQL flexible servers", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.DBforMySQL/flexibleServers", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.DBforMySQL/flexibleServers", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlListServersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "mysql_list_servers");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: MysqlListServersInput = serde_json::from_value(json).unwrap();
    }
}
