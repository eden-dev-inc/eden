...
const API_INFO: ApiInfo<AzureApi, MysqlListDatabasesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MysqlListDatabases, "List MySQL databases", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlListDatabasesInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "mysql_list_databases");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: MysqlListDatabasesInput = serde_json::from_value(json).unwrap();
    }
}
