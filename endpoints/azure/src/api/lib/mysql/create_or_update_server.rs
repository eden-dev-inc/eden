...
const API_INFO: ApiInfo<AzureApi, MysqlCreateOrUpdateServerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MysqlCreateOrUpdateServer,
    "Create or update a MySQL flexible server",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlCreateOrUpdateServerInputBuilder::default()
            .resource_group("my-rg")
            .server_name("my-server")
            .location("eastus")
            .properties(serde_json::json!({"version": "8.0.21"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "mysql_create_or_update_server");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "location": "eastus", "properties": {}});
        let _: MysqlCreateOrUpdateServerInput = serde_json::from_value(json).unwrap();
    }
}
