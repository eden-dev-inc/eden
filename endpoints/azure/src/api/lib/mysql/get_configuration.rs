...
const API_INFO: ApiInfo<AzureApi, MysqlGetConfigurationInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MysqlGetConfiguration, "Get a MySQL configuration", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlGetConfigurationInputBuilder::default()
        ...
        assert_eq!(json["type"], "mysql_get_configuration");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server", "configuration_name": "max_connections"});
        let _: MysqlGetConfigurationInput = serde_json::from_value(json).unwrap();
    }
}
