...
const API_INFO: ApiInfo<AzureApi, MysqlStartServerInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MysqlStartServer, "Start a MySQL flexible server", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlStartServerInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "mysql_start_server");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: MysqlStartServerInput = serde_json::from_value(json).unwrap();
    }
}
