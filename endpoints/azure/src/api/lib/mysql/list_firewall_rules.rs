...
const API_INFO: ApiInfo<AzureApi, MysqlListFirewallRulesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MysqlListFirewallRules, "List MySQL firewall rules", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlListFirewallRulesInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "mysql_list_firewall_rules");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: MysqlListFirewallRulesInput = serde_json::from_value(json).unwrap();
    }
}
