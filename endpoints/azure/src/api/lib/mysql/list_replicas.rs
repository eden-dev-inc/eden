...
const API_INFO: ApiInfo<AzureApi, MysqlListReplicasInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MysqlListReplicas,
    "List MySQL flexible server replicas",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MysqlListReplicasInputBuilder::default().resource_group("my-rg").server_name("my-server").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "mysql_list_replicas");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "server_name": "my-server"});
        let _: MysqlListReplicasInput = serde_json::from_value(json).unwrap();
    }
}
