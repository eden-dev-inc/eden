...
const API_VERSION: &str = "2022-01-01";

const API_INFO: ApiInfo<AzureApi, SecurityDismissAlertInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SecurityDismissAlert, "Dismiss security alert", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Security/alerts/{}/dismiss", sub, self.alert_name);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SecurityDismissAlertInputBuilder::default().alert_name("my-alert").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "security_dismiss_alert");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"alert_name": "my-alert"});
        let _: SecurityDismissAlertInput = serde_json::from_value(json).unwrap();
    }
}
