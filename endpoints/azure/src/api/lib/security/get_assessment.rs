...
const API_INFO: ApiInfo<AzureApi, SecurityGetAssessmentInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SecurityGetAssessment, "Get security assessment", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("{}/providers/Microsoft.Security/assessments/{}", self.resource_id, self.assessment_name);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SecurityGetAssessmentInputBuilder::default()
            .resource_id("/subscriptions/sub-id")
            .assessment_name("my-assessment")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "security_get_assessment");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_id": "/subscriptions/sub-id", "assessment_name": "my-assessment"});
        let _: SecurityGetAssessmentInput = serde_json::from_value(json).unwrap();
    }
}
