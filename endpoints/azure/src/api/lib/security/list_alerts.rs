...
const API_INFO: ApiInfo<AzureApi, SecurityListAlertsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SecurityListAlerts, "List security alerts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Security/alerts", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SecurityListAlertsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "security_list_alerts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SecurityListAlertsInput = serde_json::from_value(json).unwrap();
    }
}
