...
const API_INFO: ApiInfo<AzureApi, SecurityGetAlertInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SecurityGetAlert, "Get security alert", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Security/alerts/{}", sub, self.alert_name);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SecurityGetAlertInputBuilder::default().alert_name("my-alert").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "security_get_alert");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"alert_name": "my-alert"});
        let _: SecurityGetAlertInput = serde_json::from_value(json).unwrap();
    }
}
