...
const API_VERSION: &str = "2020-01-01-preview";

const API_INFO: ApiInfo<AzureApi, SecurityListSecurityContactsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SecurityListSecurityContacts, "List security contacts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Security/securityContacts", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SecurityListSecurityContactsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "security_list_security_contacts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SecurityListSecurityContactsInput = serde_json::from_value(json).unwrap();
    }
}
