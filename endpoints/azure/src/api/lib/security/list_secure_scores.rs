...
const API_INFO: ApiInfo<AzureApi, SecurityListSecureScoresInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SecurityListSecureScores, "List secure scores", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Security/secureScores", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SecurityListSecureScoresInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "security_list_secure_scores");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SecurityListSecureScoresInput = serde_json::from_value(json).unwrap();
    }
}
