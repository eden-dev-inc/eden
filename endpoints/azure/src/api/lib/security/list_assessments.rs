...
const API_INFO: ApiInfo<AzureApi, SecurityListAssessmentsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SecurityListAssessments, "List security assessments", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Security/assessments", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SecurityListAssessmentsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "security_list_assessments");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SecurityListAssessmentsInput = serde_json::from_value(json).unwrap();
    }
}
