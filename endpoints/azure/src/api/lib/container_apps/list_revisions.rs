...
const API_INFO: ApiInfo<AzureApi, ContainerAppsListRevisionsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerAppsListRevisions,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerAppsListRevisionsInputBuilder::default().resource_group("my-rg").container_app_name("my-app").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_apps_list_revisions");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_app_name": "my-app"});
        let _: ContainerAppsListRevisionsInput = serde_json::from_value(json).unwrap();
    }
}
