...
const API_INFO: ApiInfo<AzureApi, ContainerAppsDeleteEnvironmentInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerAppsDeleteEnvironment,
    "Delete a Container Apps managed environment",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerAppsDeleteEnvironmentInputBuilder::default()
            .resource_group("my-rg")
            .environment_name("my-env")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_apps_delete_environment");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "environment_name": "my-env"});
        let _: ContainerAppsDeleteEnvironmentInput = serde_json::from_value(json).unwrap();
    }
}
