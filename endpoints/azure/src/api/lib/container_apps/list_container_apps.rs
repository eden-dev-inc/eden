...
const API_INFO: ApiInfo<AzureApi, ContainerAppsListContainerAppsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerAppsListContainerApps, "List Container Apps", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.App/containerApps", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.App/containerApps", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerAppsListContainerAppsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_apps_list_container_apps");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ContainerAppsListContainerAppsInput = serde_json::from_value(json).unwrap();
    }
}
