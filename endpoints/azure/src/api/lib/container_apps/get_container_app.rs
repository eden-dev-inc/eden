...
const API_INFO: ApiInfo<AzureApi, ContainerAppsGetContainerAppInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerAppsGetContainerApp, "Get a Container App", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerAppsGetContainerAppInputBuilder::default()
            .resource_group("my-rg")
            .container_app_name("my-app")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_apps_get_container_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_app_name": "my-app"});
        let _: ContainerAppsGetContainerAppInput = serde_json::from_value(json).unwrap();
    }
}
