...
const API_INFO: ApiInfo<AzureApi, ContainerAppsCreateOrUpdateEnvironmentInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerAppsCreateOrUpdateEnvironment,
    "Create or update a Container Apps managed environment",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerAppsCreateOrUpdateEnvironmentInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_apps_create_or_update_environment");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "environment_name": "my-env", "location": "eastus", "properties": {}});
        let _: ContainerAppsCreateOrUpdateEnvironmentInput = serde_json::from_value(json).unwrap();
    }
}
