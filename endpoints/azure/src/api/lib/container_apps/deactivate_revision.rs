...
const API_INFO: ApiInfo<AzureApi, ContainerAppsDeactivateRevisionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerAppsDeactivateRevision,
    "Deactivate a revision for a Container App",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerAppsDeactivateRevisionInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_apps_deactivate_revision");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_app_name": "my-app", "revision_name": "my-rev"});
        let _: ContainerAppsDeactivateRevisionInput = serde_json::from_value(json).unwrap();
    }
}
