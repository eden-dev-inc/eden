...
const API_INFO: ApiInfo<AzureApi, ContainerAppsCreateOrUpdateContainerAppInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerAppsCreateOrUpdateContainerApp,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.container_app_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerAppsCreateOrUpdateContainerAppInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_apps_create_or_update_container_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_app_name": "my-app", "location": "eastus", "properties": {}});
        let _: ContainerAppsCreateOrUpdateContainerAppInput = serde_json::from_value(json).unwrap();
    }
}
