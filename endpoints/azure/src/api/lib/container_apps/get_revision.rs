...
const API_INFO: ApiInfo<AzureApi, ContainerAppsGetRevisionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerAppsGetRevision,
    "Get a revision for a Container App",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerAppsGetRevisionInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_apps_get_revision");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_app_name": "my-app", "revision_name": "my-rev"});
        let _: ContainerAppsGetRevisionInput = serde_json::from_value(json).unwrap();
    }
}
