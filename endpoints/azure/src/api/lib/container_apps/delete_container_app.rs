...
const API_INFO: ApiInfo<AzureApi, ContainerAppsDeleteContainerAppInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerAppsDeleteContainerApp,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerAppsDeleteContainerAppInputBuilder::default()
            .resource_group("my-rg")
            .container_app_name("my-app")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_apps_delete_container_app");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "container_app_name": "my-app"});
        let _: ContainerAppsDeleteContainerAppInput = serde_json::from_value(json).unwrap();
    }
}
