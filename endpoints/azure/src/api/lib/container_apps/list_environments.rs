...
const API_INFO: ApiInfo<AzureApi, ContainerAppsListEnvironmentsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerAppsListEnvironments,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.App/managedEnvironments", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.App/managedEnvironments", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerAppsListEnvironmentsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_apps_list_environments");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ContainerAppsListEnvironmentsInput = serde_json::from_value(json).unwrap();
    }
}
