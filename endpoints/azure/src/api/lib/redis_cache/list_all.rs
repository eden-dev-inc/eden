...
const API_INFO: ApiInfo<AzureApi, RedisCacheListAllInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::RedisCacheListAll, "List all Redis Cache instances", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Cache/redis", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Cache/redis", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RedisCacheListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "redis_cache_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: RedisCacheListAllInput = serde_json::from_value(json).unwrap();
    }
}
