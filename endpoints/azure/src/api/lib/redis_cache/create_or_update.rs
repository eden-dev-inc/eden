...
const API_INFO: ApiInfo<AzureApi, RedisCacheCreateOrUpdateInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RedisCacheCreateOrUpdate,
    "Create or update a Redis Cache instance",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.cache_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RedisCacheCreateOrUpdateInputBuilder::default()
            .resource_group("my-rg")
            .cache_name("my-cache")
            .location("eastus")
            .properties(serde_json::json!({"sku": {"name": "Basic", "family": "C", "capacity": 0}}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "redis_cache_create_or_update");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cache_name": "my-cache", "location": "eastus", "properties": {}});
        let _: RedisCacheCreateOrUpdateInput = serde_json::from_value(json).unwrap();
    }
}
