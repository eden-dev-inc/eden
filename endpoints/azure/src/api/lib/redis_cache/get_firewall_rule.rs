...
const API_INFO: ApiInfo<AzureApi, RedisCacheGetFirewallRuleInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RedisCacheGetFirewallRule,
    "Get a Redis Cache firewall rule",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RedisCacheGetFirewallRuleInputBuilder::default()
        ...
        assert_eq!(json["type"], "redis_cache_get_firewall_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cache_name": "my-cache", "rule_name": "my-rule"});
        let _: RedisCacheGetFirewallRuleInput = serde_json::from_value(json).unwrap();
    }
}
