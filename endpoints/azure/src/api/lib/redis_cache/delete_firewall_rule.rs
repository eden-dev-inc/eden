...
const API_INFO: ApiInfo<AzureApi, RedisCacheDeleteFirewallRuleInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RedisCacheDeleteFirewallRule,
    "Delete a Redis Cache firewall rule",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.cache_name, self.rule_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RedisCacheDeleteFirewallRuleInputBuilder::default()
        ...
        assert_eq!(json["type"], "redis_cache_delete_firewall_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cache_name": "my-cache", "rule_name": "my-rule"});
        let _: RedisCacheDeleteFirewallRuleInput = serde_json::from_value(json).unwrap();
    }
}
