...
const API_INFO: ApiInfo<AzureApi, RedisCacheListFirewallRulesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RedisCacheListFirewallRules,
    "List Redis Cache firewall rules",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RedisCacheListFirewallRulesInputBuilder::default().resource_group("my-rg").cache_name("my-cache").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "redis_cache_list_firewall_rules");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cache_name": "my-cache"});
        let _: RedisCacheListFirewallRulesInput = serde_json::from_value(json).unwrap();
    }
}
