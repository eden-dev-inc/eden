...
const API_INFO: ApiInfo<AzureApi, RedisCacheListKeysInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RedisCacheListKeys,
    "List keys for a Redis Cache instance",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RedisCacheListKeysInputBuilder::default().resource_group("my-rg").cache_name("my-cache").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "redis_cache_list_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cache_name": "my-cache"});
        let _: RedisCacheListKeysInput = serde_json::from_value(json).unwrap();
    }
}
