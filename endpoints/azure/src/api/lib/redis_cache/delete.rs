...
const API_INFO: ApiInfo<AzureApi, RedisCacheDeleteInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::RedisCacheDelete, "Delete a Redis Cache instance", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RedisCacheDeleteInputBuilder::default().resource_group("my-rg").cache_name("my-cache").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "redis_cache_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cache_name": "my-cache"});
        let _: RedisCacheDeleteInput = serde_json::from_value(json).unwrap();
    }
}
