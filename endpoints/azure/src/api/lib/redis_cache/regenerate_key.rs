...
const API_INFO: ApiInfo<AzureApi, RedisCacheRegenerateKeyInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RedisCacheRegenerateKey,
    "Regenerate a Redis Cache key",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RedisCacheRegenerateKeyInputBuilder::default()
        ...
        assert_eq!(json["type"], "redis_cache_regenerate_key");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cache_name": "my-cache", "key_type": "Primary"});
        let _: RedisCacheRegenerateKeyInput = serde_json::from_value(json).unwrap();
    }
}
