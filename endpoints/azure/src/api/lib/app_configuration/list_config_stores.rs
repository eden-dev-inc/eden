...
const API_INFO: ApiInfo<AzureApi, AppConfigListConfigStoresInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppConfigListConfigStores, "List configuration stores", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.AppConfiguration/configurationStores", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppConfigListConfigStoresInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_config_list_config_stores");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AppConfigListConfigStoresInput = serde_json::from_value(json).unwrap();
    }
}
