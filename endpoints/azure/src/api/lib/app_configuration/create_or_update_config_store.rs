...
const API_INFO: ApiInfo<AzureApi, AppConfigCreateOrUpdateConfigStoreInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppConfigCreateOrUpdateConfigStore,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppConfigCreateOrUpdateConfigStoreInputBuilder::default()
            .resource_group("my-rg")
            .config_store_name("my-store")
            .location("eastus")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_config_create_or_update_config_store");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "config_store_name": "my-store", "location": "eastus"});
        let _: AppConfigCreateOrUpdateConfigStoreInput = serde_json::from_value(json).unwrap();
    }
}
