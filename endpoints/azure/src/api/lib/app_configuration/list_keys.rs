...
const API_INFO: ApiInfo<AzureApi, AppConfigListKeysInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppConfigListKeys, "List configuration store keys", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppConfigListKeysInputBuilder::default().resource_group("my-rg").config_store_name("my-store").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_config_list_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "config_store_name": "my-store"});
        let _: AppConfigListKeysInput = serde_json::from_value(json).unwrap();
    }
}
