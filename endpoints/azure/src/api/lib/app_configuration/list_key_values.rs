...
const API_INFO: ApiInfo<AzureApi, AppConfigListKeyValuesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppConfigListKeyValues,
    "List configuration store key values",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppConfigListKeyValuesInputBuilder::default().resource_group("my-rg").config_store_name("my-store").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_config_list_key_values");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "config_store_name": "my-store"});
        let _: AppConfigListKeyValuesInput = serde_json::from_value(json).unwrap();
    }
}
