...
const API_INFO: ApiInfo<AzureApi, AppConfigGetConfigStoreInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppConfigGetConfigStore, "Get configuration store", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.config_store_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppConfigGetConfigStoreInputBuilder::default().resource_group("my-rg").config_store_name("my-store").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_config_get_config_store");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "config_store_name": "my-store"});
        let _: AppConfigGetConfigStoreInput = serde_json::from_value(json).unwrap();
    }
}
