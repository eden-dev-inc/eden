...
const API_INFO: ApiInfo<AzureApi, AppConfigDeleteConfigStoreInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppConfigDeleteConfigStore,
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            AppConfigDeleteConfigStoreInputBuilder::default().resource_group("my-rg").config_store_name("my-store").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_config_delete_config_store");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "config_store_name": "my-store"});
        let _: AppConfigDeleteConfigStoreInput = serde_json::from_value(json).unwrap();
    }
}
