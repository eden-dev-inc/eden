...
const API_INFO: ApiInfo<AzureApi, DatabricksGetWorkspaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DatabricksGetWorkspace, "Get a Databricks workspace", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DatabricksGetWorkspaceInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "databricks_get_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: DatabricksGetWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
