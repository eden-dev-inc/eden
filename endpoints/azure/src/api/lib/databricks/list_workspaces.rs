...
const API_INFO: ApiInfo<AzureApi, DatabricksListWorkspacesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DatabricksListWorkspaces, "List Databricks workspaces", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Databricks/workspaces", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Databricks/workspaces", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DatabricksListWorkspacesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "databricks_list_workspaces");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DatabricksListWorkspacesInput = serde_json::from_value(json).unwrap();
    }
}
