...
const API_INFO: ApiInfo<AzureApi, DatabricksCreateOrUpdateWorkspaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DatabricksCreateOrUpdateWorkspace,
    "Create or update a Databricks workspace",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.workspace_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DatabricksCreateOrUpdateWorkspaceInputBuilder::default()
            .resource_group("my-rg")
            .workspace_name("my-ws")
            .location("eastus")
            .properties(serde_json::json!({"managedResourceGroupId": "/subscriptions/sub/resourceGroups/managed-rg"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "databricks_create_or_update_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "location": "eastus", "properties": {}});
        let _: DatabricksCreateOrUpdateWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
