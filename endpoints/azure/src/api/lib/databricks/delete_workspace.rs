...
const API_INFO: ApiInfo<AzureApi, DatabricksDeleteWorkspaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DatabricksDeleteWorkspace,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DatabricksDeleteWorkspaceInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "databricks_delete_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: DatabricksDeleteWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
