...
const API_INFO: ApiInfo<AzureApi, CosmosDbCreateOrUpdateGremlinDatabaseInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbCreateOrUpdateGremlinDatabase,
    "Create or update a Cosmos DB Gremlin database",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbCreateOrUpdateGremlinDatabaseInputBuilder::default()
            .resource_group("my-rg")
            .account_name("my-cosmos")
            .database_name("my-db")
            .properties(serde_json::json!({"resource": {"id": "my-db"}}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_create_or_update_gremlin_database");
    }
    ...
    fn deserialize_minimal() {
        let json = serde_json::json!({
            ...
            "account_name": "my-cosmos",
            "database_name": "my-db",
            "properties": {"resource": {"id": "my-db"}}
        });
        let _: CosmosDbCreateOrUpdateGremlinDatabaseInput = serde_json::from_value(json).unwrap();
    }
}
