...
const API_INFO: ApiInfo<AzureApi, CosmosDbListConnectionStringsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbListConnectionStrings,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbListConnectionStringsInputBuilder::default().resource_group("my-rg").account_name("my-cosmos").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_list_connection_strings");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbListConnectionStringsInput = serde_json::from_value(json).unwrap();
    }
}
