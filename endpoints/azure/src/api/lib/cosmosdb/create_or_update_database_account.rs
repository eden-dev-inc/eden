...
const API_INFO: ApiInfo<AzureApi, CosmosDbCreateOrUpdateDatabaseAccountInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbCreateOrUpdateDatabaseAccount,
    "Create or update a Cosmos DB database account",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbCreateOrUpdateDatabaseAccountInputBuilder::default()
            .resource_group("my-rg")
            .account_name("my-cosmos")
            .location("eastus")
            .properties(serde_json::json!({"databaseAccountOfferType": "Standard"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_create_or_update_database_account");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbCreateOrUpdateDatabaseAccountInput = serde_json::from_value(json).unwrap();
    }
}
