...
const API_INFO: ApiInfo<AzureApi, CosmosDbListDatabaseAccountsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbListDatabaseAccounts,
    "List Cosmos DB database accounts",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.DocumentDB/databaseAccounts", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbListDatabaseAccountsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_list_database_accounts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: CosmosDbListDatabaseAccountsInput = serde_json::from_value(json).unwrap();
    }
}
