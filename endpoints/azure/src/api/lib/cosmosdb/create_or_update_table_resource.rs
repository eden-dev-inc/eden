...
const API_INFO: ApiInfo<AzureApi, CosmosDbCreateOrUpdateTableResourceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbCreateOrUpdateTableResource,
    "Create or update a Cosmos DB table resource",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.account_name, self.table_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbCreateOrUpdateTableResourceInputBuilder::default()
            .resource_group("my-rg")
            .account_name("my-cosmos")
            .table_name("my-table")
            .properties(serde_json::json!({"resource": {"id": "my-table"}}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_create_or_update_table_resource");
    }
    ...
    fn deserialize_minimal() {
        let json = serde_json::json!({
            ...
            "account_name": "my-cosmos",
            "table_name": "my-table",
            "properties": {"resource": {"id": "my-table"}}
        });
        let _: CosmosDbCreateOrUpdateTableResourceInput = serde_json::from_value(json).unwrap();
    }
}
