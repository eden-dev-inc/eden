...
const API_INFO: ApiInfo<AzureApi, CosmosDbRegenerateKeyInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbRegenerateKey,
    "Regenerate a Cosmos DB account key",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbRegenerateKeyInputBuilder::default()
        ...
        assert_eq!(json["type"], "cosmosdb_regenerate_key");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbRegenerateKeyInput = serde_json::from_value(json).unwrap();
    }
}
