...
const API_INFO: ApiInfo<AzureApi, CosmosDbListCassandraTablesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbListCassandraTables,
    "List Cosmos DB Cassandra tables",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbListCassandraTablesInputBuilder::default()
        ...
        assert_eq!(json["type"], "cosmosdb_list_cassandra_tables");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbListCassandraTablesInput = serde_json::from_value(json).unwrap();
    }
}
