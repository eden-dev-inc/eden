...
const API_INFO: ApiInfo<AzureApi, CosmosDbListSqlDatabasesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbListSqlDatabases,
    "List Cosmos DB SQL databases",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbListSqlDatabasesInputBuilder::default().resource_group("my-rg").account_name("my-cosmos").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_list_sql_databases");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-cosmos"});
        let _: CosmosDbListSqlDatabasesInput = serde_json::from_value(json).unwrap();
    }
}
