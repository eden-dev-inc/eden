...
const API_INFO: ApiInfo<AzureApi, CosmosDbDeleteDatabaseAccountInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbDeleteDatabaseAccount,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbDeleteDatabaseAccountInputBuilder::default().resource_group("my-rg").account_name("my-cosmos").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_delete_database_account");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbDeleteDatabaseAccountInput = serde_json::from_value(json).unwrap();
    }
}
