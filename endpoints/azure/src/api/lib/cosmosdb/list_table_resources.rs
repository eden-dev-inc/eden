...
const API_INFO: ApiInfo<AzureApi, CosmosDbListTableResourcesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbListTableResources,
    "List Cosmos DB table resources",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbListTableResourcesInputBuilder::default().resource_group("my-rg").account_name("my-cosmos").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_list_table_resources");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbListTableResourcesInput = serde_json::from_value(json).unwrap();
    }
}
