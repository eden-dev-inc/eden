...
const API_INFO: ApiInfo<AzureApi, CosmosDbListMongoDatabasesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbListMongoDatabases,
    "List Cosmos DB MongoDB databases",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbListMongoDatabasesInputBuilder::default().resource_group("my-rg").account_name("my-cosmos").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_list_mongo_databases");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbListMongoDatabasesInput = serde_json::from_value(json).unwrap();
    }
}
