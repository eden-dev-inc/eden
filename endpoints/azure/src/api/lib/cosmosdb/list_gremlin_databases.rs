...
const API_INFO: ApiInfo<AzureApi, CosmosDbListGremlinDatabasesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbListGremlinDatabases,
    "List Cosmos DB Gremlin databases",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbListGremlinDatabasesInputBuilder::default().resource_group("my-rg").account_name("my-cosmos").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_list_gremlin_databases");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbListGremlinDatabasesInput = serde_json::from_value(json).unwrap();
    }
}
