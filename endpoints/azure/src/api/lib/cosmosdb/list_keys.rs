...
const API_INFO: ApiInfo<AzureApi, CosmosDbListKeysInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::CosmosDbListKeys, "List Cosmos DB account keys", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbListKeysInputBuilder::default().resource_group("my-rg").account_name("my-cosmos").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_list_keys");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbListKeysInput = serde_json::from_value(json).unwrap();
    }
}
