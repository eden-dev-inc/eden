...
const API_INFO: ApiInfo<AzureApi, CosmosDbCreateOrUpdateSqlContainerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbCreateOrUpdateSqlContainer,
    "Create or update a Cosmos DB SQL container",
    ReqType::Write,
    true,
);
...
crate::azure_endpoint! {
    ...
    struct {
        ...
        partition_key_kind: Option<String>,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.account_name, self.database_name, self.container_name
        );

        let kind = self.partition_key_kind.as_deref().unwrap_or("Hash");
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbCreateOrUpdateSqlContainerInputBuilder::default()
            .resource_group("my-rg")
            .account_name("my-cosmos")
            .database_name("my-db")
            .container_name("my-container")
            .partition_key_paths(vec!["/id".to_string()])
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_create_or_update_sql_container");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbCreateOrUpdateSqlContainerInput = serde_json::from_value(json).unwrap();
    }
}
