...
const API_INFO: ApiInfo<AzureApi, CosmosDbCreateOrUpdateSqlDatabaseInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbCreateOrUpdateSqlDatabase,
    "Create or update a Cosmos DB SQL database",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbCreateOrUpdateSqlDatabaseInputBuilder::default()
        ...
        assert_eq!(json["type"], "cosmosdb_create_or_update_sql_database");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbCreateOrUpdateSqlDatabaseInput = serde_json::from_value(json).unwrap();
    }
}
