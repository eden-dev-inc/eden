...
const API_INFO: ApiInfo<AzureApi, CosmosDbUpdateSqlDatabaseThroughputInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbUpdateSqlDatabaseThroughput,
    "Update throughput for a Cosmos DB SQL database",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbUpdateSqlDatabaseThroughputInputBuilder::default()
        ...
        assert_eq!(json["type"], "cosmosdb_update_sql_database_throughput");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbUpdateSqlDatabaseThroughputInput = serde_json::from_value(json).unwrap();
    }
}
