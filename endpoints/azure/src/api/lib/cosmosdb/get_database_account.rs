...
const API_INFO: ApiInfo<AzureApi, CosmosDbGetDatabaseAccountInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbGetDatabaseAccount,
    "Get a Cosmos DB database account",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbGetDatabaseAccountInputBuilder::default().resource_group("my-rg").account_name("my-cosmos").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_get_database_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-cosmos"});
        let _: CosmosDbGetDatabaseAccountInput = serde_json::from_value(json).unwrap();
    }
}
