...
const API_INFO: ApiInfo<AzureApi, CosmosDbGetMongoCollectionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbGetMongoCollection,
    "Get a Cosmos DB MongoDB collection",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbGetMongoCollectionInputBuilder::default()
        ...
        assert_eq!(json["type"], "cosmosdb_get_mongo_collection");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbGetMongoCollectionInput = serde_json::from_value(json).unwrap();
    }
}
