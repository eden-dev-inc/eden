...
const API_INFO: ApiInfo<AzureApi, CosmosDbCreateOrUpdateMongoCollectionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbCreateOrUpdateMongoCollection,
    "Create or update a Cosmos DB MongoDB collection",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.account_name, self.database_name, self.collection_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = CosmosDbCreateOrUpdateMongoCollectionInputBuilder::default()
            .resource_group("my-rg")
            .account_name("my-cosmos")
            .database_name("my-db")
            .collection_name("my-coll")
            .properties(serde_json::json!({"resource": {"id": "my-coll"}}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_create_or_update_mongo_collection");
    }
    ...
    fn deserialize_minimal() {
        let json = serde_json::json!({
            ...
            "database_name": "my-db",
            "collection_name": "my-coll",
            "properties": {"resource": {"id": "my-coll"}}
        });
        let _: CosmosDbCreateOrUpdateMongoCollectionInput = serde_json::from_value(json).unwrap();
    }
}
