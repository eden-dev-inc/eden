...
const API_INFO: ApiInfo<AzureApi, CosmosDbListCassandraKeyspacesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::CosmosDbListCassandraKeyspaces,
    "List Cosmos DB Cassandra keyspaces",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            CosmosDbListCassandraKeyspacesInputBuilder::default().resource_group("my-rg").account_name("my-cosmos").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "cosmosdb_list_cassandra_keyspaces");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: CosmosDbListCassandraKeyspacesInput = serde_json::from_value(json).unwrap();
    }
}
