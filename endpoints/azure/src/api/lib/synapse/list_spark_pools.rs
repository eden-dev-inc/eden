...
const API_INFO: ApiInfo<AzureApi, SynapseListSparkPoolsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SynapseListSparkPools,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SynapseListSparkPoolsInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "synapse_list_spark_pools");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: SynapseListSparkPoolsInput = serde_json::from_value(json).unwrap();
    }
}
