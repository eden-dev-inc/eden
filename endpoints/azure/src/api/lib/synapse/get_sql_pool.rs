...
const API_INFO: ApiInfo<AzureApi, SynapseGetSqlPoolInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SynapseGetSqlPool,
    "Get a SQL pool in a Synapse workspace",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SynapseGetSqlPoolInputBuilder::default()
        ...
        assert_eq!(json["type"], "synapse_get_sql_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "sql_pool_name": "my-pool"});
        let _: SynapseGetSqlPoolInput = serde_json::from_value(json).unwrap();
    }
}
