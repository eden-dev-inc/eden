...
const API_INFO: ApiInfo<AzureApi, SynapseResumeSqlPoolInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SynapseResumeSqlPool, "Resume a SQL pool", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.workspace_name, self.sql_pool_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SynapseResumeSqlPoolInputBuilder::default()
            .resource_group("my-rg")
            .workspace_name("my-ws")
            .sql_pool_name("my-pool")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "synapse_resume_sql_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "sql_pool_name": "my-pool"});
        let _: SynapseResumeSqlPoolInput = serde_json::from_value(json).unwrap();
    }
}
