...
const API_INFO: ApiInfo<AzureApi, SynapseDeleteWorkspaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SynapseDeleteWorkspace, "Delete a Synapse workspace", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SynapseDeleteWorkspaceInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "synapse_delete_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: SynapseDeleteWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
