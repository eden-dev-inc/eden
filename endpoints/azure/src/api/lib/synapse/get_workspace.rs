...
const API_INFO: ApiInfo<AzureApi, SynapseGetWorkspaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SynapseGetWorkspace, "Get a Synapse workspace", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SynapseGetWorkspaceInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "synapse_get_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: SynapseGetWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
