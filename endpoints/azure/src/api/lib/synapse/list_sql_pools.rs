...
const API_INFO: ApiInfo<AzureApi, SynapseListSqlPoolsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SynapseListSqlPools,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SynapseListSqlPoolsInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "synapse_list_sql_pools");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: SynapseListSqlPoolsInput = serde_json::from_value(json).unwrap();
    }
}
