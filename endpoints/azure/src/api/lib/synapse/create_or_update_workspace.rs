...
const API_INFO: ApiInfo<AzureApi, SynapseCreateOrUpdateWorkspaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SynapseCreateOrUpdateWorkspace,
    "Create or update a Synapse workspace",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SynapseCreateOrUpdateWorkspaceInputBuilder::default()
        ...
        assert_eq!(json["type"], "synapse_create_or_update_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "location": "eastus", "properties": {}});
        let _: SynapseCreateOrUpdateWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
