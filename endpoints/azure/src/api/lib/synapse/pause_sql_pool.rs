...
const API_INFO: ApiInfo<AzureApi, SynapsePauseSqlPoolInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SynapsePauseSqlPool, "Pause a SQL pool", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SynapsePauseSqlPoolInputBuilder::default()
        ...
        assert_eq!(json["type"], "synapse_pause_sql_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "sql_pool_name": "my-pool"});
        let _: SynapsePauseSqlPoolInput = serde_json::from_value(json).unwrap();
    }
}
