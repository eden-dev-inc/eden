...
const API_INFO: ApiInfo<AzureApi, SynapseListWorkspacesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SynapseListWorkspaces, "List Synapse workspaces", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Synapse/workspaces", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Synapse/workspaces", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SynapseListWorkspacesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "synapse_list_workspaces");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SynapseListWorkspacesInput = serde_json::from_value(json).unwrap();
    }
}
