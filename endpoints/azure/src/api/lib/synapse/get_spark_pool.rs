...
const API_INFO: ApiInfo<AzureApi, SynapseGetSparkPoolInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SynapseGetSparkPool,
    "Get a Spark pool in a Synapse workspace",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.workspace_name, self.spark_pool_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SynapseGetSparkPoolInputBuilder::default()
        ...
        assert_eq!(json["type"], "synapse_get_spark_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "spark_pool_name": "my-pool"});
        let _: SynapseGetSparkPoolInput = serde_json::from_value(json).unwrap();
    }
}
