...
const API_INFO: ApiInfo<AzureApi, AttestationDeleteProviderInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AttestationDeleteProvider,
    "Delete an attestation provider",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AttestationDeleteProviderInputBuilder::default().resource_group("my-rg").provider_name("my-provider").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "attestation_delete_provider");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "provider_name": "my-provider"});
        let _: AttestationDeleteProviderInput = serde_json::from_value(json).unwrap();
    }
}
