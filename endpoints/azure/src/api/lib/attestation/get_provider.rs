...
const API_INFO: ApiInfo<AzureApi, AttestationGetProviderInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AttestationGetProvider, "Get an attestation provider", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.provider_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AttestationGetProviderInputBuilder::default().resource_group("my-rg").provider_name("my-provider").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "attestation_get_provider");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "provider_name": "my-provider"});
        let _: AttestationGetProviderInput = serde_json::from_value(json).unwrap();
    }
}
