...
const API_INFO: ApiInfo<AzureApi, AttestationListProvidersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AttestationListProviders, "List attestation providers", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Attestation/attestationProviders", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Attestation/attestationProviders", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AttestationListProvidersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "attestation_list_providers");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AttestationListProvidersInput = serde_json::from_value(json).unwrap();
    }
}
