...
const API_INFO: ApiInfo<AzureApi, MlListOnlineDeploymentsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MlListOnlineDeployments,
    "List deployments for an online endpoint",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.MachineLearningServices/workspaces/{}/onlineEndpoints/{}/deployments",
            sub, self.resource_group, self.workspace_name, self.endpoint_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlListOnlineDeploymentsInputBuilder::default()
        ...
        assert_eq!(json["type"], "ml_list_online_deployments");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "endpoint_name": "my-ep"});
        let _: MlListOnlineDeploymentsInput = serde_json::from_value(json).unwrap();
    }
}
