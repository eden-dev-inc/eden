...
const API_INFO: ApiInfo<AzureApi, MlListWorkspacesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MlListWorkspaces, "List Machine Learning workspaces", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            ...
            format!("/subscriptions/{}/providers/Microsoft.MachineLearningServices/workspaces", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlListWorkspacesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "ml_list_workspaces");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: MlListWorkspacesInput = serde_json::from_value(json).unwrap();
    }
}
