...
const API_INFO: ApiInfo<AzureApi, MlGetComputeInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MlGetCompute,
    "Get a compute in a Machine Learning workspace",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.workspace_name, self.compute_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlGetComputeInputBuilder::default()
        ...
        assert_eq!(json["type"], "ml_get_compute");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "compute_name": "my-compute"});
        let _: MlGetComputeInput = serde_json::from_value(json).unwrap();
    }
}
