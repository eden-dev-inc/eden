...
const API_INFO: ApiInfo<AzureApi, MlListComputeInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MlListCompute,
    "List computes in a Machine Learning workspace",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlListComputeInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "ml_list_compute");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: MlListComputeInput = serde_json::from_value(json).unwrap();
    }
}
