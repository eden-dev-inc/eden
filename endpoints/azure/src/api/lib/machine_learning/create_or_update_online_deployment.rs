...
const API_INFO: ApiInfo<AzureApi, MlCreateOrUpdateOnlineDeploymentInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MlCreateOrUpdateOnlineDeployment,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.workspace_name, self.endpoint_name, self.deployment_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlCreateOrUpdateOnlineDeploymentInputBuilder::default()
        ...
        assert_eq!(json["type"], "ml_create_or_update_online_deployment");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: MlCreateOrUpdateOnlineDeploymentInput = serde_json::from_value(json).unwrap();
    }
}
