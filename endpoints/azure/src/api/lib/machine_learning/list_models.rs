...
const API_INFO: ApiInfo<AzureApi, MlListModelsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MlListModels,
    "List models in a Machine Learning workspace",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlListModelsInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "ml_list_models");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: MlListModelsInput = serde_json::from_value(json).unwrap();
    }
}
