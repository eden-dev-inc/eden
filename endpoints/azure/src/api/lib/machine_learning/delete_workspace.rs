...
const API_INFO: ApiInfo<AzureApi, MlDeleteWorkspaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MlDeleteWorkspace,
    "Delete a Machine Learning workspace",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlDeleteWorkspaceInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "ml_delete_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: MlDeleteWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
