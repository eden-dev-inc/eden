...
const API_INFO: ApiInfo<AzureApi, MlCreateOrUpdateWorkspaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MlCreateOrUpdateWorkspace,
    "Create or update a Machine Learning workspace",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlCreateOrUpdateWorkspaceInputBuilder::default()
        ...
        assert_eq!(json["type"], "ml_create_or_update_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "location": "eastus", "properties": {}});
        let _: MlCreateOrUpdateWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
