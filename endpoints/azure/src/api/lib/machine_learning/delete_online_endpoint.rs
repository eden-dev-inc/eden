...
const API_INFO: ApiInfo<AzureApi, MlDeleteOnlineEndpointInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MlDeleteOnlineEndpoint, "Delete an online endpoint", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlDeleteOnlineEndpointInputBuilder::default()
        ...
        assert_eq!(json["type"], "ml_delete_online_endpoint");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "endpoint_name": "my-ep"});
        let _: MlDeleteOnlineEndpointInput = serde_json::from_value(json).unwrap();
    }
}
