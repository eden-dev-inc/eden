...
const API_INFO: ApiInfo<AzureApi, MlGetWorkspaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::MlGetWorkspace, "Get a Machine Learning workspace", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlGetWorkspaceInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "ml_get_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: MlGetWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
