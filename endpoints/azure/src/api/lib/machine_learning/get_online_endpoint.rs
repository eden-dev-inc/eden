...
const API_INFO: ApiInfo<AzureApi, MlGetOnlineEndpointInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MlGetOnlineEndpoint,
    "Get an online endpoint in a Machine Learning workspace",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MlGetOnlineEndpointInputBuilder::default()
        ...
        assert_eq!(json["type"], "ml_get_online_endpoint");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "endpoint_name": "my-ep"});
        let _: MlGetOnlineEndpointInput = serde_json::from_value(json).unwrap();
    }
}
