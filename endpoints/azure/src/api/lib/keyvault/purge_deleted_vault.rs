...
const API_INFO: ApiInfo<AzureApi, KeyVaultPurgeDeletedVaultInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::KeyVaultPurgeDeletedVault,
    "Purge a deleted key vault",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultPurgeDeletedVaultInputBuilder::default().vault_name("my-vault").location("eastus").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_purge_deleted_vault");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"vault_name": "my-vault", "location": "eastus"});
        let _: KeyVaultPurgeDeletedVaultInput = serde_json::from_value(json).unwrap();
    }
}
