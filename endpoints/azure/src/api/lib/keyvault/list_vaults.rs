...
const API_INFO: ApiInfo<AzureApi, KeyVaultListVaultsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultListVaults, "List key vaults", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.KeyVault/vaults", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultListVaultsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_list_vaults");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: KeyVaultListVaultsInput = serde_json::from_value(json).unwrap();
    }
}
