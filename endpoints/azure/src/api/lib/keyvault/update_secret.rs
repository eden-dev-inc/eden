...
const API_INFO: ApiInfo<AzureApi, KeyVaultUpdateSecretInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultUpdateSecret, "Update a secret in key vault", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.vault_name, self.secret_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultUpdateSecretInputBuilder::default()
            .resource_group("my-rg")
            .vault_name("my-vault")
            .secret_name("my-secret")
            .properties(serde_json::json!({"value": "new-value"}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_update_secret");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: KeyVaultUpdateSecretInput = serde_json::from_value(json).unwrap();
    }
}
