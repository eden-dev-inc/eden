...
const API_INFO: ApiInfo<AzureApi, KeyVaultGetSecretInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultGetSecret, "Get a secret from key vault", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(ver) = &self.version {
            format!("/secrets/{}/{}", self.secret_name, ver)
        } else {
            format!("/secrets/{}", self.secret_name)
        };

        let result = client.execute_data_plane(&self.vault_url, "GET", &path, "7.4", None).await?;
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultGetSecretInputBuilder::default()
            .vault_url("https://my-vault.vault.azure.net")
            .secret_name("my-secret")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_get_secret");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: KeyVaultGetSecretInput = serde_json::from_value(json).unwrap();
    }
}
