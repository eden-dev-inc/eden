...
const API_INFO: ApiInfo<AzureApi, KeyVaultListCertificatesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::KeyVaultListCertificates,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultListCertificatesInputBuilder::default().vault_url("https://my-vault.vault.azure.net").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_list_certificates");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: KeyVaultListCertificatesInput = serde_json::from_value(json).unwrap();
    }
}
