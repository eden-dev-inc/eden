...
const API_INFO: ApiInfo<AzureApi, KeyVaultBackupKeyInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultBackupKey, "Backup a key from key vault", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/keys/{}/backup", self.key_name);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultBackupKeyInputBuilder::default()
            .vault_url("https://my-vault.vault.azure.net")
            .key_name("my-key")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_backup_key");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: KeyVaultBackupKeyInput = serde_json::from_value(json).unwrap();
    }
}
