...
const API_INFO: ApiInfo<AzureApi, KeyVaultListSecretsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultListSecrets, "List secrets in key vault", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let result = client.execute_data_plane(&self.vault_url, "GET", "/secrets", "7.4", None).await?;
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultListSecretsInputBuilder::default().vault_url("https://my-vault.vault.azure.net").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_list_secrets");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"vault_url": "https://my-vault.vault.azure.net"});
        let _: KeyVaultListSecretsInput = serde_json::from_value(json).unwrap();
    }
}
