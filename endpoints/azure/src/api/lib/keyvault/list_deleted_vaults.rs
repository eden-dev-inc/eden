...
const API_INFO: ApiInfo<AzureApi, KeyVaultListDeletedVaultsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultListDeletedVaults, "List deleted key vaults", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.KeyVault/deletedVaults", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultListDeletedVaultsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_list_deleted_vaults");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: KeyVaultListDeletedVaultsInput = serde_json::from_value(json).unwrap();
    }
}
