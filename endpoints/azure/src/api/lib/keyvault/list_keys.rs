...
const API_INFO: ApiInfo<AzureApi, KeyVaultListKeysInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultListKeys, "List keys in key vault", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let result = client.execute_data_plane(&self.vault_url, "GET", "/keys", "7.4", None).await?;
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultListKeysInputBuilder::default().vault_url("https://my-vault.vault.azure.net").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_list_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"vault_url": "https://my-vault.vault.azure.net"});
        let _: KeyVaultListKeysInput = serde_json::from_value(json).unwrap();
    }
}
