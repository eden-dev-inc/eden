...
const API_INFO: ApiInfo<AzureApi, KeyVaultGetKeyInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultGetKey, "Get a key from key vault", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultGetKeyInputBuilder::default().vault_url("https://my-vault.vault.azure.net").key_name("my-key").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_get_key");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: KeyVaultGetKeyInput = serde_json::from_value(json).unwrap();
    }
}
