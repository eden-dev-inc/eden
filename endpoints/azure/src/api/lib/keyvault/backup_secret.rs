...
const API_INFO: ApiInfo<AzureApi, KeyVaultBackupSecretInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::KeyVaultBackupSecret,
    "Backup a secret from key vault",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/secrets/{}/backup", self.secret_name);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultBackupSecretInputBuilder::default()
            .vault_url("https://my-vault.vault.azure.net")
            .secret_name("my-secret")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_backup_secret");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: KeyVaultBackupSecretInput = serde_json::from_value(json).unwrap();
    }
}
