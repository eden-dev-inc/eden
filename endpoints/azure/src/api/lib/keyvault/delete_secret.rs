...
const API_INFO: ApiInfo<AzureApi, KeyVaultDeleteSecretInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::KeyVaultDeleteSecret,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultDeleteSecretInputBuilder::default()
            .vault_url("https://my-vault.vault.azure.net")
            .secret_name("my-secret")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_delete_secret");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: KeyVaultDeleteSecretInput = serde_json::from_value(json).unwrap();
    }
}
