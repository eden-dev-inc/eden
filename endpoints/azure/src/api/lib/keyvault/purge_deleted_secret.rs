...
const API_INFO: ApiInfo<AzureApi, KeyVaultPurgeDeletedSecretInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::KeyVaultPurgeDeletedSecret,
    "Purge a deleted secret from key vault",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/deletedsecrets/{}", self.secret_name);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultPurgeDeletedSecretInputBuilder::default()
            .vault_url("https://my-vault.vault.azure.net")
            .secret_name("my-secret")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_purge_deleted_secret");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: KeyVaultPurgeDeletedSecretInput = serde_json::from_value(json).unwrap();
    }
}
