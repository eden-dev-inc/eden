...
const API_INFO: ApiInfo<AzureApi, KeyVaultGetDeletedVaultInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultGetDeletedVault, "Get a deleted key vault", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.location, self.vault_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultGetDeletedVaultInputBuilder::default().vault_name("my-vault").location("eastus").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_get_deleted_vault");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"vault_name": "my-vault", "location": "eastus"});
        let _: KeyVaultGetDeletedVaultInput = serde_json::from_value(json).unwrap();
    }
}
