...
const API_INFO: ApiInfo<AzureApi, KeyVaultDeleteCertificateInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::KeyVaultDeleteCertificate,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/certificates/{}", self.cert_name);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultDeleteCertificateInputBuilder::default()
            .vault_url("https://my-vault.vault.azure.net")
            .cert_name("my-cert")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_delete_certificate");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: KeyVaultDeleteCertificateInput = serde_json::from_value(json).unwrap();
    }
}
