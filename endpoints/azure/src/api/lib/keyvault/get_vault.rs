...
const API_INFO: ApiInfo<AzureApi, KeyVaultGetVaultInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultGetVault, "Get a key vault", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultGetVaultInputBuilder::default().resource_group("my-rg").vault_name("my-vault").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_get_vault");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "vault_name": "my-vault"});
        let _: KeyVaultGetVaultInput = serde_json::from_value(json).unwrap();
    }
}
