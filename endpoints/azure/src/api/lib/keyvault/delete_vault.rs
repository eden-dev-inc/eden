...
const API_INFO: ApiInfo<AzureApi, KeyVaultDeleteVaultInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KeyVaultDeleteVault, "Delete a key vault", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KeyVaultDeleteVaultInputBuilder::default().resource_group("my-rg").vault_name("my-vault").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "keyvault_delete_vault");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: KeyVaultDeleteVaultInput = serde_json::from_value(json).unwrap();
    }
}
