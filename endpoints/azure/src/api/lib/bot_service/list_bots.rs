...
const API_INFO: ApiInfo<AzureApi, BotServiceListBotsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::BotServiceListBots, "List Bot Service bots", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.BotService/botServices", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.BotService/botServices", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BotServiceListBotsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "bot_service_list_bots");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: BotServiceListBotsInput = serde_json::from_value(json).unwrap();
    }
}
