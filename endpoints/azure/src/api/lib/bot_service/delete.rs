...
const API_INFO: ApiInfo<AzureApi, BotServiceDeleteInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::BotServiceDelete, "Delete a Bot Service bot", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BotServiceDeleteInputBuilder::default().resource_group("my-rg").bot_name("my-bot").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "bot_service_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "bot_name": "my-bot"});
        let _: BotServiceDeleteInput = serde_json::from_value(json).unwrap();
    }
}
