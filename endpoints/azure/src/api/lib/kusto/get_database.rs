...
const API_INFO: ApiInfo<AzureApi, KustoGetDatabaseInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KustoGetDatabase, "Get a Kusto database", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KustoGetDatabaseInputBuilder::default()
        ...
        assert_eq!(json["type"], "kusto_get_database");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "database_name": "my-db"});
        let _: KustoGetDatabaseInput = serde_json::from_value(json).unwrap();
    }
}
