...
const API_INFO: ApiInfo<AzureApi, KustoListClustersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KustoListClusters, "List all Kusto clusters", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Kusto/clusters", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Kusto/clusters", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KustoListClustersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "kusto_list_clusters");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: KustoListClustersInput = serde_json::from_value(json).unwrap();
    }
}
