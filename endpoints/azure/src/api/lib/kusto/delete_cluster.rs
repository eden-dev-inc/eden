...
const API_INFO: ApiInfo<AzureApi, KustoDeleteClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KustoDeleteCluster, "Delete a Kusto cluster", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KustoDeleteClusterInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "kusto_delete_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: KustoDeleteClusterInput = serde_json::from_value(json).unwrap();
    }
}
