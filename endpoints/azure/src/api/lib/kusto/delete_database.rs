...
const API_INFO: ApiInfo<AzureApi, KustoDeleteDatabaseInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KustoDeleteDatabase, "Delete a Kusto database", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KustoDeleteDatabaseInputBuilder::default()
        ...
        assert_eq!(json["type"], "kusto_delete_database");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "database_name": "my-db"});
        let _: KustoDeleteDatabaseInput = serde_json::from_value(json).unwrap();
    }
}
