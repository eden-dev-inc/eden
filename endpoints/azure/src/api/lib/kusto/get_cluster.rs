...
const API_INFO: ApiInfo<AzureApi, KustoGetClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::KustoGetCluster, "Get a Kusto cluster", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KustoGetClusterInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "kusto_get_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: KustoGetClusterInput = serde_json::from_value(json).unwrap();
    }
}
