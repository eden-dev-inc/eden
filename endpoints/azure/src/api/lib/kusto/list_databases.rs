...
const API_INFO: ApiInfo<AzureApi, KustoListDatabasesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::KustoListDatabases,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KustoListDatabasesInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "kusto_list_databases");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: KustoListDatabasesInput = serde_json::from_value(json).unwrap();
    }
}
