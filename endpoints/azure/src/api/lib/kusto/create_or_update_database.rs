...
const API_VERSION: &str = "2023-08-15";

const API_INFO: ApiInfo<AzureApi, KustoCreateOrUpdateDatabaseInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::KustoCreateOrUpdateDatabase,
    "Create or update a Kusto database",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.cluster_name, self.database_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KustoCreateOrUpdateDatabaseInputBuilder::default()
        ...
        assert_eq!(json["type"], "kusto_create_or_update_database");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "database_name": "my-db", "location": "eastus", "properties": {}});
        let _: KustoCreateOrUpdateDatabaseInput = serde_json::from_value(json).unwrap();
    }
}
