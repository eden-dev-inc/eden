...
const API_INFO: ApiInfo<AzureApi, KustoCreateOrUpdateClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::KustoCreateOrUpdateCluster,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = KustoCreateOrUpdateClusterInputBuilder::default()
            .resource_group("my-rg")
            .cluster_name("my-cluster")
            .location("eastus")
            .properties(serde_json::json!({"sku": {"name": "Standard_D13_v2", "tier": "Standard", "capacity": 2}}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "kusto_create_or_update_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "location": "eastus", "properties": {}});
        let _: KustoCreateOrUpdateClusterInput = serde_json::from_value(json).unwrap();
    }
}
