...
const API_INFO: ApiInfo<AzureApi, PurviewListKeysInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::PurviewListKeys, "List Purview account keys", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PurviewListKeysInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "purview_list_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: PurviewListKeysInput = serde_json::from_value(json).unwrap();
    }
}
