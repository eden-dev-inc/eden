...
const API_VERSION: &str = "2021-12-01";

const API_INFO: ApiInfo<AzureApi, PurviewDeleteAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::PurviewDeleteAccount, "Delete Purview account", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PurviewDeleteAccountInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "purview_delete_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: PurviewDeleteAccountInput = serde_json::from_value(json).unwrap();
    }
}
