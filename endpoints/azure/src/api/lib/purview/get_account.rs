...
const API_INFO: ApiInfo<AzureApi, PurviewGetAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::PurviewGetAccount, "Get Purview account", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PurviewGetAccountInputBuilder::default().resource_group("my-rg").account_name("my-account").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "purview_get_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-account"});
        let _: PurviewGetAccountInput = serde_json::from_value(json).unwrap();
    }
}
