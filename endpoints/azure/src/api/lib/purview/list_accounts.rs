...
const API_INFO: ApiInfo<AzureApi, PurviewListAccountsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::PurviewListAccounts, "List Purview accounts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Purview/accounts", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PurviewListAccountsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "purview_list_accounts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: PurviewListAccountsInput = serde_json::from_value(json).unwrap();
    }
}
