...
const API_INFO: ApiInfo<AzureApi, PolicyGetPolicyDefinitionInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::PolicyGetPolicyDefinition, "Get policy definition", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PolicyGetPolicyDefinitionInputBuilder::default().policy_definition_name("my-policy").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "policy_get_policy_definition");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"policy_definition_name": "my-policy"});
        let _: PolicyGetPolicyDefinitionInput = serde_json::from_value(json).unwrap();
    }
}
