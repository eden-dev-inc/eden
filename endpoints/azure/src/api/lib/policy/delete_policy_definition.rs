...
const API_INFO: ApiInfo<AzureApi, PolicyDeletePolicyDefinitionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PolicyDeletePolicyDefinition,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PolicyDeletePolicyDefinitionInputBuilder::default().policy_definition_name("my-policy").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "policy_delete_policy_definition");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"policy_definition_name": "my-policy"});
        let _: PolicyDeletePolicyDefinitionInput = serde_json::from_value(json).unwrap();
    }
}
