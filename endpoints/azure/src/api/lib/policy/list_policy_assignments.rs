...
const API_INFO: ApiInfo<AzureApi, PolicyListPolicyAssignmentsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::PolicyListPolicyAssignments, "List policy assignments", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Authorization/policyAssignments", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PolicyListPolicyAssignmentsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "policy_list_policy_assignments");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: PolicyListPolicyAssignmentsInput = serde_json::from_value(json).unwrap();
    }
}
