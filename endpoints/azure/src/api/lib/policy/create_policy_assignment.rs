...
const API_INFO: ApiInfo<AzureApi, PolicyCreatePolicyAssignmentInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PolicyCreatePolicyAssignment,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PolicyCreatePolicyAssignmentInputBuilder::default()
        ...
        assert_eq!(json["type"], "policy_create_policy_assignment");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"scope": "/subscriptions/sub-id", "policy_assignment_name": "my-assignment", "properties": {}});
        let _: PolicyCreatePolicyAssignmentInput = serde_json::from_value(json).unwrap();
    }
}
