...
const API_INFO: ApiInfo<AzureApi, PolicyCreateOrUpdatePolicyDefinitionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PolicyCreateOrUpdatePolicyDefinition,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.policy_definition_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PolicyCreateOrUpdatePolicyDefinitionInputBuilder::default()
            .policy_definition_name("my-policy")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "policy_create_or_update_policy_definition");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"policy_definition_name": "my-policy", "properties": {}});
        let _: PolicyCreateOrUpdatePolicyDefinitionInput = serde_json::from_value(json).unwrap();
    }
}
