...
const API_INFO: ApiInfo<AzureApi, PolicyListPolicyComplianceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PolicyListPolicyCompliance,
    "List policy compliance summary",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.PolicyInsights/policyStates/latest/summarize", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PolicyListPolicyComplianceInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "policy_list_policy_compliance");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: PolicyListPolicyComplianceInput = serde_json::from_value(json).unwrap();
    }
}
