...
const API_INFO: ApiInfo<AzureApi, PolicyListPolicyDefinitionsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::PolicyListPolicyDefinitions, "List policy definitions", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Authorization/policyDefinitions", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PolicyListPolicyDefinitionsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "policy_list_policy_definitions");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: PolicyListPolicyDefinitionsInput = serde_json::from_value(json).unwrap();
    }
}
