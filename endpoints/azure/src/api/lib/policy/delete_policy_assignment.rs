...
const API_INFO: ApiInfo<AzureApi, PolicyDeletePolicyAssignmentInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::PolicyDeletePolicyAssignment,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = PolicyDeletePolicyAssignmentInputBuilder::default()
            .scope("/subscriptions/sub-id")
            .policy_assignment_name("my-assignment")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "policy_delete_policy_assignment");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"scope": "/subscriptions/sub-id", "policy_assignment_name": "my-assignment"});
        let _: PolicyDeletePolicyAssignmentInput = serde_json::from_value(json).unwrap();
    }
}
