...
const API_INFO: ApiInfo<AzureApi, AppInsightsGetApiKeysInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppInsightsGetApiKeys,
    "Get API keys for an Application Insights component",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.component_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppInsightsGetApiKeysInputBuilder::default().resource_group("my-rg").component_name("my-component").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_insights_get_api_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "component_name": "my-component"});
        let _: AppInsightsGetApiKeysInput = serde_json::from_value(json).unwrap();
    }
}
