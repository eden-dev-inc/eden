...
const API_INFO: ApiInfo<AzureApi, AppInsightsGetInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppInsightsGet,
    "Get an Application Insights component",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppInsightsGetInputBuilder::default().resource_group("my-rg").component_name("my-component").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_insights_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "component_name": "my-component"});
        let _: AppInsightsGetInput = serde_json::from_value(json).unwrap();
    }
}
