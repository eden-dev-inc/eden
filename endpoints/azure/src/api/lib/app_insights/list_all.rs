...
const API_INFO: ApiInfo<AzureApi, AppInsightsListAllInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppInsightsListAll,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Insights/components", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Insights/components", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppInsightsListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_insights_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AppInsightsListAllInput = serde_json::from_value(json).unwrap();
    }
}
