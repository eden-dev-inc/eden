...
const API_VERSION: &str = "2020-02-02";

const API_INFO: ApiInfo<AzureApi, AppInsightsListWebTestsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::AppInsightsListWebTests, "List web tests", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Insights/webtests", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Insights/webtests", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppInsightsListWebTestsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_insights_list_web_tests");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AppInsightsListWebTestsInput = serde_json::from_value(json).unwrap();
    }
}
