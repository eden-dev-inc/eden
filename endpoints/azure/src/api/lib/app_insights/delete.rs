...
const API_INFO: ApiInfo<AzureApi, AppInsightsDeleteInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::AppInsightsDelete,
    "Delete an Application Insights component",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AppInsightsDeleteInputBuilder::default().resource_group("my-rg").component_name("my-component").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "app_insights_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "component_name": "my-component"});
        let _: AppInsightsDeleteInput = serde_json::from_value(json).unwrap();
    }
}
