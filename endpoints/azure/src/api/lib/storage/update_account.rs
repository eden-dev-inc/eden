...
const API_INFO: ApiInfo<AzureApi, StorageUpdateAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageUpdateAccount, "Update a storage account", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageUpdateAccountInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_update_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "properties": {}});
        let _: StorageUpdateAccountInput = serde_json::from_value(json).unwrap();
    }
}
