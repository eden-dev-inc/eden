...
const API_INFO: ApiInfo<AzureApi, StorageGetBlobServicePropertiesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::StorageGetBlobServiceProperties,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageGetBlobServicePropertiesInputBuilder::default()
            .resource_group("my-rg")
            .account_name("mystorage")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "storage_get_blob_service_properties");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage"});
        let _: StorageGetBlobServicePropertiesInput = serde_json::from_value(json).unwrap();
    }
}
