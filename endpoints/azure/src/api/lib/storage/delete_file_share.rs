...
const API_INFO: ApiInfo<AzureApi, StorageDeleteFileShareInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageDeleteFileShare, "Delete a file share", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageDeleteFileShareInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_delete_file_share");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "share_name": "myshare"});
        let _: StorageDeleteFileShareInput = serde_json::from_value(json).unwrap();
    }
}
