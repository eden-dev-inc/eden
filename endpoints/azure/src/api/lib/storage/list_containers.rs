...
const API_INFO: ApiInfo<AzureApi, StorageListContainersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageListContainers, "List blob containers", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageListContainersInputBuilder::default().resource_group("my-rg").account_name("mystorage").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "storage_list_containers");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage"});
        let _: StorageListContainersInput = serde_json::from_value(json).unwrap();
    }
}
