...
const API_INFO: ApiInfo<AzureApi, StorageDeleteQueueInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageDeleteQueue, "Delete a storage queue", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageDeleteQueueInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_delete_queue");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "queue_name": "myqueue"});
        let _: StorageDeleteQueueInput = serde_json::from_value(json).unwrap();
    }
}
