...
const API_INFO: ApiInfo<AzureApi, StorageCreateTableInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageCreateTable, "Create a storage table", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageCreateTableInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_create_table");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "table_name": "mytable"});
        let _: StorageCreateTableInput = serde_json::from_value(json).unwrap();
    }
}
