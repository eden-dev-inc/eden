...
const API_INFO: ApiInfo<AzureApi, StorageListAccountsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageListAccounts, "List storage accounts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.Storage/storageAccounts", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageListAccountsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "storage_list_accounts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: StorageListAccountsInput = serde_json::from_value(json).unwrap();
    }
}
