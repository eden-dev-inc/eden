...
const API_INFO: ApiInfo<AzureApi, StorageListFileSharesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageListFileShares, "List file shares", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageListFileSharesInputBuilder::default().resource_group("my-rg").account_name("mystorage").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "storage_list_file_shares");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage"});
        let _: StorageListFileSharesInput = serde_json::from_value(json).unwrap();
    }
}
