...
const API_INFO: ApiInfo<AzureApi, StorageDeleteContainerInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageDeleteContainer, "Delete a blob container", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageDeleteContainerInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_delete_container");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "container_name": "mycontainer"});
        let _: StorageDeleteContainerInput = serde_json::from_value(json).unwrap();
    }
}
