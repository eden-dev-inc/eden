...
const API_INFO: ApiInfo<AzureApi, StorageCreateQueueInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageCreateQueue, "Create a storage queue", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.account_name, self.queue_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageCreateQueueInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_create_queue");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "queue_name": "myqueue"});
        let _: StorageCreateQueueInput = serde_json::from_value(json).unwrap();
    }
}
