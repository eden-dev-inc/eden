...
const API_INFO: ApiInfo<AzureApi, StorageDeleteBlobInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageDeleteBlob, "Delete a blob", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Storage/storageAccounts/{}/blobServices/default/containers/{}/blobs/{}",
            sub, self.resource_group, self.account_name, self.container_name, self.blob_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageDeleteBlobInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_delete_blob");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "container_name": "mycontainer", "blob_name": "myblob"});
        let _: StorageDeleteBlobInput = serde_json::from_value(json).unwrap();
    }
}
