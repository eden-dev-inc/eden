...
const API_INFO: ApiInfo<AzureApi, StorageListAccountKeysInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageListAccountKeys, "List storage account keys", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageListAccountKeysInputBuilder::default().resource_group("my-rg").account_name("mystorage").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "storage_list_account_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage"});
        let _: StorageListAccountKeysInput = serde_json::from_value(json).unwrap();
    }
}
