...
const API_INFO: ApiInfo<AzureApi, StorageDeleteAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageDeleteAccount, "Delete a storage account", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageDeleteAccountInputBuilder::default().resource_group("my-rg").account_name("mystorage").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "storage_delete_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage"});
        let _: StorageDeleteAccountInput = serde_json::from_value(json).unwrap();
    }
}
