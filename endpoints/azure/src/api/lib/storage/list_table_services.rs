...
const API_INFO: ApiInfo<AzureApi, StorageListTableServicesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::StorageListTableServices,
    "List storage table services",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageListTableServicesInputBuilder::default().resource_group("my-rg").account_name("mystorage").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "storage_list_table_services");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage"});
        let _: StorageListTableServicesInput = serde_json::from_value(json).unwrap();
    }
}
