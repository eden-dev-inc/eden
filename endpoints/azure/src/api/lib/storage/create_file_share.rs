...
const API_INFO: ApiInfo<AzureApi, StorageCreateFileShareInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageCreateFileShare, "Create a file share", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.account_name, self.share_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageCreateFileShareInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_create_file_share");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "share_name": "myshare"});
        let _: StorageCreateFileShareInput = serde_json::from_value(json).unwrap();
    }
}
