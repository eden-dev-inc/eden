...
const API_INFO: ApiInfo<AzureApi, StorageGetFileShareInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageGetFileShare, "Get a file share", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageGetFileShareInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_get_file_share");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "share_name": "myshare"});
        let _: StorageGetFileShareInput = serde_json::from_value(json).unwrap();
    }
}
