...
const API_INFO: ApiInfo<AzureApi, StorageCreateAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageCreateAccount, "Create a storage account", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        body_map.insert("location".to_string(), Value::String(self.location.clone()));
        body_map.insert("kind".to_string(), Value::String(self.kind.clone()));
        body_map.insert("sku".to_string(), serde_json::json!({"name": self.sku_name}));
        if let Some(props) = &self.properties {
            body_map.insert("properties".to_string(), props.clone());
        }
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageCreateAccountInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_create_account");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: StorageCreateAccountInput = serde_json::from_value(json).unwrap();
    }
}
