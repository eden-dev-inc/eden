...
const API_INFO: ApiInfo<AzureApi, StorageGetTableInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageGetTable, "Get a table", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageGetTableInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_get_table");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "table_name": "mytable"});
        let _: StorageGetTableInput = serde_json::from_value(json).unwrap();
    }
}
