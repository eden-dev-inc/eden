...
const API_INFO: ApiInfo<AzureApi, StorageGetContainerInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageGetContainer, "Get a blob container", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageGetContainerInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_get_container");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "container_name": "mycontainer"});
        let _: StorageGetContainerInput = serde_json::from_value(json).unwrap();
    }
}
