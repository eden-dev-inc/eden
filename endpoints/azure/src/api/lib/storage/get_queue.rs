...
const API_INFO: ApiInfo<AzureApi, StorageGetQueueInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageGetQueue, "Get a queue", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageGetQueueInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_get_queue");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "queue_name": "myqueue"});
        let _: StorageGetQueueInput = serde_json::from_value(json).unwrap();
    }
}
