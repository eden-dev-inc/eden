...
const API_INFO: ApiInfo<AzureApi, StorageDeleteTableInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageDeleteTable, "Delete a storage table", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageDeleteTableInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_delete_table");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "table_name": "mytable"});
        let _: StorageDeleteTableInput = serde_json::from_value(json).unwrap();
    }
}
