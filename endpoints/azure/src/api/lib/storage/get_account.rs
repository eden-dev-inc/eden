...
const API_INFO: ApiInfo<AzureApi, StorageGetAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageGetAccount, "Get a storage account", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageGetAccountInputBuilder::default().resource_group("my-rg").account_name("mystorage").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "storage_get_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage"});
        let _: StorageGetAccountInput = serde_json::from_value(json).unwrap();
    }
}
