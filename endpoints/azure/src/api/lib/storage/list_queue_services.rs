...
const API_INFO: ApiInfo<AzureApi, StorageListQueueServicesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageListQueueServices, "List queue services", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageListQueueServicesInputBuilder::default().resource_group("my-rg").account_name("mystorage").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "storage_list_queue_services");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage"});
        let _: StorageListQueueServicesInput = serde_json::from_value(json).unwrap();
    }
}
