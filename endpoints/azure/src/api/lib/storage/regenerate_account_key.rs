...
const API_INFO: ApiInfo<AzureApi, StorageRegenerateAccountKeyInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::StorageRegenerateAccountKey,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageRegenerateAccountKeyInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_regenerate_account_key");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "key_name": "key1"});
        let _: StorageRegenerateAccountKeyInput = serde_json::from_value(json).unwrap();
    }
}
