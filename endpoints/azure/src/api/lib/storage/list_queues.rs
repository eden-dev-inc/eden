...
const API_INFO: ApiInfo<AzureApi, StorageListQueuesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageListQueues, "List storage queues", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageListQueuesInputBuilder::default().resource_group("my-rg").account_name("mystorage").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "storage_list_queues");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage"});
        let _: StorageListQueuesInput = serde_json::from_value(json).unwrap();
    }
}
