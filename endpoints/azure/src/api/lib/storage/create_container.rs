...
const API_INFO: ApiInfo<AzureApi, StorageCreateContainerInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StorageCreateContainer, "Create a blob container", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.account_name, self.container_name
        );

        let body = if let Some(props) = &self.properties {
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StorageCreateContainerInputBuilder::default()
        ...
        assert_eq!(json["type"], "storage_create_container");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "mystorage", "container_name": "mycontainer"});
        let _: StorageCreateContainerInput = serde_json::from_value(json).unwrap();
    }
}
