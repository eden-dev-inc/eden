...
const API_INFO: ApiInfo<AzureApi, StreamAnalyticsListStreamingJobsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::StreamAnalyticsListStreamingJobs,
    "List streaming jobs",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/providers/Microsoft.StreamAnalytics/streamingjobs", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StreamAnalyticsListStreamingJobsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "stream_analytics_list_streaming_jobs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: StreamAnalyticsListStreamingJobsInput = serde_json::from_value(json).unwrap();
    }
}
