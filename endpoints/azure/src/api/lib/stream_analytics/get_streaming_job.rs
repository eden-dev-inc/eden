...
const API_INFO: ApiInfo<AzureApi, StreamAnalyticsGetStreamingJobInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StreamAnalyticsGetStreamingJob, "Get streaming job", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StreamAnalyticsGetStreamingJobInputBuilder::default().resource_group("my-rg").job_name("my-job").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "stream_analytics_get_streaming_job");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "job_name": "my-job"});
        let _: StreamAnalyticsGetStreamingJobInput = serde_json::from_value(json).unwrap();
    }
}
