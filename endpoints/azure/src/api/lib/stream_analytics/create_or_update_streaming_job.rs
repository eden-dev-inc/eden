...
const API_VERSION: &str = "2021-10-01-preview";

const API_INFO: ApiInfo<AzureApi, StreamAnalyticsCreateOrUpdateStreamingJobInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::StreamAnalyticsCreateOrUpdateStreamingJob,
    "Create or update streaming job",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.job_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StreamAnalyticsCreateOrUpdateStreamingJobInputBuilder::default()
        ...
        assert_eq!(json["type"], "stream_analytics_create_or_update_streaming_job");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "job_name": "my-job", "location": "eastus", "properties": {}});
        let _: StreamAnalyticsCreateOrUpdateStreamingJobInput = serde_json::from_value(json).unwrap();
    }
}
