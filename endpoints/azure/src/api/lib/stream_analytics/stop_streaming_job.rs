...
const API_INFO: ApiInfo<AzureApi, StreamAnalyticsStopStreamingJobInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StreamAnalyticsStopStreamingJob, "Stop streaming job", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StreamAnalyticsStopStreamingJobInputBuilder::default().resource_group("my-rg").job_name("my-job").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "stream_analytics_stop_streaming_job");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "job_name": "my-job"});
        let _: StreamAnalyticsStopStreamingJobInput = serde_json::from_value(json).unwrap();
    }
}
