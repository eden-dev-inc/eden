...
const API_INFO: ApiInfo<AzureApi, StackHciCreateOrUpdateClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::StackHciCreateOrUpdateCluster,
    "Create or update Stack HCI cluster",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StackHciCreateOrUpdateClusterInputBuilder::default()
        ...
        assert_eq!(json["type"], "stack_hci_create_or_update_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "location": "eastus", "properties": {}});
        let _: StackHciCreateOrUpdateClusterInput = serde_json::from_value(json).unwrap();
    }
}
