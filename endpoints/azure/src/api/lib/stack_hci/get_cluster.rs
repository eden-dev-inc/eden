...
const API_INFO: ApiInfo<AzureApi, StackHciGetClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StackHciGetCluster, "Get Stack HCI cluster", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StackHciGetClusterInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "stack_hci_get_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: StackHciGetClusterInput = serde_json::from_value(json).unwrap();
    }
}
