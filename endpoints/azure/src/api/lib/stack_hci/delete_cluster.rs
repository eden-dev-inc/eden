...
const API_INFO: ApiInfo<AzureApi, StackHciDeleteClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StackHciDeleteCluster, "Delete Stack HCI cluster", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StackHciDeleteClusterInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "stack_hci_delete_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: StackHciDeleteClusterInput = serde_json::from_value(json).unwrap();
    }
}
