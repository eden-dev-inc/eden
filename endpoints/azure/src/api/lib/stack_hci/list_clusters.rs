...
const API_INFO: ApiInfo<AzureApi, StackHciListClustersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::StackHciListClusters, "List Stack HCI clusters", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.AzureStackHCI/clusters", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.AzureStackHCI/clusters", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = StackHciListClustersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "stack_hci_list_clusters");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: StackHciListClustersInput = serde_json::from_value(json).unwrap();
    }
}
