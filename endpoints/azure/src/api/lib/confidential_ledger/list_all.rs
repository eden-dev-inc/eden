...
const API_VERSION: &str = "2023-06-28-preview";

const API_INFO: ApiInfo<AzureApi, ConfidentialLedgerListAllInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ConfidentialLedgerListAll,
    "List all confidential ledgers",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.ConfidentialLedger/ledgers", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.ConfidentialLedger/ledgers", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ConfidentialLedgerListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "confidential_ledger_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ConfidentialLedgerListAllInput = serde_json::from_value(json).unwrap();
    }
}
