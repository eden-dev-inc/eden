...
const API_INFO: ApiInfo<AzureApi, ConfidentialLedgerDeleteInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ConfidentialLedgerDelete,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ConfidentialLedgerDeleteInputBuilder::default().resource_group("my-rg").ledger_name("my-ledger").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "confidential_ledger_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "ledger_name": "my-ledger"});
        let _: ConfidentialLedgerDeleteInput = serde_json::from_value(json).unwrap();
    }
}
