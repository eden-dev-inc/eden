...
const API_INFO: ApiInfo<AzureApi, ConfidentialLedgerCreateOrUpdateInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ConfidentialLedgerCreateOrUpdate,
    "Create or update confidential ledger",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.ledger_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ConfidentialLedgerCreateOrUpdateInputBuilder::default()
        ...
        assert_eq!(json["type"], "confidential_ledger_create_or_update");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "ledger_name": "my-ledger", "location": "eastus", "properties": {}});
        let _: ConfidentialLedgerCreateOrUpdateInput = serde_json::from_value(json).unwrap();
    }
}
