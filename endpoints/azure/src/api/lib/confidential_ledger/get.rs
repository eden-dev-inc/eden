...
const API_INFO: ApiInfo<AzureApi, ConfidentialLedgerGetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ConfidentialLedgerGet, "Get confidential ledger", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ConfidentialLedgerGetInputBuilder::default().resource_group("my-rg").ledger_name("my-ledger").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "confidential_ledger_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "ledger_name": "my-ledger"});
        let _: ConfidentialLedgerGetInput = serde_json::from_value(json).unwrap();
    }
}
