...
const API_INFO: ApiInfo<AzureApi, MaintenanceGetConfigurationInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MaintenanceGetConfiguration,
    "Get a maintenance configuration",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MaintenanceGetConfigurationInputBuilder::default()
            .resource_group("my-rg")
            .configuration_name("my-config")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "maintenance_get_configuration");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "configuration_name": "my-config"});
        let _: MaintenanceGetConfigurationInput = serde_json::from_value(json).unwrap();
    }
}
