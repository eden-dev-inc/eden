...
const API_INFO: ApiInfo<AzureApi, MaintenanceListConfigurationsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::MaintenanceListConfigurations,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            ...
            format!("/subscriptions/{}/providers/Microsoft.Maintenance/maintenanceConfigurations", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = MaintenanceListConfigurationsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "maintenance_list_configurations");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: MaintenanceListConfigurationsInput = serde_json::from_value(json).unwrap();
    }
}
