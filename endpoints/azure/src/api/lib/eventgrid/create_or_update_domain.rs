...
const API_INFO: ApiInfo<AzureApi, EventGridCreateOrUpdateDomainInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventGridCreateOrUpdateDomain,
    "Create or update an EventGrid domain",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventGridCreateOrUpdateDomainInputBuilder::default()
        ...
        assert_eq!(json["type"], "event_grid_create_or_update_domain");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "domain_name": "my-domain", "location": "eastus", "properties": {}});
        let _: EventGridCreateOrUpdateDomainInput = serde_json::from_value(json).unwrap();
    }
}
