...
const API_INFO: ApiInfo<AzureApi, EventGridDeleteEventSubscriptionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventGridDeleteEventSubscription,
    "Delete an EventGrid event subscription",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventGridDeleteEventSubscriptionInputBuilder::default()
            .scope("subscriptions/sub-id/resourceGroups/my-rg")
            .event_subscription_name("my-sub")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "event_grid_delete_event_subscription");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"scope": "subscriptions/sub-id/resourceGroups/my-rg", "event_subscription_name": "my-sub"});
        let _: EventGridDeleteEventSubscriptionInput = serde_json::from_value(json).unwrap();
    }
}
