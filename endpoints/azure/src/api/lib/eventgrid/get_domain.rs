...
const API_INFO: ApiInfo<AzureApi, EventGridGetDomainInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::EventGridGetDomain, "Get an EventGrid domain", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventGridGetDomainInputBuilder::default().resource_group("my-rg").domain_name("my-domain").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "event_grid_get_domain");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "domain_name": "my-domain"});
        let _: EventGridGetDomainInput = serde_json::from_value(json).unwrap();
    }
}
