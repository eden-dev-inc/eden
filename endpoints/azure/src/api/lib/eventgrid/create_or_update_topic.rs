...
const API_INFO: ApiInfo<AzureApi, EventGridCreateOrUpdateTopicInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventGridCreateOrUpdateTopic,
    "Create or update an EventGrid topic",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventGridCreateOrUpdateTopicInputBuilder::default()
        ...
        assert_eq!(json["type"], "event_grid_create_or_update_topic");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "topic_name": "my-topic", "location": "eastus", "properties": {}});
        let _: EventGridCreateOrUpdateTopicInput = serde_json::from_value(json).unwrap();
    }
}
