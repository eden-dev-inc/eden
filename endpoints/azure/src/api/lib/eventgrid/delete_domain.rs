...
const API_INFO: ApiInfo<AzureApi, EventGridDeleteDomainInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::EventGridDeleteDomain, "Delete an EventGrid domain", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.domain_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventGridDeleteDomainInputBuilder::default().resource_group("my-rg").domain_name("my-domain").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "event_grid_delete_domain");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "domain_name": "my-domain"});
        let _: EventGridDeleteDomainInput = serde_json::from_value(json).unwrap();
    }
}
