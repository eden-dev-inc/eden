...
const API_INFO: ApiInfo<AzureApi, EventGridCreateOrUpdateEventSubscriptionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventGridCreateOrUpdateEventSubscription,
    "Create or update an EventGrid event subscription",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/{}/providers/Microsoft.EventGrid/eventSubscriptions/{}", self.scope, self.event_subscription_name);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventGridCreateOrUpdateEventSubscriptionInputBuilder::default()
            .scope("subscriptions/sub-id/resourceGroups/my-rg")
            .event_subscription_name("my-sub")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "event_grid_create_or_update_event_subscription");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"scope": "subscriptions/sub-id/resourceGroups/my-rg", "event_subscription_name": "my-sub", "properties": {}});
        let _: EventGridCreateOrUpdateEventSubscriptionInput = serde_json::from_value(json).unwrap();
    }
}
