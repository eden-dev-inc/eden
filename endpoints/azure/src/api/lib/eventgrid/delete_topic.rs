...
const API_INFO: ApiInfo<AzureApi, EventGridDeleteTopicInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::EventGridDeleteTopic, "Delete an EventGrid topic", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.topic_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventGridDeleteTopicInputBuilder::default().resource_group("my-rg").topic_name("my-topic").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "event_grid_delete_topic");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "topic_name": "my-topic"});
        let _: EventGridDeleteTopicInput = serde_json::from_value(json).unwrap();
    }
}
