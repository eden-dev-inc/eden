...
const API_INFO: ApiInfo<AzureApi, EventGridGetTopicInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::EventGridGetTopic, "Get an EventGrid topic", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventGridGetTopicInputBuilder::default().resource_group("my-rg").topic_name("my-topic").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "event_grid_get_topic");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "topic_name": "my-topic"});
        let _: EventGridGetTopicInput = serde_json::from_value(json).unwrap();
    }
}
