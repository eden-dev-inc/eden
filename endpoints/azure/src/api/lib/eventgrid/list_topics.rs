...
const API_INFO: ApiInfo<AzureApi, EventGridListTopicsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::EventGridListTopics, "List EventGrid topics", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.EventGrid/topics", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.EventGrid/topics", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventGridListTopicsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "event_grid_list_topics");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: EventGridListTopicsInput = serde_json::from_value(json).unwrap();
    }
}
