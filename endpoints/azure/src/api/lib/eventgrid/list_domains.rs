...
const API_INFO: ApiInfo<AzureApi, EventGridListDomainsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::EventGridListDomains, "List EventGrid domains", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.EventGrid/domains", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.EventGrid/domains", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventGridListDomainsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "event_grid_list_domains");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: EventGridListDomainsInput = serde_json::from_value(json).unwrap();
    }
}
