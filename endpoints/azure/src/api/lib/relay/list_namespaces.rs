...
const API_INFO: ApiInfo<AzureApi, RelayListNamespacesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::RelayListNamespaces, "List Relay namespaces", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Relay/namespaces", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Relay/namespaces", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RelayListNamespacesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "relay_list_namespaces");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: RelayListNamespacesInput = serde_json::from_value(json).unwrap();
    }
}
