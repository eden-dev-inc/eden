...
const API_INFO: ApiInfo<AzureApi, RelayListHybridConnectionsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RelayListHybridConnections,
    "List Relay hybrid connections",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RelayListHybridConnectionsInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "relay_list_hybrid_connections");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: RelayListHybridConnectionsInput = serde_json::from_value(json).unwrap();
    }
}
