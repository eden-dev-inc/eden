...
const API_INFO: ApiInfo<AzureApi, RelayDeleteNamespaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::RelayDeleteNamespace, "Delete a Relay namespace", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RelayDeleteNamespaceInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "relay_delete_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: RelayDeleteNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
