...
const API_INFO: ApiInfo<AzureApi, RelayCreateOrUpdateNamespaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RelayCreateOrUpdateNamespace,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RelayCreateOrUpdateNamespaceInputBuilder::default()
        ...
        assert_eq!(json["type"], "relay_create_or_update_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "location": "eastus", "properties": {}});
        let _: RelayCreateOrUpdateNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
