...
const API_INFO: ApiInfo<AzureApi, RelayDeleteHybridConnectionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RelayDeleteHybridConnection,
    "Delete a Relay hybrid connection",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RelayDeleteHybridConnectionInputBuilder::default()
        ...
        assert_eq!(json["type"], "relay_delete_hybrid_connection");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "hybrid_connection_name": "my-hc"});
        let _: RelayDeleteHybridConnectionInput = serde_json::from_value(json).unwrap();
    }
}
