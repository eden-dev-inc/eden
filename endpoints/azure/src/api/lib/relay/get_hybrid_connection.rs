...
const API_INFO: ApiInfo<AzureApi, RelayGetHybridConnectionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RelayGetHybridConnection,
    "Get a Relay hybrid connection",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RelayGetHybridConnectionInputBuilder::default()
        ...
        assert_eq!(json["type"], "relay_get_hybrid_connection");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "hybrid_connection_name": "my-hc"});
        let _: RelayGetHybridConnectionInput = serde_json::from_value(json).unwrap();
    }
}
