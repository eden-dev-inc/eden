...
const API_INFO: ApiInfo<AzureApi, RelayGetNamespaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::RelayGetNamespace, "Get a Relay namespace", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RelayGetNamespaceInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "relay_get_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: RelayGetNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
