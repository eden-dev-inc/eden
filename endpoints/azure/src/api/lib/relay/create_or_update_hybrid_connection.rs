...
const API_INFO: ApiInfo<AzureApi, RelayCreateOrUpdateHybridConnectionInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::RelayCreateOrUpdateHybridConnection,
    "Create or update a Relay hybrid connection",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.namespace_name, self.hybrid_connection_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RelayCreateOrUpdateHybridConnectionInputBuilder::default()
            .resource_group("my-rg")
            .namespace_name("my-ns")
            .hybrid_connection_name("my-hc")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "relay_create_or_update_hybrid_connection");
    }

    #[test]
    fn deserialize_minimal() {
        let json =
            serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "hybrid_connection_name": "my-hc", "properties": {}});
        let _: RelayCreateOrUpdateHybridConnectionInput = serde_json::from_value(json).unwrap();
    }
}
