...
const API_INFO: ApiInfo<AzureApi, QuantumDeleteWorkspaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::QuantumDeleteWorkspace, "Delete a Quantum workspace", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = QuantumDeleteWorkspaceInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "quantum_delete_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: QuantumDeleteWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
