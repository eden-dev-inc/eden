...
const API_INFO: ApiInfo<AzureApi, QuantumGetWorkspaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::QuantumGetWorkspace, "Get a Quantum workspace", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = QuantumGetWorkspaceInputBuilder::default().resource_group("my-rg").workspace_name("my-ws").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "quantum_get_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws"});
        let _: QuantumGetWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
