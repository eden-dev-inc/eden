...
const API_INFO: ApiInfo<AzureApi, QuantumCreateOrUpdateWorkspaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::QuantumCreateOrUpdateWorkspace,
    "Create or update a Quantum workspace",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = QuantumCreateOrUpdateWorkspaceInputBuilder::default()
        ...
        assert_eq!(json["type"], "quantum_create_or_update_workspace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "workspace_name": "my-ws", "location": "eastus", "properties": {}});
        let _: QuantumCreateOrUpdateWorkspaceInput = serde_json::from_value(json).unwrap();
    }
}
