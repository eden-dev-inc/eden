...
const API_VERSION: &str = "2023-11-13-preview";

const API_INFO: ApiInfo<AzureApi, QuantumListWorkspacesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::QuantumListWorkspaces, "List Quantum workspaces", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Quantum/workspaces", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Quantum/workspaces", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = QuantumListWorkspacesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "quantum_list_workspaces");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: QuantumListWorkspacesInput = serde_json::from_value(json).unwrap();
    }
}
