...
const API_INFO: ApiInfo<AzureApi, DataFactoryDeletePipelineInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryDeletePipeline,
    "Delete a Data Factory pipeline",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryDeletePipelineInputBuilder::default()
        ...
        assert_eq!(json["type"], "data_factory_delete_pipeline");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "pipeline_name": "my-pipeline"});
        let _: DataFactoryDeletePipelineInput = serde_json::from_value(json).unwrap();
    }
}
