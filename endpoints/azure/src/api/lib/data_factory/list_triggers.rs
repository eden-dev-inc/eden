...
const API_INFO: ApiInfo<AzureApi, DataFactoryListTriggersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DataFactoryListTriggers, "List Data Factory triggers", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryListTriggersInputBuilder::default().resource_group("my-rg").factory_name("my-factory").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "data_factory_list_triggers");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory"});
        let _: DataFactoryListTriggersInput = serde_json::from_value(json).unwrap();
    }
}
