...
const API_INFO: ApiInfo<AzureApi, DataFactoryGetLinkedServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryGetLinkedService,
    "Get a Data Factory linked service",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryGetLinkedServiceInputBuilder::default()
            .resource_group("my-rg")
            .factory_name("my-factory")
            .linked_service_name("my-ls")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "data_factory_get_linked_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "linked_service_name": "my-ls"});
        let _: DataFactoryGetLinkedServiceInput = serde_json::from_value(json).unwrap();
    }
}
