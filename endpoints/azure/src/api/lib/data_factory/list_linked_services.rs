...
const API_INFO: ApiInfo<AzureApi, DataFactoryListLinkedServicesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryListLinkedServices,
...
mod tests {
    ...
    fn builder_serde() {
        let input =
            DataFactoryListLinkedServicesInputBuilder::default().resource_group("my-rg").factory_name("my-factory").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "data_factory_list_linked_services");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory"});
        let _: DataFactoryListLinkedServicesInput = serde_json::from_value(json).unwrap();
    }
}
