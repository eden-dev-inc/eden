...
const API_INFO: ApiInfo<AzureApi, DataFactoryCreatePipelineRunInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryCreatePipelineRun,
    "Create a Data Factory pipeline run",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.factory_name, self.pipeline_name
        );

        let result = client.execute("POST", &path, API_VERSION, self.parameters.as_ref(), None).await?;
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryCreatePipelineRunInputBuilder::default()
        ...
        assert_eq!(json["type"], "data_factory_create_pipeline_run");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "pipeline_name": "my-pipeline"});
        let _: DataFactoryCreatePipelineRunInput = serde_json::from_value(json).unwrap();
    }
}
