...
const API_INFO: ApiInfo<AzureApi, DataFactoryDeleteFactoryInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryDeleteFactory,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryDeleteFactoryInputBuilder::default().resource_group("my-rg").factory_name("my-factory").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "data_factory_delete_factory");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory"});
        let _: DataFactoryDeleteFactoryInput = serde_json::from_value(json).unwrap();
    }
}
