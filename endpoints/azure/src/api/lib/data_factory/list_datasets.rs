...
const API_INFO: ApiInfo<AzureApi, DataFactoryListDatasetsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DataFactoryListDatasets, "List Data Factory datasets", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.factory_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryListDatasetsInputBuilder::default().resource_group("my-rg").factory_name("my-factory").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "data_factory_list_datasets");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory"});
        let _: DataFactoryListDatasetsInput = serde_json::from_value(json).unwrap();
    }
}
