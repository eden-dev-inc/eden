...
const API_INFO: ApiInfo<AzureApi, DataFactoryDeleteTriggerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryDeleteTrigger,
    "Delete a Data Factory trigger",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryDeleteTriggerInputBuilder::default()
        ...
        assert_eq!(json["type"], "data_factory_delete_trigger");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "trigger_name": "my-trigger"});
        let _: DataFactoryDeleteTriggerInput = serde_json::from_value(json).unwrap();
    }
}
