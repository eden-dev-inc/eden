...
const API_INFO: ApiInfo<AzureApi, DataFactoryGetDatasetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DataFactoryGetDataset, "Get a Data Factory dataset", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryGetDatasetInputBuilder::default()
        ...
        assert_eq!(json["type"], "data_factory_get_dataset");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "dataset_name": "my-dataset"});
        let _: DataFactoryGetDatasetInput = serde_json::from_value(json).unwrap();
    }
}
