...
const API_INFO: ApiInfo<AzureApi, DataFactoryCreateOrUpdateLinkedServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryCreateOrUpdateLinkedService,
    "Create or update a Data Factory linked service",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.factory_name, self.linked_service_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryCreateOrUpdateLinkedServiceInputBuilder::default()
        ...
        assert_eq!(json["type"], "data_factory_create_or_update_linked_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "linked_service_name": "my-linked-service", "properties": {}});
        let _: DataFactoryCreateOrUpdateLinkedServiceInput = serde_json::from_value(json).unwrap();
    }
}
