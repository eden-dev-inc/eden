...
const API_INFO: ApiInfo<AzureApi, DataFactoryCreateOrUpdateTriggerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryCreateOrUpdateTrigger,
    "Create or update a Data Factory trigger",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.factory_name, self.trigger_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryCreateOrUpdateTriggerInputBuilder::default()
        ...
        assert_eq!(json["type"], "data_factory_create_or_update_trigger");
    }

    #[test]
    fn deserialize_minimal() {
        let json =
            serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "trigger_name": "my-trigger", "properties": {}});
        let _: DataFactoryCreateOrUpdateTriggerInput = serde_json::from_value(json).unwrap();
    }
}
