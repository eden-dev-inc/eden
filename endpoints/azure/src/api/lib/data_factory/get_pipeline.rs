...
const API_INFO: ApiInfo<AzureApi, DataFactoryGetPipelineInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DataFactoryGetPipeline, "Get a Data Factory pipeline", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryGetPipelineInputBuilder::default()
        ...
        assert_eq!(json["type"], "data_factory_get_pipeline");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "pipeline_name": "my-pipeline"});
        let _: DataFactoryGetPipelineInput = serde_json::from_value(json).unwrap();
    }
}
