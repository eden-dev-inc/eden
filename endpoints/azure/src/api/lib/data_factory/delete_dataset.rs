...
const API_INFO: ApiInfo<AzureApi, DataFactoryDeleteDatasetInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryDeleteDataset,
    "Delete a Data Factory dataset",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryDeleteDatasetInputBuilder::default()
        ...
        assert_eq!(json["type"], "data_factory_delete_dataset");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "dataset_name": "my-dataset"});
        let _: DataFactoryDeleteDatasetInput = serde_json::from_value(json).unwrap();
    }
}
