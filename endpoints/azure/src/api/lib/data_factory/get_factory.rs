...
const API_INFO: ApiInfo<AzureApi, DataFactoryGetFactoryInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DataFactoryGetFactory, "Get a Data Factory instance", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryGetFactoryInputBuilder::default().resource_group("my-rg").factory_name("my-factory").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "data_factory_get_factory");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory"});
        let _: DataFactoryGetFactoryInput = serde_json::from_value(json).unwrap();
    }
}
