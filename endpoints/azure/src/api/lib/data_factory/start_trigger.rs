...
const API_INFO: ApiInfo<AzureApi, DataFactoryStartTriggerInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryStartTrigger,
    "Start a Data Factory trigger",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryStartTriggerInputBuilder::default()
        ...
        assert_eq!(json["type"], "data_factory_start_trigger");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "trigger_name": "my-trigger"});
        let _: DataFactoryStartTriggerInput = serde_json::from_value(json).unwrap();
    }
}
