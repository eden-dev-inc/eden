...
const API_INFO: ApiInfo<AzureApi, DataFactoryListFactoriesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryListFactories,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.DataFactory/factories", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.DataFactory/factories", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryListFactoriesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "data_factory_list_factories");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DataFactoryListFactoriesInput = serde_json::from_value(json).unwrap();
    }
}
