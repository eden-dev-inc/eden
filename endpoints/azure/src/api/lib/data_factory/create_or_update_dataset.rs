...
const API_INFO: ApiInfo<AzureApi, DataFactoryCreateOrUpdateDatasetInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryCreateOrUpdateDataset,
    "Create or update a Data Factory dataset",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.factory_name, self.dataset_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryCreateOrUpdateDatasetInputBuilder::default()
        ...
        assert_eq!(json["type"], "data_factory_create_or_update_dataset");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory", "dataset_name": "my-dataset"});
        let _: DataFactoryCreateOrUpdateDatasetInput = serde_json::from_value(json).unwrap();
    }
}
