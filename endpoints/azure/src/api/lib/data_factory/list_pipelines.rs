...
const API_INFO: ApiInfo<AzureApi, DataFactoryListPipelinesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DataFactoryListPipelines,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DataFactoryListPipelinesInputBuilder::default().resource_group("my-rg").factory_name("my-factory").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "data_factory_list_pipelines");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "factory_name": "my-factory"});
        let _: DataFactoryListPipelinesInput = serde_json::from_value(json).unwrap();
    }
}
