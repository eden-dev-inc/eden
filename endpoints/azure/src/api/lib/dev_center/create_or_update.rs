...
const API_INFO: ApiInfo<AzureApi, DevCenterCreateOrUpdateInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::DevCenterCreateOrUpdate,
    "Create or update dev center",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.dev_center_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DevCenterCreateOrUpdateInputBuilder::default()
            .resource_group("my-rg")
            .dev_center_name("my-dc")
            .location("eastus")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "dev_center_create_or_update");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "dev_center_name": "my-dc", "location": "eastus", "properties": {}});
        let _: DevCenterCreateOrUpdateInput = serde_json::from_value(json).unwrap();
    }
}
