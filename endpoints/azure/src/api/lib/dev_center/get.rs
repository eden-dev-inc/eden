...
const API_INFO: ApiInfo<AzureApi, DevCenterGetInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DevCenterGet, "Get dev center", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DevCenterGetInputBuilder::default().resource_group("my-rg").dev_center_name("my-dc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "dev_center_get");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "dev_center_name": "my-dc"});
        let _: DevCenterGetInput = serde_json::from_value(json).unwrap();
    }
}
