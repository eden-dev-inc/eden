...
const API_INFO: ApiInfo<AzureApi, DevCenterDeleteInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DevCenterDelete, "Delete dev center", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DevCenterDeleteInputBuilder::default().resource_group("my-rg").dev_center_name("my-dc").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "dev_center_delete");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "dev_center_name": "my-dc"});
        let _: DevCenterDeleteInput = serde_json::from_value(json).unwrap();
    }
}
