...
const API_INFO: ApiInfo<AzureApi, DevCenterListProjectsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DevCenterListProjects, "List Dev Center projects", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.DevCenter/projects", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.DevCenter/projects", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DevCenterListProjectsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "dev_center_list_projects");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DevCenterListProjectsInput = serde_json::from_value(json).unwrap();
    }
}
