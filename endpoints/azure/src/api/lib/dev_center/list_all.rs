...
const API_INFO: ApiInfo<AzureApi, DevCenterListAllInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::DevCenterListAll, "List all dev centers", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.DevCenter/devcenters", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.DevCenter/devcenters", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DevCenterListAllInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "dev_center_list_all");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DevCenterListAllInput = serde_json::from_value(json).unwrap();
    }
}
