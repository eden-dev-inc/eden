...
const API_INFO: ApiInfo<AzureApi, ContainerDeleteAgentPoolInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerDeleteAgentPool, "Delete an agent pool", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.cluster_name, self.pool_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerDeleteAgentPoolInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_delete_agent_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "pool_name": "pool1"});
        let _: ContainerDeleteAgentPoolInput = serde_json::from_value(json).unwrap();
    }
}
