...
const API_INFO: ApiInfo<AzureApi, ContainerStopManagedClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerStopManagedCluster, "Stop a managed cluster", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerStopManagedClusterInputBuilder::default().resource_group("my-rg").name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_stop_managed_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-cluster"});
        let _: ContainerStopManagedClusterInput = serde_json::from_value(json).unwrap();
    }
}
