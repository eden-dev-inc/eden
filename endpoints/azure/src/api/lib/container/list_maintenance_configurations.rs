...
const API_INFO: ApiInfo<AzureApi, ContainerListMaintenanceConfigurationsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerListMaintenanceConfigurations,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerListMaintenanceConfigurationsInputBuilder::default()
            .resource_group("my-rg")
            .cluster_name("my-cluster")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_list_maintenance_configurations");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: ContainerListMaintenanceConfigurationsInput = serde_json::from_value(json).unwrap();
    }
}
