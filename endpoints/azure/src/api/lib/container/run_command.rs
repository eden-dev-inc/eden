...
const API_INFO: ApiInfo<AzureApi, ContainerRunCommandInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRunCommand,
    "Run a command on a managed cluster",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRunCommandInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_run_command");
    }
    ...
    fn deserialize_minimal() {
        ...
        let _: ContainerRunCommandInput = serde_json::from_value(json).unwrap();
    }
}
