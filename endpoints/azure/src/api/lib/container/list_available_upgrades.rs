...
const API_INFO: ApiInfo<AzureApi, ContainerListAvailableUpgradesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerListAvailableUpgrades,
    "List available upgrades for a managed cluster",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            "/subscriptions/{}/resourceGroups/{}/providers/Microsoft.ContainerService/managedClusters/{}/upgradeProfiles",
            sub, self.resource_group, self.name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerListAvailableUpgradesInputBuilder::default().resource_group("my-rg").name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_list_available_upgrades");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-cluster"});
        let _: ContainerListAvailableUpgradesInput = serde_json::from_value(json).unwrap();
    }
}
