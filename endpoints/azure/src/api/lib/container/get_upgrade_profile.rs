...
const API_INFO: ApiInfo<AzureApi, ContainerGetUpgradeProfileInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerGetUpgradeProfile, "Get upgrade profile", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerGetUpgradeProfileInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_get_upgrade_profile");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: ContainerGetUpgradeProfileInput = serde_json::from_value(json).unwrap();
    }
}
