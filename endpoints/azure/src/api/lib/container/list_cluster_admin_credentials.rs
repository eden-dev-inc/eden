...
const API_INFO: ApiInfo<AzureApi, ContainerListClusterAdminCredentialsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerListClusterAdminCredentials,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerListClusterAdminCredentialsInputBuilder::default().resource_group("my-rg").name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_list_cluster_admin_credentials");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-cluster"});
        let _: ContainerListClusterAdminCredentialsInput = serde_json::from_value(json).unwrap();
    }
}
