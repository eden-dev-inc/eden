...
const API_INFO: ApiInfo<AzureApi, ContainerStartManagedClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerStartManagedCluster,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerStartManagedClusterInputBuilder::default().resource_group("my-rg").name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_start_managed_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-cluster"});
        let _: ContainerStartManagedClusterInput = serde_json::from_value(json).unwrap();
    }
}
