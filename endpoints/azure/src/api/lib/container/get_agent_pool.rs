...
const API_INFO: ApiInfo<AzureApi, ContainerGetAgentPoolInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerGetAgentPool, "Get an agent pool", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerGetAgentPoolInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_get_agent_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "pool_name": "pool1"});
        let _: ContainerGetAgentPoolInput = serde_json::from_value(json).unwrap();
    }
}
