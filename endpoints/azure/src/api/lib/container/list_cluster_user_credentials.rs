...
const API_INFO: ApiInfo<AzureApi, ContainerListClusterUserCredentialsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerListClusterUserCredentials,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerListClusterUserCredentialsInputBuilder::default().resource_group("my-rg").name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_list_cluster_user_credentials");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-cluster"});
        let _: ContainerListClusterUserCredentialsInput = serde_json::from_value(json).unwrap();
    }
}
