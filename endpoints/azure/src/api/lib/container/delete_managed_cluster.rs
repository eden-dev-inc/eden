...
const API_INFO: ApiInfo<AzureApi, ContainerDeleteManagedClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerDeleteManagedCluster,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerDeleteManagedClusterInputBuilder::default().resource_group("my-rg").name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_delete_managed_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-cluster"});
        let _: ContainerDeleteManagedClusterInput = serde_json::from_value(json).unwrap();
    }
}
