...
const API_INFO: ApiInfo<AzureApi, ContainerRotateClusterCertificatesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerRotateClusterCertificates,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerRotateClusterCertificatesInputBuilder::default().resource_group("my-rg").name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_rotate_cluster_certificates");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-cluster"});
        let _: ContainerRotateClusterCertificatesInput = serde_json::from_value(json).unwrap();
    }
}
