...
const API_INFO: ApiInfo<AzureApi, ContainerListManagedClustersInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerListManagedClusters, "List managed clusters", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.ContainerService/managedClusters", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.ContainerService/managedClusters", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerListManagedClustersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_list_managed_clusters");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ContainerListManagedClustersInput = serde_json::from_value(json).unwrap();
    }
}
