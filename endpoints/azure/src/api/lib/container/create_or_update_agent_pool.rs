...
const API_INFO: ApiInfo<AzureApi, ContainerCreateOrUpdateAgentPoolInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerCreateOrUpdateAgentPool,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerCreateOrUpdateAgentPoolInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_create_or_update_agent_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster", "pool_name": "pool1"});
        let _: ContainerCreateOrUpdateAgentPoolInput = serde_json::from_value(json).unwrap();
    }
}
