...
const API_INFO: ApiInfo<AzureApi, ContainerGetManagedClusterInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ContainerGetManagedCluster, "Get a managed cluster", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerGetManagedClusterInputBuilder::default().resource_group("my-rg").name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_get_managed_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-cluster"});
        let _: ContainerGetManagedClusterInput = serde_json::from_value(json).unwrap();
    }
}
