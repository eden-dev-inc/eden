...
const API_INFO: ApiInfo<AzureApi, ContainerListAgentPoolsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerListAgentPools,
    "List agent pools for a managed cluster",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerListAgentPoolsInputBuilder::default().resource_group("my-rg").cluster_name("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "container_list_agent_pools");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "cluster_name": "my-cluster"});
        let _: ContainerListAgentPoolsInput = serde_json::from_value(json).unwrap();
    }
}
