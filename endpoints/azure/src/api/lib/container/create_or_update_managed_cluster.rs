...
const API_INFO: ApiInfo<AzureApi, ContainerCreateOrUpdateManagedClusterInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ContainerCreateOrUpdateManagedCluster,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ContainerCreateOrUpdateManagedClusterInputBuilder::default()
        ...
        assert_eq!(json["type"], "container_create_or_update_managed_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "name": "my-cluster", "location": "eastus"});
        let _: ContainerCreateOrUpdateManagedClusterInput = serde_json::from_value(json).unwrap();
    }
}
