...
const API_INFO: ApiInfo<AzureApi, BatchGetAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::BatchGetAccount, "Get a Batch account", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchGetAccountInputBuilder::default().resource_group("my-rg").account_name("my-batch").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "batch_get_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-batch"});
        let _: BatchGetAccountInput = serde_json::from_value(json).unwrap();
    }
}
