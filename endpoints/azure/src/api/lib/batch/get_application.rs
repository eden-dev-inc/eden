...
const API_INFO: ApiInfo<AzureApi, BatchGetApplicationInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::BatchGetApplication,
    "Get an application in a Batch account",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchGetApplicationInputBuilder::default()
        ...
        assert_eq!(json["type"], "batch_get_application");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-batch", "application_name": "my-app"});
        let _: BatchGetApplicationInput = serde_json::from_value(json).unwrap();
    }
}
