...
const API_INFO: ApiInfo<AzureApi, BatchListApplicationsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::BatchListApplications,
    "List applications in a Batch account",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchListApplicationsInputBuilder::default().resource_group("my-rg").account_name("my-batch").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "batch_list_applications");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-batch"});
        let _: BatchListApplicationsInput = serde_json::from_value(json).unwrap();
    }
}
