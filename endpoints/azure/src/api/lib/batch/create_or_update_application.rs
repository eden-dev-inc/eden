...
const API_INFO: ApiInfo<AzureApi, BatchCreateOrUpdateApplicationInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::BatchCreateOrUpdateApplication,
    "Create or update an application in a Batch account",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.account_name, self.application_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchCreateOrUpdateApplicationInputBuilder::default()
        ...
        assert_eq!(json["type"], "batch_create_or_update_application");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-batch", "application_name": "my-app"});
        let _: BatchCreateOrUpdateApplicationInput = serde_json::from_value(json).unwrap();
    }
}
