...
const API_INFO: ApiInfo<AzureApi, BatchDeleteAccountInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::BatchDeleteAccount, "Delete a Batch account", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchDeleteAccountInputBuilder::default().resource_group("my-rg").account_name("my-batch").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "batch_delete_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-batch"});
        let _: BatchDeleteAccountInput = serde_json::from_value(json).unwrap();
    }
}
