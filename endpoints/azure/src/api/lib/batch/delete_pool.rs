...
const API_INFO: ApiInfo<AzureApi, BatchDeletePoolInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::BatchDeletePool, "Delete a pool in a Batch account", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchDeletePoolInputBuilder::default()
        ...
        assert_eq!(json["type"], "batch_delete_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-batch", "pool_name": "my-pool"});
        let _: BatchDeletePoolInput = serde_json::from_value(json).unwrap();
    }
}
