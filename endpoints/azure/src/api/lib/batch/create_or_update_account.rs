...
const API_INFO: ApiInfo<AzureApi, BatchCreateOrUpdateAccountInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::BatchCreateOrUpdateAccount,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchCreateOrUpdateAccountInputBuilder::default()
        ...
        assert_eq!(json["type"], "batch_create_or_update_account");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-batch", "location": "eastus"});
        let _: BatchCreateOrUpdateAccountInput = serde_json::from_value(json).unwrap();
    }
}
