...
const API_INFO: ApiInfo<AzureApi, BatchUpdatePoolInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::BatchUpdatePool, "Update a pool in a Batch account", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.account_name, self.pool_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchUpdatePoolInputBuilder::default()
            .resource_group("my-rg")
            .account_name("my-batch")
            .pool_name("my-pool")
            .properties(serde_json::json!({}))
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "batch_update_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-batch", "pool_name": "my-pool", "properties": {}});
        let _: BatchUpdatePoolInput = serde_json::from_value(json).unwrap();
    }
}
