...
const API_INFO: ApiInfo<AzureApi, BatchGetPoolInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::BatchGetPool, "Get a pool in a Batch account", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchGetPoolInputBuilder::default()
        ...
        assert_eq!(json["type"], "batch_get_pool");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-batch", "pool_name": "my-pool"});
        let _: BatchGetPoolInput = serde_json::from_value(json).unwrap();
    }
}
