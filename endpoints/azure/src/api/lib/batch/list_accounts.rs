...
const API_INFO: ApiInfo<AzureApi, BatchListAccountsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::BatchListAccounts, "List Batch accounts", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Batch/batchAccounts", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Batch/batchAccounts", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchListAccountsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "batch_list_accounts");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: BatchListAccountsInput = serde_json::from_value(json).unwrap();
    }
}
