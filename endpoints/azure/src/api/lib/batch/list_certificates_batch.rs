...
const API_INFO: ApiInfo<AzureApi, BatchListCertificatesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::BatchListCertificates,
    "List certificates in a Batch account",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = BatchListCertificatesInputBuilder::default().resource_group("my-rg").account_name("my-batch").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "batch_list_certificates");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "account_name": "my-batch"});
        let _: BatchListCertificatesInput = serde_json::from_value(json).unwrap();
    }
}
