...
const API_INFO: ApiInfo<AzureApi, ResourceListResourcesInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ResourceListResources,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/resources", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ResourceListResourcesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "resource_list_resources");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ResourceListResourcesInput = serde_json::from_value(json).unwrap();
    }
}
