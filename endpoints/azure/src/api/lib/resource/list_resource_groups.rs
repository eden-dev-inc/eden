...
const API_INFO: ApiInfo<AzureApi, ResourceListResourceGroupsInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ResourceListResourceGroups, "List resource groups", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/resourcegroups", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ResourceListResourceGroupsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "resource_list_resource_groups");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ResourceListResourceGroupsInput = serde_json::from_value(json).unwrap();
    }
}
