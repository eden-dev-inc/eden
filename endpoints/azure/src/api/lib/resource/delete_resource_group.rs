...
const API_INFO: ApiInfo<AzureApi, ResourceDeleteResourceGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ResourceDeleteResourceGroup,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ResourceDeleteResourceGroupInputBuilder::default().resource_group("my-rg").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "resource_delete_resource_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg"});
        let _: ResourceDeleteResourceGroupInput = serde_json::from_value(json).unwrap();
    }
}
