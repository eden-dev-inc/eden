...
const API_INFO: ApiInfo<AzureApi, ResourceCreateOrUpdateResourceGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::ResourceCreateOrUpdateResourceGroup,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}/resourcegroups/{}", sub, self.resource_group);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ResourceCreateOrUpdateResourceGroupInputBuilder::default().resource_group("my-rg").location("eastus").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "resource_create_or_update_resource_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "location": "eastus"});
        let _: ResourceCreateOrUpdateResourceGroupInput = serde_json::from_value(json).unwrap();
    }
}
