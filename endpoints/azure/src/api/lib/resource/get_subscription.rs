...
const API_INFO: ApiInfo<AzureApi, ResourceGetSubscriptionInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ResourceGetSubscription, "Get a subscription", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/subscriptions/{}", sub);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ResourceGetSubscriptionInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "resource_get_subscription");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ResourceGetSubscriptionInput = serde_json::from_value(json).unwrap();
    }
}
