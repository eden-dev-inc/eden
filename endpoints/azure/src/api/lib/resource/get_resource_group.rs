...
const API_INFO: ApiInfo<AzureApi, ResourceGetResourceGroupInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::ResourceGetResourceGroup, "Get a resource group", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ResourceGetResourceGroupInputBuilder::default().resource_group("my-rg").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "resource_get_resource_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg"});
        let _: ResourceGetResourceGroupInput = serde_json::from_value(json).unwrap();
    }
}
