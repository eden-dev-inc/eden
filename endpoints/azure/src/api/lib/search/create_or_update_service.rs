...
const API_INFO: ApiInfo<AzureApi, SearchCreateOrUpdateServiceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::SearchCreateOrUpdateService,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SearchCreateOrUpdateServiceInputBuilder::default()
        ...
        assert_eq!(json["type"], "search_create_or_update_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json =
            serde_json::json!({"resource_group": "my-rg", "service_name": "my-search", "location": "eastus", "sku": {"name": "basic"}});
        let _: SearchCreateOrUpdateServiceInput = serde_json::from_value(json).unwrap();
    }
}
