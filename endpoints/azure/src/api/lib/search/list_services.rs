...
const API_INFO: ApiInfo<AzureApi, SearchListServicesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SearchListServices, "List Search services", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.Search/searchServices", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.Search/searchServices", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SearchListServicesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "search_list_services");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SearchListServicesInput = serde_json::from_value(json).unwrap();
    }
}
