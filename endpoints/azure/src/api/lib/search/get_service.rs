...
const API_INFO: ApiInfo<AzureApi, SearchGetServiceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SearchGetService, "Get a Search service", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SearchGetServiceInputBuilder::default().resource_group("my-rg").service_name("my-search").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "search_get_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-search"});
        let _: SearchGetServiceInput = serde_json::from_value(json).unwrap();
    }
}
