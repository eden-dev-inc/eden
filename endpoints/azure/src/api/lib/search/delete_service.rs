...
const API_INFO: ApiInfo<AzureApi, SearchDeleteServiceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::SearchDeleteService, "Delete a Search service", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SearchDeleteServiceInputBuilder::default().resource_group("my-rg").service_name("my-search").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "search_delete_service");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "service_name": "my-search"});
        let _: SearchDeleteServiceInput = serde_json::from_value(json).unwrap();
    }
}
