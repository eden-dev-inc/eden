...
const API_INFO: ApiInfo<AzureApi, EventHubGetNamespaceInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::EventHubGetNamespace, "Get an Event Hub namespace", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubGetNamespaceInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "eventhub_get_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: EventHubGetNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
