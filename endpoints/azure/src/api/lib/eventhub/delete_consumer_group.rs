...
const API_INFO: ApiInfo<AzureApi, EventHubDeleteConsumerGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventHubDeleteConsumerGroup,
    "Delete a consumer group for an event hub",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubDeleteConsumerGroupInputBuilder::default()
        ...
        assert_eq!(json["type"], "eventhub_delete_consumer_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "event_hub_name": "my-hub", "consumer_group_name": "my-cg"});
        let _: EventHubDeleteConsumerGroupInput = serde_json::from_value(json).unwrap();
    }
}
