...
const API_INFO: ApiInfo<AzureApi, EventHubListConsumerGroupsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventHubListConsumerGroups,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubListConsumerGroupsInputBuilder::default()
        ...
        assert_eq!(json["type"], "eventhub_list_consumer_groups");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "event_hub_name": "my-hub"});
        let _: EventHubListConsumerGroupsInput = serde_json::from_value(json).unwrap();
    }
}
