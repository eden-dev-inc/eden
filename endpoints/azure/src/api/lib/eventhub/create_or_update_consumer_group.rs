...
const API_INFO: ApiInfo<AzureApi, EventHubCreateOrUpdateConsumerGroupInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventHubCreateOrUpdateConsumerGroup,
    "Create or update a consumer group for an event hub",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.namespace_name, self.event_hub_name, self.consumer_group_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubCreateOrUpdateConsumerGroupInputBuilder::default()
        ...
        assert_eq!(json["type"], "eventhub_create_or_update_consumer_group");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "event_hub_name": "my-hub", "consumer_group_name": "my-cg", "properties": {}});
        let _: EventHubCreateOrUpdateConsumerGroupInput = serde_json::from_value(json).unwrap();
    }
}
