...
const API_INFO: ApiInfo<AzureApi, EventHubDeleteNamespaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventHubDeleteNamespace,
    "Delete an Event Hub namespace",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubDeleteNamespaceInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "eventhub_delete_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: EventHubDeleteNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
