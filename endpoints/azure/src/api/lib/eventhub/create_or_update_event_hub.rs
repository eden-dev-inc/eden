...
const API_INFO: ApiInfo<AzureApi, EventHubCreateOrUpdateEventHubInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventHubCreateOrUpdateEventHub,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubCreateOrUpdateEventHubInputBuilder::default()
        ...
        assert_eq!(json["type"], "eventhub_create_or_update_event_hub");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "event_hub_name": "my-hub"});
        let _: EventHubCreateOrUpdateEventHubInput = serde_json::from_value(json).unwrap();
    }
}
