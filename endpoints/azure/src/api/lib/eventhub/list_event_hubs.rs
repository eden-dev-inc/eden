...
const API_INFO: ApiInfo<AzureApi, EventHubListEventHubsInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventHubListEventHubs,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubListEventHubsInputBuilder::default().resource_group("my-rg").namespace_name("my-ns").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "eventhub_list_event_hubs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns"});
        let _: EventHubListEventHubsInput = serde_json::from_value(json).unwrap();
    }
}
