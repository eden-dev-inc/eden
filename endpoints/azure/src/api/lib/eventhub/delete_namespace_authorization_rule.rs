...
const API_INFO: ApiInfo<AzureApi, EventHubDeleteNamespaceAuthorizationRuleInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventHubDeleteNamespaceAuthorizationRule,
    "Delete an Event Hub namespace authorization rule",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.namespace_name, self.authorization_rule_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubDeleteNamespaceAuthorizationRuleInputBuilder::default()
            .resource_group("my-rg")
            .namespace_name("my-ns")
            .authorization_rule_name("my-rule")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "event_hub_delete_namespace_authorization_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "authorization_rule_name": "my-rule"});
        let _: EventHubDeleteNamespaceAuthorizationRuleInput = serde_json::from_value(json).unwrap();
    }
}
