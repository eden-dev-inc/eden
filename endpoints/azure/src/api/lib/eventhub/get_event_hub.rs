...
const API_INFO: ApiInfo<AzureApi, EventHubGetEventHubInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::EventHubGetEventHub, "Get an event hub", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubGetEventHubInputBuilder::default()
        ...
        assert_eq!(json["type"], "eventhub_get_event_hub");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "event_hub_name": "my-hub"});
        let _: EventHubGetEventHubInput = serde_json::from_value(json).unwrap();
    }
}
