...
const API_INFO: ApiInfo<AzureApi, EventHubDeleteEventHubInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::EventHubDeleteEventHub, "Delete an event hub", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!(
            ...
            sub, self.resource_group, self.namespace_name, self.event_hub_name
        );
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubDeleteEventHubInputBuilder::default()
        ...
        assert_eq!(json["type"], "eventhub_delete_event_hub");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "event_hub_name": "my-hub"});
        let _: EventHubDeleteEventHubInput = serde_json::from_value(json).unwrap();
    }
}
