...
const API_INFO: ApiInfo<AzureApi, EventHubListNamespacesInput> =
    ApiInfo::new(EpKind::Azure, AzureApi::EventHubListNamespaces, "List Event Hub namespaces", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AzureAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = if let Some(rg) = &self.resource_group {
            format!("/subscriptions/{}/resourceGroups/{}/providers/Microsoft.EventHub/namespaces", sub, rg)
        } else {
            format!("/subscriptions/{}/providers/Microsoft.EventHub/namespaces", sub)
        };
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubListNamespacesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "eventhub_list_namespaces");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: EventHubListNamespacesInput = serde_json::from_value(json).unwrap();
    }
}
