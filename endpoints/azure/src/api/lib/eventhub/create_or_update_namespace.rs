...
const API_INFO: ApiInfo<AzureApi, EventHubCreateOrUpdateNamespaceInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventHubCreateOrUpdateNamespace,
    "Create or update an Event Hub namespace",
    ReqType::Write,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubCreateOrUpdateNamespaceInputBuilder::default()
        ...
        assert_eq!(json["type"], "eventhub_create_or_update_namespace");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "location": "eastus"});
        let _: EventHubCreateOrUpdateNamespaceInput = serde_json::from_value(json).unwrap();
    }
}
