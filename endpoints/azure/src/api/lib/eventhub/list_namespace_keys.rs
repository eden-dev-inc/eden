...
const API_INFO: ApiInfo<AzureApi, EventHubListNamespaceKeysInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventHubListNamespaceKeys,
    "List Event Hub namespace keys",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubListNamespaceKeysInputBuilder::default()
        ...
        assert_eq!(json["type"], "event_hub_list_namespace_keys");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "authorization_rule_name": "my-rule"});
        let _: EventHubListNamespaceKeysInput = serde_json::from_value(json).unwrap();
    }
}
