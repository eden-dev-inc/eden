...
const API_INFO: ApiInfo<AzureApi, EventHubGetNamespaceAuthorizationRuleInput> = ApiInfo::new(
    EpKind::Azure,
    AzureApi::EventHubGetNamespaceAuthorizationRule,
    "Get an Event Hub namespace authorization rule",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EventHubGetNamespaceAuthorizationRuleInputBuilder::default()
        ...
        assert_eq!(json["type"], "event_hub_get_namespace_authorization_rule");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_group": "my-rg", "namespace_name": "my-ns", "authorization_rule_name": "my-rule"});
        let _: EventHubGetNamespaceAuthorizationRuleInput = serde_json::from_value(json).unwrap();
    }
}
