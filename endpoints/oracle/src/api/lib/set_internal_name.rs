...
const API_INFO: ApiInfo<OracleApi, SetInternalNameInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::SetInternalName, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct SetInternalNameInput {
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.set_internal_name(&self.internal_name).map_err(EpError::request)?;
