...
const API_INFO: ApiInfo<OracleApi, ChangePasswordInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::ChangePassword, "Oracle Change Password", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.change_password(&self.username, &self.old_password, &self.new_password).map_err(EpError::request)?;
