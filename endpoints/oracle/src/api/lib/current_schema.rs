...
const API_INFO: ApiInfo<OracleApi, CurrentSchemaInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::CurrentSchema, "Oracle current schema", ReqType::Read, true);

crate::oracle_endpoint! {
    struct CurrentSchemaInput {}
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let result = client.current_schema().map_err(EpError::request)?;
