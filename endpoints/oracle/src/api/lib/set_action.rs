...
const API_INFO: ApiInfo<OracleApi, SetActionInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::SetAction, "Oracle execute named", ReqType::Read, true);

crate::oracle_endpoint! {
    struct SetActionInput {
        action: String
    }
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.set_action(&self.action).map_err(EpError::request)?;
