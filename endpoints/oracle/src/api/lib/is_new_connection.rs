...
use crate::api::output::BoolOutput;
...
const API_INFO: ApiInfo<OracleApi, IsNewConnectionInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::IsNewConnection, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct IsNewConnectionInput {}
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let result = client.is_new_connection();
        ...
        Ok(Box::new(BoolOutput(result).to_output()) as Box<dyn EpOutput>)
    }
    #[named]
