...
use format::timestamp::DurationWrapper;
...
const API_INFO: ApiInfo<OracleApi, SetCallTimeoutInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::SetCallTimeout, "Oracle execute named", ReqType::Read, true);

crate::oracle_endpoint! {
    struct SetCallTimeoutInput {
        dur: Option<DurationWrapper>
    }
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        match &self.dur {
            Some(dur) => client.set_call_timeout(Some(dur.as_duration().to_std().map_err(EpError::request)?)),
            None => client.set_call_timeout(None),
        }
