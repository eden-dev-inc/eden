...
const API_INFO: ApiInfo<OracleApi, SetStmtCacheSizeInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::SetStmtCacheSize, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct SetStmtCacheSizeInput {
        size: u32,
    }
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.set_stmt_cache_size(self.size).map_err(EpError::request)?;
