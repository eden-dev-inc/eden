use crate::api::input::ShutdownModeWrapper;
...
const API_INFO: ApiInfo<OracleApi, ShutdownDatabaseInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::ShutdownDatabase, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct ShutdownDatabaseInput {
        mode: ShutdownModeWrapper,
    }
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.shutdown_database(self.mode.clone().into()).map_err(EpError::request)?;
