...
const API_INFO: ApiInfo<OracleApi, ClearObjectTypeCacheInput> = ApiInfo::new(
    EpKind::Oracle,
    OracleApi::ClearObjectTypeCache,
    "Oracle clear object type cache",
    ReqType::Read,
    true,
);

crate::oracle_endpoint! {
    struct ClearObjectTypeCacheInput {}
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.clear_object_type_cache().map_err(EpError::request)?;
