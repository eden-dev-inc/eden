...
const API_INFO: ApiInfo<OracleApi, SetClientIdentifierInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::SetClientIdentifier, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct SetClientIdentifierInput {
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.set_client_identifier(&self.client_identifier).map_err(EpError::request)?;
