...
const API_INFO: ApiInfo<OracleApi, PingInput> = ApiInfo::new(EpKind::Oracle, OracleApi::Ping, "Oracle execute named", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.ping().map_err(EpError::request)?;
