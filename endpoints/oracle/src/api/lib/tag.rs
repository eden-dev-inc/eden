...
const API_INFO: ApiInfo<OracleApi, TagInput> = ApiInfo::new(EpKind::Oracle, OracleApi::Tag, "Oracle execute named", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        Ok(Box::new(StringOutput(result.to_string()).to_output()) as Box<dyn EpOutput>)
    }
    #[named]
