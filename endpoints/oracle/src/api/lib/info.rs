...
const API_INFO: ApiInfo<OracleApi, InfoInput> = ApiInfo::new(EpKind::Oracle, OracleApi::Info, "Oracle execute named", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.info().map_err(EpError::request)?;
        ...
        Err(EpError::request("info output not yet implementd"))
    }
    #[named]
