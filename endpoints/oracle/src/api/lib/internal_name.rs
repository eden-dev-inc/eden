...
const API_INFO: ApiInfo<OracleApi, InternalNameInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::InternalName, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct InternalNameInput {}
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let result = client.internal_name().map_err(EpError::request)?;
