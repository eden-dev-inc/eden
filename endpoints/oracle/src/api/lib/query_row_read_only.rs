...
const API_INFO: ApiInfo<OracleApi, QueryRowReadOnlyInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::QueryRowReadOnly, "Oracle Read-Only Query Row", ReqType::Read, true);

crate::oracle_endpoint! {
    struct QueryRowReadOnlyInput {
        query: String,
        params: Vec<SqlParam>
    }
}
...
impl SimpleInput {
    ...
    pub async fn run_query(&self, context: OracleAsync) -> ResultEP<Row> {
        ...
        client.query_row(self.query(), self.param_refs().as_slice()).map_err(EpError::request)
    }
