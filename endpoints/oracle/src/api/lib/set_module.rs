...
const API_INFO: ApiInfo<OracleApi, SetModuleInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::SetModule, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct SetModuleInput {
        module: String
    }
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.set_module(&self.module).map_err(EpError::request)?;
