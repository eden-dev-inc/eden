...
const API_INFO: ApiInfo<OracleApi, QueryRowAsReadOnlyInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::QueryRowAsReadOnly, "Oracle Read-Only Query Row As", ReqType::Read, true);

crate::oracle_endpoint! {
    struct QueryRowAsReadOnlyInput {
        query: String,
        params: Vec<SqlParam>
    }
}
