...
const API_INFO: ApiInfo<OracleApi, StatementInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::Statement, "Oracle execute named", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let _ = client.statement(&self.sql);
        ...
        Err(EpError::request("statement builder output failed"))
    }
    #[named]
