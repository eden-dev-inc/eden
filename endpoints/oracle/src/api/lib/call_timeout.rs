...
const API_INFO: ApiInfo<OracleApi, CallTimeoutInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::CallTimeout, "Oracle Call Timeout", ReqType::Write, true);

crate::oracle_endpoint! {
    struct CallTimeoutInput {}
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.call_timeout().map_err(EpError::request)?;
        ...
        Err(EpError::Request(RequestError::ErrorOutputNotImplemented))
    }
    #[named]
