...
const API_INFO: ApiInfo<OracleApi, QueryAsReadOnlyInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::QueryAsReadOnly, "Oracle Read-Only Query As", ReqType::Read, true);

crate::oracle_endpoint! {
    struct QueryAsReadOnlyInput {
        query: String,
        params: Vec<SqlParam>
    }
}
...
impl SimpleInput {
    ...
    pub async fn run_query(&self, context: OracleAsync) -> ResultEP<Vec<Row>> {
        ...
        let mut rows = vec![];
        for result in client.query_as(self.query(), self.param_refs().as_slice()).map_err(EpError::request)? {
