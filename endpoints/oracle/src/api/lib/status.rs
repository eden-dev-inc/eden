...
const API_INFO: ApiInfo<OracleApi, StatusInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::Status, "Oracle execute named", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.status().map_err(EpError::request)?;
        ...
        Err(EpError::request("status output failed"))
    }
    #[named]
