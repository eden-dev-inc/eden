...
const API_INFO: ApiInfo<OracleApi, ExternalNameInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::ExternalName, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct ExternalNameInput {}
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let result = client.external_name().map_err(EpError::request)?;
