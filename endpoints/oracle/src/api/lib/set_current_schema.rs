...
const API_INFO: ApiInfo<OracleApi, SetCurrentSchemaInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::SetCurrentSchema, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct SetCurrentSchemaInput {
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.set_current_schema(&self.current_schema).map_err(EpError::request)?;
