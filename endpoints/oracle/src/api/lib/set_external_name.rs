...
const API_INFO: ApiInfo<OracleApi, SetExternalNameInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::SetExternalName, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct SetExternalNameInput {
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.set_external_name(&self.external_name).map_err(EpError::request)?;
