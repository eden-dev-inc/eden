...
const API_INFO: ApiInfo<OracleApi, SetClientInfoInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::SetClientInfo, "Oracle execute named", ReqType::Write, true);

crate::oracle_endpoint! {
    struct SetClientInfoInput {
        client_info: String
    }
}
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.set_client_info(&self.client_info).map_err(EpError::request)?;
