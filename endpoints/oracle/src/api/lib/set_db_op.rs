...
const API_INFO: ApiInfo<OracleApi, SetDbOpInput> =
    ApiInfo::new(EpKind::Oracle, OracleApi::SetDbOp, "Oracle execute named", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: OracleAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.set_db_op(&self.db_op).map_err(EpError::request)?;
