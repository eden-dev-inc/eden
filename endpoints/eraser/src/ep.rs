...
use crate::metadata::EraserMetadata;
...
use eraser_core::comm::EraserClient;
...
impl_endpoint!(EpKind::Eraser => Eraser,  EraserAsync);

ep_core::impl_endpoint_lifecycle_spec!(EraserEp, EraserAsync, EraserConfig, EraserRequest, EraserMetadata, EraserApi, EraserTx);

impl EP<EraserAsync, EraserConfig, EraserRequest, EraserMetadata, EraserApi, EraserTx> for EraserEp {
    fn new() -> Self {
        Self::default()
    }
    #[named]
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<EraserRequest>>() {
            Some(tx) => tx.to_owned(),
