...
const API_INFO: ApiInfo<EraserApi, DeleteDiagramInput> =
    ApiInfo::new(EpKind::Eraser, EraserApi::DeleteDiagram, "Delete a diagram from an Eraser file", ReqType::Write);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: EraserAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let diagram_id = self.body.get("diagramId").and_then(|v| v.as_str()).ok_or_else(|| EpError::parse("missing diagramId"))?;
        let path = format!("/api/v1/files/{file_id}/diagrams/{diagram_id}");
