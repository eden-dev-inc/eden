...
const API_INFO: ApiInfo<EraserApi, CreateDiagramInput> =
    ApiInfo::new(EpKind::Eraser, EraserApi::CreateDiagram, "Create a new diagram in an Eraser file", ReqType::Write);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: EraserAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/api/v1/files/{file_id}/diagrams");

        let client = context.get().await.map_err(EpError::connect)?;
        let value = client.post(&path, self.body.clone()).await?;
