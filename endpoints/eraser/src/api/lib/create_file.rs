...
const API_INFO: ApiInfo<EraserApi, CreateFileInput> =
    ApiInfo::new(EpKind::Eraser, EraserApi::CreateFile, "Create a new Eraser file", ReqType::Write);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: EraserAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = client.post("/api/v1/files", self.body.clone()).await?;
