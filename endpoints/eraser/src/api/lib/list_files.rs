...
const API_INFO: ApiInfo<EraserApi, ListFilesInput> =
    ApiInfo::new(EpKind::Eraser, EraserApi::ListFiles, "List all Eraser files", ReqType::Read);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: EraserAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = client.get("/api/v1/files").await?;
