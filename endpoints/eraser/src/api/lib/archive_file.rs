...
const API_INFO: ApiInfo<EraserApi, ArchiveFileInput> =
    ApiInfo::new(EpKind::Eraser, EraserApi::ArchiveFile, "Archive an Eraser file", ReqType::Write);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: EraserAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let path = format!("/api/v1/files/{file_id}/archive");
