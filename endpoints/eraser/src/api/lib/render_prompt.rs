...
const API_INFO: ApiInfo<EraserApi, RenderPromptInput> = ApiInfo::new(
    EpKind::Eraser,
    EraserApi::RenderPrompt,
    "Generate a diagram from a natural language prompt using Eraser AI",
    ReqType::Read,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: EraserAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = client.post("/api/render/prompt", self.body.clone()).await?;
