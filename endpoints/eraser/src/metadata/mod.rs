...
impl SyncMetadata<EraserAsync> for EraserMetadata {
    fn jobs(&mut self, _: SyncFrequency) -> Vec<MetadataJob<EraserAsync, Self>> {
        Vec::new()
    }
}

impl EpMetadata for EraserMetadata {
    fn kind(&self) -> EpKind {
        EpKind::Eraser
    }
...
define_metadata_serializer_stuff!(EpKind::Eraser => EraserMetadata);
