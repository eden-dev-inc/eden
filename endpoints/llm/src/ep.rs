...
use error::{EpError, RequestError, ResultEP};
...
impl_endpoint!(EpKind::Llm => Llm,  LlmAsync);

ep_core::impl_endpoint_lifecycle_spec!(LlmEp, LlmAsync, LlmConfig, LlmRequest, LlmMetadata, LlmApi, LlmTx);

impl EP<LlmAsync, LlmConfig, LlmRequest, LlmMetadata, LlmApi, LlmTx> for LlmEp {
    fn new() -> Self {
        Self::default()
    }
    #[named]
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<LlmRequest>>() {
            Some(tx) => tx.to_owned(),
    ...
    async fn health_check(&self, endpoint_cache_uuid: &EndpointCacheUuid, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<()> {
        ...
        // Send a minimal chat request to verify the provider is accessible and credentials work
        client.health_ping().await?;
        Ok(())
    }
