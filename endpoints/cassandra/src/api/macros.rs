...
#[macro_export]
macro_rules! cassandra_endpoint {
    (
        $kind:ident,
        $api_info:expr,
        struct {
        ...
        paste::paste! {
            ...
            pub type SimpleInput = [<$kind Input>];

            impl $crate::EndpointOperation for [<$kind Input>] {}

            impl [<$kind Input>] {
                #[allow(dead_code)]
                fn cassandra_operation_todo() {
                    todo!("Determine if we want to derive here or in the cassandra crate")
                }
            ...
            #[ctor::ctor]
            fn [<__register_cassandra_operation_for_ $kind>]() {
