...
use crate::metadata::GoogleWorkspaceMetadata;
...
use gworkspace_core::comm::GoogleWorkspaceClient;
...
impl_endpoint!(EpKind::GoogleWorkspace => GoogleWorkspace, GoogleWorkspaceAsync);

ep_core::impl_endpoint_lifecycle_spec!(
    GoogleWorkspaceEp,
    GoogleWorkspaceAsync,
...
impl EP<GoogleWorkspaceAsync, GoogleWorkspaceConfig, GoogleWorkspaceRequest, GoogleWorkspaceMetadata, GoogleWorkspaceApi, GoogleWorkspaceTx>
    for GoogleWorkspaceEp
{
    fn new() -> Self {
        Self::default()
    }
    ...
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<GoogleWorkspaceRequest>>() {
            Some(tx) => tx.to_owned(),
        ...
        span.add_event("commiting transaction", vec![]);
