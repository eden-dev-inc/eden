...
impl SyncMetadata<FunctionAsync> for FunctionMetadata {
    fn jobs(&mut self, _: SyncFrequency) -> Vec<MetadataJob<FunctionAsync, Self>> {
        Vec::new()
    }
}

impl EpMetadata for FunctionMetadata {
    fn kind(&self) -> EpKind {
        EpKind::Function
    }
...
define_metadata_serializer_stuff!(EpKind::Function => FunctionMetadata);
