...
use crate::metadata::AwsMetadata;
use crate::request::AwsRequest;
use crate::{EP, EpTransaction, Transaction};
use aws_core::comm::AwsClient;
...
impl_endpoint!(EpKind::Aws => Aws, AwsAsync);

ep_core::impl_endpoint_lifecycle_spec!(AwsEp, AwsAsync, AwsConfig, AwsRequest, AwsMetadata, AwsApi, AwsTx);

impl EP<AwsAsync, AwsConfig, AwsRequest, AwsMetadata, AwsApi, AwsTx> for AwsEp {
    fn new() -> Self {
        Self::default()
    }
    #[named]
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<AwsRequest>>() {
            Some(tx) => tx.to_owned(),
