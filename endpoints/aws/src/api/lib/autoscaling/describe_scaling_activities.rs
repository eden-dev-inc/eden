...
const API_INFO: ApiInfo<AwsApi, AutoScalingDescribeScalingActivitiesInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::AutoScalingDescribeScalingActivities,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("DescribeScalingActivities", "2011-01-01", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = AutoScalingDescribeScalingActivitiesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "autoscaling_describe_scaling_activities");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: AutoScalingDescribeScalingActivitiesInput = serde_json::from_value(json).unwrap();
    }
}
