...
const API_INFO: ApiInfo<AwsApi, RedshiftDescribeClustersInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::RedshiftDescribeClusters, "redshift_describe_clusters", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("DescribeClusters", "2012-12-01", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RedshiftDescribeClustersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "redshift_describe_clusters");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: RedshiftDescribeClustersInput = serde_json::from_value(json).unwrap();
    }
}
