...
const API_INFO: ApiInfo<AwsApi, RedshiftModifyClusterInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::RedshiftModifyCluster, "redshift_modify_cluster", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("ModifyCluster", "2012-12-01", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RedshiftModifyClusterInputBuilder::default().cluster_identifier("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "redshift_modify_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"cluster_identifier": "my-cluster"});
        let _: RedshiftModifyClusterInput = serde_json::from_value(json).unwrap();
    }
}
