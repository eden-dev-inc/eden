...
const API_INFO: ApiInfo<AwsApi, IamTagRoleInput> = ApiInfo::new(EpKind::Aws, AwsApi::IamTagRole, "iam_tag_role", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("TagRole", "2010-05-08", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IamTagRoleInputBuilder::default().role_name("r").tags(vec![]).build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iam_tag_role");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"role_name": "r", "tags": []});
        let _: IamTagRoleInput = serde_json::from_value(json).unwrap();
    }
}
