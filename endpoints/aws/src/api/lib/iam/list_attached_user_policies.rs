...
const API_INFO: ApiInfo<AwsApi, IamListAttachedUserPoliciesInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::IamListAttachedUserPolicies,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("ListAttachedUserPolicies", "2010-05-08", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IamListAttachedUserPoliciesInputBuilder::default().user_name("u").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iam_list_attached_user_policies");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"user_name": "u"});
        let _: IamListAttachedUserPoliciesInput = serde_json::from_value(json).unwrap();
    }
}
