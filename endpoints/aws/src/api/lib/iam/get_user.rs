...
const API_INFO: ApiInfo<AwsApi, IamGetUserInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::IamGetUser,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("GetUser", "2010-05-08", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = IamGetUserInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "iam_get_user");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: IamGetUserInput = serde_json::from_value(json).unwrap();
    }
}
