...
const API_INFO: ApiInfo<AwsApi, EksListNodegroupsInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::EksListNodegroups, "Lists EKS nodegroups", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = EksListNodegroupsInputBuilder::default().cluster_name("c").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "eks_list_nodegroups");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"cluster_name": "c"});
        let _: EksListNodegroupsInput = serde_json::from_value(json).unwrap();
    }
}
