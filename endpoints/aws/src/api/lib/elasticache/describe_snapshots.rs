...
const API_INFO: ApiInfo<AwsApi, ElastiCacheDescribeSnapshotsInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::ElastiCacheDescribeSnapshots,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(v) = &self.snapshot_name {
            params.insert("SnapshotName".to_string(), v.clone());
        }
        ...
        let form_body = build_query_body("DescribeSnapshots", "2015-02-02", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ElastiCacheDescribeSnapshotsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "elasticache_describe_snapshots");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: ElastiCacheDescribeSnapshotsInput = serde_json::from_value(json).unwrap();
    }
}
