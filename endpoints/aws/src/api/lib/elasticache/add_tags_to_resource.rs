...
const API_INFO: ApiInfo<AwsApi, ElastiCacheAddTagsToResourceInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::ElastiCacheAddTagsToResource,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("AddTagsToResource", "2015-02-02", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = ElastiCacheAddTagsToResourceInputBuilder::default()
            .resource_name("arn:aws:elasticache:us-east-1:123:cluster:my-cache")
            .tags(vec![serde_json::json!({"Key": "env", "Value": "prod"})])
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "elasticache_add_tags_to_resource");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resource_name": "arn", "tags": [{"Key": "k", "Value": "v"}]});
        let _: ElastiCacheAddTagsToResourceInput = serde_json::from_value(json).unwrap();
    }
}
