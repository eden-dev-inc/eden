...
const API_INFO: ApiInfo<AwsApi, LambdaUpdateFunctionUrlConfigInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::LambdaUpdateFunctionUrlConfig,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(auth) = &self.auth_type {
            body.insert("AuthType".to_string(), Value::String(auth.clone()));
        }
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LambdaUpdateFunctionUrlConfigInputBuilder::default().function_name("f").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "lambda_update_function_url_config");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"function_name": "f"});
        let _: LambdaUpdateFunctionUrlConfigInput = serde_json::from_value(json).unwrap();
    }
}
