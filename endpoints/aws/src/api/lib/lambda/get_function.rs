...
const API_INFO: ApiInfo<AwsApi, LambdaGetFunctionInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::LambdaGetFunction,
    "Returns information about a Lambda function",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LambdaGetFunctionInputBuilder::default().function_name("f").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "lambda_get_function");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"function_name": "f"});
        let _: LambdaGetFunctionInput = serde_json::from_value(json).unwrap();
    }
}
