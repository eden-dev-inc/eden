...
const API_INFO: ApiInfo<AwsApi, LambdaDeleteFunctionUrlConfigInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::LambdaDeleteFunctionUrlConfig,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LambdaDeleteFunctionUrlConfigInputBuilder::default().function_name("f").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "lambda_delete_function_url_config");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"function_name": "f"});
        let _: LambdaDeleteFunctionUrlConfigInput = serde_json::from_value(json).unwrap();
    }
}
