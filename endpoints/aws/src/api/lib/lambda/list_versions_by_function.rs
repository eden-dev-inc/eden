...
const API_INFO: ApiInfo<AwsApi, LambdaListVersionsByFunctionInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::LambdaListVersionsByFunction,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LambdaListVersionsByFunctionInputBuilder::default().function_name("f").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "lambda_list_versions_by_function");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"function_name": "f"});
        let _: LambdaListVersionsByFunctionInput = serde_json::from_value(json).unwrap();
    }
}
