...
const API_INFO: ApiInfo<AwsApi, LambdaGetFunctionUrlConfigInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::LambdaGetFunctionUrlConfig,
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = LambdaGetFunctionUrlConfigInputBuilder::default().function_name("f").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "lambda_get_function_url_config");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"function_name": "f"});
        let _: LambdaGetFunctionUrlConfigInput = serde_json::from_value(json).unwrap();
    }
}
