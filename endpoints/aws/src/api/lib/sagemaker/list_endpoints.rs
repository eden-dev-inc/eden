...
const API_INFO: ApiInfo<AwsApi, SageMakerListEndpointsInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::SageMakerListEndpoints, "sagemaker_list_endpoints", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let result = client.execute_json_target("sagemaker", "SageMaker.ListEndpoints", Some(&body_val), "1.1").await?;
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SageMakerListEndpointsInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "sagemaker_list_endpoints");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SageMakerListEndpointsInput = serde_json::from_value(json).unwrap();
    }
}
