...
const API_INFO: ApiInfo<AwsApi, SageMakerListNotebookInstancesInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::SageMakerListNotebookInstances,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let result = client.execute_json_target("sagemaker", "SageMaker.ListNotebookInstances", Some(&body_val), "1.1").await?;
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = SageMakerListNotebookInstancesInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "sagemaker_list_notebook_instances");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: SageMakerListNotebookInstancesInput = serde_json::from_value(json).unwrap();
    }
}
