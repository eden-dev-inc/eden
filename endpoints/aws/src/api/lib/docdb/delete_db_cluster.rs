...
const API_INFO: ApiInfo<AwsApi, DocDbDeleteDbClusterInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::DocDbDeleteDbCluster, "docdb_delete_db_cluster", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(v) = self.skip_final_snapshot {
            params.insert("SkipFinalSnapshot".to_string(), v.to_string());
        }
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DocDbDeleteDbClusterInputBuilder::default().db_cluster_identifier("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "docdb_delete_db_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"db_cluster_identifier": "my-cluster"});
        let _: DocDbDeleteDbClusterInput = serde_json::from_value(json).unwrap();
    }
}
