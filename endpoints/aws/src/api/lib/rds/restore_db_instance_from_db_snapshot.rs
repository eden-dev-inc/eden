...
const API_INFO: ApiInfo<AwsApi, RdsRestoreDbInstanceFromDbSnapshotInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::RdsRestoreDbInstanceFromDbSnapshot,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("RestoreDBInstanceFromDBSnapshot", "2014-10-31", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RdsRestoreDbInstanceFromDbSnapshotInputBuilder::default()
            .db_instance_identifier("my-db")
            .db_snapshot_identifier("my-snap")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "rds_restore_db_instance_from_db_snapshot");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"db_instance_identifier": "my-db", "db_snapshot_identifier": "my-snap"});
        let _: RdsRestoreDbInstanceFromDbSnapshotInput = serde_json::from_value(json).unwrap();
    }
}
