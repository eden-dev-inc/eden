...
const API_INFO: ApiInfo<AwsApi, RdsDeleteDbClusterInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::RdsDeleteDbCluster, "rds_delete_db_cluster", ReqType::Write, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RdsDeleteDbClusterInputBuilder::default().db_cluster_identifier("my-cluster").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "rds_delete_db_cluster");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"db_cluster_identifier": "my-cluster"});
        let _: RdsDeleteDbClusterInput = serde_json::from_value(json).unwrap();
    }
}
