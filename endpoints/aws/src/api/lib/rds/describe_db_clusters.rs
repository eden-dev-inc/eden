...
const API_INFO: ApiInfo<AwsApi, RdsDescribeDbClustersInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::RdsDescribeDbClusters, "rds_describe_db_clusters", ReqType::Read, true);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = RdsDescribeDbClustersInputBuilder::default().build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "rds_describe_db_clusters");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: RdsDescribeDbClustersInput = serde_json::from_value(json).unwrap();
    }
}
