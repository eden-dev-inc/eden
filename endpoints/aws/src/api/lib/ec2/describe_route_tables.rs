...
const API_INFO: ApiInfo<AwsApi, Ec2DescribeRouteTablesInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::Ec2DescribeRouteTables, "ec2_describe_route_tables", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("DescribeRouteTables", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = Ec2DescribeRouteTablesInputBuilder::default()
            .route_table_ids(None::<Vec<String>>)
        ...
        assert_eq!(json["type"], "ec2_describe_route_tables");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: Ec2DescribeRouteTablesInput = serde_json::from_value(json).unwrap();
    }
}
