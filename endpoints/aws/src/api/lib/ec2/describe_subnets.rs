...
const API_INFO: ApiInfo<AwsApi, DescribeSubnetsInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::Ec2DescribeSubnets, "Describes one or more subnets", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("DescribeSubnets", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DescribeSubnetsInputBuilder::default()
            .subnet_ids(None::<Vec<String>>)
        ...
        assert_eq!(json["type"], "ec2_describe_subnets");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DescribeSubnetsInput = serde_json::from_value(json).unwrap();
    }
}
