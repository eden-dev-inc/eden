...
const API_INFO: ApiInfo<AwsApi, Ec2DescribeFlowLogsInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::Ec2DescribeFlowLogs, "ec2_describe_flow_logs", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(ids) = &self.flow_log_ids {
            params.extend(indexed_list_params("FlowLogId", ids));
        }
        ...
        let form_body = build_query_body("DescribeFlowLogs", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = Ec2DescribeFlowLogsInputBuilder::default()
            .flow_log_ids(None::<Vec<String>>)
        ...
        assert_eq!(json["type"], "ec2_describe_flow_logs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: Ec2DescribeFlowLogsInput = serde_json::from_value(json).unwrap();
    }
}
