...
const API_INFO: ApiInfo<AwsApi, DescribeSecurityGroupsInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::Ec2DescribeSecurityGroups,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(ids) = &self.group_ids {
            params.extend(indexed_list_params("GroupId", ids));
        }
        if let Some(names) = &self.group_names {
            params.extend(indexed_list_params("GroupName", names));
        }
        ...
        let form_body = build_query_body("DescribeSecurityGroups", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DescribeSecurityGroupsInputBuilder::default()
            .group_ids(None::<Vec<String>>)
            .group_names(None::<Vec<String>>)
        ...
        assert_eq!(json["type"], "ec2_describe_security_groups");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DescribeSecurityGroupsInput = serde_json::from_value(json).unwrap();
    }
}
