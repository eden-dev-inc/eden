...
const API_INFO: ApiInfo<AwsApi, Ec2DeleteTagsInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::Ec2DeleteTags, "ec2_delete_tags", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("DeleteTags", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = Ec2DeleteTagsInputBuilder::default()
            .resources(vec!["i-12345".to_string()])
            .tags(vec![AwsTag::new("Name", "")])
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "ec2_delete_tags");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"resources": ["i-12345"], "tags": [{"key": "Name", "value": ""}]});
        let _: Ec2DeleteTagsInput = serde_json::from_value(json).unwrap();
    }
}
