...
const API_INFO: ApiInfo<AwsApi, DescribeVolumesInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::Ec2DescribeVolumes, "Describes one or more EBS volumes", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(ids) = &self.volume_ids {
            params.extend(indexed_list_params("VolumeId", ids));
        }
        ...
        let form_body = build_query_body("DescribeVolumes", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DescribeVolumesInputBuilder::default()
            .volume_ids(None::<Vec<String>>)
        ...
        assert_eq!(json["type"], "ec2_describe_volumes");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DescribeVolumesInput = serde_json::from_value(json).unwrap();
    }
}
