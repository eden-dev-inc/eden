...
const API_INFO: ApiInfo<AwsApi, Ec2DescribeVpcEndpointsInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::Ec2DescribeVpcEndpoints, "ec2_describe_vpc_endpoints", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(ids) = &self.vpc_endpoint_ids {
            params.extend(indexed_list_params("VpcEndpointId", ids));
        }
        ...
        let form_body = build_query_body("DescribeVpcEndpoints", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = Ec2DescribeVpcEndpointsInputBuilder::default()
            .vpc_endpoint_ids(None::<Vec<String>>)
        ...
        assert_eq!(json["type"], "ec2_describe_vpc_endpoints");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: Ec2DescribeVpcEndpointsInput = serde_json::from_value(json).unwrap();
    }
}
