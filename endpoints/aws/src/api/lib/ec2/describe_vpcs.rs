...
const API_INFO: ApiInfo<AwsApi, DescribeVpcsInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::Ec2DescribeVpcs, "Describes one or more VPCs", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(ids) = &self.vpc_ids {
            params.extend(indexed_list_params("VpcId", ids));
        }
        ...
        let form_body = build_query_body("DescribeVpcs", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DescribeVpcsInputBuilder::default()
        ...
        assert_eq!(json["type"], "ec2_describe_vpcs");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DescribeVpcsInput = serde_json::from_value(json).unwrap();
    }
}
