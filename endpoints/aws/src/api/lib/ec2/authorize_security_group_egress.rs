...
const API_INFO: ApiInfo<AwsApi, Ec2AuthorizeSecurityGroupEgressInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::Ec2AuthorizeSecurityGroupEgress,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("AuthorizeSecurityGroupEgress", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = Ec2AuthorizeSecurityGroupEgressInputBuilder::default().group_id("sg-123").build().unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "ec2_authorize_security_group_egress");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"group_id": "sg-123"});
        let _: Ec2AuthorizeSecurityGroupEgressInput = serde_json::from_value(json).unwrap();
    }
}
