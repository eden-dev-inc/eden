...
const API_INFO: ApiInfo<AwsApi, Ec2DescribeInstanceTypesInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::Ec2DescribeInstanceTypes, "ec2_describe_instance_types", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(types) = &self.instance_types {
            params.extend(indexed_list_params("InstanceType", types));
        }
        ...
        let form_body = build_query_body("DescribeInstanceTypes", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = Ec2DescribeInstanceTypesInputBuilder::default()
            .instance_types(None::<Vec<String>>)
        ...
        assert_eq!(json["type"], "ec2_describe_instance_types");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: Ec2DescribeInstanceTypesInput = serde_json::from_value(json).unwrap();
    }
}
