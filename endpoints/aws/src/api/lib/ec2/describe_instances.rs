...
const API_INFO: ApiInfo<AwsApi, DescribeInstancesInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::Ec2DescribeInstances,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let form_body = build_query_body("DescribeInstances", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DescribeInstancesInputBuilder::default()
        ...
        assert_eq!(json["type"], "ec2_describe_instances");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: DescribeInstancesInput = serde_json::from_value(json).unwrap();
    }
}
