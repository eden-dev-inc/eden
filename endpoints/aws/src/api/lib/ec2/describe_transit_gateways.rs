...
const API_INFO: ApiInfo<AwsApi, Ec2DescribeTransitGatewaysInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::Ec2DescribeTransitGateways,
...
crate::aws_endpoint! {
    ...
    API_INFO,
    struct {
        transit_gateway_ids: Option<Vec<String>>,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(ids) = &self.transit_gateway_ids {
            params.extend(indexed_list_params("TransitGatewayId", ids));
        }
        ...
        let form_body = build_query_body("DescribeTransitGateways", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = Ec2DescribeTransitGatewaysInputBuilder::default()
            .transit_gateway_ids(None::<Vec<String>>)
        ...
        assert_eq!(json["type"], "ec2_describe_transit_gateways");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: Ec2DescribeTransitGatewaysInput = serde_json::from_value(json).unwrap();
    }
}
