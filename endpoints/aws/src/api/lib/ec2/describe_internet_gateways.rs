...
const API_INFO: ApiInfo<AwsApi, Ec2DescribeInternetGatewaysInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::Ec2DescribeInternetGateways,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(ids) = &self.internet_gateway_ids {
            params.extend(indexed_list_params("InternetGatewayId", ids));
        }
        ...
        let form_body = build_query_body("DescribeInternetGateways", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = Ec2DescribeInternetGatewaysInputBuilder::default()
            .internet_gateway_ids(None::<Vec<String>>)
        ...
        assert_eq!(json["type"], "ec2_describe_internet_gateways");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: Ec2DescribeInternetGatewaysInput = serde_json::from_value(json).unwrap();
    }
}
