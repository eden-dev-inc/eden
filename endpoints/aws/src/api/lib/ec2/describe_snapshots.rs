...
const API_INFO: ApiInfo<AwsApi, Ec2DescribeSnapshotsInput> =
    ApiInfo::new(EpKind::Aws, AwsApi::Ec2DescribeSnapshots, "ec2_describe_snapshots", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        if let Some(ids) = &self.snapshot_ids {
            params.extend(indexed_list_params("SnapshotId", ids));
        }
        ...
        let form_body = build_query_body("DescribeSnapshots", "2016-11-15", &params);
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = Ec2DescribeSnapshotsInputBuilder::default()
            .snapshot_ids(None::<Vec<String>>)
        ...
        assert_eq!(json["type"], "ec2_describe_snapshots");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({});
        let _: Ec2DescribeSnapshotsInput = serde_json::from_value(json).unwrap();
    }
}
