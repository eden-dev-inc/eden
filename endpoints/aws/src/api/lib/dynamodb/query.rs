...
const API_INFO: ApiInfo<AwsApi, DynamoDbQueryInput> = ApiInfo::new(
    EpKind::Aws,
    AwsApi::DynamoDbQuery,
    "Finds items in a DynamoDB table using a key expression",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: AwsAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        body.insert("KeyConditionExpression".to_string(), Value::String(self.key_condition_expression.clone()));
        ...
        let result = client.execute_json_target("dynamodb", "DynamoDB_20120810.Query", Some(&body_val), "1.0").await?;
...
mod tests {
    ...
    #[test]
    fn builder_serde() {
        let input = DynamoDbQueryInputBuilder::default()
            .table_name("t")
            .key_condition_expression("#id = :id")
            .build()
            .unwrap();
        let json = serde_json::to_value(&input).unwrap();
        assert_eq!(json["type"], "dynamodb_query");
    }

    #[test]
    fn deserialize_minimal() {
        let json = serde_json::json!({"table_name": "t", "key_condition_expression": "expr"});
        let _: DynamoDbQueryInput = serde_json::from_value(json).unwrap();
    }
}
