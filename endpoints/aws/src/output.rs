...
pub struct AwsJsonOutput(pub Value);

impl ToOutput for AwsJsonOutput {
    fn to_output(self) -> EndpointOutput<Self> {
        EndpointOutput::new(EpKind::Aws, EndpointResponse::Response(self))
    }
...
#[derive(Debug, Deserialize)]
pub struct AwsEmptyOutput;

impl ToOutput for AwsEmptyOutput {
    fn to_output(self) -> EndpointOutput<Self> {
        EndpointOutput::new(EpKind::Aws, EndpointResponse::ok("success"))
    }
    fn try_to_bytes(self) -> ResultEP<bytes::Bytes> {
        Err(EpError::Protocol(ProtocolError::NotImplemented))
    }
