...
const API_INFO: ApiInfo<PostgresApi, QueryOptReadOnlyInput> = ApiInfo::new(
    EpKind::Postgres,
    PostgresApi::QueryOptReadOnly,
    "Executes a read-only parameterized SQL query that may return zero or one row from PostgreSQL",
    ReqType::Read,
    true,
);
