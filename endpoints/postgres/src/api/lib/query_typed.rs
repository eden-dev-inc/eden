...
const API_INFO: ApiInfo<PostgresApi, QueryTypedInput> = ApiInfo::new(
    EpKind::Postgres,
    PostgresApi::QueryTyped,
    "Executes a parameterized SQL query with explicitly typed parameters and returns results from PostgreSQL",
    ReqType::Write,
    true,
);

#[derive(Debug, Clone, Serialize, Deserialize, ToSchema, JsonSchema)]
pub struct SqlParamType(pub SqlParam, pub SqlType);

impl Default for SqlParamType {
    fn default() -> Self {
        Self(SqlParam::Null, SqlType::Bool)
    }
}
