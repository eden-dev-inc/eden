...
const API_INFO: ApiInfo<PostgresApi, QueryOneReadOnlyInput> = ApiInfo::new(
    EpKind::Postgres,
    PostgresApi::QueryOneReadOnly,
    "Executes a read-only parameterized SQL query and returns exactly one row from PostgreSQL",
    ReqType::Read,
    true,
);
