...
use crate::api::wrapper::output::PostgresSimpleQueryOutput;
...
const API_INFO: ApiInfo<PostgresApi, SimpleQueryReadOnlyInput> = ApiInfo::new(
    EpKind::Postgres,
    PostgresApi::SimpleQueryReadOnly,
    "Executes a read-only simple SQL query without parameters in PostgreSQL",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: PostgresAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let raw = client.simple_query_raw(self.query()).await?;
        ...
        Ok(Box::new(PostgresSimpleQueryOutput(raw).to_output()) as Box<dyn EpOutput>)
    }
    #[named]
