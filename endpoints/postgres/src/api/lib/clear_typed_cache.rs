...
const API_INFO: ApiInfo<PostgresApi, ClearTypeCacheInput> = ApiInfo::new(
    EpKind::Postgres,
    PostgresApi::ClearTypeCache,
    "Clears the PostgreSQL type cache to refresh custom type definitions",
    ReqType::Write,
    true,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: PostgresAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        // No-op: raw wire protocol client has no type cache
