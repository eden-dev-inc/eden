...
const API_INFO: ApiInfo<PostgresApi, QueryRawReadOnlyInput> = ApiInfo::new(
    EpKind::Postgres,
    PostgresApi::QueryRawReadOnly,
    "Executes a read-only parameterized SQL query and returns raw, unprocessed results from PostgreSQL",
    ReqType::Read,
    true,
);
