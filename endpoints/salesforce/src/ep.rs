...
use crate::metadata::SalesforceMetadata;
...
use salesforce_core::comm::SalesforceClient;
...
impl_endpoint!(EpKind::Salesforce => Salesforce, SalesforceAsync);
...
impl EP<SalesforceAsync, SalesforceConfig, SalesforceRequest, SalesforceMetadata, SalesforceApi, SalesforceTx> for SalesforceEp {
    fn new() -> Self {
        Self::default()
    }
    #[named]
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<SalesforceRequest>>() {
            Some(tx) => tx.to_owned(),
