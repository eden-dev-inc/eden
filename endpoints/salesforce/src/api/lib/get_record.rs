...
const API_INFO: ApiInfo<SalesforceApi, GetRecordInput> =
    ApiInfo::new(EpKind::Salesforce, SalesforceApi::GetRecord, "Retrieve a Salesforce record by ID", ReqType::Read);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: SalesforceAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let object_name = self.body["object_name"]
            .as_str()
            .ok_or_else(|| EpError::request("Salesforce get_record requires an 'object_name' field"))?;
        let record_id =
            self.body["record_id"].as_str().ok_or_else(|| EpError::request("Salesforce get_record requires a 'record_id' field"))?;

        let client = context.get().await.map_err(EpError::connect)?;

        let value = if let Some(fields) = self.body["fields"].as_str() {
            client.get_with_query(&format!("/sobjects/{}/{}", object_name, record_id), &[("fields", fields)]).await?
        } else {
            client.get(&format!("/sobjects/{}/{}", object_name, record_id)).await?
        };
