...
const API_INFO: ApiInfo<SalesforceApi, SearchInput> =
    ApiInfo::new(EpKind::Salesforce, SalesforceApi::Search, "Execute a SOSL search against Salesforce", ReqType::Read);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: SalesforceAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let sosl = self.body["sosl"].as_str().ok_or_else(|| EpError::request("Salesforce search requires a 'sosl' field"))?;

        let client = context.get().await.map_err(EpError::connect)?;
        let value = client.get_with_query("/search", &[("q", sosl)]).await?;
