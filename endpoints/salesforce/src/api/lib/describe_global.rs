...
const API_INFO: ApiInfo<SalesforceApi, DescribeGlobalInput> = ApiInfo::new(
    EpKind::Salesforce,
    SalesforceApi::DescribeGlobal,
    "List all available Salesforce sObjects",
    ReqType::Read,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: SalesforceAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = client.get("/sobjects/").await?;
