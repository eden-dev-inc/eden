...
const API_INFO: ApiInfo<SalesforceApi, DescribeObjectInput> = ApiInfo::new(
    EpKind::Salesforce,
    SalesforceApi::DescribeObject,
    "Retrieve metadata for a Salesforce sObject",
    ReqType::Read,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: SalesforceAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let object_name = self.body["object_name"]
            .as_str()
            .ok_or_else(|| EpError::request("Salesforce describe_object requires an 'object_name' field"))?;

        let client = context.get().await.map_err(EpError::connect)?;
        let value = client.get(&format!("/sobjects/{}/describe", object_name)).await?;
