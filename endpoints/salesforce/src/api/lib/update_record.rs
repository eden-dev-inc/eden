...
const API_INFO: ApiInfo<SalesforceApi, UpdateRecordInput> = ApiInfo::new(
    EpKind::Salesforce,
    SalesforceApi::UpdateRecord,
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: SalesforceAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let object_name = self.body["object_name"]
            .as_str()
            .ok_or_else(|| EpError::request("Salesforce update_record requires an 'object_name' field"))?;
        let record_id = self.body["record_id"]
            .as_str()
            .ok_or_else(|| EpError::request("Salesforce update_record requires a 'record_id' field"))?;
        let fields =
            self.body["fields"].as_object().ok_or_else(|| EpError::request("Salesforce update_record requires a 'fields' object"))?;

        let client = context.get().await.map_err(EpError::connect)?;
        let value = client.patch(&format!("/sobjects/{}/{}", object_name, record_id), Value::Object(fields.clone())).await?;
