...
impl SyncMetadata<SalesforceAsync> for SalesforceMetadata {
    fn jobs(&mut self, _: SyncFrequency) -> Vec<MetadataJob<SalesforceAsync, Self>> {
        Vec::new()
    }
}

impl EpMetadata for SalesforceMetadata {
    fn kind(&self) -> EpKind {
        EpKind::Salesforce
    }
...
define_metadata_serializer_stuff!(EpKind::Salesforce => SalesforceMetadata);
