...
use s3_core::{S3Async, S3Tx, config::S3Config};
...
impl_endpoint!(EpKind::S3 => S3, S3Async);

ep_core::impl_endpoint_lifecycle_spec!(S3Ep, S3Async, S3Config, S3Request, S3Metadata, S3Api, S3Tx);

impl EP<S3Async, S3Config, S3Request, S3Metadata, S3Api, S3Tx> for S3Ep {
    fn new() -> Self {
        Self::default()
    }
    ...
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<S3Request>>() {
            Some(tx) => tx.to_owned(),
