...
const API_INFO: ApiInfo<S3Api, CreateBucketInput> =
    ApiInfo::new(EpKind::S3, S3Api::CreateBucket, "Create an S3-compatible bucket", ReqType::Write, true);
...
impl SimpleInput {
    async fn run_async_generic(&self, context: S3Async, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        client.create_bucket(self.bucket()).await?;
