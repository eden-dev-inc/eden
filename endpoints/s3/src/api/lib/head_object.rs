...
use crate::output::S3HeadObjectOutput;
...
const API_INFO: ApiInfo<S3Api, HeadObjectInput> = ApiInfo::new(
    EpKind::S3,
    S3Api::HeadObject,
    "Fetch object metadata from S3-compatible storage",
    ReqType::Read,
    true,
);
...
impl SimpleInput {
    async fn run_async_generic(&self, context: S3Async, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let output = client.head_object(self.bucket().as_deref(), self.key()).await?;
        Ok(Box::new(S3HeadObjectOutput::from(output).to_output()) as Box<dyn EpOutput>)
    }
