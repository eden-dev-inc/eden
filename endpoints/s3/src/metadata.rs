...
impl SyncMetadata<S3Async> for S3Metadata {
    fn jobs(&mut self, _: SyncFrequency) -> Vec<MetadataJob<S3Async, Self>> {
        Vec::new()
    }
}

impl EpMetadata for S3Metadata {
    fn kind(&self) -> EpKind {
        EpKind::S3
    }
...
define_metadata_serializer_stuff!(EpKind::S3 => S3Metadata);
