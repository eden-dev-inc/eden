...
const API_INFO: ApiInfo<RedisApi, SremInput> = ApiInfo::new(
    EpKind::Redis,
    RedisApi::Srem,
...
/// See official Redis documentation for `SREM`
...
impl Serialize for SremInput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("SremInput", 3)?;
...
impl RedisCommandInput for SremInput {
    ...
    {
        if args.len() < 2 {
            return Err(EpError::request("SREM requires at least 2 arguments"));
        }
...
/// Output for Redis SREM command
///
/// Returns the number of members removed from the set.
#[derive(Debug, Deserialize, Clone, ToSchema, JsonSchema)]
pub struct SremOutput {
    removed: i64,
}
...
impl SremOutput {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, EpError> {
        ...
        let removed = match frame {
            ...
            _ => return Err(EpError::parse("SREM must return integer")),
        };
...
impl Serialize for SremOutput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("SremOutput", 1)?;
...
mod tests {
    ...
    mod unit {
        ...
        fn test_encode_command_single() {
            let input = SremInput::single(RedisKey::String("myset".into()), "member1");
        ...
        fn test_decode_output_removed() {
            let output = SremOutput::decode(b":3\r\n").unwrap();
        ...
        fn test_decode_output_zero() {
            let output = SremOutput::decode(b":0\r\n").unwrap();
        ...
        fn test_decode_error() {
            let err = SremOutput::decode(b"-WRONGTYPE Operation against a key holding the wrong kind of value\r\n").unwrap_err();
        ...
        fn test_decode_input_valid() {
            ...
            let input = SremInput::decode(args).unwrap();
        ...
        fn test_decode_input_too_few_args() {
            ...
            let err = SremInput::decode(args).unwrap_err();
            assert!(err.to_string().contains("requires at least 2"));
        }

        #[test]
        fn test_keys_returns_single_key() {
            let input = SremInput::single(RedisKey::String("myset".into()), "m");
    ...
    mod integration {
        ...
        async fn test_srem_single() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ...
                    let result =
                        ctx.raw(&SremInput::single(RedisKey::String("srem_single".into()), "one").command()).await.expect("raw failed");

                    let output = SremOutput::decode(&result).expect("decode failed");
            ...
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ...
                    let result = ctx
                        .raw(
                            &SremInput::new(
                                RedisKey::String("srem_multi".into()),
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_srem_nonexistent_member() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(b"*2\r\n$3\r\nDEL\r\n$13\r\nsrem_nomember\r\n").await.expect("raw failed");
                    ...
                    let result = ctx
                        .raw(&SremInput::single(RedisKey::String("srem_nomember".into()), "nonexistent").command())
                        .await
                        .expect("raw failed");
        ...
        async fn test_srem_nonexistent_key() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ...
                    let result =
                        ctx.raw(&SremInput::single(RedisKey::String("srem_nokey".into()), "member").command()).await.expect("raw failed");
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_srem_wrongtype() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(b"*3\r\n$3\r\nSET\r\n$10\r\nsrem_wrong\r\n$5\r\nvalue\r\n").await.expect("raw failed");

                    let result =
                        ctx.raw(&SremInput::single(RedisKey::String("srem_wrong".into()), "member").command()).await.expect("raw failed");

                    let err = SremOutput::decode(&result).unwrap_err();
                    assert!(err.to_string().contains("WRONGTYPE"));
                })
            })
            .await;
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_srem_resp2_format() {
            ...
            let result = ctx.raw(&SremInput::single(RedisKey::String("srem_r2".into()), "a").command()).await.expect("raw failed");
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_srem_resp3_format() {
            ...
            let result = ctx.raw(&SremInput::single(RedisKey::String("srem_r3".into()), "a").command()).await.expect("raw failed");
