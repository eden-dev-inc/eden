...
const API_INFO: ApiInfo<RedisApi, ZpopminInput> = ApiInfo::new(
    EpKind::Redis,
    RedisApi::Zpopmin,
...
/// See official Redis documentation for `ZPOPMIN`
...
impl Serialize for ZpopminInput {
        ...
        S: Serializer,
    {
        ...
        let mut state = serializer.serialize_struct("ZpopminInput", fields)?;
...
impl RedisCommandInput for ZpopminInput {
    ...
    {
        if args.is_empty() {
            return Err(EpError::request(format!("ZPOPMIN requires at least 1 argument, given {}", args.len())));
        }
...
/// Output for Redis ZPOPMIN command
///
/// Returns an array of members with their scores (lowest score first),
/// or an empty array if the sorted set is empty or doesn't exist.
#[derive(Debug, Deserialize, Clone, ToSchema, JsonSchema)]
pub struct ZpopminOutput {
    elements: Vec<Scores>,
}
...
impl ZpopminOutput {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, EpError> {
        let (frame, _) = RedisProtocol::decode_buffer(bytes).ok_or_else(|| EpError::parse("incomplete RESP frame for ZpopminOutput"))?;
        ...
        match frame {
            DecoderRespFrame::Resp2(Resp2Frame::Array(arr)) => {
                ...
                while let Some(member_frame) = iter.next() {
                    let score_frame = iter.next().ok_or_else(|| EpError::parse("ZPOPMIN missing score for member"))?;
                ...
                if !data.is_empty() && !matches!(data[0], Resp3Frame::Array { .. }) {
                    ...
                    for frame in data {
                        match frame {
                            ...
                            _ => {
                                return Err(EpError::parse("ZPOPMIN element must be [member, score] array"));
                            }
                        }
                    }
                }
            ...
            _ => Err(EpError::parse("ZPOPMIN must return an array")),
        }
    }
}

impl Serialize for ZpopminOutput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("ZpopminOutput", 1)?;
...
mod tests {
    ...
    mod unit {
        ...
        fn test_encode_command_no_count() {
            let input = ZpopminInput::new(RedisKey::String("myzset".into()));
        ...
        fn test_encode_command_with_count() {
            let input = ZpopminInput::new(RedisKey::String("myzset".into())).with_count(RedisJsonValue::Integer(3));
            let cmd = input.command();
        ...
        fn test_decode_output_single_element() {
            ...
            let output = ZpopminOutput::decode(b"*2\r\n$6\r\nmember\r\n$3\r\n1.5\r\n").unwrap();
        ...
        fn test_decode_output_multiple_elements() {
            ...
            let output = ZpopminOutput::decode(b"*4\r\n$1\r\na\r\n$1\r\n1\r\n$1\r\nb\r\n$1\r\n2\r\n").unwrap();
            assert_eq!(output.len(), 2);
        }

        #[test]
        fn test_decode_output_empty() {
            let output = ZpopminOutput::decode(b"*0\r\n").unwrap();
            assert!(output.is_empty());
        ...
        fn test_decode_error() {
            let err = ZpopminOutput::decode(b"-WRONGTYPE Operation against a key holding the wrong kind of value\r\n").unwrap_err();
        ...
        fn test_decode_input_valid() {
            ...
            let input = ZpopminInput::decode(args).unwrap();
        ...
        fn test_decode_input_too_few_args() {
            ...
            let err = ZpopminInput::decode(args).unwrap_err();
    ...
    mod integration {
        ...
        async fn test_zpopmin_single() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ...
                    // ZADD zpopmin_single 1 one 2 two 3 three
                    ctx.raw(b"*8\r\n$4\r\nZADD\r\n$14\r\nzpopmin_single\r\n$1\r\n1\r\n$3\r\none\r\n$1\r\n2\r\n$3\r\ntwo\r\n$1\r\n3\r\n$5\r\nthree\r\n")
                        .await
                        .expect("raw failed");

                    let result = ctx
                        .raw(&ZpopminInput::new(RedisKey::String("zpopmin_single".into())).command())
                        .await
                        .expect("raw failed");

                    let output = ZpopminOutput::decode(&result).expect("decode failed");
                    assert_eq!(output.len(), 1);
                    // Should pop "one" with score 1 (lowest)
                })
            })
                .await;
        }
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_zpopmin_with_count() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(b"*2\r\n$3\r\nDEL\r\n$13\r\nzpopmin_count\r\n")
                        .await
                    ...
                    let result = ctx
                        .raw(
                            &ZpopminInput::new(RedisKey::String("zpopmin_count".into()))
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_zpopmin_empty_set() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(b"*2\r\n$3\r\nDEL\r\n$13\r\nzpopmin_empty\r\n").await.expect("raw failed");

                    let result = ctx.raw(&ZpopminInput::new(RedisKey::String("zpopmin_empty".into())).command()).await.expect("raw failed");
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_zpopmin_wrongtype() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(b"*3\r\n$3\r\nSET\r\n$13\r\nzpopmin_wrong\r\n$5\r\nvalue\r\n").await.expect("raw failed");

                    let result = ctx.raw(&ZpopminInput::new(RedisKey::String("zpopmin_wrong".into())).command()).await.expect("raw failed");

                    let err = ZpopminOutput::decode(&result).unwrap_err();
                    assert!(err.to_string().contains("WRONGTYPE"));
                })
            })
            .await;
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_zpopmin_resp2_format() {
            ...
            let result = ctx.raw(&ZpopminInput::new(RedisKey::String("zpopmin_r2".into())).command()).await.expect("raw failed");
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_zpopmin_resp3_format() {
            ...
            let result = ctx.raw(&ZpopminInput::new(RedisKey::String("zpopmin_r3".into())).command()).await.expect("raw failed");
