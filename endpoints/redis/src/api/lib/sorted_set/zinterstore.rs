...
const API_INFO: ApiInfo<RedisApi, ZinterstoreInput> = ApiInfo::new(
    EpKind::Redis,
...
/// See official Redis documentation for `ZINTERSTORE`
...
impl Serialize for ZinterstoreInput {
        ...
        S: Serializer,
    {
        ...
        let mut state = serializer.serialize_struct("ZinterstoreInput", fields)?;
...
impl RedisCommandInput for ZinterstoreInput {
    ...
    {
        if args.len() < 2 {
            return Err(EpError::request("ZINTERSTORE requires at least destination and numkeys arguments"));
        }
...
/// Output for Redis ZINTERSTORE command
///
/// Returns the number of elements in the resulting sorted set.
#[derive(Debug, Deserialize, Clone, ToSchema, JsonSchema)]
pub struct ZinterstoreOutput {
    count: i64,
}

impl ZinterstoreOutput {
...
impl Serialize for ZinterstoreOutput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("ZinterstoreOutput", 1)?;
...
mod tests {
    ...
    mod unit {
        ...
        fn test_encode_command_with_weights() {
            let input =
                ZinterstoreInput::new(RedisKey::String("dest".into()), vec![RedisKey::String("zset1".into())]).with_weights(vec![2.0]);
            let cmd = input.command();
            let cmd_str = String::from_utf8_lossy(&cmd);
        ...
        fn test_encode_command_with_aggregate() {
            let input = ZinterstoreInput::new(RedisKey::String("dest".into()), vec![RedisKey::String("zset1".into())])
        ...
        fn test_decode_output_zero() {
            let output = ZinterstoreOutput::decode(b":0\r\n").unwrap();
            assert_eq!(output.count(), 0);
        }

        #[test]
        fn test_decode_output_positive() {
            let output = ZinterstoreOutput::decode(b":5\r\n").unwrap();
            assert_eq!(output.count(), 5);
        }

        #[test]
        fn test_decode_error() {
            let err = ZinterstoreOutput::decode(b"-WRONGTYPE Operation against a key holding the wrong kind of value\r\n").unwrap_err();
        ...
        fn test_decode_input_valid() {
            ...
            let input = ZinterstoreInput::decode(args).unwrap();
        ...
        #[test]
        fn test_decode_input_with_aggregate() {
            let args = vec![
                RedisJsonValue::String("dest".into()),
                RedisJsonValue::Integer(1),
                RedisJsonValue::String("zset1".into()),
            ...
            let input = ZinterstoreInput::decode(args).unwrap();
            assert!(matches!(input.aggregate, Some(Aggregate::MIN)));
        }

        #[test]
        fn test_decode_input_too_few_args() {
            let args = vec![RedisJsonValue::String("dest".into()), RedisJsonValue::Integer(2)];
            let err = ZinterstoreInput::decode(args).unwrap_err();
    ...
    mod integration {
        ...
        async fn test_zinterstore_basic() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ...
                    // ZADD zinterstoretest1 1 a 2 b 3 c
                    ctx.raw(b"*8\r\n$4\r\nZADD\r\n$16\r\nzinterstoretest1\r\n$1\r\n1\r\n$1\r\na\r\n$1\r\n2\r\n$1\r\nb\r\n$1\r\n3\r\n$1\r\nc\r\n")
                        .await
                        .expect("raw failed");

                    // ZADD zinterstoretest2 2 b 3 c 4 d
                    ctx.raw(b"*8\r\n$4\r\nZADD\r\n$16\r\nzinterstoretest2\r\n$1\r\n2\r\n$1\r\nb\r\n$1\r\n3\r\n$1\r\nc\r\n$1\r\n4\r\n$1\r\nd\r\n")
                        .await
                        .expect("raw failed");
                    ...
                    let result = ctx
                        .raw(
                            &ZinterstoreInput::new(
                                RedisKey::String("zinterstoreresult".into()),
                                vec![
                                    RedisKey::String("zinterstoretest1".into()),
                                    RedisKey::String("zinterstoretest2".into()),
                                ],
                            )
                                .command(),
                        )
                    ...
                    let output = ZinterstoreOutput::decode(&result).expect("decode failed");
            ...
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ...
                    let result = ctx
                        .raw(
                            &ZinterstoreInput::new(
                                RedisKey::String("zistore_wgt_dest".into()),
                                vec![RedisKey::String("zistore_wgt_s1".into()), RedisKey::String("zistore_wgt_s2".into())],
                            )
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_zinterstore_aggregate_min() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(b"*2\r\n$3\r\nDEL\r\n$14\r\nzistore_min_s1\r\n").await.expect("raw failed");
                    ctx.raw(b"*2\r\n$3\r\nDEL\r\n$14\r\nzistore_min_s2\r\n").await.expect("raw failed");
                    ...
                    let result = ctx
                        .raw(
                            &ZinterstoreInput::new(
                                RedisKey::String("zistore_min_dest".into()),
                                vec![RedisKey::String("zistore_min_s1".into()), RedisKey::String("zistore_min_s2".into())],
                            )
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_zinterstore_empty_intersection() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(b"*2\r\n$3\r\nDEL\r\n$18\r\nzistore_empty_dest\r\n").await.expect("raw failed");
                    ctx.raw(b"*2\r\n$3\r\nDEL\r\n$16\r\nzistore_empty_s1\r\n").await.expect("raw failed");
                    ...
                    let result = ctx
                        .raw(
                            &ZinterstoreInput::new(
                                RedisKey::String("zistore_empty_dest".into()),
                                vec![
                                    RedisKey::String("zistore_empty_s1".into()),
                                    RedisKey::String("zistore_empty_s2".into()),
                                ],
                            )
                            .command(),
                        )
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_zinterstore_wrongtype() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(b"*3\r\n$3\r\nSET\r\n$13\r\nzistore_wrong\r\n$5\r\nvalue\r\n").await.expect("raw failed");
                    ...
                    let result = ctx
                        .raw(
                            &ZinterstoreInput::new(RedisKey::String("zistore_dest".into()), vec![RedisKey::String("zistore_wrong".into())])
                                .command(),
                        )
                        .await
                        .expect("raw failed");

                    let err = ZinterstoreOutput::decode(&result).unwrap_err();
                    assert!(err.to_string().contains("WRONGTYPE"));
                })
            })
            .await;
