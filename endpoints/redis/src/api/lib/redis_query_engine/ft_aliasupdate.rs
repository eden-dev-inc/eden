...
const API_INFO: ApiInfo<RedisApi, FtAliasupdateInput> = ApiInfo::new(
    EpKind::Redis,
...
/// See official Redis documentation for `FT.ALIASUPDATE`
/// https://redis.io/docs/latest/commands/ft.aliasupdate/
#[derive(Debug, Deserialize, Clone, Builder, ToSchema, DocumentInput, JsonSchema)]
pub struct FtAliasupdateInput {
    alias: RedisJsonValue,
    index: RedisJsonValue,
}

impl Serialize for FtAliasupdateInput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("FtAliasupdateInput", 3)?;
...
impl_redis_operation!(FtAliasupdateInput, API_INFO, { alias, index });

impl RedisCommandInput for FtAliasupdateInput {
    ...
    {
        if args.len() != 2 {
            return Err(EpError::request(format!("FT.ALIASUPDATE requires 2 arguments, given {}", args.len())));
        }
...
/// Output for Redis `FT.ALIASUPDATE` command.
///
/// Returns OK on success.
#[derive(Debug, Deserialize, Clone, ToSchema, JsonSchema)]
pub struct FtAliasupdateOutput {
    success: bool,
}

impl Serialize for FtAliasupdateOutput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("FtAliasupdateOutput", 1)?;
        state.serialize_field("success", &self.success)?;
        state.end()
    }
}

impl FtAliasupdateOutput {
    pub fn new(success: bool) -> Self {
        Self { success }
    }

    /// Check if the alias was updated successfully
    pub fn is_success(&self) -> bool {
        self.success
    }

    /// Decode the Redis protocol response into a FtAliasupdateOutput
    pub fn decode(bytes: &[u8]) -> Result<Self, EpError> {
        ...
        let success = match frame {
            DecoderRespFrame::Resp2(resp2_frame) => match resp2_frame {
                ...
                other => {
                    return Err(EpError::parse(format!("unexpected FT.ALIASUPDATE response: {:?}", other)));
                }
            },
...
mod tests {
    ...
    mod unit {
        ...
        fn test_decode_input_valid() {
            ...
            let input = FtAliasupdateInput::decode(args).unwrap();
        ...
        fn test_decode_input_too_few_args() {
            ...
            let err = FtAliasupdateInput::decode(args).unwrap_err();
        ...
        fn test_decode_output_ok() {
            let output = FtAliasupdateOutput::decode(b"+OK\r\n").unwrap();
            assert!(output.is_success());
        }

        #[test]
        fn test_decode_output_error() {
            let err = FtAliasupdateOutput::decode(b"-ERR unknown index\r\n").unwrap_err();
        ...
        fn test_serialize_output() {
            let output = FtAliasupdateOutput::new(true);
    ...
    mod integration {
        ...
        // Note: FT.ALIASUPDATE requires RediSearch module.
        // These tests will skip if the module is not available.

        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_ft_aliasupdate_nonexistent_index() {
