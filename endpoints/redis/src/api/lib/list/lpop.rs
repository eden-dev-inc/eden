...
const API_INFO: ApiInfo<RedisApi, LpopInput> = ApiInfo::new(
    EpKind::Redis,
    RedisApi::Lpop,
...
/// See official Redis documentation for `LPOP`
...
impl Serialize for LpopInput {
        ...
        S: Serializer,
    {
        ...
        let mut state = serializer.serialize_struct("LpopInput", fields)?;
...
impl_redis_operation!(LpopInput, API_INFO, { key, count });

impl RedisCommandInput for LpopInput {
    ...
    {
        if args.is_empty() {
            return Err(EpError::request("LPOP requires at least 1 argument"));
        }
...
/// Output for Redis LPOP command
#[derive(Debug, Deserialize, Clone, ToSchema, JsonSchema)]
pub struct LpopOutput {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, EpError> {
        ...
        match frame {
            DecoderRespFrame::Resp2(resp2_frame) => match resp2_frame {
                ...
                other => Err(EpError::parse(format!("unexpected LPOP response: {:?}", other))),
            },
...
impl Serialize for LpopOutput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("LpopOutput", 2)?;
...
mod tests {
    ...
    mod unit {
        ...
        fn test_encode_command_without_count() {
            let input = LpopInput { key: RedisKey::String("mylist".into()), count: None };
        ...
        fn test_decode_input_valid() {
            ...
            let input = LpopInput::decode(args).unwrap();
        ...
        async fn test_lpop_single() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(
                        &RpushInput {
                            key: RedisKey::String("lpop_single".into()),
                    ...
                    let result = ctx
                        .raw(&LpopInput { key: RedisKey::String("lpop_single".into()), count: None }.command())
                        .await
                        .expect("lpop failed");

                    let output = LpopOutput::decode(&result).expect("decode failed");
        ...
        async fn test_lpop_with_count() {
            test_all_protocols_min_version("6.2", |ctx| {
                Box::pin(async move {
                    ctx.raw(
                        &RpushInput {
                            key: RedisKey::String("lpop_count".into()),
                            elements: vec![
            ...
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    let result = ctx
                        .raw(
                            &LpopInput {
                                key: RedisKey::String("nonexistent_lpop".into()),
                                count: None,
                            }
                            .command(),
                        )
