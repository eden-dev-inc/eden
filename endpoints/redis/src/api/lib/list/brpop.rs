...
const API_INFO: ApiInfo<RedisApi, BrpopInput> = ApiInfo::new(
    EpKind::Redis,
    RedisApi::Brpop,
...
/// See official Redis documentation for `BRPOP`
/// https://redis.io/docs/latest/commands/brpop/
#[derive(Debug, Deserialize, Clone, Builder, ToSchema, DocumentInput, JsonSchema)]
pub struct BrpopInput {
    pub(crate) keys: Vec<RedisKey>,
    pub(crate) timeout: RedisJsonValue,
}

impl Serialize for BrpopInput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("BrpopInput", 3)?;
...
impl_redis_operation!(BrpopInput, API_INFO, { keys, timeout });

impl RedisCommandInput for BrpopInput {
    ...
    {
        if args.len() < 2 {
            return Err(EpError::request(format!("BRPOP requires at least 2 arguments, given {}", args.len())));
        }
...
impl BrpopOutput {
    ...
    /// Decode the Redis protocol response into a BrpopOutput
    pub fn decode(bytes: &[u8]) -> Result<Self, EpError> {
        ...
        match frame {
            DecoderRespFrame::Resp2(resp2_frame) => match resp2_frame {
                ...
                other => Err(EpError::parse(format!("unexpected BRPOP response: {:?}", other))),
            },
...
impl Serialize for BrpopOutput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("BrpopOutput", 2)?;
...
mod tests {
    ...
    mod unit {
        ...
        fn test_decode_input_valid() {
            ...
            let input = BrpopInput::decode(args).unwrap();
        ...
        fn test_decode_output_null_resp3() {
            let output = BrpopOutput::decode(b"_\r\n").unwrap();
            assert!(output.timed_out());
        }

        #[test]
        fn test_decode_output_array() {
            let output = BrpopOutput::decode(b"*2\r\n$6\r\nmylist\r\n$5\r\nvalue\r\n").unwrap();
            assert!(!output.timed_out());
        ...
        fn test_decode_output_error() {
            let err = BrpopOutput::decode(b"-ERR wrong type\r\n").unwrap_err();
        ...
        async fn test_brpop_with_data() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ...
                    ctx.raw(
                        &RpushInput {
                            key: RedisKey::String("brpoplist".into()),
                    ...
                    let result = ctx
                        .raw(
                            &BrpopInput {
                                keys: vec![RedisKey::String("brpoplist".into())],
                                timeout: RedisJsonValue::Integer(0),
                            }
                            .command(),
                        )
                    ...
                    let output = BrpopOutput::decode(&result).expect("decode failed");
                    assert!(!output.timed_out());
                    assert_eq!(output.key(), Some("brpoplist"));
            ...
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    let result = ctx
                        .raw(
                            &BrpopInput {
                                keys: vec![RedisKey::String("nonexistent_brpop".into())],
        ...
        async fn test_brpop_pops_from_right() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ...
                    ctx.raw(
                        &RpushInput {
                            key: RedisKey::String("brpop_order".into()),
                    ...
                    let result = ctx
                        .raw(
                            &BrpopInput {
                                keys: vec![RedisKey::String("brpop_order".into())],
                                timeout: RedisJsonValue::Integer(0),
                            }
                            .command(),
                        )
                    ...
                    let output = BrpopOutput::decode(&result).expect("decode failed");
                    assert_eq!(output.value(), Some(&RedisJsonValue::from("last")), "BRPOP should pop from right");
                })
            })
            .await;
        }
