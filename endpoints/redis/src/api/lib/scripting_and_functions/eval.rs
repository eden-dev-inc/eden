...
const API_INFO: ApiInfo<RedisApi, EvalInput> =
    ApiInfo::new(EpKind::Redis, RedisApi::Eval, "Executes a server-side Lua script", ReqType::Write, true);

/// Input for Redis `EVAL` command.
///
/// Executes a Lua script server side. Scripts are cached on the server and can be
/// called later using EVALSHA.
///
/// See official Redis documentation for `EVAL`:
...
impl Serialize for EvalInput {
        ...
        S: Serializer,
    {
        ...
        let mut state = serializer.serialize_struct("EvalInput", fields)?;
...
impl_redis_operation!(EvalInput, API_INFO, { script, numkeys, keys, args });

impl RedisCommandInput for EvalInput {
    ...
    {
        if args.len() < 2 {
            return Err(EpError::request(format!("EVAL requires at least 2 arguments, given {}", args.len())));
        }
...
impl EvalOutput {
    ...
    /// Decode the Redis protocol response into an EvalOutput
    ...
    fn resp3_to_value(frame: Resp3Frame) -> Result<RedisJsonValue, EpError> {
        match frame {
            ...
            other => Err(EpError::parse(format!("unexpected EVAL response type: {:?}", other))),
        }
    }
}
...
impl Serialize for EvalOutput {
    ...
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("EvalOutput", 1)?;
...
mod tests {
    ...
    mod unit {
        ...
        fn test_decode_integer_response() {
            let output = EvalOutput::decode(b":42\r\n").unwrap();
        ...
        fn test_decode_string_response() {
            let output = EvalOutput::decode(b"$5\r\nhello\r\n").unwrap();
            assert_eq!(output.as_str(), Some("hello"));
        }

        #[test]
        fn test_decode_nil_response() {
            let output = EvalOutput::decode(b"$-1\r\n").unwrap();
            assert!(output.is_nil());
        }

        #[test]
        fn test_decode_array_response() {
            let output = EvalOutput::decode(b"*2\r\n:1\r\n:2\r\n").unwrap();
            match output.result() {
        ...
        fn test_decode_error_fails() {
            let err = EvalOutput::decode(b"-ERR Script error\r\n").unwrap_err();
            assert!(err.to_string().contains("ERR"));
        }

        #[test]
        fn test_decode_input_too_few_args() {
            let result = EvalInput::decode(vec![RedisJsonValue::String("return 1".into())]);
            assert!(result.is_err());
        }

        #[test]
        fn test_decode_input_basic() {
            let result = EvalInput::decode(vec![RedisJsonValue::String("return 1".into()), RedisJsonValue::Integer(0)]);
            assert!(result.is_ok());
        ...
        fn test_decode_input_with_keys() {
            let result = EvalInput::decode(vec![
                RedisJsonValue::String("return KEYS[1]".into()),
            ...
            assert_eq!(input.keys.unwrap().len(), 1);
        }

        #[test]
        fn test_decode_input_with_keys_and_args() {
            let result = EvalInput::decode(vec![
                RedisJsonValue::String("return {KEYS[1], ARGV[1]}".into()),
                RedisJsonValue::Integer(1),
                RedisJsonValue::String("key1".into()),
        ...
        #[test]
        fn test_keys_returns_provided_keys() {
            let input = EvalInput {
                script: RedisJsonValue::String("return 1".into()),
        ...
        fn test_kind() {
            ...
            assert_eq!(crate::api::lib::RedisCommandInput::kind(&input), RedisApi::Eval);
        }
        ...
        fn test_serialize_input() {
            ...
            assert!(json.contains("numkeys"));
        }

        #[test]
        fn test_serialize_output() {
            let output = EvalOutput::new(RedisJsonValue::Integer(42));
    ...
    mod integration {
        ...
        async fn test_eval_return_integer() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ...
                    let output = EvalOutput::decode(&result).expect("decode failed");
                    assert_eq!(output.as_int(), Some(42));
                })
            })
            .await;
            ...
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    let result = ctx
                        .raw(
                            &EvalInput {
                                script: RedisJsonValue::String("return 'hello'".into()),
                                numkeys: RedisJsonValue::Integer(0),
                                keys: None,
                                args: None,
                            }
        ...
        async fn test_eval_with_keys() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    // Set a key first
                    ctx.write(SetInput {
                        key: RedisKey::String("evalkey".into()),
                        value: RedisJsonValue::String("evalvalue".into()),
                        ..Default::default()
                    })
                    .await;
                    ...
                    let result = ctx
                        .raw(
                            &EvalInput {
                                ...
                                numkeys: RedisJsonValue::Integer(1),
                                keys: Some(vec![RedisKey::String("evalkey".into())]),
                                args: None,
                            }
                            .command(),
                        )
                    ...
                    let output = EvalOutput::decode(&result).expect("decode failed");
                    assert_eq!(output.as_str(), Some("evalvalue"));
                })
            })
            .await;
        }
        ...
        async fn test_eval_with_argv() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    let result = ctx
                        .raw(
                            &EvalInput {
                                ...
                                numkeys: RedisJsonValue::Integer(0),
                                keys: None,
                                args: Some(vec![RedisJsonValue::String("testarg".into())]),
                            }
                            .command(),
                        )
                        .await
                    ...
                    let output = EvalOutput::decode(&result).expect("decode failed");
                    assert_eq!(output.as_str(), Some("testarg"));
                })
            })
            .await;
        }
            ...
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    let result = ctx
                        .raw(
                            &EvalInput {
                                script: RedisJsonValue::String("return {1, 2, 3}".into()),
                                numkeys: RedisJsonValue::Integer(0),
                                keys: None,
                                args: None,
                            }
            ...
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    let result = ctx
                        .raw(
                            &EvalInput {
                                script: RedisJsonValue::String("return nil".into()),
                                numkeys: RedisJsonValue::Integer(0),
                                keys: None,
                                args: None,
                            }
        ...
        async fn test_eval_pipeline() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ...
                    let output1 = EvalOutput::decode(responses[0]).expect("decode first");
                    assert_eq!(output1.as_int(), Some(1));

                    let output2 = EvalOutput::decode(responses[1]).expect("decode second");
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_eval_resp2_format() {
            ...
            assert!(result.starts_with(b":"), "RESP2 integer should start with :");
            let output = EvalOutput::decode(&result).expect("decode failed");
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_eval_resp3_format() {
