...
const API_INFO: ApiInfo<RedisApi, TsMrevrangeInput> = ApiInfo::new(
    EpKind::Redis,
...
/// Input for Redis `TS.MREVRANGE` command.
///
/// Query multiple time series for data points within a timestamp range in reverse order.
///
/// See official Redis documentation for `TS.MREVRANGE`:
/// https://redis.io/docs/latest/commands/ts.mrevrange/
#[derive(Debug, Deserialize, Clone, Builder, ToSchema, DocumentInput, JsonSchema)]
pub struct TsMrevrangeInput {
...
impl Serialize for TsMrevrangeInput {
        ...
        S: Serializer,
    {
        ...
        let mut state = serializer.serialize_struct("TsMrevrangeInput", fields)?;
...
impl RedisCommandInput for TsMrevrangeInput {
    ...
    {
        if args.len() < 4 {
            return Err(EpError::request(format!("TS.MREVRANGE requires at least 4 arguments, given {}", args.len())));
        }
        ...
        let filter = filter.ok_or_else(|| EpError::request("TS.MREVRANGE requires FILTER parameter"))?;
...
/// Result for a single time series in TS.MREVRANGE response.
...
/// Output for Redis `TS.MREVRANGE` command.
#[derive(Debug, Deserialize, Clone, ToSchema, JsonSchema)]
pub struct TsMrevrangeOutput {
    /// Results for each matching time series
    results: Vec<TimeSeriesResult>,
}

impl TsMrevrangeOutput {
    ...
    fn parse_resp2(frame: Resp2Frame) -> Result<Vec<TimeSeriesResult>, EpError> {
        match frame {
            ...
            other => Err(EpError::parse(format!("unexpected TS.MREVRANGE response: {:?}", other))),
        }
    }
...
impl Serialize for TsMrevrangeOutput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("TsMrevrangeOutput", 1)?;
...
mod tests {
    ...
    mod unit {
        ...
        fn test_encode_command_basic() {
            ...
            assert!(cmd_str.contains("TS.MREVRANGE"));
        ...
        fn test_encode_command_with_selected_labels() {
            let input = TsMrevrangeInput {
                ...
                label: Some(Label::SELECTEDLABELS(vec![
                    ...
                    RedisJsonValue::String("area".into()),
                ])),
                count: None,
                align: None,
        ...
        #[test]
        fn test_decode_input_with_groupby() {
            let args = vec![
                RedisJsonValue::Integer(0),
                RedisJsonValue::Integer(10000),
                RedisJsonValue::String("FILTER".into()),
                RedisJsonValue::String("area=*".into()),
                RedisJsonValue::String("GROUPBY".into()),
                RedisJsonValue::String("area".into()),
                RedisJsonValue::String("REDUCE".into()),
                RedisJsonValue::String("sum".into()),
            ];
            let input = TsMrevrangeInput::decode(args).unwrap();
            assert!(input.group.is_some());
            let group = input.group.unwrap();
            assert_eq!(group.group_by, RedisJsonValue::String("area".into()));
            assert_eq!(group.reduce, RedisJsonValue::String("sum".into()));
        }
        ...
        fn test_decode_input_missing_filter() {
            ...
            let err = TsMrevrangeInput::decode(args).unwrap_err();
        ...
        fn test_decode_output_empty() {
            let output = TsMrevrangeOutput::decode(b"*0\r\n").unwrap();
            assert!(output.is_empty());
        }

        #[test]
        fn test_decode_output_error() {
            let err = TsMrevrangeOutput::decode(b"-ERR unknown command\r\n").unwrap_err();
        ...
        fn test_output_accessors() {
            let output = TsMrevrangeOutput::new(vec![TimeSeriesResult {
                key: "ts:sensor:1".into(),
                labels: vec![("sensor".into(), "1".into())],
                data_points: vec![DataPoint { timestamp: 2000, value: 2.0 }, DataPoint { timestamp: 1000, value: 1.0 }],
            }]);
            assert!(!output.is_empty());
            assert_eq!(output.len(), 1);
            // MREVRANGE returns newest first
            assert_eq!(output.results()[0].data_points[0].timestamp, 2000);
        }
    }
    ...
    mod integration {
        ...
        #[serial]
        async fn test_mrevrange_no_matches() {
            test_all_protocols_min_version("6.0", |ctx| {
                Box::pin(async move {
                    ...
                    {
                        let output = TsMrevrangeOutput::decode(&result).expect("decode failed");
                        assert!(output.is_empty());
                    }
                })
            })
