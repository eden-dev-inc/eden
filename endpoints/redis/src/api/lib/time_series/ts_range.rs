...
const API_INFO: ApiInfo<RedisApi, TsRangeInput> =
    ApiInfo::new(EpKind::Redis, RedisApi::TsRange, "Query a range in forward direction", ReqType::Read, true);

/// Input for Redis `TS.RANGE` command.
///
/// Query a time series for data points within a timestamp range.
///
/// See official Redis documentation for `TS.RANGE`:
/// https://redis.io/docs/latest/commands/ts.range/
#[derive(Debug, Deserialize, Clone, Builder, ToSchema, DocumentInput, JsonSchema)]
pub struct TsRangeInput {
...
impl Serialize for TsRangeInput {
        ...
        S: Serializer,
    {
        ...
        let mut state = serializer.serialize_struct("TsRangeInput", fields)?;
...
impl RedisCommandInput for TsRangeInput {
        ...
        Self: Sized,
    {
        if args.len() < 3 {
            return Err(EpError::request(format!(
                "TS.RANGE requires at least 3 arguments (key, fromTimestamp, toTimestamp), given {}",
                args.len()
            )));
        }
...
/// Output for Redis `TS.RANGE` command.
///
/// Contains the list of timestamp-value pairs within the queried range.
#[derive(Debug, Deserialize, Clone, ToSchema, JsonSchema)]
pub struct TsRangeOutput {
...
impl TsRangeOutput {
    ...
    /// Decode the Redis protocol response into a TsRangeOutput
    ...
    fn parse_resp2(frame: Resp2Frame) -> Result<Vec<DataPoint>, EpError> {
        match frame {
            ...
            other => Err(EpError::parse(format!("unexpected TS.RANGE response: {:?}", other))),
        }
    }
...
impl Serialize for TsRangeOutput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("TsRangeOutput", 1)?;
...
mod tests {
    ...
    mod unit {
        ...
        fn test_encode_command_basic() {
            ...
            assert!(cmd_str.contains("TS.RANGE"));
        ...
        #[test]
        fn test_encode_command_with_aggregation() {
            let input = TsRangeInput {
                key: RedisKey::String("ts:key".into()),
                from_timestamp: RedisJsonValue::Integer(0),
                to_timestamp: RedisJsonValue::Integer(10000),
                latest: None,
                filter_by_ts: None,
                filter_by_value: None,
                count: None,
        ...
        fn test_decode_input_minimal() {
            ...
            let input = TsRangeInput::decode(args).unwrap();
        ...
        fn test_decode_input_too_few_args() {
            ...
            let err = TsRangeInput::decode(args).unwrap_err();
            assert!(err.to_string().contains("requires at least 3"));
        }

        #[test]
        fn test_decode_input_filter_by_ts_missing_arg() {
            let args = vec![
                RedisJsonValue::String("key".into()),
                RedisJsonValue::Integer(1000),
                RedisJsonValue::Integer(2000),
                RedisJsonValue::String("FILTER_BY_TS".into()),
            ];
            let err = TsRangeInput::decode(args).unwrap_err();
            assert!(err.to_string().contains("FILTER_BY_TS"));
        }

        #[test]
        fn test_decode_input_filter_by_value_missing_args() {
            let args = vec![
                RedisJsonValue::String("key".into()),
                RedisJsonValue::Integer(1000),
                RedisJsonValue::Integer(2000),
            ...
            let err = TsRangeInput::decode(args).unwrap_err();
            assert!(err.to_string().contains("FILTER_BY_VALUE"));
        }

        #[test]
        fn test_decode_output_empty() {
            // RESP2 empty array
            let output = TsRangeOutput::decode(b"*0\r\n").unwrap();
        ...
        fn test_decode_output_error() {
            let err = TsRangeOutput::decode(b"-ERR key does not exist\r\n").unwrap_err();
        ...
        #[test]
        fn test_data_point_struct() {
            let dp = DataPoint { timestamp: 1609459200000, value: 25.5 };
            assert_eq!(dp.timestamp, 1609459200000);
            assert!((dp.value - 25.5).abs() < f64::EPSILON);
        }

        #[test]
        fn test_output_accessors() {
            let output = TsRangeOutput::new(vec![DataPoint { timestamp: 1000, value: 1.0 }, DataPoint { timestamp: 2000, value: 2.0 }]);
            assert!(!output.is_empty());
            assert_eq!(output.len(), 2);
            assert_eq!(output.data_points()[0].timestamp, 1000);
        }
    }
    ...
    mod integration {
        ...
        // Note: TS.RANGE requires RedisTimeSeries module.
        // These tests will skip if the module is not available.

        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_range_nonexistent_key() {
                ...
                Box::pin(async move {
                    ...
                    if let Ok(result) = result {
                        if result.starts_with(b"-") {
                            ...
                            let output = TsRangeOutput::decode(&result).expect("decode failed");
                            assert!(output.is_empty());
                        }
                    }
                })
