...
const API_INFO: ApiInfo<RedisApi, SubstrInput> =
    ApiInfo::new(EpKind::Redis, RedisApi::Substr, "Returns a substring from a string value", ReqType::Read, true);

/// See official Redis documentation for `SUBSTR`
/// https://redis.io/docs/latest/commands/substr/
///
/// Note: SUBSTR is deprecated as of Redis 2.0.
/// Use GETRANGE instead.
#[derive(Debug, Deserialize, Clone, Builder, ToSchema, DocumentInput, JsonSchema)]
pub struct SubstrInput {
    pub(crate) key: RedisKey,
    pub(crate) start: RedisJsonValue,
    pub(crate) end: RedisJsonValue,
}

impl Serialize for SubstrInput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("SubstrInput", 4)?;
...
impl RedisCommandInput for SubstrInput {
    ...
    {
        if args.len() < 3 {
            return Err(EpError::request(format!("SUBSTR requires 3 arguments, given {}", args.len())));
        }
        ...
        if args.len() > 3 {
            ...
            log_warn!(
                _ctx,
                "SUBSTR expects 3 arguments, but given {}",
...
impl SubstrOutput {
    ...
    /// Decode the Redis protocol response into a SubstrOutput
    pub fn decode(bytes: &[u8]) -> Result<Self, EpError> {
        ...
        let value = match frame {
            DecoderRespFrame::Resp2(resp2_frame) => match resp2_frame {
                ...
                other => {
                    return Err(EpError::parse(format!("unexpected SUBSTR response: {:?}", other)));
                }
            },
...
impl Serialize for SubstrOutput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("SubstrOutput", 1)?;
...
mod tests {
    ...
    mod unit {
        ...
        fn test_decode_bulk_string() {
            let output = SubstrOutput::decode(b"$5\r\nHello\r\n").unwrap();
        ...
        fn test_decode_empty_string() {
            let output = SubstrOutput::decode(b"$0\r\n\r\n").unwrap();
            assert_eq!(output.as_str(), Some(""));
            assert!(output.is_empty());
        }

        #[test]
        fn test_decode_null_as_empty() {
            let output = SubstrOutput::decode(b"$-1\r\n").unwrap();
            assert_eq!(output.as_str(), Some(""));
            assert!(output.is_empty());
        }
        ...
        fn test_decode_error_fails() {
            let err = SubstrOutput::decode(b"-WRONGTYPE Operation against a key holding the wrong kind of value\r\n").unwrap_err();
        ...
        fn test_decode_input_valid() {
            ...
            let input = SubstrInput::decode(args).unwrap();
        ...
        fn test_decode_input_too_few_args() {
            ...
            let err = SubstrInput::decode(args).unwrap_err();
    ...
    mod integration {
        ...
        // Note: SUBSTR is deprecated as of Redis 2.0, replaced by GETRANGE.
        // The command still works in all versions for backward compatibility.

        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_substr_basic() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(
                        &SetInput {
                            key: RedisKey::String("substr_key".into()),
                    ...
                    let output = SubstrOutput::decode(&result).expect("decode failed");
                    assert_eq!(output.as_str(), Some("Hello"));
                })
            })
            .await;
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_substr_negative_indices() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(
                        &SetInput {
                            key: RedisKey::String("substr_neg".into()),
                            value: RedisJsonValue::String("Hello World".into()),
                            ..Default::default()
                        }
                        .command(),
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_substr_missing_key() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    let result = ctx
                        .raw(
                            &SubstrInput {
                                key: RedisKey::String("substr_missing".into()),
                                start: RedisJsonValue::Integer(0),
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_substr_out_of_range() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(
                        &SetInput {
                            key: RedisKey::String("substr_range".into()),
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_substr_pipeline() {
            test_all_protocols(|ctx| {
                Box::pin(async move {
                    ctx.raw(
                        &SetInput {
                            key: RedisKey::String("substr_pipe".into()),
                    ...
                    let out1 = SubstrOutput::decode(responses[0]).expect("decode first");
                    assert_eq!(out1.as_str(), Some("ABC"));

                    let out2 = SubstrOutput::decode(responses[1]).expect("decode second");
                    assert_eq!(out2.as_str(), Some("DEF"));

                    let out3 = SubstrOutput::decode(responses[2]).expect("decode third");
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_substr_resp2_format() {
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_substr_resp3_format() {
