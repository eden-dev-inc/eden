...
const API_INFO: ApiInfo<RedisApi, TdigestRevrankInput> = ApiInfo::new(
    EpKind::Redis,
...
/// Input for Redis `TDIGEST.REVRANK` command.
///
/// Returns, for each input value, the estimated reverse rank of the value.
///
/// See official Redis documentation for `TDIGEST.REVRANK`:
/// https://redis.io/docs/latest/commands/tdigest.revrank/
#[derive(Debug, Deserialize, Clone, Builder, ToSchema, DocumentInput, JsonSchema)]
pub struct TdigestRevrankInput {
    /// The key name for the t-digest sketch
    pub(crate) key: RedisKey,
    /// One or more values to query reverse ranks for
    pub(crate) value: Vec<RedisJsonValue>,
}

impl Serialize for TdigestRevrankInput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("TdigestRevrankInput", 3)?;
...
impl RedisCommandInput for TdigestRevrankInput {
        ...
        Self: Sized,
    {
        if args.len() < 2 {
            return Err(EpError::parse(format!(
                "TDIGEST.REVRANK requires at least 2 arguments (key, value...), given {}",
                args.len()
            )));
        }
        ...
        if value.is_empty() {
            return Err(EpError::parse("TDIGEST.REVRANK requires at least one value to query"));
        }

        Ok(TdigestRevrankInput { key, value })
    }
}

/// Output for Redis `TDIGEST.REVRANK` command.
///
/// Contains the estimated reverse ranks for each queried value.
#[derive(Debug, Deserialize, Clone, ToSchema, JsonSchema)]
pub struct TdigestRevrankOutput {
    /// Estimated reverse ranks for each value (may be -1 for values above maximum)
    ranks: Vec<i64>,
}

impl TdigestRevrankOutput {
    pub fn new(ranks: Vec<i64>) -> Self {
        Self { ranks }
    }

    /// Get the estimated reverse ranks
    ...
    /// Decode the Redis protocol response into a TdigestRevrankOutput
    ...
    fn parse_resp2(frame: Resp2Frame) -> Result<Vec<i64>, EpError> {
        match frame {
            ...
            other => Err(EpError::parse(format!("unexpected TDIGEST.REVRANK response: {:?}", other))),
        }
    }
...
impl Serialize for TdigestRevrankOutput {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("TdigestRevrankOutput", 1)?;
...
mod tests {
    ...
    mod unit {
        ...
        fn test_decode_output_array() {
            let output = TdigestRevrankOutput::decode(b"*3\r\n:2\r\n:1\r\n:0\r\n").unwrap();
            assert_eq!(output.len(), 3);
            assert_eq!(output.ranks()[0], 2);
            assert_eq!(output.ranks()[1], 1);
            assert_eq!(output.ranks()[2], 0);
        }

        #[test]
        fn test_decode_output_negative_rank() {
            let output = TdigestRevrankOutput::decode(b"*1\r\n:-1\r\n").unwrap();
        ...
        fn test_decode_output_error() {
            let err = TdigestRevrankOutput::decode(b"-ERR unknown command\r\n").unwrap_err();
        ...
        fn test_decode_input_valid() {
            ...
            let input = TdigestRevrankInput::decode(args).unwrap();
        ...
        fn test_decode_input_missing_value() {
            ...
            let err = TdigestRevrankInput::decode(args).unwrap_err();
        ...
        fn test_serialize_output() {
            let output = TdigestRevrankOutput::new(vec![2, 1, 0]);
        ...
        #[test]
        fn test_new_output() {
            let output = TdigestRevrankOutput::new(vec![10, 5]);
    ...
    mod integration {
        ...
        #[serial]
        async fn test_tdigest_revrank_basic() {
            test_all_protocols_min_version("6.0", |ctx| {
                Box::pin(async move {
                    let Ok(create_result) = ctx
                        .raw(
                            &TdigestCreateInput {
                                key: RedisKey::String("td_revrank_test".into()),
                                compression: None,
                            }
                            .command(),
                        )
                    ...
                    let output = TdigestRevrankOutput::decode(&result).expect("decode failed");
                    assert!(!output.is_empty());
                    // Reverse rank should be >= 0
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_tdigest_revrank_multiple_values() {
            test_all_protocols_min_version("6.0", |ctx| {
                Box::pin(async move {
                    let Ok(create_result) = ctx
                        .raw(
                            &TdigestCreateInput {
                                key: RedisKey::String("td_revrank_multi".into()),
                                compression: None,
                            }
                            .command(),
                        )
                    ...
                    let output = TdigestRevrankOutput::decode(&result).expect("decode failed");
                    assert_eq!(output.len(), 3);
                    // Reverse ranks should be monotonically decreasing for increasing values
        ...
        #[tokio::test(flavor = "multi_thread")]
        #[serial]
        async fn test_tdigest_revrank_nonexistent_key() {
            test_all_protocols_min_version("6.0", |ctx| {
                Box::pin(async move {
                    let result = ctx
                        .raw(
