...
use crate::metadata::DatabricksMetadata;
use crate::request::DatabricksRequest;
use crate::{EP, EpTransaction, Transaction};
use dashmap::DashMap;
use databricks_core::client::DatabricksClient;
...
impl_endpoint!(EpKind::Databricks => Databricks, DatabricksAsync);
...
impl EP<DatabricksAsync, DatabricksConfig, DatabricksRequest, DatabricksMetadata, DatabricksApi, DatabricksTx> for DatabricksEp {
    fn new() -> Self {
        Self::default()
    }
    #[named]
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<DatabricksRequest>>() {
            Some(tx) => tx.to_owned(),
