...
use crate::metadata::GitlabMetadata;
...
use gitlab_core::comm::GitlabClient;
...
impl_endpoint!(EpKind::Gitlab => Gitlab, GitlabAsync);

ep_core::impl_endpoint_lifecycle_spec!(GitlabEp, GitlabAsync, GitlabConfig, GitlabRequest, GitlabMetadata, GitlabApi, GitlabTx);

impl EP<GitlabAsync, GitlabConfig, GitlabRequest, GitlabMetadata, GitlabApi, GitlabTx> for GitlabEp {
    fn new() -> Self {
        Self::default()
    }
    #[named]
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<GitlabRequest>>() {
            Some(tx) => tx.to_owned(),
