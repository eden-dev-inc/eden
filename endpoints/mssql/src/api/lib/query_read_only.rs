...
const API_INFO: ApiInfo<MssqlApi, QueryReadOnlyInput> =
    ApiInfo::new(EpKind::Mssql, MssqlApi::QueryReadOnly, "Execute a read-only SQL query against MSSQL", ReqType::Read);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, _context: MssqlAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        Err(EpError::request("MSSQL read-only query execution not yet implemented"))
    }
