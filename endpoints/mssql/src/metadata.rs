...
use std::any::Any;
use {ep_core::define_metadata_serializer_stuff, mssql_core::MssqlAsync};
...
impl SyncMetadata<MssqlAsync> for MssqlMetadata {
    fn jobs(&mut self, _: SyncFrequency) -> Vec<MetadataJob<MssqlAsync, Self>> {
        Vec::new()
    }
}
...
define_metadata_serializer_stuff!(EpKind::Mssql => MssqlMetadata);
