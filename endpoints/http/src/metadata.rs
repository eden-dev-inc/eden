...
impl SyncMetadata<HttpAsync> for HttpMetadata {
    fn jobs(&mut self, _: SyncFrequency) -> Vec<MetadataJob<HttpAsync, Self>> {
        Vec::new()
    }
}

impl EpMetadata for HttpMetadata {
    fn kind(&self) -> EpKind {
        EpKind::Http
    }
...
define_metadata_serializer_stuff!(EpKind::Http => HttpMetadata);
