...
use crate::metadata::DatadogMetadata;
...
impl_endpoint!(EpKind::Datadog => Datadog, DatadogAsync);

ep_core::impl_endpoint_lifecycle_spec!(DatadogEp, DatadogAsync, DatadogConfig, DatadogRequest, DatadogMetadata, DatadogApi, DatadogTx);

impl EP<DatadogAsync, DatadogConfig, DatadogRequest, DatadogMetadata, DatadogApi, DatadogTx> for DatadogEp {
    fn new() -> Self {
        Self::default()
    }
    #[named]
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<DatadogRequest>>() {
            Some(tx) => tx.to_owned(),
