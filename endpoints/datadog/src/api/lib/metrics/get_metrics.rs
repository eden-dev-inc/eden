...
const API_INFO: ApiInfo<DatadogApi, GetMetricsInput> = ApiInfo::new(
    EpKind::Datadog,
    DatadogApi::GetMetrics,
    "Queries timeseries metric data from Datadog",
    ReqType::Read,
    true,
);
...
mod tests {
    ...
    #[test]
    fn get_metrics_builder_serde() {
        let input = GetMetricsInputBuilder::default()
            .from(1609459200i64)
            .to(1609545600i64)
            .query("avg:system.cpu.user{*}")
            .build()
            .expect("Failed to build GetMetricsInput");
    ...
    #[test]
    fn get_metrics_deserialize() {
        let json = serde_json::json!({
            "from": 1609459200,
            "to": 1609545600,
            "query": "avg:system.cpu.user{*}"
        });
        let input: GetMetricsInput = serde_json::from_value(json).expect("Failed to deserialize");
