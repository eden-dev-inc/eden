...
const API_INFO: ApiInfo<MongoApi, RawCommandInput> = ApiInfo::new(
    EpKind::Mongo,
    MongoApi::RawCommand,
    "Execute a raw MongoDB command string (for tools compatibility)",
    ReqType::Write,
    true,
);
...
impl_simple_operation!(RawCommandInput, MongoAsync, MongoTx, MongoApi, MongoRequest);

impl RawCommandInput {
    #[named]
