...
use crate::output::{ClientOutput, CollectionDocumentOutput};
...
const API_INFO: ApiInfo<MongoApi, DatabaseClientInput> = ApiInfo::new(
    EpKind::Mongo,
    MongoApi::Database(Some(DatabaseApi::DatabaseClient)),
...
crate::mongo_endpoint! {
    API_INFO,
    struct DatabaseClientInput {
        database: String,
        collection: String,
    }
}

type OutputWrapper = ClientOutput;
type ExpectedInput = CollectionDocumentOutput;

impl_simple_operation!(DatabaseClientInput, MongoAsync, MongoTx, MongoApi, MongoRequest);

impl DatabaseClientInput {
    #[named]
