...
use crate::metadata::TavilyMetadata;
...
use tavily_core::comm::TavilyClient;
...
impl_endpoint!(EpKind::Tavily => Tavily,  TavilyAsync);

ep_core::impl_endpoint_lifecycle_spec!(TavilyEp, TavilyAsync, TavilyConfig, TavilyRequest, TavilyMetadata, TavilyApi, TavilyTx);

impl EP<TavilyAsync, TavilyConfig, TavilyRequest, TavilyMetadata, TavilyApi, TavilyTx> for TavilyEp {
    fn new() -> Self {
        Self::default()
    }
    #[named]
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<TavilyRequest>>() {
            Some(tx) => tx.to_owned(),
