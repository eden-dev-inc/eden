...
impl SyncMetadata<TavilyAsync> for TavilyMetadata {
    fn jobs(&mut self, _: SyncFrequency) -> Vec<MetadataJob<TavilyAsync, Self>> {
        Vec::new()
    }
}

impl EpMetadata for TavilyMetadata {
    fn kind(&self) -> EpKind {
        EpKind::Tavily
    }
...
define_metadata_serializer_stuff!(EpKind::Tavily => TavilyMetadata);
