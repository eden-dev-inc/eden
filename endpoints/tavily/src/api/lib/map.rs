...
const API_INFO: ApiInfo<TavilyApi, MapInput> = ApiInfo::new(
    EpKind::Tavily,
    TavilyApi::Map,
    "Map website structure by discovering linked URLs using Tavily",
    ReqType::Read,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: TavilyAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = client.post("/map", self.body.clone()).await?;
