...
const API_INFO: ApiInfo<TavilyApi, ResearchStatusInput> = ApiInfo::new(
    EpKind::Tavily,
    TavilyApi::ResearchStatus,
    "Check the status of an ongoing Tavily research task",
    ReqType::Read,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: TavilyAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = client.post("/research/status", self.body.clone()).await?;
