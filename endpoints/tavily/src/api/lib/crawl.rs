...
const API_INFO: ApiInfo<TavilyApi, CrawlInput> = ApiInfo::new(
    EpKind::Tavily,
    TavilyApi::Crawl,
    "Crawl a website starting from a root URL using Tavily",
    ReqType::Read,
);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: TavilyAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = client.post("/crawl", self.body.clone()).await?;
