...
const API_INFO: ApiInfo<TavilyApi, SearchInput> =
    ApiInfo::new(EpKind::Tavily, TavilyApi::Search, "Search the web using Tavily", ReqType::Read);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: TavilyAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = client.post("/search", self.body.clone()).await?;
