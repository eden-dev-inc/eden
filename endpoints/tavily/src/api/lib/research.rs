...
const API_INFO: ApiInfo<TavilyApi, ResearchInput> =
    ApiInfo::new(EpKind::Tavily, TavilyApi::Research, "Conduct deep research on a topic using Tavily", ReqType::Read);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: TavilyAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = client.post("/research", self.body.clone()).await?;
