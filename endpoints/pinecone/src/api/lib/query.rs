...
const API_INFO: ApiInfo<PineconeApi, QueryInput> = ApiInfo::new(EpKind::Mongo, PineconeApi::Query, "Pinecone Query", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: PineconeAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = context.query(self.body().to_owned()).await?;
