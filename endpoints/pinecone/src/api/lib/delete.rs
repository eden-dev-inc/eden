...
const API_INFO: ApiInfo<PineconeApi, DeleteInput> =
    ApiInfo::new(EpKind::Mongo, PineconeApi::Delete, "Pinecone Delete", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: PineconeAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = context.delete(self.body().to_owned()).await?;
