...
const API_INFO: ApiInfo<PineconeApi, FetchInput> = ApiInfo::new(EpKind::Mongo, PineconeApi::Fetch, "Pinecone Fetch", ReqType::Read, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: PineconeAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = context.fetch(&self.ids, &self.namespace).await?;
