...
const API_INFO: ApiInfo<PineconeApi, UpsertInput> =
    ApiInfo::new(EpKind::Mongo, PineconeApi::Upsert, "Pinecone Upsert", ReqType::Write, true);
...
impl SimpleInput {
    ...
    async fn run_async_generic(&self, context: PineconeAsync, telemetry_wrapper: &mut TelemetryWrapper) -> ResultEP<Box<dyn EpOutput>> {
        ...
        let value = context.upsert(self.body().to_owned()).await?;
