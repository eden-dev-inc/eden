...
use crate::metadata::PosthogMetadata;
...
use posthog_core::comm::PosthogClient;
...
impl_endpoint!(EpKind::Posthog => Posthog, PosthogAsync);

ep_core::impl_endpoint_lifecycle_spec!(PosthogEp, PosthogAsync, PosthogConfig, PosthogRequest, PosthogMetadata, PosthogApi, PosthogTx);

impl EP<PosthogAsync, PosthogConfig, PosthogRequest, PosthogMetadata, PosthogApi, PosthogTx> for PosthogEp {
    fn new() -> Self {
        Self::default()
    }
    #[named]
    async fn transaction(
        ...
        let tx = match transaction.as_any().downcast_ref::<Transaction<PosthogRequest>>() {
            Some(tx) => tx.to_owned(),
