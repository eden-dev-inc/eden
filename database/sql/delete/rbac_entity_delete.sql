WITH control_delete AS (
    WITH tombstone AS (
        INSERT INTO rbac_control_entity_tombstones (org_uuid, entity_kind, entity_uuid, version_ms, version_seq, updated_at)
        ...
        DO UPDATE SET
            ...
            updated_at = NOW()
        WHERE (rbac_control_entity_tombstones.version_ms, rbac_control_entity_tombstones.version_seq) < (EXCLUDED.version_ms, EXCLUDED.version_seq)
        RETURNING 1
    )
    UPDATE rbac_control
    SET is_active = FALSE,
...
data_delete AS (
    WITH tombstone AS (
        ...
        SELECT $1::UUID, $3::UUID, $4::BIGINT, $5::BIGINT, NOW()
