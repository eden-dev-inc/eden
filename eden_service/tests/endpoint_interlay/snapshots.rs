...
async fn test_list_snapshots() {
    test_server(
        async || {
            ...
            assert!(result.is_some(), "List snapshots should return a response body");
        },
    ...
    test_server(
        async || {
            ...
            let response = reqwest::Client::default()
                .post(format!("{}/snapshots", get_base_url()))
                .bearer_auth(&admin_jwt.token)
                .json(&body)
                .send()
                .await
                .expect("Failed to send create snapshot request");

            let status = response.status();
            assert_ne!(status.as_u16(), 404, "POST /api/v1/snapshots should not return 404");
        },
        Some(EDEN_NEW_ORG_TOKEN_VALUE.to_string()),
    )
}
