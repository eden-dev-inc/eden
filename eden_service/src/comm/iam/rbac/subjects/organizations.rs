...
#[utoipa::path(
    get,
    tags = ["RBAC"],
    path="/iam/control/subjects/{subject}/organizations",
    operation_id = "get_rbac_subject_organizations",
...
pub async fn get(
    ...
    let entries: Vec<ControlPlaneRbacData> = all_entries.into_iter().filter(|e| e.entity_kind == IdKind::Organization.as_str()).collect();
