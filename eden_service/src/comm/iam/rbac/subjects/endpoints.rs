...
#[utoipa::path(
    get,
    tags = ["RBAC"],
    path="/iam/control/subjects/{subject}/endpoints",
    operation_id = "get_rbac_subject_endpoints",
...
pub async fn get(
    ...
    let entries: Vec<ControlPlaneRbacData> = all_entries.into_iter().filter(|e| e.entity_kind == IdKind::Endpoint.as_str()).collect();
