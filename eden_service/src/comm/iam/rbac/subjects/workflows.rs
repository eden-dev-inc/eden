...
#[utoipa::path(
    get,
    tags = ["RBAC"],
    path="/iam/control/subjects/{subject}/workflows",
    operation_id = "get_rbac_subject_workflows",
...
pub async fn get(
    ...
    let entries: Vec<ControlPlaneRbacData> = all_entries.into_iter().filter(|e| e.entity_kind == IdKind::Workflow.as_str()).collect();
